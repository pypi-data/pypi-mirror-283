...
class SettingsDialog extends Widget {
  ...
  private buildDialog() {
    ...
    customUrlInput.addEventListener('input', () => {
      ...
      this.updateSettings({ urlPath: customUrlInput.value, openAsNotebook: false });
      this.updateUrlDisplay(urlDisplay, customUrlInput.value);
    });

    // Initial URL display update
    this.updateUrlDisplay(urlDisplay, this.settings.urlPath);
  ...
  private updateSettings(newSettings: Partial<{ copyOutput: boolean; urlPath: string; openAsNotebook: boolean }>) {
