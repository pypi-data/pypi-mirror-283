...
function createButton(label: string, icon: string, callback: () => void): ToolbarButton {
...
// Function to create the settings dialog
function createSettingsDialog(settings: any, updateSettings: (newSettings: any) => void): void {
  const dialog = new Dialog({
    title: 'Save to URL Settings',
    body: new SettingsDialog(updateSettings, settings), // Ensure SettingsDialog is a valid widget
...
class SplitButtonWidget extends Widget {
  constructor() {
    ...
    const settingsButton = createButton('Settings', 'ui-components:settings', () => {
      createSettingsDialog({}, (newSettings: any) => {
        console.log('Settings updated:', newSettings);
      });
    });

    // Append the button elements to the SplitButtonWidget's node
