...
from prodt_lan.app.models.asset_instance import AssetInstance
...
from prodt_lan.app.models.project import ProjectTaskTypeLink

from prodt_lan.app.utils import fields, events

from prodt_lan.app.services import (
    assets_service,
    entities_service,
    shots_service,
)
...
def create_casting_link(entity_in_id, asset_id, nb_occurences=1, label=""):
    ...
    try:
        ...
        current_app.logger.warning(
            "Attempt to create duplicated entity links (concurrent modification) via prodt_lan.app.services.breakdown_service.create_casting_link (raised by sqlalchemy.orm.exc.StaleDataError)"
        )
    except DetachedInstanceError:
        current_app.logger.warning(
            "Attempt to create duplicated entity links (attempt to access unloaded attributes on a mapped instance that is detached) via prodt_lan.app.services.breakdown_service.create_casting_link (raised by sqlalchemy.orm.exc.DetachedInstanceError)"
        )
    return link
