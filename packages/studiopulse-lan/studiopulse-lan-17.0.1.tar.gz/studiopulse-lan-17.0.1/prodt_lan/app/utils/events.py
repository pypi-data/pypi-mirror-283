...
from prodt_lan.app.stores import publisher_store
from prodt_lan.app.models.event import ApiEvent
from prodt_lan.app.utils import fields
...
def emit(event, data={}, persist=True, project_id=None):
    ...
    from prodt_lan.app.config import ENABLE_JOB_QUEUE

    for func in event_handlers.values():
        if ENABLE_JOB_QUEUE:
            from prodt_lan.app.stores.queue_store import job_queue
...
def save_event(event, data, project_id=None):
    ...
    """
    try:
        from prodt_lan.app.services.persons_service import get_current_user_raw
