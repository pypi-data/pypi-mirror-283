...
class iAgent:
    ...
    async def bootstrap(self):
        ...
        i = -1
        for i, task in enumerate(self._bootstrap()):
            task.setdefault(FUNC_KEY, 'get_data')
            log.info("+ [%s]: [%s] %s", i, self.name, task)
            # kind = kwargs.get(KIND_KEY)  #  must exists!
            # path = kwargs.get(PATH_KEY, kind)
            # _source = parse_duri(self.app_url)
            ...
            # uri = build_uri(**_uri)
            # uri
            # 'sqlite://test/tmp/1720347683.769698.177810408/db.sqlite/mysensor_stream:mysensor_stream'
            # uri
            # 'sqlite:///tmp/1720347683.769698.177810408/db.sqlite/mysensor_stream'

            # uri = f"{self.app_url}/{kind}"
...
class iCrawler(iAgent):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.app_url = app_url or self.cfg.get("app_url")
        # self.app_url = (
