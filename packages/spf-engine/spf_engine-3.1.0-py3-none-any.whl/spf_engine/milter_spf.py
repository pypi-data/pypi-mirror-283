...
class spfMilter(Milter.Base):
            ...
            if milterconfig['IntHosts']:
                if milterconfig['IntHosts'].match(ipaddr):
                    ...
                    if milterconfig.get('debugLevel') >= 2:
                        syslog.syslog('Internal connection: InternalHosts {0}'
                                .format(str(ipaddr)))
        else:
            ipaddr = ''
    ...
    def envfrom(self, f, *stri):
        # This gets rid of any non-UTF-8, which is invalid, but common in
        # junk mail.  l macro not supported if non-ascii, per RFC 8616.
        if self.getsymval('{auth_authen}'):
            # SMTP Auth, so internal
            self.internal_connection = True
            if milterconfig.get('debugLevel') >= 2:
                syslog.syslog('Internal connection: SMTP Auth user {0}'
                              .format(str(self.getsymval('{auth_authen}'))))
        try:
            test = bytes(f, encoding='utf-8')
