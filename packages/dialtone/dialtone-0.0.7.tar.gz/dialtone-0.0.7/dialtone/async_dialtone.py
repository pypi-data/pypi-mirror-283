...
DEFAULT_BASE_URL = "https://dialtone-app.fly.dev"
...
class Completions(BaseModel):
    ...
    async def create(self, messages: list[ChatMessage | dict], tools: list[Tool] = []):
        params = {
            "api_key": self.client.api_key,
            "messages": [
                message.to_dict() if isinstance(message, ChatMessage) else message
...
class Chat(BaseModel):
    ...
    async def route(self, messages: list[ChatMessage], tools: list[Tool] = []):
        params = {
            "api_key": self.client.api_key,
        ...
        try:
            async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
                response = await client.post(
                    f"{self.client.base_url}/chat/route", json=params
                )
                response.raise_for_status()
                response_json = response.json()
        except httpx.RequestError as e:
