...
def test_key_working():
    ...
    assert len(response.AnswerBuilder.answers[0].data.content) > 10
...
def test_upload_files():
    ...
    response = client.upload_files(files=file_paths, meta=meta, priority_queue=True)
...
def test_client_upload_files_from_bytes():
    ...
    response = client.upload_files(files=tuple_files, meta=meta, priority_queue=False)
    assert response["message"] == "Files Submitted. Indexing tasks created."
