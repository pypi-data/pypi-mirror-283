...
def create_film_component(bl, bw, ft):
    """建立桥区模型, 相关参数: BL/BW/FT"""
    ViewHelper.SetSketchPlane(Plane.PlaneXY, None)
    SketchHelper.StartConstraintSketching()
    ...
    # 拉伸实体
    ViewHelper.SetViewMode(InteractionMode.Solid)
    extrude_faces([GetRootPart().Bodies[0].Faces[0]], MM(ft))
...
def main():
    ...
    film_comp = create_film_component(*(x.value for x in (PS['BL'], PS['BW'], PS['FT'])))
    ...
    set_named_selection([film_comp.Content.Bodies[0].Faces[4]], 'NSF_FILM_TOP')
