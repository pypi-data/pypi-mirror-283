...
def get_merchant_config_host(merchant_id, key_host):
    ...
    if not redis_value:
        ...
        RedisClient().set_value_expire(key_cache=key_cache, value_cache=value_cache, timecache=3600)
        return value_cache
    else:
...
def get_list_app_merchant_register(merchant_id, data_type):
    ...
    if not redis_value:
        ...
        result = response.json()
        data = result.get("data", []) if result and result.get("data", []) else []
        RedisClient().set_value_expire(key_cache=key_cache, value_cache=json.dumps(data, ensure_ascii=False))
        return data
    else:
...
class AppConfig:
    def __init__(self):
        self.connector_register = LRUCacheDict(expiration=1800)
