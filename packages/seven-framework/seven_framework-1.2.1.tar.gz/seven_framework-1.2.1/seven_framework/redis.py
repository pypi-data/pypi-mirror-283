...
:LastEditTime: 2024-07-04 20:01:11
...
class RedisHelper:
    ...
    def redis_cluster_init(cls, config_dict):
        ...
        if "cluster_nodes" in config_dict:
            redis_nodes = config_dict["cluster_nodes"]
        ...
        if not redis_nodes or len(redis_nodes) == 0 or int(db) < 0:
            raise RedisError("Config Value Eroor")
        ...
        # 使用连接池创建RedisCluster对象
        return RedisCluster(connection_pool=connection_pool)
