...
def cli(ctx, verbose, debug, logfile, vendor, workbook, worksheet):  # noqa: PLR0913
    ...
    * A website order
    ...
    logargs = {"format":"%(asctime)s %(levelname)-8s %(message)s",
               "datefmt":"%Y-%m-%d %H:%M:%S"}
    if logfile:
...
def sheet_waves_wrapper(vendor, workbook, worksheet, out, scraped_items_db):
    spreadsheet.sheet_waves(vendor, workbook, worksheet, out, scraped_items_db)
