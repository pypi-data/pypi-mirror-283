...
def deferred_date_modify_widget(node, kw):
    ...
    if not request.has_permission(
        "edit.business_payment_deadline.date", request.context
    ):
...
def customize_business_payment_deadline_schema(schema, request, fields, invoices):
    ...
    if len(invoices) > 0:
        ...
        schema[
            "invoice_id"
        ].description = "Rattacher une facture si elle correspond à cette échéance"
...
def get_business_payment_deadline_edit_schema(request):
    ...
    if request.has_permission(
        "edit.business_payment_deadline.invoice_id", request.context
    ):
        invoices = get_invoices_without_deadline(request, request.context.business)
        if len(invoices) > 0:
            fields += ("invoice_id",)
    if request.has_permission("edit.business_payment_deadline.amount", request.context):
        fields += ("amount_ttc",)
    if request.has_permission("edit.business_payment_deadline.date", request.context):
        fields += ("date",)
