...
def infer_timestamp_column(fields: List):
    ...
    candidate_stubs = ['timestamp', 'Timestamp', 'Timestamp [ms]', 'timestamp [ms]', 'time', 'offset', 'ts', 'seconds']
...
def process_header(header: List[str],
    ...
    if timestamp_key:
        if timestamp_key not in field_names:
            print(f"Error: {timestamp_key} must be a valid key in file {csv_file}: {field_names}")
            timestamp_column = infer_timestamp_column_from_header(field_names)
            print(f"Inferring timestamp column as {field_names[timestamp_column]}")
        else:
...
def csv_to_mcap(
    ...
    with open(csv_file, 'r') as f:
        ...
        field_type_mappings, json_field_schemes = None, None
        start_time_ns = None
        end_time_ns = None
