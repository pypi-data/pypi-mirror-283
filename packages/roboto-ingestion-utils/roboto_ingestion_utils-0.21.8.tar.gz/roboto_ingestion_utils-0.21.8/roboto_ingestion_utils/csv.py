...
def process_header(header: List[str],
    ...
    if timestamp_key:
        assert timestamp_key in field_names, f"Error: {timestamp_key} must be a valid key in file {csv_file}: {field_names}"
...
def csv_to_mcap(
    ...
    field_names, raw_msg_lists, field_type_mappings, json_field_schemes, timestamp_column = initialize_variables()
    ...
    with open(csv_file, 'r') as f:
        ...
        for i, line in enumerate(reader):
            ...
            if i == 0:
                ...
                raw_msg_lists = {field: [] for field in field_names}
            ...
            for j, val in enumerate(line):
                ...
                raw_msg_lists[field].append((timestamp, raw_msg))

    topic_stats = defaultdict(RunningStats)

    topic_mcap_files = dict()

    json_schema_topic = create_json_schema(field_type_dict=field_type_mappings)
    schema_checksum = compute_checksum(json_schema_topic)

    for field_name in field_names:
        file_name = sanitize_filename(f"{csv_file.stem}_{field_name}.mcap")
        topic_mcap_file = output_dir / file_name
        topic_mcap_files[field_name] = str(topic_mcap_file)
        raw_msg_list = raw_msg_lists[field_name]
        # create json schema for topic
        json_schema_topic_per_field = create_json_schema(field_type_dict={field_name: field_type_mappings[field_name]})
        schema_checksum = compute_checksum(json_schema_topic_per_field)

        # parse more advanced schema if the type is json
        if field_type_mappings[field_name] == "json":
            schema = json_field_schemes[field_name].to_schema()
            # update default schema with json info
            json_schema_topic_per_field["properties"][field_name] = schema
        ...
        with open(topic_mcap_file, "wb") as stream:
            ...
            for i, (timestamp, raw_msg) in enumerate(raw_msg_list):
                if timestamp > MAX_UINT64:
                    continue
                json_msg_instance = {field_name: convert_value(raw_msg[0], raw_msg[1])}
    ...
    topic_info_entry["mcap_path"] = topic_mcap_files
    ...
    topic_info_entry["msg_type"] = json_schema_topic
