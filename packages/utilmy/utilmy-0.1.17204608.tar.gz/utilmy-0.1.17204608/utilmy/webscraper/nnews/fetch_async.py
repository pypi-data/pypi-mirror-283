...
no need of assert, just code working

#### You need to understand the concept

Am building a collection of functions

Autonomous Function :

   Input: some argument (ie list of URLS)
   Output: json on disk ONLY. (we dont receive the output, this is only on disk.)

   The function is autonomous: independent, does not depend anything else.


2) How to check the error, failed URL
    we scan the json on disk and check if URL exist and content is fine.
    If not fine, --> do a 2nd pass with new functions.


def read_all_json_on_disk_check_invalid_ones(path )

   new_url_failed= ""


### Fetch again those URL list
...
### this log is alias for  print
from utilmy import log, loge, log2
import threading
...
def test_pipeline_multiprocessing_5_process():
    """
        python fetch_async.py test_pipeline_multiprocessing_5_process
            ...
            ### Pick up one file not yet done #############################
            ### Pick up one file not yet done ################################ Pick up one file not yet done #############################

            ### Pick up one file not yet done #############################
            ### Pick up one file not yet done #############################
            (1, 'The process cannot access the file because another process has locked a portion of the file.')(1, 'The process cannot access the file because another process has locked a portion of the file.')(1, 'The process cannot access the file because another process has locked a portion of the file.')
            ...
            (1, 'The process cannot access the file because another process has locked a portion of the file.')
            (1, 'The process cannot access the file because another process has locked a portion of the file.')
            ### Read url ################################(1, 'The process cannot access the file because another process has locked a portion of the file.')

            ### start fetch ################################(1, 'The process cannot access the file because another process has locked a portion of the file.')(1, 'The process cannot access the file because another process has locked a portion of the file.')
            ...
            starts0 0
            starts 0
            0[['https://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html']]starts0
            [['https://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html']]0
            starts
            0 0[['https://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html']]

            [['https://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html']]0
            [['https://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html']]
            starts 1
            1 startsstartsstarts[['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']]starts 1
            1
            1
            11[['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']]1

            11
            [['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']] [['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']][['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']]
            ...
            job finished
            n_processed 2
            ### start missing 1 ###########################n_processed
            url json files: 20
            1
            job finished
            ### start missing 1 ###########################2
            url json files:n_processed  [['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html'], ['https://books.toscrap2

            0### start missing 1 ###########################
            1
            url json files:job finished 0
            2
            [['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html'], ['https://books.toscrap
            2n_processed  [['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html'], ['https://books.toscrap2
            ...
            thread  1 1
            starts 0
            startsstartsstarts0 0
            00[['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']]

            0[['https://books.toscrape.com/catalogue/tipping-the-velvet_999/index.html']]0
            ...
            Pipeline scrape completed successfully.
            0 job finished
            1 job finished
            0 job finished
            n_processed10  2 job finishedjob finished


            1url json files:  0job finished

            ### start missing 2 ###########################
            #### Report as done ###########################n_processed
            2
            n_processed 2
            url json files: 0url json files:
            ### start missing 2 ###########################0

            Circular reference detected
            ### start missing 2 ############################### Report as done ###########################done


            #### Report as done ###########################
            Pipeline scrape completed successfully.
            Circular reference detected
            Circular reference detecteddone

            donePipeline scrape completed successfully.
...
def pipeline_scrape(dirurlist="ztmp/urlist",
    ...
    dir_index = direport + "filelist_index.txt"
...
def fetch_and_save_googlenews(keywords, dirout="./ztmp/data/urls_search/"):
    """
      output in. json :
    ...
    """
    keywords_list = keywords.split("/n")

    for keywords_k in keywords_list:
        urlsearch, urls_list = urls_fetch_googlenews_single(keywords_k,  pagemax=2,)      
        ddict  = { 'urlsearch': urlsearch,
                   'keywords':  keywords,
                   'url_list':  urls_list 
        }
        ts, tag  = int(time.time()),random.randint(10000, 99999)
...
# @diskcache_decorator
def urls_fetch_googlenews_single(keywords="microsoft funding", pagemax=2,):
    ...
    urls2 = [ {"url": str(prefix + x['url']),  'keywords': comp_name, 
...
def pipeline_scrape_search(dirurlist="ztmp/urlist",
                    dirout="ztmp/urldone",
                    direport="ztmp/report/", npool=4):
    """
        alias scrape=" python fetch_async.py "
    ...
    log("### start fetch ################################")
    fun_async =  fetch_and_save_googlenews
    fetch_parallel_from_textfile(fun_async, urls, npool=len(urls))


    log("### start missing 1 ###########################")
    done_urls2 = get_previous_done_urls(dirout)
    missing_urls2 = list(set(urls) - set(done_urls))
    ...
    ts, tag    = int(time.time()), random.randint(10000, 99999)
    json_save(dd, direport + f"/{ymdhm}_{ts}_{tag}.json")
    log("All done")
...
class IndexLock(object):
    ...
    def __init__(self, findex, file_lock=None, min_size=5, skip_comment=True, ntry=20):
        ...
        os.makedirs(
            os.path.dirname(os.path.abspath(self.findex)),
            exist_ok=True
        )

        if file_lock is None:
            file_lock = os.path.dirname(
                findex) + "/" + findex.split("/")[-1].replace(".", "_lock.")
    ...
    def get_block(self, flist_ref:list, **kw):
        ...
        while i < self.ntry:
            try:
                ...
                fnew = list( set(flist_ref).intersection(set(flist_global_index)))
