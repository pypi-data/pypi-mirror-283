...
def get_llm_response(prompt_request: promptRequest):
    """
...
def get_llm_response_with_enforced_schema(prompt_request: promptRequest):
    """
...
class LLM(object):
    def __init__(self, service='openai', model="gpt-3.5-turbo",
        ...
        """ Various util functions for quick LLM : One Liner method
           ...
           Groq:  800 token /sec. for small Mistral.
                   70b llama3  300 token

           Groq2:  Text cleaning , LLM is very good,         

             20,000 calls to Groq (dataset toc lean up) -->  1 USD
                 text; 1500 token per call.
        ...
        from multiprocessing import Pool
        self.pool0 = Pool()  ### global pool

    def get_save(self, prompt: str, dirout: str = None, output_schema: object = None):
        """
        for single request.Saves output in a json file
        """
        pool = Pool()
        prompt_request = promptRequest(prompt=prompt, service=self.service, modelid=self.modelid, json_schema=output_schema)
        # parellel calls to llm, not required though as single request
        if not output_schema:
            llm_output = pool.apply_async(get_llm_response, [prompt_request])
        else:
            llm_output = pool.apply_async(get_llm_response_with_enforced_schema, [prompt_request])
        result = llm_output.get(timeout=100)
    ...
    def get_save_bulk(self, prompts: list, dirout: str = None, output_schema: object = None):
        """
        multiple request
        saves output in a jsonl file
        """
        # parellel calls to llm

        pool = Pool()
        if not output_schema:
            # parellel calls to llm
            multiple_results = [pool.apply_async(get_llm_response, [
                promptRequest(prompt=prompt, service=self.service, modelid=self.modelid)]) for prompt in prompts]
        else:
            multiple_results = [pool.apply_async(get_llm_response_with_enforced_schema, [
                promptRequest(prompt=prompt, service=self.service, modelid=self.modelid, json_schema=output_schema)
            ]) for prompt in prompts]
            results = list([res.get(timeout=100) for res in multiple_results])
            ...
            # json_save expects dict so using custom method
