    ...
    def combine_sheets(cls, file_path, data_product_id, environment):
        ...
        with tracer.start_as_current_span("combine_sheets", attributes={"data_product_id": str(data_product_id), "environment": str(environment)}):
            try:
                ...
                new_columns = ['data_product_id', 'code_type', 'valueset_code', 'feature_name', 'parent_valueset_code', 'parent_concept_code', 'concept_code', 'concept_name',  'concept_sort', 'description','category_valueset_code','category_concept_code', 'item_visibility', 'item_color', 'sheet_name']
                ...
                for sheet in sheets[last_list_index + 1:]:
                    ...
                    for col in new_columns:
                        if col not in df_sheet.columns:
                            if col == 'data_product_id':
                                df_sheet[col] = data_product_id
                            elif col == 'code_type':
                            ...
                            elif col == 'concept_code':
                                df_sheet[col] =  df_sheet['item_code'].astype(str)
                            elif col == 'concept_name':
                                df_sheet[col] =  df_sheet['item_name'].astype(str)
                                ...
                                df_sheet[col] =  df_sheet['category_list'].astype(str)
