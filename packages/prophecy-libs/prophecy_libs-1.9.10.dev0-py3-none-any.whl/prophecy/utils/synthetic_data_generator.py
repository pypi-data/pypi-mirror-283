...
from pyspark.sql.types import StructType, StructField, StringType, IntegerType, FloatType, BooleanType, DateType, TimestampType, LongType
from .faker import Faker
import random
from datetime import datetime
from typing import Any, Optional
from .dill import pickle # using in-house dill
...
class FakeDataFrame:
    ...
    def addColumn(self, column_name, faker_function: BaseFakerFunction, data_type=None, nulls=0, unique=False):
        ...
        if unique:
            self.faker_methods_dict[column_name] = ("unique_" + self.faker_methods_dict[column_name][0], self.faker_methods_dict[column_name][1])
    ...
    def build(self):
        ...
        null_indices = {column: random.sample(range(total_rows), nulls) for column, nulls in self.null_columns_dict.items()}
