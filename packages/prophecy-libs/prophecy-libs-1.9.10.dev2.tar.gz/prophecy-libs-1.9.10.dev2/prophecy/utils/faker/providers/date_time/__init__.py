...
from ...custom_typing import Country, DateParseType
...
def timestamp_to_datetime(timestamp: Union[int, float], tzinfo: Optional[TzInfo]) -> datetime:
    ...
    if tzinfo is None:
        ...
        pick = convert_timestamp_to_datetime(timestamp, local_timezone)
...
class Provider(BaseProvider):
    ...
    def pytimezone(self, *args: Any, **kwargs: Any) -> Optional[TzInfo]:
        ...
        COMMON_TIMEZONES = [
            ...
            ("CST", 8 * 3600),  # China Standard Time, UTC + 8
            ("JST", 9 * 3600),  # Japan Standard Time, UTC + 9
        ]

        name, offset = random.choice(COMMON_TIMEZONES)
        return timezone(timedelta(seconds=offset), name)

        # timezone_str = self.timezone(*args, **kwargs)
