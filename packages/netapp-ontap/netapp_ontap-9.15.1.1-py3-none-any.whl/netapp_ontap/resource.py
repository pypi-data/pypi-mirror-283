...
class Resource:  # pylint: disable=too-many-instance-attributes
    ...
    def instance_location(self) -> str:
        ...
        if self._last_state:
            representative = self.__class__.from_dict(self._last_state)
        keys = self._keys.copy() #make a copy so that we can add svm.uuid the DNS resource
        if self.__module__ == "netapp_ontap.resources.dns" and not getattr(self, "uuid", None):
        ...
        if config.STRICT_GET:
            ...
            for key in [key for key in self._keys if key in key_diff]:
                try:
                    ...
                    raise NetAppRestError(message=f"Not all of the keys required are present. Missing a value for '{key}'.") from None
    ...
    def _parse_json_response(self, response: requests.Response) -> None:
        ...
        if 'record' in response_dict:
            ...
            if response_dict.get("num_records", 0) == 1:
                ...
                num_records = "more than one record" if response_dict.get("num_records", 0) > 1 else "no records"
                raise NetAppRestError(
                    message="Received %s in the response %s."
                    % (num_records, self.__class__.__name__)
                )
