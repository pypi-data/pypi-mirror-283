...
def _to_qfs_vector(
    ...
    *,
    ...
    if all(isinstance(x, int) for x in collection):
        if data_type == "int[]":
            array = _to_java_array(
                collection, gateway=gateway, array_type=gateway.jvm.int
            )
    ...
    if all(isinstance(x, (float, int)) for x in collection):
        if data_type == "float[]":
            array = _to_java_array(
                [float(cast(Union[int, float], x)) for x in collection],
                gateway=gateway,
    ...
    array = to_java_object_array(collection, gateway=gateway)
...
def to_python_list(
    ...
    return list(cast(Any, java_list).toArray())
