...
class CumulativeScope(BaseScope):
    ...
    ...     m["Quantity L"], scope=tt.CumulativeScope(level=l["Day"])
    ... )
    >>> m["Dense cumulative quantity L"] = tt.agg.sum(
    ...     m["Quantity L"], scope=tt.CumulativeScope(level=l["Day"], dense=True)
    ... )
    >>> cube.query(
    ...     m["Quantity.SUM"],
    ...     m["Quantity L"],
    ...
    If not ``None``, :attr:`partitioning` must part of the same hierarchy as :attr:`level` and be "above" it (i.e. before it in :attr:`atoti.Hierarchy.levels`).
