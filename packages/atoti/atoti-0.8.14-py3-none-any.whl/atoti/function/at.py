...
def at(
    ...
    coordinates: Condition[
        LevelIdentifier,
        Literal["eq"],
        Union[Constant, LevelIdentifier],
    ...
    target_values: list[Optional[ConstantValue]] = []

    for subject, target in condition_to_dict(coordinates).items():
        levels.append(subject._java_description)

        if isinstance(target, LevelIdentifier):
