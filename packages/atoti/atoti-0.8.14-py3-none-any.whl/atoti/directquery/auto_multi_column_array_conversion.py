...
class AutoMultiColumnArrayConversion:
    """Pass it to a DirectQuery ``*ConnectionInfo`` class to automatically convert all external tables with array values stored with one element per column to tables with array columns.
    ...
    Example:
        ...
        ...     options=SnowflakeTableOptions(keys=["PRODUCT"]),
        ... )
