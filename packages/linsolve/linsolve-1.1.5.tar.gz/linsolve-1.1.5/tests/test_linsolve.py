...
class TestLinSolve:
    ...
    def test_infer_dtype(self):
        assert linsolve.infer_dtype([1.0, 2.0]) == np.float32
        ...
        assert linsolve.infer_dtype([np.complex64(1), 4.0]) == np.complex64
        assert (
            linsolve.infer_dtype([np.complex128(1), np.float64(4.0)]) == np.complex128
        )
        assert linsolve.infer_dtype([np.complex64(1), np.float64(4.0)]) == np.complex128
        assert linsolve.infer_dtype([np.complex64(1), np.int32(4.0)]) == np.complex64
        assert linsolve.infer_dtype([np.complex64(1), np.int64(4.0)]) == np.complex64
...
class TestLinearSolver:
    ...
    def test_get_A(self):
        ...
        np.testing.assert_equal(A, np.array([[[1.0], [1]], [[1.0], [-1]]]))

    def test_solve(self):
        sol = self.ls.solve()
        np.testing.assert_almost_equal(sol["x"], 1.0)
        np.testing.assert_almost_equal(sol["y"], 2.0)
    ...
    def test_A_shape(self):
        ...
        ls = linsolve.LinearSolver({"a*x+b*y": 0.0}, {"a*x+b*y": 1}, **consts)
    ...
    def test_nonunity_wgts(self):
        ...
        d, w = {}, {}
        for eq in eqs:
            d[eq], w[eq] = eval(eq) * np.ones(4), 2 * np.ones(4)
        ls = linsolve.LinearSolver(d, w, a=a, b=b, sparse=self.sparse)
        sol = ls.solve()
        np.testing.assert_almost_equal(sol["x"], x * np.ones(4, dtype=np.float64))
        np.testing.assert_almost_equal(sol["y"], y * np.ones(4, dtype=np.float64))
    ...
    def test_chisq(self):
        ...
        sol = ls.solve()
        chisq = ls.chisq(sol)
        np.testing.assert_almost_equal(chisq, 0.5)
        x = 1.0
        d = {"x": 1, "1.0*x": 2}
    ...
    def test_dtypes(self):
        ...
        ls = linsolve.LinearSolver(
            {"x_": np.ones(1, dtype=np.complex64)[0]}, sparse=self.sparse
        )
        # conjugation should trigger re_im_split, splitting the
        # complex64 type into two float32 types
        assert ls.dtype == np.float32
        ...
        ls = linsolve.LinearSolver(
            {"x": np.ones(1, dtype=np.complex64)[0]}, sparse=self.sparse
        )
        assert ls.dtype, np.complex64
        assert type(ls.solve()["x"]) == np.complex64

        ls = linsolve.LinearSolver(
            {"c*x": np.array(1.0, dtype=np.float32)}, c=1.0 + 1.0j, sparse=self.sparse
        )
        assert ls.dtype == np.complex64
        assert type(ls.solve()["x"]) == np.complex64
    ...
    def test_degen_sol(self):
        ...
        d = {"x+y": 1.0, "2*x+2*y": 2.0}
        ls = linsolve.LinearSolver(d, sparse=self.sparse)
        for mode in ("pinv", "lsqr"):
            sol = ls.solve(mode=mode)
            np.testing.assert_almost_equal(sol["x"] + sol["y"], 1.0, 6)
...
class TestLogProductSolver:
    ...
    def test_init(self):
        x, y, z = np.exp(1.0 + 0j), np.exp(2.0), np.exp(3.0)
        keys = ["x*y*z", "x*y", "y*z"]
        d, w = {}, {}
        for k in keys:
            d[k], w[k] = eval(k), 1.0
    ...
    def test_solve(self):
        x, y, z = np.exp(1.0 + 1j), np.exp(2.0 + 2j), np.exp(3.0 + 3j)
        keys = ["x*y*z", "x*y", "y*z"]
        d, w = {}, {}
        for k in keys:
            d[k], w[k] = eval(k), 1.0
    ...
    def test_conj_solve(self):
        x, y = np.exp(1.0 + 2j), np.exp(2.0 + 1j)
    ...
    def test_no_abs_phs_solve(self):
        ...
        np.testing.assert_almost_equal(np.angle(x * y.conjugate()), 0.0)
        np.testing.assert_almost_equal(np.angle(x * z.conjugate()), 0.0)
        np.testing.assert_almost_equal(np.angle(y * z.conjugate()), 0.0)
        # check projection of degenerate mode
        np.testing.assert_almost_equal(np.angle(x), 0.0)
        np.testing.assert_almost_equal(np.angle(y), 0.0)
        np.testing.assert_almost_equal(np.angle(z), 0.0)

    def test_dtype(self):
        for dtype in (np.float32, np.float64, np.complex64, np.complex128):
            x, y, z = np.exp(1.0), np.exp(2.0), np.exp(3.0)
            keys = ["x*y*z", "x*y", "y*z"]
            d, w = {}, {}
            for k in keys:
...
class TestLinProductSolver:
    ...
    def test_complex_array_solve(self):
        x = np.arange(30, dtype=np.complex128)
        x.shape = (3, 10)
        y = np.arange(30, dtype=np.complex128)
        y.shape = (3, 10)
        z = np.arange(30, dtype=np.complex128)
        z.shape = (3, 10)
        d, w = {"x*y": x * y, "x*z": x * z, "y*z": y * z}, {}
        for k in list(d.keys()):
            w[k] = np.ones(d[k].shape)
        sol0 = {}
        for k in "xyz":
            sol0[k] = eval(k) + 0.01
        ls = linsolve.LinProductSolver(d, sol0, w, sparse=self.sparse)
    ...
    def test_complex_array_NtimesNfreqs1_solve(self):
        x = np.arange(1, dtype=np.complex128)
        x.shape = (1, 1)
        y = np.arange(1, dtype=np.complex128)
        y.shape = (1, 1)
        z = np.arange(1, dtype=np.complex128)
        z.shape = (1, 1)
        d, w = {"x*y": x * y, "x*z": x * z, "y*z": y * z}, {}
        for k in list(d.keys()):
            w[k] = np.ones(d[k].shape)
    ...
    def test_sums_of_products(self):
        ...
        for ex in expressions:
            data[ex] = eval(ex)
        currentSol = {"x": 1.1 * x, "y": 0.9 * y, "z": 1.1 * z, "w": 1.2 * w}
        for i in range(
            5
