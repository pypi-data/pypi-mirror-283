...
class MergeEntities (GenericCRUDML):
    def merge_entities(self, entity_id1: int, entity_id2: int, main_entity_ml_id: int = None):
        ...
        old_id = end_id
        new_id = main_id
        mlids = gcrml.get_ml_ids_by_id(table_id=old_id,ml_column_name='city_ml_id',order_by=DEFAULT_ORDER_BY, ml_table_name='city_ml_table', column_name='city_id', compare_view_name='city_ml_view')
                # Set the main id
