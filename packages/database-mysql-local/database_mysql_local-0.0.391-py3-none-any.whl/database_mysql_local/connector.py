...
connections_pool = defaultdict(
    dict)  # TODO: test multiple users. Maybe we have an issue with MySQL connection pool, that we should also check the user when we return the connection pool from the cache. Shall we add RDS_*2 environment variables, and you will add test to connect to the same schema with another RDS_USER
...
class Connector(metaclass=MetaLogger, object=LOGGER_CONNECTOR_CODE_OBJECT):
    ...
    def _connect_to_db(self):
        ...
        #         self.logger.error(f"Failed to connect to the database via SSH tunnel: {exception}")
        #         if self.tunnel:
        #             self.tunnel.stop()
        #         raise exception
