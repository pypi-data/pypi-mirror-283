...
connections_pool = defaultdict(dict)  # TODO: test multiple users. Maybe we have an issue with MySQL connection pool, that we should also check the user when we return the connection pool from the cache. Shall we add RDS_*2 environment variables, and you will add test to connect to the same schema with another RDS_USER
...
class Connector(metaclass=MetaLogger, object=LOGGER_CONNECTOR_CODE_OBJECT):
    @staticmethod
    def connect(schema_name: str, *, user: str = get_sql_hostname(), ignore_cache: bool = False) -> Connector:
        if ignore_cache and schema_name in connections_pool[user]:
            connections_pool[user].pop(schema_name)

        if (schema_name in connections_pool[user] and
                connections_pool[user][schema_name] and
                connections_pool[user][schema_name].connection):
