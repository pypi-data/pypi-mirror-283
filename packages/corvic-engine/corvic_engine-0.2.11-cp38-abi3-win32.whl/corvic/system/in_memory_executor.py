...
def _patch_list_field(
    ...
    old_list_type = pa.list_(old_field_type.value_field.type)
    ...
    if not pa.types.is_null(new_list_field.type):
        return new_field
    return new_field.with_type(old_list_type)
...
def _patch_struct_field(
    ...
    for field_index in range(new_field_type.num_fields):
        field = new_field_type.field(field_index)
        old_nested_field_type = old_struct.field(
            old_struct.get_field_index(field.name)
        ).type
        if pa.types.is_struct(field.type):
            patched_nested_fields.append(
                _patch_struct_field(field, field.with_type(old_nested_field_type))
            )
        elif pa.types.is_list(field.type) or pa.types.is_large_list(field.type):
            patched_nested_fields.append(
                _patch_list_field(field, field.with_type(old_nested_field_type))
            )
        elif not pa.types.is_null(field.type):
            patched_nested_fields.append(field)
        elif old_nested_field_type and not pa.types.is_null(old_nested_field_type):
            patched_nested_fields.append(field.with_type(old_nested_field_type))
        else:
            patched_nested_fields.append(field)
