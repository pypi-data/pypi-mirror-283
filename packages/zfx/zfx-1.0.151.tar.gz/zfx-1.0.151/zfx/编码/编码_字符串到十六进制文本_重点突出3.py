def 编码_字符串到十六进制文本_重点突出3(完整字符串, 重点字符串1, 重点字符串2, 重点字符串3):
    """
    将字符串转换为16进制文本，并将属于重点字符的十六进制字节以不同颜色显示。

    参数:
        - 完整字符串 (str): 要转换的字符串。
        - 重点字符串1 (str): 需要重点突出的第一个字符。
        - 重点字符串2 (str): 需要重点突出的第二个字符。
        - 重点字符串3 (str): 需要重点突出的第三个字符。
    ...
    示例:
        ...
        重点字符串2 = "World"
        重点字符串3 = "o"
        编码_字符串到十六进制文本_重点突出(完整字符串, 重点字符串1, 重点字符串2, 重点字符串3)
        # 输出: 48 65 6c 6c 6f 2c 20 57 6f 72 6c 64 21
        # 并且 "Hello" 的十六进制文本以红色输出，"World" 的十六进制文本以绿色输出，"o" 的十六进制文本以黄色输出
    """
    try:
        ...
        while i < len(byte_text):
            ...
            if char in 重点字符串1:
                ...
                # 黄色显示
                hex_text_list.append(f"\033[93m{hex_str}\033[0m")
            else:
