...
def save_states(state: TrainState, samples):
    ...
    global i_checkpoint
    checkpoint_dir = ARTEFACT_DIR / MODEL / "checkpoints"
    checkpoint_dir.mkdir(exist_ok=True, parents=True)
    sample_dir = ARTEFACT_DIR / MODEL / "samples"
...
def train_step(train_state: TrainState, key):
    ...
    logs.add_metric("perplexity(train)", loss_train)
    loss_test = evaluate(train_state.params, states, X_test)
    logs.add_metric("perplexity(test)", loss_test)
...
keys = random.split(key, num=n_checkpoint_iterations)
state, history, elapsed = ciclo.loop(
    ...
    tasks={
        ciclo.every(1): [
            train_step,
            ciclo.checkpoint(ARTEFACT_DIR / MODEL),
            ciclo.keras_bar(n_checkpoint_iterations),
        ],
    },
    stop=n_checkpoint_iterations,
)
