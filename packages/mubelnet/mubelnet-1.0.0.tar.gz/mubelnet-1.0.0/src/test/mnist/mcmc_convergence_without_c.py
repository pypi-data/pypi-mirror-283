...
def build_gamma_belief_net():
    ...
    model.layers.layers[1].frozen = model.layers.layers[1].frozen_ = frozenset(["c"])
    return model
...
c = param_init["multinomial_dirichlet_believe/~/dirichlet_layer"]["c"]
param_init["multinomial_dirichlet_believe/~/dirichlet_layer"]["c"] = jnp.ones_like(c)
