    ...
    def _verify_indices(self, start: int, stop: Optional[int]) -> tuple[int, Optional[int]]:
        ...
        if start < 0 or start >= array_length:
            message = (
                ...
                f"{start}. The index is outside the valid start index range ({0}:{array_length - 1})."
            )
        ...
        if stop is not None and (stop < 1 or stop > array_length):
            message = (
                ...
                f"{stop}. The index is outside the valid stop index range ({1}:{array_length})."
            )
            console.error(message=message, error=IndexError)
            raise IndexError(message)
        ...
        if stop is not None and start > stop:
            message = (
                ...
                f"index ({start}) is greater than the end index ({stop}), which is not allowed."
            )
            console.error(message=message, error=ValueError)
