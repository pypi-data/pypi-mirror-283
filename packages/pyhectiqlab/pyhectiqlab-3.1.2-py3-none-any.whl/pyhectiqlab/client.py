...
from pyhectiqlab.utils import ProgressHandler
...
class Client:
    ...
    def upload_fragment_sync(local_path: str, policy: dict, chunk_size: int = 1024 * 1024 * 32) -> None:
        ...
        task_id = Client._progress.add_task(task_desc, total=upload.total_bytes)
        ...
        while upload.finished == False:
            ...
            Client._progress.update(task_id, advance=upload.bytes_uploaded - bytes_uploaded)
            bytes_uploaded = upload.bytes_uploaded
        Client._progress.stop_task(task_id)
    ...
    def upload_single_sync(local_path: str, policy: dict, bucket: str) -> ResponseType:
        ...
        with open(local_path, "rb") as content:
            ...
            filename = os.path.basename(local_path)
            task_desc = f"Upload {filename if len(filename) <= 15 else filename[:12] + '...'}"
            task_id = Client._progress.add_task(task_desc, total=num_bytes)

            res = upload_method(url, data=policy.get("fields"), files=files)
        Client._progress.update(task_id, advance=num_bytes)
        Client._progress.stop_task(task_id)
        return res
        ...
        with open(local_path, "wb") as f:
            ...
            with httpx.stream("GET", url) as r:
                for data in r.iter_bytes():
                    f.write(data)
                    Client._progress.update(task_id, advance=len(data))
        return local_path
    ...
    def initiate_progress():
        ...
        try:
            ...
            msg = (
                ...
                "the script. Continuing without progress."
            )
            logger.warning(msg)
