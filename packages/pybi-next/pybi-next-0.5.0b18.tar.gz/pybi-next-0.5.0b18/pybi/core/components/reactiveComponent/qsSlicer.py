...
class QsSlicer(SingleReactiveComponent):
    def __init__(self, sql: SqlInfo) -> None:
        super().__init__(ComponentTag.QsSlicer, sql)
        self.title = ""
    ...
    def set_multiple(self, multiple: bool):
        return self.set_props({"multiple": multiple})

    def set_props(self, props: Dict):
        """
        [slicer props](http://www.quasarchs.com/vue-components/select#qselect-api)
        e.g
