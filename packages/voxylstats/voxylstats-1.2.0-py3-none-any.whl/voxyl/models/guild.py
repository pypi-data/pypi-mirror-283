...
class Guild:
    ...
    def __post_init__(self, _info_data: dict, _member_data: dict):
        ...
        for i in _member_data:
            self.all_members.append(GuildMember(i, guild_id=self.guild_id))

            if i["role"] == "OWNER":
                self.owner = GuildMember(i, guild_id=self.guild_id)

            else:
                self.__dict__[f'{i["role"].lower()}s'].append(
                    GuildMember(i, guild_id=self.guild_id)
                )
