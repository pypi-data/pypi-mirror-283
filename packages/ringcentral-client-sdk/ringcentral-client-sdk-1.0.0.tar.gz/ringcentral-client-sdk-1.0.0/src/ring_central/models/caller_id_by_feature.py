...
class CallerIdByFeatureFeature(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, CallerIdByFeatureFeature._member_map_.values())
        )


@JsonMap({"caller_id": "callerId"})
class CallerIdByFeature(BaseModel):
    """Caller ID settings by feature

    :param feature: feature, defaults to None
    :type feature: CallerIdByFeatureFeature, optional
    :param caller_id: caller_id, defaults to None
    :type caller_id: CallerIdByFeatureInfo, optional
    """

    def __init__(
        self,
        feature: CallerIdByFeatureFeature = None,
        caller_id: CallerIdByFeatureInfo = None,
    ):
        if feature is not None:
            self.feature = self._enum_matching(
                feature, CallerIdByFeatureFeature.list(), "feature"
            )
        if caller_id is not None:
            self.caller_id = self._define_object(caller_id, CallerIdByFeatureInfo)
