...
You'll also need to make sure the URL "https://pypi.org/project/examplor-pypi-package/" (replace `examplor-pypi-package` by your package name, with all `_` becoming `-`) is not occupied.
...
Don't be fooled by the URL "[pypi.org/project/examplor-pypi-package/](https://pypi.org/project/examplor-pypi-package/)" and the name "examplor-pypi-package" on pypi.org. pypi.org and pip system convert all `_` to `-` and use the latter on the website / in `pip` command, but the real name is still with `_`, which users should use when importing the package.
...
After your package is published on PyPI, go to [https://pypi.org/project/examplor-pypi-package/](https://pypi.org/project/examplor-pypi-package/) (`_` becomes `-`). Copy the command on the page, execute it to download and install your package from PyPI. (or test.pypi.org if you use that)
