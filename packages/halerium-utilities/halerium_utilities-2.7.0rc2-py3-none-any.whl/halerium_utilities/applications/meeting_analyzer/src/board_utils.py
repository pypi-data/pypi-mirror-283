...
def create_meeting_board(
    ...
    if meeting_template:
        ...
        # if there is a meeting template, load it
        with open(meeting_template, "r") as template:
            meeting_template = json.load(template)

        # instantiate a new board from the template
        board = Board(meeting_template)

        # get all notes in the board that are of type "note" and have the title "Transcript"
        transcript_notes = [
            n
            for n in board.cards
            if n.type == "note" and n.type_specific.title == "Transcript"
        ]
    ...
    with open(meeting_board_path, "w") as meeting_board:
        json.dump(board.to_dict(), meeting_board)
