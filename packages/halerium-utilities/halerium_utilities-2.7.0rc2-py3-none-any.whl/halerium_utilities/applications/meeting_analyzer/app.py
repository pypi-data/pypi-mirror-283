...
from .src.infostore_handler import push_summaries_to_infostore, push_transcript_to_infostore, fetch_infostore
...
try:
    ...
    print(f"No store with the name {summary_info_store_name} found.")
    raise exc
...
root_path = (
    f"/apps/{os.getenv('HALERIUM_ID')}/{str(port_app)}/" if os.getenv("HALERIUM_ID") else ""
)
frontend_path = (
...
# chatbot link
chatbot_link = f"{os.getenv('HALERIUM_BASE_URL')}/apps/{os.getenv('HALERIUM_ID')}/{port_chatbot}"
app_link = f"{os.getenv('HALERIUM_BASE_URL')}/apps/{os.getenv('HALERIUM_ID')}/{port_app}"
...
async def record_and_transcribe(websocket: WebSocket):
    """
    ...
    try:
        ...
        while True:
            ...
            if "text" in data:
                ...
                if message["type"] == "audio-start":
                    ...
                    print(f"User started recording. Samplerate: {samplerate}")

                elif message["type"] == "audio-end":
                    print("Stopped recording. Sending session id to frontend.")
                    filename = audio_processor.export_mp3()
                    print(f"Received filename: {filename}")
                    try:
                        audio_b64 = ""
                        with open(filename, "rb") as uploaded_file:
                            audio_b64 = base64.b64encode(uploaded_file.read()).decode(
                                "utf-8"
                            )
        ...
        logger.debug(f"AudioWebSocket disconnected.")
...
@app.get("/analyze_transcript", response_class=HTMLResponse)
async def analyze_transcript(request: Request, session_id: str):
    ...
    meeting_board_path = create_meeting_board(transcript,
...
async def transcribe_and_analyze_files(
    ...
    # create a meeting board from the transcript
    meeting_board_path = create_meeting_board(full_transcript,
                                              meetings_path=meetings_path,
                                              meeting_template=meeting_template)
...
async def approve_and_postprocess(
    ...
    if meeting_board_path and approved_analysis:
        ...
        push_summaries_to_infostore([summaries], info_store_name=summary_info_store_name) # push the summaries to infostore
        push_transcript_to_infostore(transcript, info_store_name=transcript_info_store_name)


        return {"status": "success", "message": "Analysis successfully approved and pushed to infostore."}
