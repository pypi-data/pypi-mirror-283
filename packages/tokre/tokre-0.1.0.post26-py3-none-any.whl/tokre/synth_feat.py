...
class SynthFeat(nn.Module):
    def __init__(self, pattern, match_aggr: str='longest', prefix: Optional[str]=None, feat_tag: Optional[str]=None, lr=1e-2, disable_parallel=False, disable_tqdm=False):
        super().__init__()
    ...
    def train(self, acts, tok_ids, n_actors=None, batch_size=1):
        ...
        pbar = range(0, len(train_data), batch_size)
        ...
        for i in pbar:
            ...
            if i % 200 == 0 and not self.disable_tqdm:
    ...
    def get_groups_per_doc(self, tok_ids, n_actors=None, batch_size=50, max_per_doc=False):
        if self.disable_parallel is True:
            pbar = tok_ids
            if not self.disable_tqdm:
                pbar = tqdm(pbar, desc='Loading tok_ids with matcher.findall')
            groups_per_doc = [self.matcher.findall(doc_ids, return_group=True) for doc_ids in pbar]
        else:
    ...
    def get_acts(self, tok_ids, n_actors=None, match_batch_size=50, pred_batch_size=50):
        if n_actors is None and self.disable_parallel is False:
        ...
        if self.disable_parallel:
            with torch.no_grad():
                per_match_preds =[(doc_idx, self.model(group_data)) for doc_idx, group_data in doc_group_pairs]
        else:
