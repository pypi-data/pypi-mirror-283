...
class SpotDIPy:
    ...
    def set_limb_darkening_params(self, mh, law='linear', model='mps2', mu_min=0.1, data_path='exotic_ld_data'):
        ...
        if dipu.isfloatable(mh) is False:
            raise ValueError("'mh' keyword must be a float value")
        ...
        self.ld_params = {'mh': mh, 'law': law, 'model': model, 'mu_min': mu_min, 'data_path': data_path}
    ...
    def prep_ld_and_int_lpt(self, info=False, grid_search=False):
        ...
        for mode in self.conf:
            if mode != 'lc':
                ld_rgi = dipu.ldcs_rgi_prep(teffps=teffps, teffcs=teffcs, teffhs=teffhs, loggs=loggs,
                                            law=self.ld_params['law'], mh=self.ld_params['mh'],
                                            wrange=self.conf[mode]['wave_range'], model=self.ld_params['model'],
                                            data_path=self.ld_params['data_path'], mu_min=self.ld_params['mu_min'],
                ...
                ld_rgi = dipu.ldcs_rgi_prep(teffps=teffps, teffcs=teffcs, teffhs=teffhs, loggs=loggs,
                                            law=self.ld_params['law'],  mh=self.ld_params['mh'],
                                            passband=self.conf[mode]['passband'], model=self.ld_params['model'],
                                            data_path=self.ld_params['data_path'],  mu_min=self.ld_params['mu_min'],
                                            cpu_num=self.cpu_num)
        ...
        if not grid_search:
            if 'lc' not in self.conf and len(self.conf) == 1:
                ...
                ld_rgi = dipu.ldcs_rgi_prep(teffps=teffps, teffcs=teffcs, teffhs=teffhs, loggs=loggs,
                                            law=self.ld_params['law'], mh=self.ld_params['mh'],
                                            wrange=[np.min(wrange), np.max(wrange)], model=self.ld_params['model'],
                                            data_path=self.ld_params['data_path'],  mu_min=self.ld_params['mu_min'],
                                            cpu_num=self.cpu_num)
    ...
    def objective_func(self, x0s, alpha, beta, gamma, delta, lmbd, cpcs):
        ...
        for mode in self.conf:
            if mode != 'lc':
                ...
                """"""
                # for i in range(len(sprf)):
                #     rle = (0.0 + jnp.mean(sprf[i]) / x0s[-1] * jrnd.normal(self.key, shape=(self.idc[mode]['nop'],)))
                #
                #     sprf = sprf.at[i].set(sprf[i] + rle)
                """"""
    ...
    def minimize(self, x0s, minx0s, maxx0s, maxiter, tol, alpha, beta, gamma, delta, lmbd, cpcs, disp):
        ...
        """"""
        # fake_fssc = np.loadtxt('fake_fssc.txt')
        # fake_fssh = np.loadtxt('fake_fssh.txt')
        # x0s = np.hstack((fake_fssc, fake_fssh))
        # ftot, others = self.objective_func(x0s, alpha, beta, gamma, delta, lmbd, cpcs)
        """"""
    ...
    def reconstructor(self, alpha=1.0, beta=1.0, gamma=1.0, delta=1.0, lmbd=1.0, maxiter=100, tol=1e-10, disp=True,
        ...
        if cpcs and 'lc' not in self.conf:
            self.opt_results['lc']['ntimes'] = (self.params['t0'] + np.linspace(0, 2, 200)
                                                * self.params['period'])
        ...
        """"""
        # spotless_fss = jnp.ones(2 * noes) * self.fsl
        # x0s = jnp.hstack((spotless_fss, jnp.zeros(self.idc['nofp']), 100))
        #
        # minx0s = jnp.array([self.fsl] * (2 * noes) + [-jnp.inf] * self.idc['nofp'] + [1])
        # maxx0s = jnp.array([self.fsu] * (2 * noes) + [jnp.inf] * self.idc['nofp'] + [jnp.inf])
        """"""
    ...
    def test(self, spots_params, modes_input, opt_params=None, plot_params=None):
        ...
        # np.savetxt('fake_fssc.txt', fake_fssc)
        # np.savetxt('fake_fssh.txt', fake_fssh)
