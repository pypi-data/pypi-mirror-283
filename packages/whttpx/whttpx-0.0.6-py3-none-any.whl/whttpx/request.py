...
class Request:
  ...
  async def get(self, server:str, url:str='', timeout:int=60, need_token=False, type_response='JSON'):
    '''
    ...
    return self.prepare_response(type_response, response)
  
  async def post(self, server:str, url:str, appJson:dict=None, FormData:dict=None, Files=None, timeout:int=60, need_token=False, type_response='JSON'):
    '''
    If you need to authenticate with jtw, go through the option of the server to authenticate
    `type_response` refers to how the server will respond, can be `STRING` or `JSON`
    '''
