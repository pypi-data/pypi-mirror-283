...
class PortfolioOne:
    ...
    def __init__(self, data, security_dao, etf_dao, config=None):
        ...
        self.etf_dao = etf_dao
        self.data = self.security_dao.map(data)
        self._init_weight()

    def _get_weight_one(self, df):
        """
        Get a weight pd.Series for one user portfolio/basket.
        """
    ...
    def _get_weight_list(self):
        """
        Get the weight dict for all user portfolios/baskets.
        """
        data = self.data
        config = self.security_dao.config
        portfolio_col = config.get('portfolio_col')
    ...
    def get_portfolio_weight_list(self):
        """Get the weight dict for all user portfolios/baskets (cached).

        Returns:
            dict: Dictionary of weight pd.Series
        """
    ...
    def get_etf_weight_list(self):
        """Get the weight dict for ETFs (cached).
    ...
    def get_weight_list(self):
        weight_list = self.get_portfolio_weight_list().copy()
