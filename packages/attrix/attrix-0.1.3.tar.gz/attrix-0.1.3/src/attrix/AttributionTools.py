...
class AttributionData:
    def __init__(self, portfolio, rmd, security_return):
        ...
        self.user_basket_ids = portfolio.get_user_basket_ids()
        self._init_data(rmd)
    ...
    def _init_idm(self, rmd):
        index_H = self.index_H
        etf_ids = self.etf_ids
        user_basket_ids = self.user_basket_ids

        logging.debug('Reserve meta data for stocks')
        idm = rmd.get('idm')
        meta = idm.columns  # TODO: meta columns customization
        idm = idm.loc[index_H, meta]

        logging.debug('Reserve meta data for ETFs')
        etf_idm = pd.DataFrame(index=etf_ids, columns=meta)
        if 'comp_idm' in rmd.keys():
            comp_idm = rmd.get('comp_idm')
            comp_ids = list(set(etf_ids) & set(comp_idm.index))
            etf_idm.loc[comp_ids, :] = comp_idm.loc[comp_ids, :].values
        
        logging.debug('Reserve meta data for user baskets')
        user_basket_idm = pd.DataFrame(index=user_basket_ids, columns=meta)

        idm = pd.concat([idm, etf_idm, user_basket_idm], axis=0)
        self.idm = idm
...
class AttributionOne:
    ...
    def get_stock_master(self):
        return self.idm.loc[self.index_h,:]
