...
def parse_row(tr: Tag) -> Either[ParsingError, Pairing]:
  try:
    cols = tr.find_all('td')
    ...
    if black_player == "No Opponent":
      return Right(Unpaired(player=white_player, reason='No Opponent'))
    else:
      return Right(Paired(
...
def parse_round(soup: BeautifulSoup) -> RoundPairings:
  ...
  pairs = list(E.filter([parse_row(r) for r in rows]))
  if not pairs:
