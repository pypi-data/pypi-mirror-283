...
from aliyun.instrumentation.context.context import get_custom_attributes
...
def payload_to_semantic_attributes(
    ...
    if EventPayload.CHUNKS in payload and EventPayload.EMBEDDINGS in payload:
        attributes[EMBEDDING_EMBEDDINGS] = [
            {EMBEDDING_TEXT: text, EMBEDDING_VECTOR: vector,EMBEDDING_VECTOR_SIZE: len(vector)}
    ...
    if EventPayload.PROMPT in payload:
        key = f"{LLM_PROMPTS}.0.{SpanAttributes.CONTENT}"
        attributes[key] = payload[EventPayload.PROMPT]
    ...
    if EventPayload.SERIALIZED in payload:
        ...
        if event_type is CBEventType.LLM:
            ...
            if model_name is None:
                model_name = serialized.get("model")
            if model_name:
                attributes[GEN_AI_MODEL_NAME] = model_name
                attributes[GEN_AI_REQUEST_MODEL_NAME] = model_name
            ...
            if temperature := serialized.get("temperature"):
                attributes[GEN_AI_REQUEST_TEMPERATURE] = temperature
            if max_tokens := serialized.get("max_tokens"):
...
class OpenInferenceTraceCallbackHandler(BaseCallbackHandler):
    ...
    def on_event_start(
        ...
        attributes.update(_extract_custom_attributes())
        start_time = time_ns()
        context = None
        ...
        if event_type.value == CBEventType.RETRIEVE:
            if RETRIEVER_CLASS_NAME in kwargs:
                retriever_name = kwargs[RETRIEVER_CLASS_NAME]
                attributes.update({RETRIEVER_CLASS_NAME: retriever_name})
                del kwargs[RETRIEVER_CLASS_NAME]
        ...
        span.set_attribute(GEN_AI_FRAMEWORK, AliyunSpanKindValues.LLAMA_INDEX.value)
        span.set_attribute(ALI_TRACE_FLAG, AliyunSpanKindValues.ARMS.value)
...
def _get_span_kind(event_type: Optional[CBEventType]) -> AliyunSpanKindValues:
    ...
    return _SPAN_KINDS.get(event_type, AliyunSpanKindValues.CHAIN)
...
def _get_output_messages(raw: Mapping[str, Any]) -> Iterator[Tuple[str, Any]]:
    ...
    if not (choices := raw.get("choices")) and not (choices := output.get("choices")):
        return
    ...
    for choice in choices:
        if finish_reason := getattr(choice, "finish_reason", None):
            assert isinstance(finish_reason, str), f"content must be str, found {type(finish_reason)}"
            # logger.info(f"{finish_reason}")
            yield GEN_AI_RESPOGENNSE_FINISH_REASON, finish_reason
    if messages := [
        dict(_get_message(message))
        for choice in choices
        if (message := getattr(choice, "message", None)) is not None
    ]:
        yield GEN_AI_COMPLETION, messages
...
def _get_token_counts_from_object(usage: object) -> Iterator[Tuple[str, Any]]:
    ...
    if (prompt_tokens := getattr(usage, "prompt_tokens", None) ) is not None:
        # countInfo += f"prompt.token: {prompt_tokens}"
        yield GEN_AI_USAGE_PROMPT_TOKENS, prompt_tokens
    if (completion_tokens := getattr(usage, "completion_tokens", None)) is not None:
        # countInfo += f"completion.token: {completion_tokens}"
        yield GEN_AI_USAGE_COMPLETION_TOKENS, completion_tokens
    if (input_tokens := getattr(usage, "input_tokens", None) ) is not None:
        # countInfo += f"prompt.token: {prompt_tokens}"
        yield GEN_AI_USAGE_PROMPT_TOKENS, input_tokens
    if (output_tokens := getattr(usage, "output_tokens", None)) is not None:
        # countInfo += f"completion.token: {completion_tokens}"
        yield GEN_AI_USAGE_COMPLETION_TOKENS, output_tokens
    if (total_tokens := getattr(usage, "total_tokens", None)) is not None:
        # countInfo += f"total.token: {total_tokens}"
        yield GEN_AI_USAGE_TOTAL_TOKENS, total_tokens
    # yield HTTP_STATUS_CODE,countInfo
...
def _get_token_counts_from_mapping(
    ...
    if (completion_tokens := usage_mapping.get("completion_tokens")) is not None:
        logger.info("LLM_USAGE_COMPLETION_TOKENS : %s", GEN_AI_USAGE_COMPLETION_TOKENS)
...
def _template_attributes(payload: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    ...
    if template := payload.get(EventPayload.TEMPLATE):
        yield GEN_AI_PROMPT_TEMPLATE, template
    if template_vars := payload.get(EventPayload.TEMPLATE_VARS):
        yield GEN_AI_PROMPT_VARIABLES, json.dumps(template_vars, ensure_ascii=False)


def _extract_custom_attributes() -> Iterator[Tuple[str, Any]]:
    custom_attributes = get_custom_attributes()
    if custom_attributes is not None:
        for custom_attribute_key in custom_attributes:
            yield custom_attribute_key, str(custom_attributes[custom_attribute_key])
...
_SPAN_KINDS = MappingProxyType(
    {
        CBEventType.EMBEDDING: AliyunSpanKindValues.EMBEDDING,
        CBEventType.LLM: AliyunSpanKindValues.LLM,
        CBEventType.RETRIEVE: AliyunSpanKindValues.RETRIEVER,
        CBEventType.FUNCTION_CALL: AliyunSpanKindValues.TOOL,
        CBEventType.AGENT_STEP: AliyunSpanKindValues.AGENT,
        CBEventType.RERANKING: AliyunSpanKindValues.RERANKER,
    }
)
...
EMBEDDING_VECTOR_SIZE = EmbeddingAttributes.EMBEDDING_VECTOR_SIZE
INPUT_MIME_TYPE = SpanAttributes.INPUT_MIME_TYPE
INPUT_VALUE = SpanAttributes.INPUT_VALUE
GEN_AI_PROMPT = SpanAttributes.GEN_AI_PROMPT
GEN_AI_MODEL_NAME = SpanAttributes.GEN_AI_MODEL_NAME
GEN_AI_COMPLETION = SpanAttributes.GEN_AI_COMPLETION
LLM_PROMPTS = SpanAttributes.GEN_AI_PROMPT
GEN_AI_PROMPT_TEMPLATE = SpanAttributes.GEN_AI_PROMPT_TEMPLATE
GEN_AI_PROMPT_VARIABLES = SpanAttributes.GEN_AI_PROMPT_VARIABLES
GEN_AI_USAGE_PROMPT_TOKENS = SpanAttributes.GEN_AI_USAGE_PROMPT_TOKENS
GEN_AI_USAGE_COMPLETION_TOKENS = SpanAttributes.GEN_AI_USAGE_COMPLETION_TOKENS
GEN_AI_USAGE_TOTAL_TOKENS = SpanAttributes.GEN_AI_USAGE_TOTAL_TOKENS
GEN_AI_REQUEST_MODEL_NAME = SpanAttributes.GEN_AI_REQUEST_MODEL_NAME
GEN_AI_REQUEST_MAX_TOKENS = SpanAttributes.GEN_AI_REQUEST_MAX_TOKENS
GEN_AI_REQUEST_TEMPERATURE = SpanAttributes.GEN_AI_REQUEST_TEMPERATURE
GEN_AI_REQUEST_TOP_P = SpanAttributes.GEN_AI_REQUEST_TOP_P
GEN_AI_RESPOGENNSE_FINISH_REASON = SpanAttributes.GEN_AI_RESPONSE_FINISH_REASON
...
GEN_AI_FRAMEWORK = SpanAttributes.GEN_AI_FRAMEWORK
ALI_TRACE_FLAG = SpanAttributes.ALI_TRACE_FLAG
