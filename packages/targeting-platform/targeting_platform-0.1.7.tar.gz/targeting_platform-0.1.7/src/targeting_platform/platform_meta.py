...
class PlatformMeta(Platform):
                    ...
                    for batch_suffixes in generate_batches(suffixes, self._CHUNK_SIZE):
                        ...
                        for suffix in batch_suffixes:
                            self.get_placement(first_level_id=first_level_id, placement_id=placement_id).create_copy(
                                params={"deep_copy": True, "rename_options": {"rename_suffix": suffix}},
                                batch=api_batch,
            ...
            for batch_adset_ids in generate_batches(placement_ids, self._CHUNK_SIZE):
                ...
                for adset_id in batch_adset_ids:
                    self.get_placement(first_level_id=first_level_id, placement_id=adset_id).api_update(
                        {AdSet.Field.status: AdSet.Status.paused}, batch=api_batch, failure=_callback_failure
                    )
                api_batch.execute()
        return placement_ids
    ...
    def clear_placement(self, first_level_id: str, placement_ids: List[str]) -> List[str]:
        ...
        """
        placement_ids, _ = self.set_targeting_options(
            first_level_id=first_level_id,
    ...
    def set_targeting_options(
        ...
        ----
            ...
            is_set_in_each_placement (bool, optional): set the same locations and periods to all placements. If True - locations should be size 1.
                If False locations should have the same size as placement_ids. Defaults to True.
        ...
        if self._is_credentials():
            if is_set_in_each_placement:
                full_periods = flatten_concatenation(periods)
                full_locations = flatten_concatenation(locations)
            ...
            for i, adset_id in enumerate(placement_ids):
                max_locations = self.get_locations_max()
                local_placement_ids.append(adset_id)
                if len(local_locations[i]) > max_locations:
                    ...
                    duplicated_adset_ids = self.duplicate_placement(
                        first_level_id=first_level_id, placement_id=adset_id, suffixes=[f" - # {j}" for j in range(len(splitted_locations) - 1)]
                    )
                    duplicate_ids.extend(duplicated_adset_ids)
                    local_placement_ids.extend(duplicated_adset_ids)
                    periods_to_set.append(local_periods[i])
                    for _ in duplicated_adset_ids:
                        periods_to_set.append(local_periods[i])
            ...
            for batch_adset_ids in generate_batches(local_placement_ids, self._CHUNK_SIZE):
                ...
                for adset_id in batch_adset_ids:
                    ...
                    self.get_placement(first_level_id=first_level_id, placement_id=adset_id).api_update(
                        params=params, batch=api_batch, success=_callback_success, failure=_callback_failure
                    )
                    i += 1
                api_batch.execute()
        return [placement_id for i, placement_id in enumerate(local_placement_ids) if response[i]], duplicate_ids
    ...
    def get_targeting_options(
        ...
        """
        response: List[Tuple[List[str] | List[Tuple[float, float, float]], List[Tuple[str, Tuple[int, int]]]]] = []
        ...
        if self._is_credentials():
            for batch_adset_ids in generate_batches(placement_ids, self._CHUNK_SIZE):
                api_batch = self._api.new_batch()
                for adset_id in batch_adset_ids:
                    self.get_placement(first_level_id=first_level_id, placement_id=adset_id).api_get(
                        fields=[AdSet.Field.status, AdSet.Field.targeting, AdSet.Field.adset_schedule], batch=api_batch, success=_callback_success, failure=_callback_failure
                    )
                api_batch.execute()
