...
class PlatformDV360(Platform):
    ...
    def from_format_locations(locations: List[Any], format: LocationFormats = LocationFormats.h3) -> List[str] | List[Tuple[float, float, float]]:
        ...
        result: List[Any] = [
            (
                int(cell["targetingOptionId"].split(";")[0]) / 1000000,
                int(cell["targetingOptionId"].split(";")[1]) / 1000000,
                cell["proximityRadiusAmount"] * (1.60934 if cell["proximityRadiusUnit"] == "DISTANCE_UNIT_MILES" else 1.0),
            )
            for cell in locations
        ]
