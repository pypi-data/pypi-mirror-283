...
from gemini_model.well.friction_pressuredrop import DPF
...
class InjectionWellMonitoring(ApplicationAbstract):
    ...
    def init_parameters(self):
        ...
        for ii in range(1, len(well_traj)):
            if ii > 0:
                depth.append(well_traj[ii]['MD'] - well_traj[ii-1]['MD'])
            else:
                depth.append(well_traj[ii]['MD'])
        ...
        well_param['roughness'] = np.array(roughness)  # well angle in [degree]
        ...
        res_param['reservoir_pressure'] = reservoir_unit.parameters['reservoir_pressure'] * 1e5
        res_param['reservoir_permeability'] = (
            reservoir_unit.parameters['reservoir_permeability'] * 9.869233E-16
        )
        res_param['reservoir_thickness'] = reservoir_unit.parameters['reservoir_thickness']
    ...
    def calculate_skin_lines(self):
        ...
        u_well['temperature'] = self.inputs['PVT_temperature']
        u_well['Pressure'] = self.inputs['PVT_pressure']
        ...
        for skin in skin_array:
            ...
            for flow in flow_array:
                ...
                deltaP_fric = y['pressuredrop_output']

                u_res['flow'] = flow

                res_param = dict()
                res_param['reservoir_radius'] = self.inputs['reservoir_radius']
                res_param['reservoir_top'] = self.inputs['reservoir_top']

                u_res['viscosity'] = viscosity
                u_res['density'] = density
                u_res['well_radius'] = self.inputs['wellbore_radius'] * 0.0254
                # inch to meter
                ...
                result = BHP - deltaP_HH + deltaP_fric
        ...
        self.outputs['injection_pressure'] = P_inj
    ...
    def plot_skin(self):
        ...
        for idx, skin in enumerate(skin_array):
            plt.plot(flow_array, self.outputs['injection_pressure']
                     [idx], label=f'Skin = {skin}', color='dimgray', linestyle='--')
