...
class Context(ABC):
    ...
    @abstractmethod
    def build_llm_judge_client(self) -> llmjudge.LlmJudgeClient:
        """
...
class RealContext(Context):
    ...
    def build_llm_judge_client(self) -> llm_judge_client.LlmJudgeClient:
        host_creds = databricks_utils.get_databricks_host_creds()
        api_url = host_creds.host
        api_token = host_creds.token
        return llm_judge_client.LlmJudgeClient(api_url=api_url, api_token=api_token)
