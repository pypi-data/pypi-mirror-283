...
def setup_logger(app_name, env='DEV', log_dir=None):
    ...
    if log_dir:
        ...
        log_path = os.path.join('/data/log' if env in ['PROD', 'TEST'] else parent_dir, log_name)
        error_log_path = os.path.join('/data/log' if env in ['PROD', 'TEST'] else parent_dir, error_log_name)
        analysis_log_path = os.path.join('/data/log' if env in ['PROD', 'TEST'] else parent_dir, analysis_log_name)
...
if __name__ == '__main__':
    # from my_logging_sdk import setup_logger
    #
    # # 示例：指定自定义日志路径
