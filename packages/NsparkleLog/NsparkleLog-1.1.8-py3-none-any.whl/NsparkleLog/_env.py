...
class LevelColor:
    ...
    def mapLevelToColor(cls,level: Level,colorName: str) -> None:
        """ 映射日志等级到颜色 """
        setattr(cls,_levelToName[level],colorName) # type: ignore
    

default_format = "{localtime}.{msecs:03} | {level:<6} | {ProcessName}.{threadName} | {name}.{funcName} | {filename}:{lineno} - {message}"
