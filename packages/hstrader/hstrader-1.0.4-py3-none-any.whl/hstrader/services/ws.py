...
class WebSocketService:
    ...
    def register_handler(self, event: Union[Event, str], handler: Callable):
        ...
        elif event in [Event.MARKET, Event.SUMMARY, Event.ERROR]:
    ...
    def update_position(self, position: UptPosition):
    ...
    async def __on_message(self, message: str):
        ...
        try:
            ...
            if typ is None:
                return
            handler = self.__message_handlers.get(typ.value)
            if handler:
                if typ == Event.MARKET:
    ...
    def __handle_market(self, tick: Tick):
        ...
        self.__run_callback(self.__message_handlers.get(Event.MARKET.value), tick, None)
    ...
    def __get_on_connect_callback(self):
        ...
        def on_connect():
            ...
            handler = self.__message_handlers.get(Event.CONNECT.value)
            if handler:
    ...
    def __get_on_disconnect_callback(self):
        ...
        def on_disconnect():
            handler = self.__message_handlers.get(Event.DISCONNECT.value)
            if handler:
                self.__client._run(handler)
