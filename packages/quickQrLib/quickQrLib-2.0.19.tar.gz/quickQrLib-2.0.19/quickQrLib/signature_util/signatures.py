...
class SignatureActions:
    ...
    @classmethod
    def verify_header_permissions(cls, permission, signature, special_permission, special_signature) -> dict:
        json_permission = {}
        json_special_permission = {}
        if special_permission and special_signature:
            public_key = serialization.load_pem_public_key(
                settings.PUBLIC_KEY,
                backend=default_backend()
            )
            signature_bytes_for_special_permission = base64.b64decode(special_signature)
            try:
                json_special_permission= json.loads(special_permission)
                json_dumped_special_permission = json.dumps(json_special_permission, default=str)
                public_key.verify(
                    signature_bytes_for_special_permission,
                    json_dumped_special_permission.encode('utf-8'),
                    padding.PSS(
                        mgf=padding.MGF1(hashes.SHA256()),
                        salt_length=padding.PSS.MAX_LENGTH
                    ),
        ...
        if permission and signature:
            ...
            try:
                ...
                logger.error(f"Error verifying header permissions: '{type(e).__name__}'")
                return {}, {}
        if json_permission and json_special_permission:
            return json_permission, json_special_permission
        elif not json_permission and json_special_permission:
            return {}, json_special_permission
        elif json_permission and not json_special_permission:
