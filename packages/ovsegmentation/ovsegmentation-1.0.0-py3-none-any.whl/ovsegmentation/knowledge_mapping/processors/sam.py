...
def create_sam_model(model_name, version: str, mode: str, device, weights_path, **kwargs):
...
class SAM(object):
    ...
    def __init__(
        self, model_name: str, model_version: str, mode: str, device, weights_path: str = "weights", **kwargs
    ):
        ...
        self.model = create_sam_model(
            self.model_name, self.model_version, self.mode, self.device, self.weights_path, **kwargs
        )
    ...
    def process(self, img, prompt: Any = None, id: int = 0):
        ...
        for r in res:
            ...
            if (self.model_name == SAM_TYPE.EfficientVit) or (
                self.model_name == SAM_TYPE.MobileSAM and self.model_version == "v1"
            ):
