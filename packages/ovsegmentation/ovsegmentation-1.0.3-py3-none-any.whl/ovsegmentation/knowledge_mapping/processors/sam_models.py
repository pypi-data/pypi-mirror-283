...
def load_trt_mobilesamv2_model(weights_path) -> Tuple[Any, ...]:
    ...
    detector_path = os.path.join(
        weights_path,
        "MobileSAMv2",
        MODEL_DICT[SAM_TYPE.MobileSAM]["v2"]["trt_detector_engine"],
    )
    ...
    with trt.Logger() as logger, trt.Runtime(logger) as runtime:
        encoder_path = os.path.join(
            weights_path,
            "MobileSAMv2",
            MODEL_DICT[SAM_TYPE.MobileSAM]["v2"]["trt_encoder_engine"],
        )
    ...
    with trt.Logger() as logger, trt.Runtime(logger) as runtime:
        decoder_path = os.path.join(
            weights_path,
            "MobileSAMv2",
            MODEL_DICT[SAM_TYPE.MobileSAM]["v2"]["trt_decoder_engine"],
        )
...
def predict_masks_mobilesam_model(predictor, bboxes, device):
    ...
    masks = masks[:, -1, :, :].detach().cpu()
    scores = scores[:, -1].detach().cpu()
    res = []
