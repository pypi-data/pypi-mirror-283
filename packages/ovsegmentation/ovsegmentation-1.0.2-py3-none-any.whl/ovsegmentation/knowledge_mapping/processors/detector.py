...
from ovsegmentation.knowledge_mapping.processors.detector_models import GroundingDINOModel, YOLOWorldModel
...
def create_detector_model(model_name, version: str, device, weights_path, **kwargs):
    ...
    elif model_name == DETECTOR_TYPE.YOLOWorld:
        model = YOLOWorldModel(model_name, version, device, weights_path, **kwargs)
    else:
        raise ValueError(
...
class Detector(object):
    ...
    def __init__(
        self, model_name: str, model_version: str, device: str = "cuda", weights_path: str = "weights", **kwargs
    ):
        ...
        self.model = create_detector_model(
            self.model_name, self.model_version, self.device, self.weights_path, **kwargs
        )
