...
# ---|P------/S----------~Lg----------
'''
Utility functions for pyrockoeost.
...
pyrockoeost can treat timestamps either as standard double precision (64 bit)
...
:py:class:`pyrockoeost.util.hpfloat`. High precision time stamps are required when
handling data with sub-millisecond precision, i.e. kHz/MHz data streams and
event catalogs derived from such data.

Not all functions in pyrockoeost and in programs depending on pyrockoeost may work
correctly with high precision times. Therefore, pyrockoeost's high precision time
...
:py:gattr:`~pyrockoeost.config.Config.use_high_precision_time`. Calling the
...
To create a valid time stamp for use in pyrockoeost (e.g. in
:py:class:`~pyrockoeost.model.Event` or :py:class:`~pyrockoeost.trace.Trace` objects),
use:
...
import pyrockoeost
from pyrockoeost import dummy_progressbar
...
try:
    from pyrockoeost import util_ext
except ImportError:
    util_ext = None


logger = logging.getLogger('pyrockoeost.util')
...
g_setup_logging_args = 'pyrockoeost', 'warning'


def setup_logging(programname='pyrockoeost', levelname='warning'):
    '''
    Initialize logging.
    ...
    This function is called at startup by most pyrockoeost programs to set up a
...
def _setup_high_precision_time_mode(enabled_app=False):
    ...
    from pyrockoeost import config

    conf = config.config()
    enabled_config = conf.use_high_precision_time

    enabled_env = os.environ.get('pyrockoeost_USE_HIGH_PRECISION_TIME', None)
    if enabled_env is not None:
        try:
            enabled_env = int(enabled_env) == 1
        except ValueError:
            raise TimeFloatSettingError(
                'Environment variable pyrockoeost_USE_HIGH_PRECISION_TIME '
    ...
    if enabled_env is not None:
        if enabled_env != enabled:
            notify = True
        enabled = enabled_env
        mode_from = 'environment variable `pyrockoeost_USE_HIGH_PRECISION_TIME`'
    ...
    logger.debug('''
pyrockoeost high precision time mode selection (latter override earlier):
    config: %s
    env: %s
    app: %s
     -> enabled: %s'''.lstrip() % (
    ...
    if notify:
        logger.info('pyrockoeost high precision time mode %s by %s.' % (
...
def check_time_class(t, error='raise'):
    ...
    if not isinstance(t, get_time_float()):
        message = (
            ...
            "pyrockoeost's currently selected time handling mode.\n\n"
...
def time_to_str(t, format='%Y-%m-%d %H:%M:%S.3FRAC'):
    ...
    if pyrockoeost.grumpy > 0:
        check_time_class(t, 'warn' if pyrockoeost.grumpy == 1 else 'raise')
...
class Sole(object):
    ...
    Usage::
        ...
        from pyrockoeost.util import Sole, SoleError, setup_logging
        import os
...
re_compatibility = re.compile(
    r'!pyrockoeost\.(trace|gf\.(meta|seismosizer)|fomosto\.' +
...
def make_iload_family(iload_fh, doc_fmt='FMT', doc_yielded_objects='FMT'):
    import itertools
    import glob
    from pyrockoeost.io.io_common import FileLoadError
...
def parse_md(f):
    ...
    # Remve the title
    mdstr = re.sub(r'^# .*\n?', '', mdstr)
    # Append sphinx reference to `pyrockoeost.` modules
    mdstr = re.sub(r'`pyrockoeost\.(.*)`', r':py:mod:`pyrockoeost.\1`', mdstr)
