...
from pyrockoeost import ExternalProgramMissing
from pyrockoeost import pile, model, util, response
from pyrockoeost.io import eventdata

pjoin = os.path.join
logger = logging.getLogger('pyrockoeost.io.rdseed')
        ...
        if Programs.avail is not None:
            ...
            if not ms:
                ...
                if cmp_version('4.7.5', version) == 1 \
                    ...
                    logger.warning(
                        'Module pyrockoeost.rdseed has not been tested with '
...
class SeedVolumeAccess(eventdata.EventDataAccess):
    ...
    def get_stationxml(self):
        stations = self.get_pyrockoeost_stations()
        respfiles = []
        for station in stations:
        ...
        from pyrockoeost.fdsn import resp
    ...
    def get_pyrockoeost_response(self, tr, target):
        '''
