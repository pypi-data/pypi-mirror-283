...
from pyrockoeost.guts import StringChoice, Float, List, Bool
from pyrockoeost.gui.snuffler.marker import PhaseMarker, EventMarker
from pyrockoeost import gf, model, util, trace, io
from pyrockoeost.response import DifferentiationResponse
from pyrockoeost.io.io_common import FileSaveError
...
logger = logging.getLogger('pyrockoeost.scenario.targets.waveform')
...
class WaveformGenerator(TargetGenerator):
    ...
    def get_waveforms(self, engine, sources, tmin, tmax):
        ...
        for source, target, res in response.iter_results(
            ...
            tr = res.trace.pyrockoeost_trace()
                ...
                for phase in tabulated_phases:
                    ...
                    markers.append(
                        PhaseMarker(
                            ...
                            tmin=t,
                            tmax=t,
                            event=source.pyrockoeost_event(),
                            nslc_ids=(nsl+('*',),)
                            )
                        )
        return markers
    ...
    def ensure_responses(self, path):
        ...
        sxml = stationxml.FDSNStationXML.from_pyrockoeost_stations(stations)
