...
from pyrockoeost import util, guts
from pyrockoeost.io import io_common
from pyrockoeost.io import stationxml as sxml

logger = logging.getLogger('pyrockoeost.io.resp')
...
def make_stationxml(pyrockoeost_stations, channel_responses):
    '''
    Create stationxml from pyrockoeost station list and RESP information.

    :param pyrockoeost_stations: list of :py:class:`pyrockoeost.model.Station` objects
    :param channel_responses: iterable yielding :py:class:`ChannelResponse`
        objects
    :returns: :py:class:`pyrockoeost.fdsn.station.FDSNStationXML` object with
        merged information

    If no station information is available for any response information, it
    is skipped and a warning is emitted.
    '''
    pstations = dict((s.nsl(), s) for s in pyrockoeost_stations)
    networks = {}
    stations = {}
    ...
    return sxml.FDSNStationXML(
        source='Converted from pyrockoeost stations file and RESP information',
        created=time.time(),
...
if __name__ == '__main__':
    import sys
    from pyrockoeost.model.station import load_stations

    util.setup_logging(__name__)

    if len(sys.argv) < 2:
        sys.exit('usage: python -m pyrockoeost.fdsn.resp <stations> <resp> ...')
