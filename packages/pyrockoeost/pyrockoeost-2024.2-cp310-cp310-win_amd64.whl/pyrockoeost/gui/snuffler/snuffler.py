...
from pyrockoeost import pile as pile_mod
from pyrockoeost import util
from pyrockoeost import model
from pyrockoeost import config
from pyrockoeost import io
from pyrockoeost.io import stationxml

from . import marker


logger = logging.getLogger('pyrockoeost.gui.snuffler.snuffler')
...
def snuffle(pile=None, **kwargs):
    ...
    :param markers: list of `pyrockoeost.gui.snuffler.util.Marker` objects or
        ``None``
...
def snuffler_from_commandline(args=None):
    ...
    for stationxml_fn in extend_paths(options.stationxml_fns):
        stations.extend(
            stationxml.load_xml(
                filename=stationxml_fn).get_pyrockoeost_stations())
    ...
    except deps.MissingpyrockoeostDependency as e:
        logger.fatal(str(e))
        sys.exit(1)
