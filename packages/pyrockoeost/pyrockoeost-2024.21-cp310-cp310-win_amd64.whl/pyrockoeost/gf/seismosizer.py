...
from pyrockoeost.guts import (Object, Float, String, StringChoice, List,
                          Timestamp, Int, SObject, ArgumentError, Dict,
                          ValidationError, Bool)
from pyrockoeost.guts_array import Array

from pyrockoeost import moment_tensor as pmt
from pyrockoeost import trace, util, config, model, eikonal_ext
from pyrockoeost.orthodrome import ne_to_latlon
from pyrockoeost.model import Location
from pyrockoeost.modelling import OkadaSource, make_okada_coefficient_matrix, \
...
logger = logging.getLogger('pyrockoeost.gf.seismosizer')
...
class Source(Location, Cloneable):
    ...
    def pyrockoeost_event(self, store=None, target=None, **kwargs):
        duration = None
        if self.stf:
    ...
    @classmethod
    def from_pyrockoeost_event(cls, ev, **kwargs):
        if ev.depth is None:
...
class SourceWithMagnitude(Source):
    ...
    def pyrockoeost_event(self, store=None, target=None, **kwargs):
        return Source.pyrockoeost_event(
            self, store, target,
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        return super(SourceWithMagnitude, cls).from_pyrockoeost_event(ev, **d)
...
class SourceWithDerivedMagnitude(Source):
    ...
    def pyrockoeost_moment_tensor(self, store=None, target=None):
        raise NotImplementedError(
            '%s does not implement pyrockoeost_moment_tensor()'
            % self.__class__.__name__)

    def pyrockoeost_event(self, store=None, target=None, **kwargs):
        try:
            mt = self.pyrockoeost_moment_tensor(store, target)
...
class DCSource(SourceWithMagnitude):
    ...
    def pyrockoeost_event(self, store=None, target=None, **kwargs):
        return SourceWithMagnitude.pyrockoeost_event(
            self, store, target,
            moment_tensor=self.pyrockoeost_moment_tensor(store, target),
            **kwargs)
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        return super(DCSource, cls).from_pyrockoeost_event(ev, **d)
...
class MTSource(Source):
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        return super(MTSource, cls).from_pyrockoeost_event(ev, **d)
...
class RectangularSource(SourceWithDerivedMagnitude):
    ...
    def geometry(self, *args, **kwargs):
        from pyrockoeost.model import Geometry
    ...
    def pyrockoeost_event(self, store=None, target=None, **kwargs):
        return SourceWithDerivedMagnitude.pyrockoeost_event(
            self, store, target,
            **kwargs)
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        return super(RectangularSource, cls).from_pyrockoeost_event(ev, **d)
...
class PseudoDynamicRupture(SourceWithDerivedMagnitude):
    ...
    tractions = TractionField.T(
        ...
        help='Traction field the rupture plane is exposed to. See the'
             ':py:mod:`pyrockoeost.gf.tractions` module for more details. '
             'If ``tractions=None`` and ``rake`` is given'
             ' :py:class:`~pyrockoeost.gf.tractions.DirectedTractions` will'
             ' be used.')
    ...
    def get_tractions(self):
        ...
        (:py:class:`~pyrockoeost.gf.tractions.DirectedTractions`) are returned,
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        return super(PseudoDynamicRupture, cls).from_pyrockoeost_event(ev, **d)
    ...
    def _discretize_points(self, store, *args, **kwargs):
        ...
        :param store:
            ...
            source).
        :type store:
            :py:class:`~pyrockoeost.gf.store.Store`
    ...
    def _discretize_rupture_v(self, store, interpolation='nearest_neighbor',
        ...
        :param store:
            ...
            source)
        :type store:
            optional, :py:class:`~pyrockoeost.gf.store.Store`
    ...
    def discretize_basesource(self, store, target=None):
        ...
        :param target:
            Target information.
        :type target:
            optional, :py:class:`~pyrockoeost.gf.targets.Target`

        :returns:
            Source discretized by a set of moment tensors and times.
        :rtype:
            :py:class:`~pyrockoeost.gf.meta.DiscretizedMTSource`
        '''
        if not target:
            interpolation = 'nearest_neighbor'
        else:
    ...
    def get_patch_attribute(self, attr):
        ...
        :param attr:
            ...
            :py:class`pyrockoeost.modelling.okada.OkadaSource`).
        :type attr:
            str
    ...
    def get_centroid(self, store, *args, **kwargs):
        ...
        Additional ``**kwargs`` are passed to :py:meth:`pyrockoeost_moment_tensor`.
        ...
        :rtype:
            :py:class:`pyrockoeost.model.Event`
        '''
        ...
        mt = self.pyrockoeost_moment_tensor(store, *args, **kwargs)
...
class DoubleDCSource(SourceWithMagnitude):
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        ...
        d.update(kwargs)
        source = super(DoubleDCSource, cls).from_pyrockoeost_event(ev, **d)
...
class SFSource(Source):
    ...
    def from_pyrockoeost_event(cls, ev, **kwargs):
        d = {}
        d.update(kwargs)
        return super(SFSource, cls).from_pyrockoeost_event(ev, **d)
...
class Response(Object):
    ...
    def pyrockoeost_traces(self):
        '''
        Return a list of requested
        :class:`~pyrockoeost.trace.Trace` instances.
        '''
        ...
        for results in self.results_list:
            for result in results:
                if not isinstance(result, meta.Result):
                    continue
                traces.append(result.trace.pyrockoeost_trace())
    ...
    def static_results(self):
        '''
        Return a list of requested
        :class:`~pyrockoeost.gf.meta.StaticResult` instances.
        '''
        statics = []
        for results in self.results_list:
            for result in results:
    ...
    def iter_results(self, get='pyrockoeost_traces'):
        '''
        Generator function to iterate over results of request.

        Yields associated :py:class:`Source`,
        :class:`~pyrockoeost.gf.targets.Target`,
        :class:`~pyrockoeost.trace.Trace` instances in each iteration.
        '''
        ...
        for isource, source in enumerate(self.request.sources):
            for itarget, target in enumerate(self.request.targets):
                ...
                if get == 'pyrockoeost_traces':
                    yield source, target, result.trace.pyrockoeost_trace()
                elif get == 'results':
    ...
    def snuffle(self, **kwargs):
        ...
        trace.snuffle(self.pyrockoeost_traces(), **kwargs)
...
class LocalEngine(Engine):
    ...
    :param use_config: if ``True``, fill :py:attr:`store_superdirs` and
        ...
        The config file can be found at :file:`~/.pyrockoeost/config.pf`

        .. code-block :: python

            gf_store_dirs: ['/home/pyrockoeost/gf_stores/ak135/']
            gf_store_superdirs: ['/home/pyrockoeost/gf_stores/']
    '''
    ...
    def get_store(self, store_id=None):
        ...
        :returns: :py:class:`~pyrockoeost.gf.store.Store` object
