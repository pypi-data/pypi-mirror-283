...
from pyrockoeost import gf, util
from pyrockoeost import orthodrome as od
from pyrockoeost.guts import Float, Timestamp, Tuple, StringChoice, Bool, Object,\
    String
...
logger = logging.getLogger('pyrockoeost.scenario.targets.insar')
    ...
    class SatelliteGeneratorTarget(gf.SatelliteTarget):
        ...
        def post_process(self, *args, **kwargs):
            ...
            scene_config = SceneConfig(
                meta=Meta(
                    scene_title='pyrockoeost Scenario Generator - {orbit} ({time})'
                                .format(orbit=self.scene_patch.orbital_node,
                                        time=datetime.now()),
                    orbital_node=patch.orbital_node,
                    scene_id='pyrockoeost_scenario_%s'
                             % self.scene_patch.orbital_node,
                    satellite_name='Sentinel-1 (pyrockoeost-scenario)'),
                frame=FrameConfig(
                    llLat=float(llLat),
                    llLon=float(llLon),
                    dN=float(dLat),
