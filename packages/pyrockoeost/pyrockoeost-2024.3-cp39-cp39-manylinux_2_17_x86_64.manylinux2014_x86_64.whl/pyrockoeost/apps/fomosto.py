...
from pyrockoeost import util, trace, gf, cake, io, fomosto
from pyrockoeost.gui.snuffler import marker
from pyrockoeost.util import mpl_show

logger = logging.getLogger('pyrockoeost.apps.fomosto')
km = 1e3
...
def fomo_wrapper_module(name):
    try:
        ...
        modname = '.'.join(['pyrockoeost', 'fomosto', name_clean])
    ...
    try:
        ...
        try:
            ...
            for i, args in enumerate(dest.config.iter_nodes(level=-1)):
                ...
                traces = db.get_traces_pyrockoeost(distance, source_depth)
...
def stored_attribute_table_plots(phase_ids, options, args, attribute):
    import numpy as num
    from pyrockoeost.plot.cake_plot import labelspace, xscaled, yscaled, mpl_init
...
def command_server(args):
    from pyrockoeost.gf import server
...
def command_modelview(args):
    ...
    import numpy as num
    from pyrockoeost.plot.cake_plot import mpl_init, labelspace, xscaled, yscaled
    mpl_init()
...
def command_report(args):
    from pyrockoeost.fomosto.report import report_call
