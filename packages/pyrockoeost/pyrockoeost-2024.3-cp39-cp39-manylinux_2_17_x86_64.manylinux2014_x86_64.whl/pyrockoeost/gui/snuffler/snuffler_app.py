...
from pyrockoeost.gui.snuffling import MyScrollArea

from pyrockoeost.streaming import serial_hamster
from pyrockoeost.streaming import slink
from pyrockoeost.streaming import edl
from pyrockoeost.streaming import datacube

from pyrockoeost import pile            # noqa
from pyrockoeost import util            # noqa
from pyrockoeost import model           # noqa
from pyrockoeost import config          # noqa
from pyrockoeost import io              # noqa

from . import pile_viewer     # noqa

from ..qt_compat import qc, qg, qw

logger = logging.getLogger('pyrockoeost.gui.snuffler.snuffler_app')
...
class SnufflerStartWizard(qw.QWizard):
    ...
    def __init__(self, parent):
        ...
        self.addPageSurvey()
        self.addPageHelp()
        self.setWindowTitle('Bienvenue dans pyrockoeost')
    ...
    def getSystemInfo(self):
        ...
        data = {
            ...
            'pyrockoeost': pyrockoeost.__version__,
    ...
    def addPageSurvey(self):
        ...
        p.setTitle("Merci d'avoir installé pyrockoeost | <b>EOST version</b> !")

        # lyt = qw.QVBoxLayout()
        # lyt.addWidget(qw.QLabel(
        #     '<p>Your feedback is important for'
        #     ' the development and improvement of pyrockoeost.</p>'
        ...
        lyt = qw.QVBoxLayout()
        lyt.addWidget(qw.QLabel(
            "<p>Vos commentaires sont importants pour"
            " le développement et l'amélioration de pyrockoeost.</p>"
            "<p><b>N'hésitez pas à nous en faire part !</b></p>"))

        text_data = qw.QLabel(
            '<code style="font-size: small;">Vos infos :<br>%s</code>' %
            pprint.pformat(
                sys_info,
                indent=1).replace('\n', '<br>')
            )
        ...
        lyt.addWidget(qw.QLabel(
            "Vous pouvez jeter un œil à la configuration (touches, emplacement)\n dans : ~/.pyrockoeost/snuffler.pf\n\n"
            "Ce message ne sera plus affiché.\n"
            '- The pyrockoeost Developers'
            ))

        p.setLayout(lyt)
        p.setButtonText(self.CommitButton, 'Continuer')
    ...
    def addPageHelp(self):
        p = qw.QWizardPage()
        p.setTitle('Bienvenue dans Snuffler !')

        text = qw.QLabel('''<html>
<h3>- <i>Le navigateur de sismogrammes et son atelier.</i></h3>
<p>On dirait que vous démarrez Snuffler pour la première fois.<br>
Il vous permet de parcourir et de traiter de grandes archives de données sous formes d'ondes.</p>
<p>Le traitement de base est complété par des Snufflings (plugins) comme celui de l'EOST :</p>
<ul>
    <li><b>Télécharger des sismogrammes</b> depuis GEOFON IRIS et d'autres sources</li>
    <li><b>Catalogue de tremblements de terre</b> accès à GEOFON, GlobalCMT, USGS...</li>
    <li><b>Cake</b>, calculer les temps d'arrivée synthétiques</li>
    <li><b>Seismosizer</b>, générer des sismogrammes synthétiques à la volée</li>
    <li>
        <b>Carte</b>, inspecter rapidement les stations et les événements sur des cartes interactives
    </li>
</ul>
<p>Et plus encore, voir <a href="https://pyrocko.org/">https://pyrocko.org/</a></p>

<p style="width: 100%; background-color: #0071b6; margin: 5px; padding: 50;"
          align="center">
    <b>Bonne chance !</b>
</p>
</html>''')
        ...
        p.setLayout(lyt)
        self.addPage(p)
        p.setButtonText(qw.QWizard.FinishButton, "Terminer")
        return p
...
class SnufflerWindow(qw.QMainWindow):
    ...
    def __init__(
        ...
        self.add_panel(
            'Markers', self.marker_editor, visible=True,
        ...
        snufflings = self.pile_viewer.setup_snufflings()
        ...
        # EOST
        # Activation du widget EOST Hodochrone au démarrage de Snuffler
        # QDockWidget: https://doc.qt.io/qt-5/qdockwidget.html
        eost_hodochrone = None
        for widget in self.get_panel_parent_widget().dockwidgets:
            if "EOST - Hodochrones" in widget.windowTitle():
                eost_hodochrone = widget.findChildren(MyScrollArea)[0]

        try:
           self.add_panel('EOST - Hodochrones', eost_hodochrone, visible=True)
        except Exception as e:
            logger.error(str(e))
            logger.error("Impossible de charger le module : EOST - Hodochrones.")
        # EOST
        ...
        # EOST
        try:
            snufflings = self.pile_viewer.get_view().default_snufflings
            for snuffling in snufflings:
                if "EOST - Hodochrones" in snuffling.get_name():
                    snuffling.call()
        except Exception as e:
            logger.error(str(e))
            logger.error("Impossible d'initialiser le module : EOST - Hodochrones.")
        ...
        sb = self.statusBar()
        sb.clearMessage()
        sb.showMessage('Appuyez sur <?> pour obtenir de l\'aide.')
