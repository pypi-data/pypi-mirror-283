...
logger = logging.getLogger('pyrockoeost.gui.sparrow.cli')
km = 1e3
...
program_name = 'sparrow'

usage_tdata = d2u(subcommand_descriptions)
usage_tdata['program_name'] = program_name
usage_tdata['version_number'] = pyrockoeost.__version__


usage = '''%(program_name)s <subcommand> [options] [--] <arguments> ...

A quick little bird to visualize geophysical data.

The Sparrow is part of pyrockoeost version %(version_number)s.
...
Where does your journey go today?!
''' % usage_tdata
...
def command_version(args):
    def setup(parser):
        parser.add_option(
            ...
            help='only print pyrockoeost\'s version number')

    parser, options, args = cl_parse('version', args, setup)

    from pyrockoeost.print_version import print_version
