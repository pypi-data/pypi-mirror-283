...
from .stream_exceptions import StreamException, StreamStoppedException, StreamTimeoutException
...
class StreamHandler:
    ...
    def __init__(self, stream_function: Callable[[Any], Any], request: Any = None, **kwargs):
        ...
        self._response_iterator = None   # Represents the incoming stream
    ...
    def _has_outgoing_stream(function: Callable[[Any], Any]) -> bool:
        ...
        if parameter_type is None:
            raise StreamException("Unable to detect whether the given stream function requires an outgoing stream. Please pass it explicitly using the outgoing_stream kwarg.")
    ...
    @requires_active_stream
    def send_batch(self, messages: Iterable, block: bool = True, timeout: float | None = None) -> Any:
        """
        ...
        :param timeout: time in seconds to wait for the response before raising a StreamTimeoutException. None will wait forever.
        ...
        for message in messages:
            self.send(message)
        self.stop_stream(finish_sending=True)
        return self.receive(block=block, timeout=timeout)

    def stop_stream(self, finish_sending: bool = False) -> None:
        """
        Closes the gRPC stream if it is not already closed. A new StreamHandler object will have to be created to start
        another stream.
        
        :param finish_sending: whether to wait for all outgoing messages to be sent before stopping the stream.
        """
        ...
        if self._request_iterator is not None:
            ...
            if self._request_iterator.is_active:
                self._request_iterator.stop(send_remaining_messages=finish_sending)
