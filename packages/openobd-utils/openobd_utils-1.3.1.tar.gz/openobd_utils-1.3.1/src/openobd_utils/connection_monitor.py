...
class ConnectionMonitor(StreamHandler):
    ...
    def get_connection_data(self) -> list:
        ...
        """
        # Add any new ConnectionInformation messages to the self._connection_info_samples list
        connection_sample = self.receive(block=False)
        while connection_sample is not None:
            self._connection_info_samples.append(connection_sample)
            connection_sample = self.receive(block=False)

        # Make sure the list does not exceed self.sample_size, by taking only the most recent messages
        self._connection_info_samples = self._connection_info_samples[-self.sample_size:]

        # Reverse the list, so the samples are ordered latest to oldest, and return it
        return self._connection_info_samples[::-1]
