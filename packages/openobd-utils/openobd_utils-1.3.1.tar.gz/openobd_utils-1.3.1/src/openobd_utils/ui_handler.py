...
from .stream_exceptions import StreamException
...
class UserInterfaceException(StreamException):
    """
...
class UiHandler(StreamHandler):
    ...
    def __init__(self, openobd_session: OpenOBDSession, target: InterfaceType = InterfaceType.INTERFACE_USER):
        ...
        """
        super().__init__(openobd_session.open_control_stream)
        self._target = target
    ...
    def show_ui(self, control: _control_types) -> None | int | bool | str:
        ...
        self.send(Control(**kwargs), flush_incoming_messages=True)
        while True:
