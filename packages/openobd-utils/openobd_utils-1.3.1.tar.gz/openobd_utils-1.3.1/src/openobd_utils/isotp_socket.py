...
from openobd import *
from .stream_handler import StreamHandler, StreamTimeoutException
from .response_exceptions import *


class IsotpSocket(StreamHandler):
    ...
    def __init__(self, openobd_session: OpenOBDSession, isotp_channel: IsotpChannel, timeout: float | None = 10):
        ...
        """
        super().__init__(openobd_session.open_isotp_stream)
    ...
    def request(self, payload: str, timeout: float | None = None, silent: bool = False, tries: int = 1) -> str | None:
        ...
        for current_try in range(1, tries + 1):
            self.send(message, flush_incoming_messages=True)
