...
file_option = click.option('-f', '--file', 'file', default=None, help='PDF file path', type=str)
annotated_option = click.option('-a', '--annotated', 'annotated', default=None, help='Annotated PDF file path', type=str)
tags_option = click.option('-t', '--tags', 'tags', default='', help="Comma delimited tags; e.g. 'tag0, tag1, tag2'")
force_option = click.option('--force', is_flag=True, default=False, show_default=True, help='Force insert of document', type=bool)
...
@click.option('--debug', type=int, default=0, help='Debug traceback level; higher prints more: e.g. 3 prints 3 levels of traceback; a negative value prints all traceback')
...
@click.pass_context
def insert(ctx, file, annotated, tags, notes, synopsis, force, standard):
    """Insert a document into the database."""

    ####### NEED TO DO CHECKS THAT FILES EXIST BEFORE MODIFYING THE INDEX, CACHE, OR DATABASE
    ####### NEED FAIL OVER IF RSYNC OR SSH FAILS
    ...
    # build doc
    doc = document.build_doc(doc_bib_dict, file_text=file_text, file_path=file_path, annotated_path=annotated_path, tags=tags, notes=notes, synopsis=synopsis)
...
@doc_id_arg
@click.pass_context
def modify(ctx, file, annotated, tags, force, doc_id):
...
def delete(ctx, force, doc_id):
    ...
    if click.confirm('Are you sure you want to delete this document?'):
        ...
        if ctx.obj['index'].file_exists(doc_id, annotated=True):
...
@click.command()
@click.option('-o', '--output', 'output', default='counts', type=click.Choice(['minimal', 'counts', 'json', 'pretty']), help='')
@click.option('-p', '--pager', 'pager', default=False, is_flag=True)
@click.option('-b', '--bib', '--bibtex', 'bib', default=False, is_flag=True)
@click.argument('regex', type=str, default="") # add bibtex output and handle tags by specifying tag?
@click.pass_obj # passes object directly without context
def search(db, output, pager, bib, regex):
    """Searches the database with REGEX_TEXT.
    """

    def print_results(hits, output='simple', pager=False):
        """
        ...
        for doc_id in hits:
            ...
            if output == 'counts':
                ...
                attr = 'bib'
                try:
                    output_str += db['index'].index[doc_id][attr]['title'] + '\n'
                except:
                    pass

                try:
                    output_str += click.style(db['index'].index[doc_id][attr]['author'] + '\n', italic=True)
                except:
                    pass
    ...
    def print_bib(hits):
        ...
        for doc_id in hits:
            attr = 'bib'
            doc_bib = db['index'].index[doc_id][attr]
