...
from bgeditor.dao.FFmpeg import create_suource_can_loop_path, create_loop, create_loop_audio, create_video_audio, merge_intro_outro, merge_list_video, sub_clip_no_encode
...
def create_video(list_comp_data, path_video, job_id, mf_server):
    ...
    for comp in arr_comps:
        ...
        if comp.type == "video":
            ...
            if comp.is_intro:
                ...
                rs.write_videofile(introPath, bitrate=base_bitrate, fps=base_fps, codec='libx264', audio_bitrate='128000')
                is_cont = True
                rs.close()
            if comp.is_outro:
                ...
                if not rs.audio:
                    ...
                    vidPathTmp = FFmpeg.add_null_sound(comp.video_path)
                    rs = VideoFileClip(vidPathTmp)
                rs.write_videofile(outroPath, bitrate=base_bitrate, fps=base_fps, codec='libx264', audio_bitrate='128000')
                is_cont = True
                rs.close()
            if is_cont:
                continue
    ...
    if not is_music:
        ...
        if cnt_effect_vid > 1:
            ...
            clip.write_videofile(tmp_path_maked_loop, bitrate=base_bitrate, fps=24, codec='libx264', audio=False, logger=logger)
            clip.close()
            final_clip.close()
