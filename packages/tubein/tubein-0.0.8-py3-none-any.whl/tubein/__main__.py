...
"""tubeup_ac - Download a video with Youtube-dlc, then upload to Internet Archive, passing all metadata.
...
from tubein.TubeIn import TubeIn
...
def main(args):
    ...
    tu = TubeIn(verbose=not quiet_mode, output_template=args["--output"])
    ...
    try:
        for identifier, meta in tu.archive_urls(
            ...
            print("Item URL: https://s3.altcensored.com/videos/%s\n" % identifier)
            print(
                "Image URL: https://s3.altcensored.com/videos/%s/%s\n"
                % (file_name, imagefile)
            )
    except Exception:
        print(
            ...
            "https://github.com/altcensored/tubeup_ac/issues"
        )
        traceback.print_exc()
