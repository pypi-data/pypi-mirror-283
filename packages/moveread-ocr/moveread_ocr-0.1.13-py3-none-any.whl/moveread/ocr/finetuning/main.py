...
def run(
  *, train: str, val: Mapping[str, str], weights: str, base_path: str, epochs: int,
  logstream: TextIO | None = None, metrics_freq: int = 10, **hparams: Unpack[HParams]
):
  ...
  train_ds, train_n = mo.records.read_dataset(train, mode='shuffle', batch_size=hparams['batch_size'], char2num=char2num)
  train_ds = train_ds.shuffle(100)

  val_datasets = {}
  for name, path in val.items():
    val_ds, n_samples = mo.records.read_dataset(path, mode='keep_order', batch_size=hparams['batch_size'], char2num=char2num)
