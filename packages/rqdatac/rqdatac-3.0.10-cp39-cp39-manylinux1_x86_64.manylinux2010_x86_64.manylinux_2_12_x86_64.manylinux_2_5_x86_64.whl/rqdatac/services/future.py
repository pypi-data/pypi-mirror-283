...
DOMINANT_PRICE_FIELDS = {
    'tick': [
        ...
        "open_interest", "prev_settlement", "date", "time",
    ],
    'd': [
...
def get_dominant_price(
    ...
    data = get_client().execute('futures.get_dominant_price', underlying_symbols, start_date, end_date, frequency, fields, rule)
    data = [(obid, {k: np.frombuffer(*v) for k, v in d.items()}) for obid, d in data]
    df = None
    if frequency == 'tick':
        from .get_price import convert_history_tick_to_multi_df
        df = convert_history_tick_to_multi_df(data, "datetime", fields, int17_to_datetime_v)
        if df is not None:
    ...
    if adjust_type != 'none':
        # 复权调整
        factor = __internal_get_ex_factor(underlying_symbols, adjust_type, adjust_method, rule)
