...
class NPSAMImage:
    ...
    def load_image(self, filepath, select_image=None):
        ...
        # Hyperspy can't load all three channels in regular images, so we load them without
                ...
                while image_found == False:
                    if select_image is None:
                        ...
                        for subsignal in signal:
                            print(str(subsignal))
                        select_image = input(
                            "\nPlease choose the image of interest by providing an index or the image title:"
                        )
    ...
    def convert_to_units(self, units):
        ...
        try:
            convert_to_units(self.seg, scaling)
        except:
            pass
    ...
    def segment(
        ...
        **kwargs,
        ...
        """
        This function takes in an HyperSpy signal of an image (RGB or greyscale) and
        segments it using either SAM or FastSAM. It returns a HyperSpy signal with the
        masks stored in the .data attribute and the segmentation paramenters stored in
        ...
        for sub_image in sub_images:
            if SAM_model == "fast":
                masks = FastSAM_segmentation(sub_image, device)
            else:
                masks = SAM_segmentation(
                    sub_image, SAM_model, device, PPS, min_mask_area, **kwargs
                )
            ...
            if shape_filter:
                ...
                for mask in masks:
                    labels_of_masks = skimage.measure.label(mask)
                    props = skimage.measure.regionprops_table(
                        labels_of_masks, properties=["label", "area", "solidity"]
                    )
        ...
        if len(masks) == 0:
            ...
            if verbose:
                print(
                    f"0 masks found for {self.img.metadata.General.title}, so no masks were saved."
                )
    ...
    def import_segmentation_from_image(self, filepath, stepsize=1, verbose=True):
        ...
        self.seg = hs.signals.Signal2D(
            ...
            metadata={
                "General": {
                    "segmentation": f"Imported from file '{filepath.as_posix()}'",
                    "title": image.metadata.General.title,
                    "image_filepath": image.metadata.General.filepath,
                },
                "Filtering": {"Conditions": None, "passed_filter": "N/A"},
            },
        )
        set_scaling(self.seg, get_scaling(image))
        self.characterize(stepsize=stepsize, verbose=verbose)
    ...
    def characterize(self, stepsize=1, verbose=True):
        ...
        for property in [
            ...
            df[property] *= length_per_pixel
        for property in ["area", "area_convex"]:
            df[property] *= length_per_pixel**2
        ...
        if not (unique_multiple_masks is None):
            for n, unique in enumerate(unique_multiple_masks):
                if verbose:
                    print(
                        f"Finding areas with overlap: {n + 1}/{len(unique_multiple_masks)}",
                        sep=",",
    ...
    def plot_masks(self, cmap="default", figsize=[8, 4]):
        if cmap == "default":
            cmap = make_randomized_cmap()
        labels = masks_to_2D(self.seg.data)
    ...
    def plot_particle(self, mask_index, cmap="grey"):
        bbox = self.cha.loc[0, [f"bbox-{n}" for n in range(4)]].tolist()
        fig, ax = plt.subplots()
    ...
    def overview(self, property_list=["area"], bin_list=None, timestamp=False):
        overview(
            self.img,
            self.seg,
            self.cha,
    ...
    def get_filtered_masks(self):
        if self.seg.metadata.Filtering.passed_filter == "N/A":
            ...
            filtered_masks = self.seg.data[
                segmentation.metadata.Filtering.passed_filter
            ]
    ...
    def export_filtered_characteristics(self, save_as=None):
        ...
        if save_as is None:
            filepath = Path(self.filepath.split(".")[0] + "_filtered_masks.csv")
        else:
    ...
    def export_filtered_masks_png(self, save_as=None, cmap="default"):
        ...
        if save_as is None:
            filepath = Path(self.filepath.split(".")[0] + "_filtered_masks.png")
        else:
            filepath = Path(save_as)
    ...
    def export_filtered_masks_tif(self, save_as=None):
        labels = masks_to_2D(self.get_filtered_masks())
        if save_as is None:
            filepath = Path(self.filepath.split(".")[0] + "_filtered_masks.tif")
        else:
            filepath = Path(save_as)
    ...
    def export_filtered_masks_binary(self, save_as=None):
        labels = masks_to_2D(self.get_filtered_masks())
        if save_as is None:
            filepath = Path(self.filepath.split(".")[0] + "_filtered_masks_binary.tif")
        else:
            filepath = Path(save_as)
        tifffile.imwrite(
    ...
    def export_filtered_masks_numpy(self, save_as=None):
        if save_as is None:
            filepath = Path(self.filepath.split(".")[0] + "_filtered_masks_binary.npz")
        else:
            filepath = Path(save_as)
        np.savez_compressed(
    ...
    def export_all(self, save_as=None):
        ...
        self.export_filtered_masks_binary(save_as=save_as)
        self.export_filtered_masks_numpy(save_as=save_as)


class NPSAM(MutableSequence):
    """
    This class is a wrapper that eases the use of the function found in NP-SAM.
    ...
    The loaded images can then be segmented with the .segment() function. The masks are
    then characterized and the characteristics are saved in memory as a pandas DataFrame
    and to file as .csv file. If no scaling is given or extracted from the image files,
    the user is prompted for a scaling. The scaling can also be changed with
    .set_scaling() afterwards.

    Once segmented, the masks can be filtered with .filter() or .filter_nogui(). This
    will the 'removed' characteristic of a given mask to True if is was removed by the
    filter.

    Finally, a range of different outputs can be made. An overview PDF file with
    .overview(), a numpy array of the kept masks with .export_to_numpy() and a binary
    flat version of all masks with .export_binary().
    """
    ...
    def __add__(self, other):
        if isinstance(other, MasterNPSAM):
    ...
    def segment(
        ...
        **kwargs,
        ...
        for n, image in enumerate(self):
            if len(self) > 1:
                print(
                    f"{n + 1}/{len(self)} - Now working on: {image.img.metadata.General.title}"
                )
    ...
    def save_segmentation():
        ...
        # if save_as is None:
        #    filepath = Path(segmentation.metadata.General.image_filepath)
        # else:
        #    filepath = Path(save_as.split('.')[0]+f'_{counter:03}' if is_list else save_as)
...
def filter(images, segmentations, characteristics):
    ...
    filtergui = ImageFilter(images, segmentations, characteristics)
    filtergui.filter()
    ...
    if type(conditions) == dict:
        ...
        if len(conditions) == len(segmentations):
            for entry in conditions:
                if type(entry) != dict:
                    raise ValueError(
                        "The list entries must be dictionaries containing the filter conditions."
                    )
            ...
            raise ValueError(
                "The length of the list with filtering conditions does not have the same length as the list with segmentations."
            )
...
def overview(
    ...
    is_list = isinstance(segmentations, list)
    segmentations = [segmentations] if not is_list else segmentations
    images = [images] if not is_list else images
    characteristics = [characteristics] if not is_list else characteristics
    ...
    dfs = []
    imagenumber = 1
    for image, segmentation, characteristic in zip(
        images, segmentations, characteristics
    ):
        df_filtered = copy(characteristic[characteristic["passed_filter"]])
        df_filtered["imagename"] = characteristic.attrs["title"]
        df_filtered["imagenumber"] = imagenumber
        imagenumber += 1
        dfs.append(df_filtered)
    master_df = pd.concat(dfs)

    filepath = characteristics[0].attrs["filepath"]
    if len(images) > 1:
        file_p = Path(filepath).parent / (
            Path(filepath).parent.name + "_overview_filtered_dataframe.csv"
        )
    ...
    def save_image():
        ...
        file_p = Path(filepath).parent / (
            Path(filepath).parent.name + "_overview" + stamp + ".pdf"
        )
    ...
    for image, segmentation, characteristic in zip(
        ...
        ax["left"].imshow(image.data, cmap="gray")
        ax["left"].axis("off")

        labels = masks_to_2D(segmentation.data)
        ax["right"].imshow(labels, interpolation="nearest", cmap=cmap)
        ax["right"].axis("off")

        ax["right2"].axis("off")

        plt.suptitle(segmentation.metadata.General.title, fontsize=18)

        df_filtered["area"].hist(
        ...
        filters = segmentation.metadata.Filtering.Conditions
        ...
        seg_params = segmentation.metadata.Segmentation
        segmentation = (
            ...
            + (
                f", MMA={seg_params.min_mask_area}"
                if seg_params.min_mask_area != 35
                else ""
            )
        )
