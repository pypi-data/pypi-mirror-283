...
class GoogleAccountLocal(GenericCRUD, metaclass=MetaLogger,
    ...
    def authenticate(self, email: str):
        ...
        if not self.creds or not self.creds.valid:  # TODO: move this to a separate method
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                if self.creds or not refresh_token:
                    ...
                    try:
                        ...
                        self.logger.error("google-contact-local Google Refresh token failed.",
            ...
            if user_info.get("email") != email:
                ...
                raise Exception("The email address of the connected Google account does not match the provided email.")
            self.user_email = user_info.get("email")    # Cannot be "user_email" because we get it from the API
    ...
    def __authorize(self, oauth_state: str = None) -> int:
        ...
        flow = InstalledAppFlow.from_client_config(client_config, SCOPES, redirect_uri=self.google_redirect_uris,
                                                   state=state)
        auth_url, _ = flow.authorization_url(access_type='offline', prompt='consent')   # access_type='online' won't return a refresh token
        ...
        inserted_id = self.insert(data_dict={"oauth_state": state})
        # If the url is
        ...
        for i in range(TIMEOUT // SLEEP_TIME):
            ...
            auth_code = self.select_one_value_by_column_and_value(
                select_clause_value="access_token", column_value=inserted_id)
            if auth_code:
                self.logger.info(f"Auth code found in the database after {i + 1} times out of {TIMEOUT // SLEEP_TIME}.")
                break
            time.sleep(SLEEP_TIME)
            self.logger.info(f"Failed to get the auth code from the database for the {i + 1} time out of {TIMEOUT // SLEEP_TIME}.")
        if not auth_code:
