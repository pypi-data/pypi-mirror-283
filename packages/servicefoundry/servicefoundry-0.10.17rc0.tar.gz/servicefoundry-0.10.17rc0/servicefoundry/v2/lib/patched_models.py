...
class GcpTPU(models.GcpTPU, PatchedModelBase):
    type: constr(regex=r"^gcp_tpu$") = "gcp_tpu"
    name: Union[TPUType, constr(regex=r"^tpu-[a-z\d\-]+$")]
...
class PythonTaskConfig(models.PythonTaskConfig, PatchedModelBase):
    type: constr(regex=r"^python-task-config") = "python-task-config"


class ContainerTaskConfig(models.ContainerTaskConfig, PatchedModelBase):
    type: constr(regex=r"^container-task-config") = "container-task-config"


class TaskDockerFileBuild(models.TaskDockerFileBuild, PatchedModelBase):
    type: constr(regex=r"^task-dockerfile-build") = "task-dockerfile-build"


class TaskPythonBuild(models.TaskPythonBuild, PatchedModelBase):
    type: constr(regex=r"^task-python-build") = "task-python-build"
