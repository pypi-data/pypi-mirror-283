...
@click.argument("name")
@click.pass_obj
def create(ctx, cpu, ram, disk, efi, mac, iso, no_iso, connect, start, gui, fullscreen, name):
    """create VM"""
    iso = iso or ctx.iso
    if mac:
        mac = str(mac)
    if no_iso:
        iso = None
    result = ctx.create(name, cpu_count=cpu, ram_size=ram, disk_size=disk, iso_path=iso, iso_connected=connect, mac=mac, efi=efi)
    if start:
        _start(ctx, gui, fullscreen, name)
...
def vnc(ctx, name, enable, disable, port, address):
    """show or configure VM VNC settings"""
