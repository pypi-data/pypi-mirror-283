...
class BaseObjectTestCase(TestCase):
    ...
    def test_copy_config(self):
        obj1 = BaseObject()
        obj2 = copy(obj1)
        self.assertEqual(obj1.config, obj2.config)
    ...
    def test_get_state(self):
        obj1 = BaseObject()
        obj1.config = deepcopy(obj1.config)
...
class BaseDictTestCase(TestCase):
    ...
    def test_del_attribute(self):
        ...
        with self.assertRaisesRegex(AttributeError, "'dict' object has no attribute 'attr1'"):
            obj.attr1 # pylint: disable=pointless-statement
    ...
    def test_del_private_attribute(self):
        ...
        with self.assertRaisesRegex(AttributeError, "'dict' object has no attribute '_attr1'"):
            obj._attr1 # pylint: disable=pointless-statement
    ...
    def test_clear(self):
        ...
        self.assertDictEqual(obj.__dict__, dict(_private=1, public= 2))
...
class MetaClassConfigTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        TestBaseDictConfig.config.value = 1
    ...
    def test_dict_config_attribute(self):
        self.assertEqual(TestBaseDictConfig.config.value, 1)
        obj = TestBaseDictConfig()
        self.assertEqual(obj.config.value, TestBaseDictConfig.config.value)

    def test_dict_config_singleton(self):
        obj = TestBaseDictConfig()
        obj.config.value = 3
        self.assertEqual(TestBaseDictConfig.config.value, 3)
        self.assertEqual(obj.config.value, TestBaseDictConfig.config.value)
    ...
    def test_dict_config_delete(self):
        ...
        with self.assertRaisesRegex(AttributeError, "'dict' object has no attribute 'Config'"):
            obj.Config # pylint: disable=pointless-statement
    ...
    def test_class_attributes(self):
        ...
        self.assertDictEqual(
            attrs,
            {
                ...
                '_is_frozen': bool,
                '_silent': bool,
                'config': BaseObject.config.__class__,
                'p01': BaseField(attr_type=str, default='Parent field'),
                'p02': str,
                'p03': int,
                'p04': float
