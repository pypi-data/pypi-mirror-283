...
class MetaClass(type):
    ...
    def __call__(cls, *args: tuple, **kwargs: dict) -> Self:
        ...
        silent = kwargs.pop('silent', cls._silent)
        ...
        ## Here we create the object and initialize
        ...
        try:
            obj._is_frozen = obj.config.frozen # pylint: disable=attribute-defined-outside-init
...
class BaseObject(_BaseObject):
    ...
    config: BaseObjectConfig = None

    def _to_dict(self, add_class_path=True) -> dict:
        """
        ...
        for key, value in self.__dict__.items():
            if key not in self.config.exclude_keys:
                key = self.config.key_mapping.get(key, key)
...
class BaseDictConfig(BaseObjectConfig):
    keys_blacklist: frozenset[str] = frozenset(['config'])
...
class BaseDict(BaseObject):
    ...
    def __setitem__(self, key: str, item: Any, caller: str = None) -> None:
        ...
        if key in self.config.keys_blacklist:
    ...
    def is_valid_key(self, key: str) -> bool:
        ...
        are not in the self.config.keys_blacklist.
        ...
        """
        return not key.startswith('_') and key not in self.config.keys_blacklist
    ...
    def update(self, *args, **kwargs) -> None:
        ...
        for key in dict(*args, **kwargs).keys():
            if not self.is_valid_key(key=key):
                raise KeyError(f'The key cannot be called "{key}".')
