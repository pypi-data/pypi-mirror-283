...
class BigQueryConnector:
    ...
    def connect(self):
        ...
        try:
            ...
            Logger.info(f"Project = {self.project_id} on Dataset = {self.dataset_id}")
    ...
    def query_rows(self, sql_query):
        ...
        Logger.info("Start query_rows")
        try:
            Logger.info(f"SQL = {sql_query}")
            query_job = self.client.query(sql_query)
            results = query_job.result().total_rows
            Logger.info(results)
            Logger.info("Finish query_rows")
            return results
        except Exception as e:
            Logger.error(f'Error SQL Query: {str(e)}')
            return None
    ...
    def query_export(self, sql_query, output_csv_path):
        ...
        :return: integer with the number of rows."""
        Logger.info("Start query_export")
        try:
            Logger.info(f"SQL = {sql_query} to  path {output_csv_path}")
            query_job = self.client.query(sql_query)
            df = query_job.to_dataframe()
            df.to_csv(output_csv_path, index=False)
            Logger.info(df)
            Logger.info("Finish query_export")
            return 0
        except Exception as e:
    ...
    def dataset_tables(self):
        ...
        Logger.info("Start dataset_tables")
        try:
            ...
            Logger.info("Finish dataset_tables")
            return table_names
        except Exception as e:
    ...
    def insert_from_bucket(self, bucket_name, source_blob_name, destination_table_name):
        ...
        :return: None."""
        Logger.info("Start insert_from_bucket")
        try:
            ...
            Logger.info(result)
            Logger.info(f'Data loaded in table "{destination_table_name}" from bucket "{bucket_name}"')
            Logger.info("Finish insert_from_bucket")
            return result
        except Exception as e:
