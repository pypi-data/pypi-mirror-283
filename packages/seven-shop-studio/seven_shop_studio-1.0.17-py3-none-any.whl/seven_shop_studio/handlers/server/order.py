...
:LastEditTime: 2024-07-05 16:54:05
            ...
            if refund_model["real_refund_price"]>0:
                ...
                if refund_conn.update_table("status = 5,finish_time = %s","refund_order_id = %s",[TimeHelper.get_now_timestamp(),refund_order_id]):
                    ...
                    if refund_conn.check_refund_complete(refund_model["order_id"]):
                        # 退还优惠券
                        CouponRecordModel().update_table("order_id = '',use_time = 0,status = 0","id = %s",order_model["coupon_id"])
                        # 修改订单状态为已取消
                        OrderModel().update_table("status = 1,finish_time = %s","order_id = %s",[TimeHelper.get_now_timestamp(),refund_model["order_id"]])
                    return self.reponse_json_success(desc="提交成功") 
