...
:LastEditTime: 2024-06-26 15:12:36
...
class DataHelper:
    ...
    def get_shop_data(self,shop_id,begin_time,end_time):
        ...
        ret_data["total_price"] = order_conn.get_dict(where="shop_id = %s and pay_time > 0",params=shop_id,field="ifnull(sum(real_pay_price),0) as total_price")["total_price"]
        ret_data["total_count"] = order_conn.get_total(where="shop_id = %s and pay_time > 0",params=shop_id)
        
        order_list = order_conn.get_dict_list(field="real_pay_price,user_code,buy_count,channel_id,add_time", where="shop_id = %s and add_time >= %s and add_time < %s and pay_time > 0",params=[shop_id,begin_time,end_time])
        if order_list:
        ...
        refund_list = refund_conn.get_dict_list(field="real_refund_price,user_code,finish_time",where="shop_id = %s and finish_time >= %s and finish_time < %s and status = 5 and refund_status = 2",params=[shop_id,begin_time,end_time])
        if refund_list:
        ...
        while end_time > begin_time:
            ...
            if day_order_list:
                day_model["day_pay_price"] = sum([x["real_pay_price"] for x in day_order_list])
            ...
            if refund_list:
                day_model["day_refund_price"] = sum([x["real_refund_price"] for x in day_refund_list])
