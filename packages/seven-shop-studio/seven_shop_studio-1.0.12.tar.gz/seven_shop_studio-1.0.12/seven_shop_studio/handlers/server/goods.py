...
:LastEditTime: 2024-06-26 16:17:07
...
class GoodsListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        goods_list,goods_count = GoodsModelEx().get_goods_list(page_index,page_size,condition,f"{order_field} {order_by}",param_list)
        if goods_list:  
        ...
        ret_data = {
            ...
            "count": goods_count,
            "series_list": ShopSeriesModelEx().get_dict_list(where="status = 1 and shop_id = %s",params=shop_id,field="id,series_name",order_by="sort desc")
        }
...
class GoodsInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        goods_model = GoodsModel().get_dict(where="id = %s and shop_id = %s",params=[goods_id,shop_id],field="id,series_id,goods_main_images,goods_detail_images,goods_long_name,goods_short_name,goods_remark,goods_parameter,goods_service,original_price,price,goods_code,sort,status")
        ...
        # 获取sku信息
        goods_model["sku_info"] = GoodsSkuModelEx().get_goods_sku_info(goods_id)
        if not goods_model["sku_info"]["sku_list"]:
            # 获取主商品库存
            goods_model["inventory"] = InventoryModelEx().get_inventory(goods_id,0)
        else:
    ...
    def post_async(self):
        ...
        if goods_entity.id > 0:
            ...
            if not sku_entity_list:
                # 修改商品库存
                inventory_result = self.edit_inventory(goods_entity.id,0,goods_info["inventory"],self.request_user_id(),"系统库存修改")
                if inventory_result:
            ...
            if new_sku_entity:
                ...
                for item in sku_entity_list:
                    ...
                    if sku_result:
                        sku_inventory_result = self.edit_inventory(goods_entity.id,sku_result,item.inventory,self.request_user_id())
                ...
                for item in sku_entity_list:
                    ...
                    if old_sku_model:
                        ...
                        # 修改库存
                        sku_inventory_result = self.edit_inventory(goods_entity.id,item.id,item.inventory,self.request_user_id())
                return self.reponse_json_success(desc="提交成功")
            ...
            if goods_result:
                ...
                if sku_entity_list:
                    ...
                    for item in sku_entity_list:
                        ...
                        sku_result = sku_conn.add_entity(item)
                        if sku_result:
                            sku_inventory_result = self.edit_inventory(goods_result,sku_result,item.inventory,self.request_user_id())
                            add_result = sku_inventory_result if add_result==True else add_result
                    if add_result:
                        return self.reponse_json_success(desc="提交成功")
                    ...
                    # 添加商品库存信息
                    if self.edit_inventory(goods_result,0,goods_info["inventory"],self.request_user_id()):
                        return self.reponse_json_success(desc="提交成功")
    ...
    def edit_inventory(self,goods_id,sku_id,inventory_number,user_code,remark = "初始化添加"):
        """
        :description: 修改商品库存
        :last_editors: KangWenBin
        """
        ...
        if db_transaction.commit_transaction():
            # 库存数量验证
            InventoryModelEx().check_goods_inventory(goods_id,sku_id)
            return True
    ...
    def put_async(self, *args, **kwargs):
        ...
        # TODO 执行业务
        if GoodsModel().update_table("status = %s","id in %s and shop_id = %s",[status,goods_list,shop_id]):
    ...
    def delete_async(self, *args, **kwargs):
        ...
        good_model_list = GoodsModel().get_dict_list(where="id in %s and shop_id = %s",params=[goods_list,shop_id],field="status")
        if good_model_list:
        ...
        # TODO 执行业务
        GoodsModel().update_table("status = -1","id in %s and shop_id = %s",[goods_list,shop_id])
...
class GoodsRecommendListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if sel_goods_name:
            condition += " and b.goods_long_name like %s"
            param_list.append(f"%{sel_goods_name}%")
        ...
        goods_list,goods_count = GoodsModelEx().get_goods_recommend_list(page_index,page_size,condition,f"{order_field} {order_by}",param_list)
        if goods_list:  
            sku_conn = GoodsSkuModelEx()      
            for item in goods_list:
                # 商品主图
        ...
        ret_data = {
            "model_list": goods_list,
            "count": goods_count,
            "series_list": ShopSeriesModelEx().get_dict_list(where="status = 1 and shop_id = %s",params=shop_id,field="id,series_name")
        }
    ...
    def post_async(self):
        ...
        # 判断商品是否已经存在
        goods_list = GoodsModelEx().get_dict_list(where="status > -1 and shop_id = %s and id in %s",params=[shop_id,goods_id_list],field="id")
        if goods_list:
            conn = GoodsRecommendModel()
            for item in goods_list:
                recommend_entity = GoodsRecommend()
                recommend_entity.goods_id = item["id"]
                recommend_entity.sort = 0
                recommend_entity.add_time = TimeHelper.get_now_timestamp()
                conn.add_entity(recommend_entity)
            return self.reponse_json_success(desc="提交成功")
        else:
    ...
    def put_async(self):
        ...
        GoodsRecommendModel().update_table("sort = %s","goods_id = %s",[sort,goods_id])
        return self.reponse_json_success(desc="提交成功")
    ...
    def delete_async(self):
        ...
        if GoodsRecommendModel().del_entity(where="goods_id = %s",params=[goods_id]):
            return self.reponse_json_success(desc="提交成功")
