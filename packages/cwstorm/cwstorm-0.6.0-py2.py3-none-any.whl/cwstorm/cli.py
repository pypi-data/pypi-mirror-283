...
@click.option("-d", "--docinfo", is_flag=True, help="Print the docinfo JSON.")
...
@click.option(
    ...
    help=FORMAT_HELP,
    default="html",
    type=click.Choice(choices=["markdown", "html", "json", "pretty"], case_sensitive=False),
)
def schema(fmt):
    """
Display the schema.

Shows the attributes of classes in the DSL in a web page or as markdown or export to JSON.
    """
    ...
    if fmt == "markdown":
        display_schema_markdown(*klasses)
    elif fmt == "html":
        display_schema_html(*klasses)
    elif fmt in ["json", "pretty"]:
        result = get_schema_obj(*klasses)
        if fmt == "json":
...
def get_schema_obj(*klasses):
    class_docs = []
    for klass in klasses:
        class_docs.append(class_schema_object(klass))
...
def get_api_obj(attr_name, datatype):
    if datatype in ["bool", "int", "str"]:
        return [f"n.{attr_name}(value) -> self", f"n.{attr_name}() -> bool"]
