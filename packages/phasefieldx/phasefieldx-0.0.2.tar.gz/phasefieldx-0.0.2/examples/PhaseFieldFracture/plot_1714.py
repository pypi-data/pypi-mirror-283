...
# Mesh Definition
# ---------------
msh_file = os.path.join("mesh", "mesh2.msh")
gdim = 2
gmsh_model_rank = 0
...
# def top(x):
#     return np.logical_and(np.isclose(x[1], 2), np.greater(x[0], -0.25),  np.less(x[0],0.25) )
...
# dt = 10**-3
# uy_1 = np.arange(0.0, 50*dt,  dt)

# values = uy_1
# steps = np.arange(0, len(values))

# def update_boundary_conditions(bcs, step):
#     bcs[0].g.value[...] = petsc4py.PETSc.ScalarType(-values[int(step)])
#     return 0, values[int(step)], 0
...
ax_reaction.plot(displacement, -2*S.reaction_files['top.reaction']["Ry"], 'k.', linewidth=2.0, label=S.label)
