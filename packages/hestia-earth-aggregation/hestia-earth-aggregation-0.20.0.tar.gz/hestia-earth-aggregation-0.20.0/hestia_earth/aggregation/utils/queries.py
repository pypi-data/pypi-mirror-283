...
def _source_query(title: str):
    return {'match': {'defaultSource.bibliography.title.keyword': title}}
...
def _product_query(product_name: str = None, match_aggregated=False):
    return {
        'bool': {
            ...
            'must_not': list(map(_source_query, EXCLUDE_BIBLIOS)) + non_empty_list([
...
def _query_all_nodes(product_name: str, start_year: int, end_year: int, country_name: str):
...
def _country_nodes(product_name: str, start_year: int, end_year: int, country_name: str):
    ...
    nodes = _query_all_nodes(product_name, start_year, end_year, country_name)
...
def find_nodes(product: dict, start_year: int, end_year: int, country: dict):
    ...
    nodes = (
        _sub_country_nodes(
            ...
            product.get('name'), start_year, end_year, country_name
        )
