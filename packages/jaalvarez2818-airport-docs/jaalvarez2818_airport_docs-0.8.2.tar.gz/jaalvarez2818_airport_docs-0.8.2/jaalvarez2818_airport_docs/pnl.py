...
class PNL:
    ...
    def __init__(self, contact_info: ContactInfo, pax_list: List[Pax], flight: Flight, split_doc_limit: int = 58,
        ...
        self.is_adl = is_adl
        prefix = ('PNL', 'ADL')[is_adl]
        self.filename = f"{prefix}_{self.flight.flight_number}_{format(self.flight.local_departure_date, '%y%m%d')}_{self.flight.departure_airport_code}.txt"
