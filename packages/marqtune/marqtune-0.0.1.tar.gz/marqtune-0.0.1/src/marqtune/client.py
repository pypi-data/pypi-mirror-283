...
class BaseResource(ABC):
    ...
    def _wait_for_completion(self):
        ...
        while True:
            ...
            if secondary_status_lower in BaseResource.WAIT_STATUS:
            ...
            elif secondary_status_lower in BaseResource.LOGS_STATUS:
                if last_print_was_status:
                    print_status(status_to_print)
                    print()
                ...
                if len(logs) > 0:
                    last_timestamp = logs[-1]["timestamp"] + 1  # Add 1 to avoid duplicate logs
                    log_messages = [log["message"] for log in logs]
                    print("\n".join(log_messages))
            elif status_lower in BaseResource.TERMINAL_STATUS:
                print_status(status_to_print)
                print()
                break
            else:
...
class Model(BaseResource):
    ...
    @classmethod
    def train(cls, http: HttpRequests, dataset_id: str, model_name: str, base_model: str, base_checkpoint: str,
              model_type: ModelType, instance_type: InstanceType, hyperparameters: dict, max_training_time: int = None,
              wait_for_completion: bool = True) -> "Model":
        request_body = {
...
class Client:
    ...
    def train_model(self, dataset_id: str, model_name: str, base_model: str, base_checkpoint: str, model_type: ModelType,
                    instance_type: InstanceType, hyperparameters: dict, max_training_time: int = None,
                    wait_for_completion: bool = True) -> Model:
        """
        Trains a new model.
        Args:
