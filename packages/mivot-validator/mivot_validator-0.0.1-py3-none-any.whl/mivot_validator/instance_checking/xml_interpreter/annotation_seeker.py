...
from mivot_validator.instance_checking.xml_interpreter.vocabulary import Att, Ele
...
class AnnotationSeeker(object):
    ...
    def get_globals_instance_from_collection(self, sourceref, pk_value):
        ...
        :param dmid:
        """
        einst = self._globals_block.xpath(
            "//COLLECTION[@dmid='"
            + sourceref
            + "']/INSTANCE/PRIMARY_KEY[@value='"
            + pk_value
            + "']"
        )
        for inst in einst:
    ...
    def get_instance_by_dmtype(self, dmtype_pattern):
        ...
        eset = self._globals_block.xpath(
            ".//INSTANCE[contains(@dmtype,'" + dmtype_pattern + "')]"
        )
        retour["GLOBALS"] = eset

        for tableref, block in self._templates_blocks.items():
            retour["TEMPLATES"][tableref] = block.xpath(
                ".//INSTANCE[contains(@dmtype,'" + dmtype_pattern + "')]"
            )
        return retour
