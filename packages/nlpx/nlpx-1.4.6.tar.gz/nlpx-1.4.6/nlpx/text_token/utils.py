...
# -------------------------------------------------pad function--------------------------------------------------------
def _pad(tokens: List[int], max_length: int = None, truncation=True, padding=True, padding_side='right', pad_begin_end=False,
...
def pad(tokens: List[int], max_length: int = None, truncation=True, padding=True, padding_side='right', pad_begin_end=False,
        pad_value=0, bos_value=1, eos_value=2):
    """
    :param tokens: (1维)
    :param max_length:
...
def batch_pad(batch: Union[map, Iterable[Iterable[int]]], max_length: int = None, truncation=True, padding=True,
              padding_side='right', pad_begin_end=False, pad_value=0, bos_value=1, eos_value=2):
    """
    :param batch: (2维)
    :param max_length:
    :param truncation:
    ...
    if max_length is None or max_length <= 0:
        ...
        max_length = max(len(item) for item in batch_copy)
...
def _cn_char_handle(part):
    ...
    chars = map(str.strip, chars)
    return (ch for ch in chars if ch)
