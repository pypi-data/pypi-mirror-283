...
class BaseTokenizer:
    ...
    def __init__(self, file: str = None, corpus: Iterable[str] = None, cut_corpus: Iterable[Iterable[str]] = None,
                 vocab: Collection[str] = None, min_frequency=5, special_tokens: Collection[str] = None, language='cn',
                 cut_type='word', word_frequency=False, stop_words: Collection[str] = None, cut_fn=None):
        """
        ...
        if vocab:
            ...
            if cut_corpus is not None:
                if stop_words:
                    cut_corpus = filter_stop_words(cut_corpus, stop_words, language)
    ...
    @classmethod
    def from_file(cls, file: str, encoding=UTF8, pattern='*', func=read_file, min_frequency=0,
                  special_tokens: Collection[str] = None, language='cn', cut_type='word', word_frequency=False,
        ...
        return cls(corpus=corpus, min_frequency=min_frequency, special_tokens=special_tokens, language=language,
                   cut_type=cut_type, word_frequency=word_frequency, stop_words=stop_words, cut_fn=cut_fn)

    @classmethod
    def from_corpus(cls, corpus: Iterable[str], min_frequency=0, special_tokens: Collection[str] = None, language='cn',
                    cut_type='word', word_frequency=False, stop_words: Collection[str] = None,cut_fn=None):
        """
        :param corpus: 语料，每个元素是一句话，如：['上课时学生手机响个不停，老师一怒之下把手机摔了。', '家长拿发票让老师赔', ....]
        :param min_frequency: 最小词频，小于词词频的词会被忽略，默认是0，所有的词都保留
    ...
    @classmethod
    def from_cut_corpus(cls, cut_corpus: Iterable[Iterable[str]], min_frequency=0, special_tokens: Collection[str] = None,
                        language='cn', cut_type='word', word_frequency=False, stop_words: Collection[str] = None, cut_fn=None):
        """
        ...
        :param cut_fn
        """
        return cls(cut_corpus=cut_corpus, min_frequency=min_frequency, special_tokens=special_tokens, language=language,
                   cut_type=cut_type, word_frequency=word_frequency, stop_words=stop_words, cut_fn=cut_fn)
...
class Tokenizer(BaseTokenizer):
    ...
    def __init__(self, file: str = None, corpus: Iterable[str] = None, cut_corpus: Iterable[Iterable[str]] = None,
                 vocab: Iterable[str] = None, min_frequency=10, special_tokens: Collection[str] = None, language='cn',
                 cut_type='word', word_frequency=False, stop_words: Collection[str] = None, cut_fn=None):
        """
        ...
        :param cut_fn
        """
        super().__init__(file=file, corpus=corpus, cut_corpus=cut_corpus, vocab=vocab, min_frequency=min_frequency,
                         special_tokens=special_tokens,language=language, cut_type=cut_type,
                         word_frequency=word_frequency, stop_words=stop_words, cut_fn=cut_fn)
