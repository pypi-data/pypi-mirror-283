...
LOGGER = create_logger("INOOPA.INOOPA_UTILS.MAIL_CHECKER")
...
def find_personal_emails_bulk(domain: str, names: list[str], check_catchall: bool = True, normalize: bool = True) -> dict[str, list[str]] | None:
    ...
    if _check_email_is_catchall(domain):
        LOGGER.info(f"{domain} is a catch-all domain.")
        return None
...
def _generate_generic_emails(domain: str, custom_prefixes: list[str] | None = None) -> list[str]:
    ...
    common_prefixes = [
        ...
        "jobs", "customer", "office", "sales", "marketing", "hr", "accounts", "billing", "finance",
        "operations", "it", "admin", "design", "engineering", "feedback", "dev", "developer", "tech",
    ]
