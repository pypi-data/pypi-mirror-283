...
from playwright.async_api import async_playwright, Browser, Playwright as AsyncPlaywright, BrowserContext, Page
...
class BasePlugin(metaclass=ABCMeta):
    ...
    def is_busy(self, b: bool):
        if self._is_busy != b:
            ...
            if b:
                ...
                logger.info(f"busy count of plugins is {type(self)._busy_count} (incremented)")
            else:
                type(self)._busy_count -= 1
                logger.info(f"busy count of plugins is {type(self)._busy_count} (decremented)")
    ...
    async def study(self, task: StudyUrlTask) -> List[BaseTag]:
        async with study_url_browser.open(task, timeout=3000) as cm:
