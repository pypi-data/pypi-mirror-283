...
from .types import QueueRole, QueueTopicMessage, QueueMessage
...
class RabbitmqQueue(Stoppable):
        ...
        try:
            while not await self.is_stopped():
                ...
                async with connection:
                    ...
                    await channel.declare_queue(self.queue_name, durable=True, arguments=arguments)

                    # TODO: can combine those conditions into single declare_exchange() expression
                    # logger.info(f"{self.params.exchange_type=}")
                    ...
                    try:
                        while not await self.is_stopped():
                            ...
                            if message is not None:
                                ...
                                if isinstance(message, QueueTopicMessage):
    ...
    async def create_channel(self, connection):
        ...
        if self.params.x_max_priority is not None:
            arguments["x-max-priority"] = self.params.x_max_priority
        self.receiver_queue = await channel.declare_queue(self.queue_name, durable=True, arguments=arguments)
