...
from starburst_client.model import (
    Role,
    User,
    Group,
    RowFilterExpression,
...
class Starburst:
    ...
    def get_column_mask_expression(self, column_mask_expression_id):
        return ColumnMaskExpression(
            self.client.get_column_mask_expression(column_mask_expression_id),
            self.client,
        )

    def get_column_mask_expressions(self):
        for column_mask_expression in self.client.list_column_mask_expressions():
            # yield column_mask_expression
            yield ColumnMaskExpression(column_mask_expression, self.client)

    def create_column_mask_expression(
        self, name: str, expression: str, description: str | None = None
    ) -> ColumnMaskExpression:
        return ColumnMaskExpression(
            self.client.create_column_mask_expression(name, expression, description),
            self.client,
        )
