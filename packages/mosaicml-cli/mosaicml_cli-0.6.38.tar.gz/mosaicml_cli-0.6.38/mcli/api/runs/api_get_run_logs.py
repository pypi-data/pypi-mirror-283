...
def get_run_logs(
    ...
    *,
    ...
    # first-exception and (any node or gpu) inputs are mutually exclusive
    if first_exception:
        if filters.get('nodeRank') is not None:
            raise ValueError('Cannot use --first-exception with node specified. Please specify one or the other.')
        if filters.get('localGpuRank') is not None:
            raise ValueError('Cannot use --first-exception with GPU rank specified. Please specify one or the other.')
        if filters.get('globalGpuRank') is not None:
            raise ValueError(
                'Cannot use --first-exception with Global GPU rank specified. Please specify one or the other.')
