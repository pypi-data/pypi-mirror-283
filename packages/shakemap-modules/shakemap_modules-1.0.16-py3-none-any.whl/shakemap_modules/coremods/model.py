...
class ModelModule(CoreModule):
    ...
    def execute(self, indir=None, outdir=None):
        ...
        # Get a set of random finite faults
        ...
        if (
            ...
            self.ipe_gmpe = MultiGMPE.__from_config__(self.config, filter_imt=pgv_imt)
        ...
        if (
            isinstance(self.rupture_obj, PointRupture)
            and isinstance(self.ipe, VirtualIPE)
            and "MMI" in self.imt_out_set
        ):
            self.imt_out_set.add(self.ipe.imt.string)
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            if self.do_grid:
                griddict = ffsim.compute_grid(vs30grid, gmpes, list(myimts))
                gsa = GenericSiteAmplification(
                    self.origin.tectonic_region,
                    self.sites_obj_out.getVs30Grid().getData(),
                )
                oqpga = imt.PGA()
                for oqimt in myimts:
                ...
                if PGV() in myimts:
                    ...
                    # Just get anything... (this should never happen)
                    for oqimt in myimts:
                        break
                shapes = []
                for k, v in self.sx_out.__dict__.items():
                    if k == "_slots_":
                        continue
                    shapes.append(v.shape)
                    self.sx_out.__dict__[k] = np.reshape(self.sx_out.__dict__[k], (-1,))
                shapeset = set(shapes)
                if len(shapeset) != 1:
                    raise ValueError("All sites elements must have same shape.")
                orig_shape = list(shapeset)[0]
                ptdict = ffsim.compute_points(self.sx_out, gmpes[oqimt], list(myimts))
                griddict = {}
                for oqimt in myimts:
                    griddict[oqimt] = {}
            ...
            for dd in (self.sx_out, self.dx_out):
                for k, v in dd.__dict__.items():
                    if k == "_slots_":
                        continue
                    if k not in ("lons", "lats") and v is not None:
                        shapes.append(v.shape)
                        dd.__dict__[k] = np.reshape(dd.__dict__[k], (-1,))
            shapeset = set(shapes)
            if len(shapeset) != 1:
            ...
            for dd in (self.dx_out, self.sx_out):
                for k, v in dd.__dict__.items():
                    if k == "_slots_":
                        continue
                    if k not in ("lons", "lats") and v is not None:
                        dd.__dict__[k] = np.reshape(dd.__dict__[k], orig_shape)
    ...
    def _populate_data_frames(self):
        ...
        for dfid in self.dataframes:
            ...
            myimts = []
            for imt_str in imt_set:
                oqimt = imt.from_string(imt_str)
                myimts.append(oqimt)

            gmpe = MultiGMPE.__from_config__(self.config, filter_imt=PGV())
            pdict = {}
            if len(myimts) > 0:
                pdict = self.get_point_amps(gmpe, myimts, dfn)
    ...
    def get_point_amps(self, gmpe, imt_set, dfn):
        """
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            if immi in imt_set:
                ...
                pdict = {}
            if len(imt_set) > 0:
                pdict = ffs.compute_points(dfn.sx, gmpe, imt_set)
            if do_mmi:
                mdict = ffs.compute_points(dfn.sx, self.ipe, [immi])
                pdict[immi] = mdict[immi]
        else:
            pdict = {}
            for oqimt in imt_set:
    ...
    def _derive_imts_from_mmi(self):
        ...
        if PGV() in imtlist:
            ...
            gmpe = MultiGMPE.__from_config__(self.config, filter_imt=PGA())
        pdict = self.get_point_amps(gmpe, imtlist, this_df)
    ...
    def _derive_mmi_from_imts(self):
        ...
        pdict = self.get_point_amps(self.ipe, [immi], this_df)
