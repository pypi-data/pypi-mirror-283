...
@app.command(rich_help_panel=HELP_PANEL_UPDATE)
@catch_api_exception
def update_php_setting(cluster_name: str, key: str, value: str) -> None:
    """Update PHP setting."""
    cluster = get_object(get_support().clusters, name=cluster_name)
...
@app.command(rich_help_panel=HELP_PANEL_UPDATE)
@catch_api_exception
def update_http_retry_tries_amount(
    cluster_name: str, tries_amount: int
) -> None:
...
@catch_api_exception
@exit_with_status
def remove_nodejs_versions(
    cluster_name: str, nodejs_versions: List[str]
) -> int:
...
def update_mariadb_cluster_name(
    cluster_name: str, mariadb_cluster_name: str
) -> None:
