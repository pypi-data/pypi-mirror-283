...
def watch_access_logs(
    ...
    interval: float = typer.Option(
        default=2.0, help="Time in seconds between updates"
    ),
    lines: int = typer.Option(
        default=10, help="Amount of previous lines to print"
    ),
    status_code: Optional[int] = typer.Option(
        None, "--status-code", "-s", help="Filter by status code"
    ),
...
def show_logs(
    *,
    ...
    output = "\n".join(
        line.raw_message for line in logs if _show_line(line, filters)
    )
...
def _show_line(
    line: Union[AccessLog, ErrorLog], filters: dict[str, Any]
) -> bool:
