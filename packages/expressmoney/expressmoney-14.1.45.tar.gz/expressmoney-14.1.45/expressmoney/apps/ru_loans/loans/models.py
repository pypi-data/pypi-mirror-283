...
class Loan(models.Model):
    ...
    penalty_charged = models.DecimalField('Начислено пени', max_digits=7, decimal_places=0, default=Decimal(0))
    penalty_paid = models.DecimalField('Выплачено пени', max_digits=7, decimal_places=0, default=Decimal(0))
    body_paid = models.DecimalField('Выплачено тело', max_digits=7, decimal_places=0, default=Decimal(0))

    @property
    def penalty_balance(self):
        """Долг по пени"""
        return self.penalty_charged - self.penalty_paid
    ...
    def balance(self):
        """Общий долг"""
        return self.penalty_balance + self.interests_balance + self.body_balance
