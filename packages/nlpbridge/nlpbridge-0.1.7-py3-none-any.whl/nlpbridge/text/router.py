...
class Node(Callable[[dict], TypedDict]):
    ...
    def _init_agent(self):
        ...
        self.agent = ChatAgent(os.getenv("CONFIG_PATH"), self.chat_id, self.llm, self.tool_names, prompt, True)
...
class RouterManager:
    ...
    def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
                  node_cls: Node = Node) -> Tuple[CompiledGraph, dict]:
        ...
        runnable_nodes = self.convert_to_runnable_node(nodes, edges, llm, chat_id, node_cls)
    ...
    @staticmethod
    def convert_to_runnable_node(nodes: List[NodeModel], edges: List[Edge], llm, chat_id: str, node_cls: Node) -> List[Node]:
        runnable_nodes = []
        for node in nodes:
