...
def servo_motor(pin, angle, speed):
    ...
    buf = _make_buf(0x81, pin&0xFF, angle&0xFF, speed&0xFF)
    sendbuf = bytearray(buf)
    _printf(f"전송 버퍼: {buf}")
    comm.write(sendbuf)
    return True
...
def dc_motor_all_on(l1, r1, l2, r2):
    ...
    buf = _make_buf(0x82, l1&0xFF, r1&0xFF, l2&0xFF, r2&0xFF)
    sendbuf = bytearray(buf)
    _printf(f"전송 버퍼: {buf}")
    comm.write(sendbuf)
    return True
...
def dc_motor1_on(l1, r1):
    ...
    buf = _make_buf(0x85, l1&0xFF, r1&0xFF)
    sendbuf = bytearray(buf)
    _printf(f"전송 버퍼: {buf}")
    comm.write(sendbuf)
    return True
...
def dc_motor2_on(l2, r2):
    ...
    buf = _make_buf(0x86, l2&0xFF, r2&0xFF)
    sendbuf = bytearray(buf)
    _printf(f"전송 버퍼: {buf}")
    comm.write(sendbuf)
    return True
