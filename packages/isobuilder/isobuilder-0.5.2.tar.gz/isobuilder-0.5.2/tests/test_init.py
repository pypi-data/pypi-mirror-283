...
class TestIsoBuilder:
    ...
    def test_write_custom_files(self, mock_update_md5):
        ...
        assert (
            f"netcfg/choose_interface string {self.iso_host.ipv4_primary}"
            in dnsops_seed
        )
        assert (
            f"netcfg/get_ipaddress string {self.iso_host.ipaddress.ip}" in dnsops_seed
        )
        assert (
            f"netcfg/get_netmask string {self.iso_host.ipaddress.netmask}"
            in dnsops_seed
        )
        assert f"netcfg/get_gateway string {self.iso_host.gateway}" in dnsops_seed
