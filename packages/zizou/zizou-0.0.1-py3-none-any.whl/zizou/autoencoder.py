...
class AutoEncoder(AnomalyDetectionBaseClass):
    ...
    def fit(self, fq):
        ...
        try:
            ...
            data_norm_train, data_norm_test = train_test_split(data_norm,
                                                            test_size=.2,
                                                            train_size=.8)
            train_data = zizouData(data_norm_train, transform=torch.from_numpy)
            test_data = zizouData(data_norm_test, transform=torch.from_numpy)
    ...
    def transform(self, fq, cluster=True):
        ...
        transform_data = zizouData(data_norm,
