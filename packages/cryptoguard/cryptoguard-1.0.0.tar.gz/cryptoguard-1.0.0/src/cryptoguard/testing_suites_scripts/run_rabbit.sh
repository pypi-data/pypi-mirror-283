...
{ time "$TEST_SUITE_DIR/testu" "${BINARY_FILE}" "${COUNT_BITS}" rabbit > "${RESULT_DIR}/test_rabbit.log"; } 2> "$RESULT_DIR/time_rabbit"
echo -e "\nRabbit" >> "$RESULT_DIR/time"
cat "$RESULT_DIR/time_rabbit" >> "$RESULT_DIR/time"
rm "$RESULT_DIR/time_rabbit"

echo "TestU01 Rabbit test completed."
