...
{ time "$TEST_SUITE_DIR/testu" "${BINARY_FILE}" "${COUNT_BITS}" crush > "${RESULT_DIR}/test_crush.log"; } 2> "$RESULT_DIR/time_crush"
echo -e "\nCrush" >> "$RESULT_DIR/time"
cat "$RESULT_DIR/time_crush" >> "$RESULT_DIR/time"
rm "$RESULT_DIR/time_crush"

echo "TestU01 Crush test completed."
