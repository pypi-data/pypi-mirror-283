...
def get_an_history(delta_date=90):
    """Get the number of times an object appeared on the anomaly top-10 list
...
def get_data_permalink_slack(
    ztf_id, kind="Difference", slack_token_envvar="ANOMALY_SLACK_TOKEN"
):
    """Load cutout and light curve via the Fink API and copy them to the Slack server
    ...
    kind: str, optional
        Type of cutouts: Science, Difference, Template.
        Default is Difference.
    slack_token_envvar: str, optional
        Name of the environment variable containing the
        Slack token. Default is ANOMALY_SLACK_TOKEN
    ...
    """
    cutout = get_cutout(ztf_id, kind)
    curve = get_curve(ztf_id)
    if slack_token_envvar in os.environ:
        slack_client = WebClient(os.environ[slack_token_envvar])
    else:
        raise KeyError(
            "You need to set up {} in your .bashrc".format(slack_token_envvar)
        )
    try:
        curve.seek(0)
        cutout.seek(0)
...
def status_check(res):
    ...
    if res.status_code != 200:
        ...
        requests.post(
            method,
            data={"chat_id": "@fink_test", "text": str(res.status_code)},
            timeout=25,
        )
        return False
    return True
...
def msg_handler_slack(
    slack_data, channel_name, init_msg=None, slack_token_envvar="ANOMALY_SLACK_TOKEN"
):
    """Send notifications to the Slack `channel_name` channel.

    Parameters
    ----------
    slack_data: list
        List of lines. Each item is a separate notification.
        See `get_data_permalink_slack`.
    channel_name: string
        Channel name in Slack
    init_msg: str, optional
        Initial message. Default is None.
    slack_token_envvar: str, optional
        Name of the environment variable containing the
        Slack token. Default is ANOMALY_SLACK_TOKEN
    ...
    try:
        ...
        # TODO: use a Slack channel instead for consistency
        if e.response["ok"] is False:
            requests.post(
                "https://api.telegram.org/bot"
...
def get_cutout(ztf_id, kind="Difference", timeout=25):
    """
