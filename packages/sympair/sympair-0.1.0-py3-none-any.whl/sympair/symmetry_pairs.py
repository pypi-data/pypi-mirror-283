...
from utils import symbol_number, standardize_ijR, inverse_ijR
...
__all__ = ["SymmetryPair", "SymmetryPairGroup", "SymmetryPairGroupDict", "SymmetryPairFinder"]


@dataclass
class SymmetryPair:
    """
    A dataclass to store symmetry atom pairs
    i, j, R are the indices of atoms i and atom (j, R) defines a atom pair
    The igroup is the index of equivalent atom pair group. In each group, the atomic pairs are equivalent, and connected by a rotation matrix and a translation vector to a reference pair.
    """
    ...
    def __repr__(self):
        return f"SymmetryPair({self.i}, {self.j}, {self.R}, {self.igroup})"
...
class SymmetryPairGroupDict(dict):
    ...
    def get_symmetric_pairs(self, i, j, R):
        igroup = self.get_igroup(i, j, R)

    def get_igroup(self, i, j, R):
        """
        Return the igroup of (i, j, R)
        """
        return self[(i, j, R)].igroup
    ...
    def __repr__(self) -> str:
        s = f"===SymmetryPairGroupDict with {self.ngroups} groups ===\n"
        s += "\n".join(
            [
    ...
    def join_reversed_pairs(self):
        ...
        """
        new_groups = SymmetryPairGroupDict(self.atoms)
        for igroup, group1 in enumerate(self.groups):
            key1 = group1[0].reference_pair
            i_group_inv = self.get_igroup(*key1)
            if i_group_inv == igroup:
...
class SymmetryPairFinder:
    ...
    def get_symmetry_pair_group_dict(self):
        """
        Return a dictionary of SymmetryPairGroup objects
        """
        groups = self.group_pairs_by_tag_and_distance()
        ret = SymmetryPairGroupDict(self.atoms)

        for igroup, (key, group) in enumerate(groups):
            spg = SymmetryPairGroup(self.atoms)
            refpair = None
            for ipair, pair in enumerate(group):
                if ipair == 0:
                    refpair = pair
                i, j, R = pair
                sp = SymmetryPair(i=i, j=j, R=R, igroup=igroup, reference_pair=refpair)
                spg.append(sp)
                ret[(i, j, R)] = spg
...
def test_SymmetryPairFinder():
    ...
    d = spf.get_symmetry_pair_group_dict()
    print(d)
