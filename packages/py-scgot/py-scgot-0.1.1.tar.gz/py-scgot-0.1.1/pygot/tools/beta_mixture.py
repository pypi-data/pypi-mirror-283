...
# EM算法主循环
def beta_mixture(data, k, n_iters, tol=1e-4, random_state=None):
...
def beta_mixture_determine_k(adata, time_key, k_range=range(3, 10), n_iters=100):
    np.random.seed(0)  # 固定随机种子
    data = adata.obs[time_key].to_numpy() * 0.9 + 0.05
    res_df = []
    for k in k_range:
        print('fitting k: {}'.format(k))
        params, responsibilities = beta_mixture(data, k, n_iters, random_state=0)
