...
class XCentricTrainer:
    def __init__(self, adata, time_key, embedding_key, 
                 ...
                 TF_loss=False, TF_idx=None,
                 neighbor_sampling=False) -> None:
    ...
    def calcu_loss(self, model, i):
        ...
        #print(group)
        X_pred = odeint(model, X[0], t=torch.Tensor(group), method='rk4')
        loss = self.loss(X_pred, X, group)
        regu_loss = 0
        if self.TF_loss:
            for j in range(len(X) - 1):
                v = model(torch.tensor(group[j]), X[j]).detach()
                G = calcu_jacobian_without_t(model, X[j], torch.tensor(group[j]))
                regu_loss += self.tf_loss(G, X[j], v)
            loss += (regu_loss / (len(X) - 1))
        
        for j in range(len(X) - 1):
            v = model(torch.tensor(group[j]), X[j]).detach()
            G = model.compute_G(torch.tensor(group[j]), X[j])
            l1_loss =  torch.norm(G, p=1)
            #loss += 0.00001 * l1_loss
            #loss += torch.mean((v-(G @ X[j][:,:, None]).squeeze(-1))**2)
