...
def training_embedder(model, adata, loss_func, lr=1e-4, batch_size=128, n_epoch = 100):
    dataloader = AnnLoader(adata, batch_size=batch_size, shuffle=True, use_cuda=False, )
...
def DIM_training_embedder(model, adata, loss_func, dim, n_interpretator, lr=1e-4, batch_size=128, n_epoch = 100):
    dataloader = AnnLoader(adata, batch_size=batch_size, shuffle=True, use_cuda=False, )
    dim_fn = DeepInfoMaxLoss(n_interpretator=n_interpretator, dim=dim)
...
class GS_VAE:
    def __init__(self):
        ...
        print('     gs_vae.register_model')
        print('     gs_vae.fit')

        print('To use GS-VAE, run as following:')
        print('     gs_vae.precompute_gs')
        print('     gs_vae.process_gs')
        print('     gs_vae.register_model')
        print('     gs_vae.fit')
    ...
    def register_model(self, adata, latent_dim, lam=1, hinge_value=1e-2, beta=1e-6):
        if 'idx' in adata.uns.keys():
            ...
            self.trainer = partial(DIM_training_embedder, dim=latent_dim, n_interpretator=len(adata.uns['idx']))
            self.flag = True
        else:
            self.model = VAE(adata.X.shape[1], latent_dim=latent_dim)
            self.loss_func = vae_loss_func
            self.trainer = training_embedder
            self.flag = False
