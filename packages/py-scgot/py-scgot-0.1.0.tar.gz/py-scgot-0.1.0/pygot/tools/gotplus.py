...
import pygot
from pygot.tools.time_estimation import TimeEstimator
from pygot.tools.model_training import train_model
from pygot.tools.flow import pred_velocity
from pygot.tools.beta_mixture import beta_mixture_determine_k
from pygot.plotting import velocity_embedding_stream, plot_root_cell, plot_mst
from pygot.tools.root_identify import determine_source_states, init_candidiates, highlight_extrema, generate_pseudobin

from pygot.tools.mst import determine_topological_tree, adjust_time_by_structure
...
def iterative_training(adata, embedding_key, pseudotime_key, split_k, device, velocity_key=None, plot=False, basis=None, cell_type_key=None, **kwargs):
    ...
    time_estimator = TimeEstimator(adata, embedding_key=embedding_key, n_neighbors=30)
    v_net_train_func = partial(train_model,
    ...
    while True:
        ...
        generate_pseudobin(adata, k=split_k, pseudotime_key=pseudotime_key, sigma=.25)
        ...
        time_estimator.train_time_model(adata, embedding_key, time_key=pseudotime_key, v_net=model.func)
        time_estimator.update_time(adata)
        #plot velocity field
        adata.obsm[velocity_key] = pred_velocity(adata, model.func.net, embedding_key=embedding_key, time_vary=False, time_key=None)
...
def single_branch(adata, cell_type_key, kernel, pseudotime_key):
    if kernel == 'dpt':
        ...
        ot_tree, _, _ = determine_topological_tree(adata, embedding_key='X_pca', cell_type_key=cell_type_key, time_key=pseudotime_key, start_cell_type=adata.obs[cell_type_key].tolist()[adata.uns['ot_root']])
        
        adata.uns['iroot'] = adata.uns['ot_ct_root']
        sc.tl.dpt(adata)
        pseuodtime_ot_ct = adata.obs[pseudotime_key]
        ot_ct_tree, _, _ = determine_topological_tree(adata, embedding_key='X_pca', cell_type_key=cell_type_key, time_key=pseudotime_key, start_cell_type=adata.obs[cell_type_key].tolist()[adata.uns['ot_ct_root']])
    else:
...
def gotplus_pipeline(adata, embedding_key, kernel='dpt', ot_split_k=30, 
                     cytotrace=True, cell_type_key=None,  
                     single_branch_detect=True,  scale=False, 
                     plot=False, basis='umap', fig_dir='./fig', device=None, **kwargs):
    if device is None:
    ...
    if scale and cell_type_key:
        ...
        if tree is None:
            tree, _, _ = determine_topological_tree(adata, embedding_key=embedding_key, cell_type_key=cell_type_key, time_key=pseudotime_key, start_cell_type=adata.obs[cell_type_key].tolist()[adata.uns['ot_root']])
        if plot:
    ...
    split_k_df = beta_mixture_determine_k(adata, pseudotime_key)
    ...
    model = iterative_training(adata, embedding_key, pseudotime_key, split_k=split_k, device=device,
