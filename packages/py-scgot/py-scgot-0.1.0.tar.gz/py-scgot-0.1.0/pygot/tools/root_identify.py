...
def greedy_best_source(adata, embedding_key, kernel='dpt', split_k=None, graph_dist=False, n_neighbors=20, connect_anchor=None, n_waypoints=1200):
    ...
    for i in tqdm(range(len(adata))):
        ...
        generate_pseudobin(data, k=split_k, pseudotime_key=time_key, sigma=0.)
...
def generate_pseudobin(adata, k=4, pseudotime_key = 'potential_pseudotime', sigma=.5, sink=False):
    adata.obs['pseudobin'] = -1
    adata.obs['potential_noise'] = adata.obs[pseudotime_key] + np.random.rand(len(adata)) * sigma
    sorted_idx = adata.obs.sort_values('potential_noise').index
    if 'source_point' in adata.obs.columns and sink:
        adata.obs.loc[adata.obs['source_point'] == 1, 'pseudobin'] = 0
        adata.obs.loc[adata.obs['sink_point'] == 1, 'pseudobin'] = k-1
        subobs = adata.obs.loc[adata.obs['pseudobin'] == -1]
        sorted_idx = subobs.sort_values('potential_noise').index
        bin_idxs = np.array_split(sorted_idx, k-2)
        for i in range(k-2):
            adata.obs.loc[bin_idxs[i], 'pseudobin'] = i+1
    else:
        bin_idxs = np.array_split(sorted_idx, k)
        for i in range(k):
            adata.obs.loc[bin_idxs[i], 'pseudobin'] = i
    adata.obs['pseudobin'] = adata.obs['pseudobin'].astype(float)
...
def determine_source_states(adata, embedding_key, split_k=30, kernel='dpt', n_comps=15, down_sampling=True, n_obs=3000, 
    ...
    res = greedy_best_source(sub_adata, embedding_key, split_k=split_k, kernel=kernel,
