...
class Force(FaasProcessorBase):
    ...
    def submit_workflow(self, force_parameters: Any, user_id: str, job_id: str):
        return super().submit_workflow(
            force_parameters=force_parameters.json(), user_id=user_id, job_id=job_id
        )

    def get_output_stac_items(self, force_parameters: Any) -> list[Item]:
...
class Sen2Like(FaasProcessorBase):
    ...
    def submit_workflow(self, sen2like_parameters: Any, user_id: str, job_id: str):
        return super().submit_workflow(
    ...
    def get_output_stac_items(
        self, sen2like_parameters: Any, target_product: str = "L2F"
    ) -> list[Item]:
        stac_item_paths = []
        ...
        logger.info(
            f"Found {len(stac_item_paths)} items in {sen2like_parameters.output_path}"
        )
...
class Snap(FaasProcessorBase):
    ...
    def get_output_stac_items(self, snap_parameters: Any) -> list[Item]:
...
class VesselDetection(FaasProcessorBase):
    ...
    def get_output_stac_items(self, detection_parameters: Any) -> list[Item]:
        """Not needed."""
        return None
