<script lang="ts">
  ...
  async function getOptions() {
    ...
    const res = await fetch('https://openapi.test.dp.tech/openapi/v1/open/user/project/list', {
      headers: {
        accessKey,
        'x-app-key': xAppKey,
      }
    ...
    if (res.ok) {
      ...
      options = data.data.items.map((item: any) => [item.projectName, item.projectId]);
    }
  }
...
>
  ...
  <label class:container>
    ...
    <select placeholder={placeholder} disabled={!interactive} bind:value>
