...
def init_cluster(price):
    ...
    try:
        ...
        if(price==0):
            ...
            transaction = marketplace_contract.functions.makeDevice(nft_address,token_id,w3.to_wei(price,'ether'),meta_hash,True).build_transaction({
            'value':market_fee,
            'chainId': 31337, 
            'gas': 2000000,  
            'gasPrice': w3.eth.gas_price,  
...
def join_cluster(ip,token,iface):
    ...
    try:
        ...
        if(iface):
            ...
            result = subprocess.check_output(['sh','-', '--node-label', getpass.getuser()], env={'K3S_URL' : 'https://'+ip+':6443', 'K3S_TOKEN': token}, stdin=curl.stdout)
        click.echo(result)
        curl.wait()
    except FileNotFoundError:
