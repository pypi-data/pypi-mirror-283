...
XDG_CONFIG_HOME = os.getenv('XDG_CONFIG_HOME', '{}/.config'.format(os.path.expanduser("~")))
DEFAULT_CONFIG_FILES = ['{}/spacelaser/config'.format(XDG_CONFIG_HOME)]
SCHEMATICS_PATH = '{}/spacelaser/schematics.yaml'.format(XDG_CONFIG_HOME)

class Config:
    def __init__(self):
        config = configargparse.ArgParser(default_config_files=DEFAULT_CONFIG_FILES)

        config.add_argument('-c', '--config', required=False, is_config_file=True, help='config file path')
        config.add_argument('-s','--schematics',type=str, help='Path for the schematics file')
    ...
    def parse(self):
        ...
        if (not self._parsed.schematics):
            self._parsed.schematics=SCHEMATICS_PATH
