...
class TCI:
    ...
    def start(self)->None:
        if not self._run:
            ...
            try:
                ...
                getmsg = loop.create_task(self._getMsgs())
                sendmsg = loop.create_task(self._emptyMsgQ())
                asyncio.gather(getmsg, sendmsg)
            except RuntimeError:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                loop.run_until_complete(self._getMsgs())
                print("running next")
                loop.run_until_complete(self._emptyMsgQ())
    ...
    def connect(self):
        ...
        if not self._server.isConnected():
            ...
            self._login()
        print(self._server.isConnected())
    ...
    async def _getMsgs(self)->None:
        ...
        while self._run:
            if self._server.isConnected():
                try:
                    data = await self._server.receive()
    ...
    async def _emptyMsgQ(self)->None:
        """
        ...
        while self._run:
            ...
            if self._server.isConnected() and not self._sendQ.empty():
                ...
                try:
                    await self._server.send(self._sendQ.get())
                except Exception as error:
                    self._event.emit(self, self._COMMANDS.DISCONNECTED, error)
                    self._run=False
                    break
