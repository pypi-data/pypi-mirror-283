...
class Radman:
    def __init__(self, base_url, username, password,cache_dir='temp/dicom_cache'):
    ...
    def fetch_dicom_file(self, instance_id, cache=True):
        if not self.logged_in:
            self.login()

        cache_file_path = os.path.join(self.cache_dir, f'{instance_id}.dcm')
        
        if cache and os.path.exists(cache_file_path):
            print(f"File {cache_file_path} found in cache.")
        ...
        try:
            ...
            dicom_content = response.content
            
            if cache:
                with open(cache_file_path, 'wb') as file:
                    file.write(dicom_content)
                print(f"File {cache_file_path} saved to cache.")
                
            return dicom_content  # Return the raw DICOM file content
        except requests.exceptions.RequestException as e:
