...
def start_server(al360, host, port, disable_logs):
    ...
    al360._stop_server = lambda: requests.get('http://' + host + ':' + str(port) + '/shutdown?token=' + shutdown_token)
    ...
    def main():
        result = {
            'version': '1.2.0',
            'api': 'al360r_api',
            'timestamp': al360.timestamp*1000,
            'availableParams': al360.list_available_params(),
            'availablePlots': [plot.info for plot in al360.get_supported_plots()],
            'options': { 'attributes': al360.enable_attributes, 'customParams': al360.enable_custom_params }
        }
    ...
    def get_params(request):
        ...
        for param_type in ['model', 'observation', 'variable', 'dataset']:
            ...
            if (not param_label is None) and param_label.startswith('{') and param_label.endswith('}') and param_type == 'observation':
                ...
                param_value = al360.find_param_value(param_type, request.args.get(param_type))
    ...
    def get_plot(plot_type):
        if plot_type == 'timestamp':
            return Response(json.dumps({'timestamp': al360.timestamp * 1000}, default=convert), content_type='application/json')
        ...
        if not al360.enable_custom_params and custom_params:
            abort(403)
            return
        try:
            result = al360.plots_manager.get_plot(plot_type, params, cache=not custom_params)
    ...
    def get_attribute(param_type, param_label):
        if not param_type in ['model', 'variable', 'observation', 'dataset']:
            abort(404)
            return
        result = al360.get_param_attributes(param_type, param_label)
        return Response(json.dumps(result, default=convert), content_type='application/json')
