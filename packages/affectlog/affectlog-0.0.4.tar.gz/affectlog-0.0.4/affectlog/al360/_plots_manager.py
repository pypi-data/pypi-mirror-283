...
class PlotsManager:
    ...
    This class should be only created by al360 instance to manage its plots.

    Parameters
    ----------
    al360 : affectlog.AL360
        Instance of AL360.

    Attributes
    --------
    al360 : AL360
        Instance of affectlog.AL360
    cache : list of PlotContainer objects
        List of already calculated plots
    mutex : _thread.lock
        Mutex for params, plots and resources cache. Should be common with mutex from AL360 instance.
    ...
    def __init__(self, al360):
        ...
        self.al360 = al360
        self.cache = []
        self.mutex = al360.mutex
        self.plots = [vars(plots)[res] for res in getattr(plots, '__all__')]

    def get_supported_plots(self):
        """Returns plots classes that can produce at least one valid chart for parent al360.

        Returns
        -----------
        List of classes extending PlotContainer
        """
        return [plot for plot in self.plots if plot.test_al360(self.al360)]
    ...
    def fill_cache(self, fixed_params={}):
        ...
        for plot_class in self.get_supported_plots():
            ...
            if len([k for k in fixed_params.keys() if k not in required_params]) > 0:
                continue
            available_params = self.al360.get_available_params()
            ...
            if self.al360.verbose and len(combinations) > 0:
    ...
    def get_plot(self, plot_type, params_values, cache=True, wait=False):
        ...
        if result is None:
            result = plot_class(self.al360, cache=cache).fit(required_params_values)
