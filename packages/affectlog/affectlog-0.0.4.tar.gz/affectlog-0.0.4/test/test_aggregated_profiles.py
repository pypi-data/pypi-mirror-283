...
class AggregatedProfilesTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_partial(self):
        ...
        self.assertIsInstance(alias, alx.model_explanations.AggregatedProfiles)
    ...
    def test_accumulated(self):
        ...
        self.assertIsInstance(case1, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case2, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(alias, alx.model_explanations.AggregatedProfiles)
    ...
    def helper_test(self, test_type):
        ...
        case2 = self.exp.model_profile(test_type, 100, variables=['salary_in_usd'], verbose=False)
        case3 = self.exp.model_profile(test_type, 120, groups='employment_type', verbose=False)
        case4 = self.exp2.model_profile(test_type, 120, groups=['employment_type'], verbose=False)
        case5 = self.exp.model_profile(test_type, 120, groups=np.array(['employment_type', 'company_size']), verbose=False)
        case6 = self.exp2.model_profile(test_type, 120, groups=pd.Series(['employment_type', 'company_size']), verbose=False)
        case7 = self.exp.model_profile(test_type, 150, variables=['salary_in_usd'], groups='company_size', verbose=False)
        case8 = self.exp.model_profile(test_type, 100, variables=['salary_in_usd'], span=0.5, grid_points=30, verbose=False)
        case9 = self.exp2.model_profile(test_type, 100, variables='salary_in_usd', span=0.5, grid_points=30, verbose=False)
        case10 = self.exp.model_profile(test_type, None, variables=['salary_in_usd', 'employment_type', 'company_size'],
                                        groups=['company_location', 'remote_ratio'],
                                        span=0.5, grid_points=30, verbose=False)
        case11 = self.exp.model_profile(test_type, 100, variables=np.array(['salary_in_usd', 'company_size']),
                                        span=0.5, grid_points=30, verbose=False)
        case12 = self.exp2.model_profile(test_type, 100, variables=pd.Series(['salary_in_usd', 'company_size']), span=0.5,
        ...
        self.assertIsInstance(case1, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case2, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case3, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case4, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case5, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case6, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case7, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case8, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case9, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case10, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case11, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case12, alx.model_explanations.AggregatedProfiles)
        self.assertIsInstance(case13, alx.model_explanations.AggregatedProfiles)
