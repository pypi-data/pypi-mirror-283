...
class XlsxExportConfig(AppConfig):
    default_auto_field = 'django.db.models.BigAutoField'
    name = 'xlsx_export'

    def ready(self):
        # Вызывается при полной инициализации приложения
        import xlsx_export  # Подключение сигналов или других необходимых модулей
        logger.info("XlsxExportConfig is ready")

class PluginConfig(XlsxExportConfig):
    package_name = 'xlsx_export'
    verbose_name = 'XLSX Export Plugin'
    description = 'Export data to XLSX format'
    version = '0.10'
    plugin_base_url = 'xlsx_export/upload'
    index_reverse_name = 'upload_file'
    urls_module = 'xlsx_export.urls'

@receiver(post_migrate, sender=AppConfig)
def handle_post_migrate(sender, **kwargs):
    # Этот код будет выполнен после выполнения всех миграций
    import xlsx_export.views  # Подключение модулей или кода, требующего Django
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            project = Project.objects.first()  # Здесь нужно определить проект, например, первый проект
            try:
                ...
                for case_data in all_test_cases:
                    ...
                    try:
                        ...
                        for step_data in steps_data:
                            TestCaseStep.objects.create(test_case=test_case, **step_data)
                ...
                return render(request, 'xlsx_export/templates/success.html', context)
    ...
    return render(request, 'xlsx_export/templates/upload.html', {'form': form})
...
def format_test_cases(sheet_name, test_cases):
    ...
    for case in test_cases:
        ...
        test_case = {
            ...
            "suite": sheet_name,
            "setup": setup,
            "scenario": "Scenario details included in steps",
