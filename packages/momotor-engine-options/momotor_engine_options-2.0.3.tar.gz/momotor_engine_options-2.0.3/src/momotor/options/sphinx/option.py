...
if has_sphinx:
    ...
    def full_option_ref(checklet, option):
    ...
    class CheckletOptionDirective(ObjectDescription[str]):
        ...
        def _full_name(self, sig: str) -> str:
            ...
            if checklet := self._get_option_or_context('checklet'):
                ...
                sig += full_option_ref(checklet, sig)
    ...
    class MomotorDomain(Domain):
        ...
        def resolve_xref(self, env, fromdocname, builder, typ, target, node, contnode) -> nodes.reference | None:
            if typ == 'option':
                ...
                py_module = node.attributes.get('py:module')
                py_class = node.attributes.get('py:class')

                if py_class:
                    if py_module:
                        fulltarget = full_option_ref(f'{py_module}.{py_class}', target)
                    else:
                        fulltarget = full_option_ref(py_class, target)

                    if node.attributes.get('refspecific'):
                        targets.insert(0, fulltarget)
                    else:
                        targets.append(fulltarget)
            else:
                targets = [target]
