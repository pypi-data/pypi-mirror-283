...
class TREE4:
    ...
    def __init__(self,
        ...
        if self.method == "LATENT-BUDGET-TREE":
            ...
            if self.impurity_fn != "gini":
                print("Impurity function changed to gini, for the moment for simplicity")
    ...
    def impur(self,node, display = False):
        ...
        if self.problem =='regression':
            ...
            if self.impurity_fn =="within_variance":
                
                if self.method == "CART":
                    y_bar = mean(self.y[node.indexes])
                    wv = []
                    for i in node.indexes:
                        wv.append ((self.y[i] - y_bar)**2)
                    return- 1 * mean(wv) * 1/len(self.y) * len(self.y[node.indexes]) #1 /len(self.y) * sum([(self.y[i] - y_bar) **2 for i in node.indexes]) #n_t , negative for maximisation purposes 
                    #return (mean(self.y[node.indexes])**2) * len(self.y[node.indexes]) #mean(y^2 ) * n
                    ...
                    print("imputity_fn = within_variance only works for CART, FAST and TWO-STAGE")
                #y_bar = mean(self.y[node.indexes])
                #wv = []
                #for i in node.indexes:
                #    wv.append ((self.y[i] - y_bar)**2)
                #return mean(wv) * 1/len(self.y) * len(self.y[node.indexes]) #1 /len(self.y) * sum([(self.y[i] - y_bar) **2 for i in node.indexes]) #n_t
            ...
            if self.impurity_fn =="gini":
                for i in  c:
                    ...
                    if display:
                        prom += prob_i
                    elif self.method in ["FAST", "TWO-STAGE", "LATENT-BUDGET-TREE"]:
                        prom += prob_i
                    else:
    ...
    def catergorise_num(self, node, var):
        ...
        self.impurity_fn = "within_variance"
                    ...
                    for var in cat_var:  
                        ...
                        for i in combinazioni: 
                            ...
                            if self.y[stump[0].indexes].size >= self.grow_rules['min_cases_child'] \
                                ...
                                #else:
                                #    print("Error, Two-Stage and FAST algorithm require impurity_fn as tau for classifier, and pearson for regression")
                                #    return None
                                splits_k.append(i)
                                splits.append(i)
                        ...
                        for i in self.midway_points(var,node):#range(len(set(self.features[str(var)][node.indexes]))): 
                                ...
                                if self.y[stump[0].indexes].size >= self.grow_rules['min_cases_child'] \
                                    ...
                                    if self.problem == 'classifier':# and self.impurity_fn == "tau":    
                                        ...
                                        between_variance_k.append(tau)
                                        between_variance.append(tau)
                                    else:# self.problem == "regression" and self.impurity_fn == "pearson": 
                                        impurities_1.append(impur0)
                                        impurities_1.append(impur1)
                    ...
                    if int(v_label[label]) in leaf_list:
                        if self.problem == "classifier":        #For classifier problem
                            ...
                            if self.method == "LATENT-BUDGET-TREE" or self.twoing: #multiclass methods
                                ...
                                myKeys.sort()
                                class_node = {str(i): class_node[i] for i in myKeys}
                            else:
                                class_node = max(response_dict, key = response_dict.get)
                            
                            if self.method == "CART":#impurity_fn in ["gini","entropy"]:
...
class Adaboost:
    ...
    def __init__(self, df, feature_var, num_var, cat_var, _problem, impurity_fn,  method = "CART", weak_learners = 11 , max_level = 0):
        ...
        impurity_fns = ["gini", "entropy", "within_variance"]#, "pearson", "tau"]
        ...
        if not self.sklearn:
            if feature_var in num_var_full:
                if feature_var != previous_var:
                    ...
                    adaboost = Adaboost(df = complete_df, feature_var = feature_var, num_var= self.num_var, cat_var = self.cat_var, _problem = "regression", impurity_fn = "within_variance",  method = "FAST", weak_learners = self.weak_learners , max_level = 0)
                ...
                if feature_var != previous_var:
                    adaboost = Adaboost(df = complete_df, feature_var = feature_var, num_var= self.num_var, cat_var = self.cat_var, _problem = "classifier", impurity_fn = "gini",  method = "FAST", weak_learners = self.weak_learners , max_level = 0)
                    model = adaboost.adaboost() 
                    yhat = adaboost.test_prediction(y_test, num_var_1, cat_var_1, X_test_num, X_test_cat)
                else:
                    yhat = old_adaboost.test_prediction(y_test, num_var_1, cat_var_1, X_test_num, X_test_cat)
                ...
                if feature_var != previous_var:
                    ...
                    adaboost = Adaboost(df = complete_df, feature_var = feature_var, num_var= self.num_var, cat_var = self.cat_var, _problem = "classifier", impurity_fn = "gini",  method = "FAST", weak_learners = self.weak_learners , max_level = 3)
                    model = adaboost.adaboost()  
                    yhat = adaboost.test_prediction(y_test, num_var_1, cat_var_1, X_test_num, X_test_cat)
                else:
                    yhat = old_adaboost.test_prediction(y_test, num_var_1, cat_var_1, X_test_num, X_test_cat)
