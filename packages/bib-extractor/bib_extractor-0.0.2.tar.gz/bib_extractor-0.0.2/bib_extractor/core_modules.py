...
from bib_extractor.merge_parsers import ParsersMerger
from bib_extractor.multiprocess import Multiproc


def setup_logger(opts, logger_name, log_filename = None):
    ...
    if not logger.handlers:
        ...
        log_dir_path = os.path.join(opts.logs_dir)
        if log_filename:
            log_filename_path = os.path.join(opts.logs_dir, f"{log_filename}")
        else:
            log_filename_path = os.path.join(opts.logs_dir, f"{logger_name}.log")
        if not os.path.exists(log_dir_path):
            os.makedirs(log_dir_path)
            logger.info(f"Directory '{log_dir_path}' for logs created.")
        else:
            pass
            # logger.info(f"Directory '{log_dir_path}' for logs already exist.")
        handler = logging.FileHandler(log_filename_path, mode='a')
...
class BaseABC(ABC):
    def __init__(self, opts):
        super().__init__()
        self.opts = opts
        self.logger = setup_logger(self.opts, self.__class__.__name__)
