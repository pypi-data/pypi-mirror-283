...
ws      = whitespace [ comment whitespace ] ; metadata: DROP DROP
false   = "false"                           ; metadata: DROP
null    = "null"                            ; metadata: DROP
true    = "true"                            ; metadata: DROP
comment = "//" *character %x0A              ; metadata: DROP value DROP

object = begin-object [ member *( [value-separator] member) [value-separator] ] end-object  ; metadata: DROP value DROP
member = string name-separator value                                                        ; metadata: key DROP value
array  = begin-array [ value *( value-separator value ) [value-separator] ] end-array       ; metadata: DROP value DROP
...
class Visitor(ABC):
    ...
    def visit_comment(self, node: CommentNode):
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement visit_comment."
        )
...
class Parser(ABNFCore):
    ...
    def _ws(self):
        return self.map(WsNode.from_rule)(
            self.sequence(
                self.whitespace,
                self.maybe(
                    self.sequence(
    ...
    def _comment(self):
        return self.map(CommentNode.from_rule)(
            self.sequence(
                self.word("//"),
                    ...
                    self.sequence(
                        ...
                        self.collect(
                            self.zero_or_more(
                                (
                                    self.sequence(
                                        self.maybe(self.value_separator),
                                        self.member,
                                    )
                                )
                            )
