...
def get_dir():  # 功能：弹出窗口，让用户选择文件夹；返回一个完整的文件夹路径。
    # 创建一个隐藏的主窗口
    root = tk.Tk()
    root.withdraw()  # 隐藏主窗口
...
# 示例调用 dir = get_dir() 返回一个文件夹路径




# 遍历文件夹和子文件夹的所有文件 get_file_path
...
# 示例调用 paths = get_file_path(dir,"word",".pdf")，返回一个二维数组

import os

def get_file_path(get_dir, *suffixes):
    # 检查 get_dir 是否为空值
    if not get_dir:
        return []
...
def get_save_full_name():
    ...
    file_name = asksaveasfilename(
        title="保存文件",  # 对话框标题
        filetypes=(("文本文件", "*.txt"), ("所有文件", "*.*"))  # 文件过滤器
    )

    # 返回选择的文件名
    return file_name

# 调用函数并打印返回的文件名 file_name = get_save_full_name()
