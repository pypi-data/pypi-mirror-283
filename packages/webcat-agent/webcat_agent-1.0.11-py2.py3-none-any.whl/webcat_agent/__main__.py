...
def self_upgrade():
    cmd = [sys.executable, '-m', 'pip', 'install', '--upgrade', 'webcat-agent']
...
def send_system_info(config, system_info):
    ...
    with urllib.request.urlopen(req) as res:
        ...
        if res.status != 200:
            ...
            if 'latestAgentVersion' in response_obj:
                latest_agent_version = response_obj['latestAgentVersion']
                if latest_agent_version != AGENT_VERSION:
                    print('Upgrading webcat-agent from', AGENT_VERSION, 'to', latest_agent_version)
                    self_upgrade()
