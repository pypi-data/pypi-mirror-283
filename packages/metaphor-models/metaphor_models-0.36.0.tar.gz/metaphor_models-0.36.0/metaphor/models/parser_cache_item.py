...
class MapStringString:
    ...
    to_string_tag_5175: Optional[str] = None
    size: Optional[float] = None

    @staticmethod
    def from_dict(obj: Any) -> 'MapStringString':
        assert isinstance(obj, dict)
        to_string_tag_5175 = from_union([from_str, from_none], obj.get("__@toStringTag@5175"))
        size = from_union([from_float, from_none], obj.get("size"))
        return MapStringString(to_string_tag_5175, size)

    def to_dict(self) -> dict:
        result: dict = {}
        if self.to_string_tag_5175 is not None:
            result["__@toStringTag@5175"] = from_union([from_str, from_none], self.to_string_tag_5175)
        if self.size is not None:
            result["size"] = from_union([to_float, from_none], self.size)
        return result
