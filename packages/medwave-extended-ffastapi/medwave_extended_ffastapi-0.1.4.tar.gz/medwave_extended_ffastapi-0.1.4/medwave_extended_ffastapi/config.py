...
class Settings(BaseSettings):
    ...
    LOGS_POSTGRES_PASSWORD: str = Field(
        "mypgdbpass",
        env="LOGS_POSTGRES_USER",
        description="Password for Postgres user for logs database.",
    )
    LOGS_POSTGRES_DB: str = Field(
        "logs", env="LOGS_POSTGRES_DB", description="Postgres database name for logs."
    )
    LOGS_POSTGRES_HOST: str = Field(
        "localhost",
        env="LOGS_POSTGRES_HOST",
        description="Host for Postgres logs database.",
    )
    LOGS_POSTGRES_PORT: int = Field(
        5432, env="LOGS_POSTGRES_PORT", description="Port for Postgres logs database."
    )
    LOGS_POSTGRES_POOL_SIZE: int = Field(
        8,
        env="LOGS_POSTGRES_POOL_SIZE",
        description="Connection pool size for Postgres logs database.",
    )
    LOGS_POSTGRES_PRUNE: bool = Field(
        True,
        env="LOGS_POSTGRES_PRUNE",
    ...
    BACKEND_POSTGRES_USER: str = Field(
        "postgres",
        env="BACKEND_POSTGRES_USER",
        description="Postgres user for app database.",
    )
    BACKEND_POSTGRES_PASSWORD: str = Field(
        "mypgdbpass",
        env="BACKEND_POSTGRES_PASSWORD",
        description="Password for Postgres user for app database.",
    )
    BACKEND_POSTGRES_DB: str = Field(
        "APP", env="BACKEND_POSTGRES_DB", description="Postgres database name for app."
    )
    BACKEND_POSTGRES_HOST: str = Field(
        "localhost",
        env="BACKEND_POSTGRES_HOST",
        description="Host for Postgres app database.",
    )
    BACKEND_POSTGRES_PORT: int = Field(
        5432, env="BACKEND_POSTGRES_PORT", description="Port for Postgres app database."
    )
    BACKEND_POSTGRES_POOL_SIZE: int = Field(
        8,
        env="BACKEND_POSTGRES_POOL_SIZE",
        description="Connection pool size for Postgres APP database.",
    )
    ...
    BACKEND_LOGGER_NAME: str = Field(
        "app",
        env="BACKEND_LOGGER_NAME",
        description="Logger name for application logs.",
    )
