...
def __full_sounding(clean_data, color_blind, dark_mode, storm_motion, special_parcels=None, 
    ...
    # kawrgs - 'param': the value to round; 'dec': the decimal places to round to; 'mag': bool, remove units (magintude of the value)
...
def __build_dropsonde(drop_data, dark_mode, color_blind, add_sat):
    
    '''
    SOUNDERPY DROPSONDE PLOT 
    
    (C) KYLE J GILLETT, 2024
    
    ACKNOWLEDGEMENTS: TROPYCAL, CARTOPY, METPY
    '''
    ...
    # SounderPy 'Mag' function to make pretty text
    def mag(param):
        if ma.is_masked(param):
            fixed = '---'
        else:
    ...
    # TroPycal wind deg -> dir txt function
    def deg2dir(x):
    ...
    # SounderPy get satellite data function 
    def get_sat_img(clean_data):
    
        # Create variables for URL generation
        image_date = datetime(int(clean_data['drop-info']['valid-time'][0]), 
                              int(clean_data['drop-info']['valid-time'][1]), 
                              int(clean_data['drop-info']['valid-time'][2])).date()
        region = 'Mesoscale-1'
        channel = 7
        ...
        for i in range(0, len(cat.datasets)):
            file_names.append(str(cat.datasets[i]).split('_')[3])

        dataset = cat.datasets[find_file(int(str(clean_data['drop-info']['valid-time'][3][0:2] + clean_data['drop-info']['valid-time'][3][3:5])), file_names)]
        ...
        return sat_data, geos, sat_x, sat_y
    #########################################################################
    ...
    ### CREATE FIGURE ###
    #################################################################
    fig = plt.figure(figsize=(17, 11),  linewidth=10, edgecolor=brdr_clr)

    # PLOT SKEW T -----------------------------------------------------------------------------------------
    skew_ax = plt.axes((0, 0, 0.5, 1))
    skew_ax.spines["top"].set_color(brdr_clr)
    skew_ax.spines["left"].set_color(brdr_clr)
    skew_ax.spines["right"].set_color(brdr_clr)
    skew_ax.spines["bottom"].set_color(brdr_clr)
    fig.set_facecolor(bckgrnd_clr)         
    skew_ax.set_facecolor(bckgrnd_clr) 
    plt.yscale('log')
    ytop = int(np.nanmin(clean_data['p']) - 50)
    yticks = np.arange(1000, ytop, -100)
    xticks = np.arange(-20, 61, 10)
    plt.yticks(yticks, [f'{i:d}' for i in yticks], color=gen_txt_clr, fontsize=12, weight='bold', ha='left')
    plt.xticks(xticks, [f'{i:d}' for i in xticks], color=gen_txt_clr, fontsize=12, weight='bold')
    plt.xlabel(' ')
    plt.ylabel(' ')
    skew_ax.set_ylim(1070, ytop)
    skew_ax.set_xlim(-25, 65)
    ...
    ### PLOT SKEW T LINES ###
    #################################################################
    # DEF SKEW MATH FOR T AND TD
    def skew_t(t, p):
        t0 = np.log(p / 1050) * 120 / np.log(100 / 1050)
        return t0 + t, p

    # PLOT ISOTHERMS AND ISOBARS
    for y in range(1000, ytop, -70):
        skew_ax.plot([-30, 70], [y] * 2, color=gen_txt_clr, lw=0.5, alpha=0.8)
    for x in range(-30 - 80, 70, 10):
        skew_ax.plot([x, x + 120], [1070, 100],
                 color=gen_txt_clr, linestyle='--', lw=0.5, alpha=0.8)
        
    # ADD 'WATER' MASK     
    skew_ax.fill_betweenx(y=[1100, 1013.25], x1=-50, x2=70, color='blue', alpha=0.1, zorder=1)

    # PLOT 'SKEWED' TEMPERATURE AND DEWPOINT LINES
    skew_ax.plot(*skew_t(clean_data['T'][~np.isnan(clean_data['T'])], clean_data['p'][~np.isnan(clean_data['T'])]), 'x-', color='r', lw=3)
    skew_ax.plot(*skew_t(clean_data['Td'][~np.isnan(clean_data['Td'])], clean_data['p'][~np.isnan(clean_data['Td'])]), 'x-', color=td_color, lw=3)
    #################################################################
    ...
    ### PLOT SKEW T WIND BARBS ###
    #################################################################  
    barb_ax = plt.axes((0.49, 0, 0.05, 1), sharey=skew_ax)
    barbs = {k: [v[-1]] for k, v in zip(('P', 'U', 'V'), (clean_data['p'], clean_data['u'], clean_data['v']))}
    for P, U, V in zip(clean_data['p'], clean_data['u'], clean_data['v']):
        if abs(P - barbs['P'][-1]) > 10 and not np.isnan(U):
            for K, V in zip(('P', 'U', 'V'), (P, U, V)):
                barbs[K].append(V)
    plt.barbs([50] * len(barbs['P']), barbs['P'],
              barbs['U'], barbs['V'], pivot='middle', color=gen_txt_clr)
    plt.axis('off')
    ##################################################################
    ...
    ### DEFINE HODOGRAPH BOUNDS ###
    #################################################################
    x_min = np.array(barbs['U']).min()
    y_min = np.array(barbs['V']).min()
    x_max = np.array(barbs['U']).max()
    y_max = np.array(barbs['V']).max()

    # if statements to determine approprate x axis and y axis 
    # limits (to change dynamically with the data)
    if y_max >= 0:
        y_Maxlimit = (y_max + 15)
    if y_max < 0:
        y_Maxlimit = (y_max + 15)
    ...
    ### CREATE HODOGRAPH OBJECT ###
    #################################################################
    hod_ax = plt.axes((0.53, 0.7, 0.3, 0.3))
    h = Hodograph(hod_ax, component_range=180.)
    h.ax.set_xlim(-150, 150)
    h.ax.set_ylim(-150, 150)                                                            
    # add hodograph grid lines
    h.add_grid(increment=50, color=gen_txt_clr, linestyle='-', linewidth=1.5, alpha=0.2) 
    h.add_grid(increment=25, color=gen_txt_clr, linewidth=1, linestyle='--', alpha=0.2) 
    # add background color
    h.ax.set_facecolor(bckgrnd_clr)
    # add spines 
    h.ax.spines["top"].set_color(brdr_clr)
    ...
    #################################################################
    cmap=plt.cm.cool
    hodo_plot = h.plot_colormapped(u=np.array(barbs['U']), v=np.array(barbs['V']), c=np.array(barbs['P']), lw=4)
    h.wind_vectors(u=np.array(barbs['U']), v=np.array(barbs['V']), alpha=0.4)
    #################################################################
    ...
    ### PLOT VELOCITY AND HEIGHT MARKERS ###
    #################################################################
    # add hodograph velocity tick marks inside the hodo axis 
    plt.xticks(np.arange(0,0,1))
    plt.yticks(np.arange(0,0,1))
    for i in range(0,200,25):
        h.ax.annotate(str(i),(i,0),xytext=(0,2),textcoords='offset pixels',clip_on=True, color=gen_txt_clr,
                      fontsize=6,weight='bold',alpha=0.6,zorder=0)
    for i in range(0,200,25):
        h.ax.annotate(str(i),(0,i),xytext=(0,2),textcoords='offset pixels',clip_on=True, color=gen_txt_clr,
                      fontsize=6,weight='bold',alpha=0.6,zorder=0)
    for i in range(0,200,25):
        h.ax.annotate(str(i),(-i,0),xytext=(0,2),textcoords='offset pixels',clip_on=True, color=gen_txt_clr,
                      fontsize=6,weight='bold',alpha=0.6,zorder=0)
    ...
    #################################################################
    ### WIND SPEED VALUES TABLE ###
    #################################################################
    # create arrays of filtered data, where wind speed values exist   
    ws_nan = [ws for ws in clean_data['ws'] if not np.isnan(ws)][::-1]
    wd_nan = [wd for wd, ws in zip(clean_data['wd'], clean_data['ws']) if not np.isnan(ws)][::-1]
    p_nan  = [p for p, ws in zip(clean_data['p'], clean_data['ws']) if not np.isnan(ws)][::-1]
    
    # define the text for each cell
    cellText = np.array([[f'{int(p)} hPa' for p in p_nan],
                        [f'{deg2dir(wd)} at {int(ws)} kt' for ws, wd in zip(ws_nan, wd_nan)]]).T
    
    # define column lables
    colLabels = ['Pressure (hPa)', 'Wind (kt)']

    # define table axis and a few table parameters
    tbl_ax = plt.axes((0.53, 0.375, 0.3, 0.3))
    tbl_ax.set_box_aspect(0.7)
    tbl_ax.xaxis.set_visible(False)
    tbl_ax.yaxis.set_visible(False)
    tbl_ax.axis('off')
    
    # plot table
    table = tbl_ax.table(cellText=cellText, colLabels=colLabels,  colColours=[bckgrnd_clr for col in colLabels],
                       cellLoc='center', bbox=[0, .05, 1, .95])
    
    # set table font size
    table.auto_set_font_size(False)
    table.set_fontsize(9)
    # make the maximum wind speed cell bold 
    table[(np.nanargmax(ws_nan)+1, 0)].get_text().set_weight('bold')
    table[(np.nanargmax(ws_nan)+1, 1)].get_text().set_weight('bold')
    # color cell backgrounds and txt color based on dark-mode settings
    for (i, j), cell in table.get_celld().items():
        cell.get_text().set_color(gen_txt_clr)
        cell.set_facecolor(bckgrnd_clr)
        cell.set_edgecolor(brdr_clr)
        
    # create a simple martplotlib 'Blues' cmap 
    # to color the ws cell backgrounds based on value
    cmap = plt.cm.Blues
    norm = plt.Normalize(vmin=np.nanmin(ws_nan) -10, 
                         vmax=np.nanmax(ws_nan))
    colors = cmap(norm(ws_nan))
    # for each cell, color it based on its value
    for i in range(len(ws_nan)):
        if not np.isnan(ws_nan[i]):
            table.get_celld()[(i + 1, 1)].set_facecolor(colors[i])
            table.get_celld()[(i + 1, 1)].set_alpha(0.6)
    #################################################################
    ...
    #################################################################
    ### SFC P & MAX WIND ANNOTATION ###
    #################################################################
    # find true sfc-p in data 
    for i in range(0, len(clean_data['p'])):
        if clean_data['z'][i] >= 0:
                   sfc_p = clean_data['p'][i]
                   break
    # plot the sfc pressure value below wind speed table            
    plt.figtext(0.76, 0.36, f"SURFACE PRESURE: {sfc_p} hPa",  color=gen_txt_clr,
                fontsize=10, weight='bold', ha='right')
    # plot full-profile max wind below wind-speed table
    plt.figtext(0.745, 0.34, f"MAXIMUM WIND: {np.nanmax(clean_data['ws'])} kt",  color=gen_txt_clr,
            fontsize=10, weight='bold', ha='right')
    #################################################################
    ...
    #################################################################
    ### PLOT MANDATORY-LEVEL DATA ANNOTATIONS ###
    #################################################################
    # for each value in the mandatory levels data, plot it on 
    # the skew-T at y = its respective pressure level and x = 0.30
    # first make some pretty text (using masked properties)
    for i, p, z, T, Td, ws, wd, in zip(range(0, len(clean_data['mand']['mand_p'])), 
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_p']], 
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_z']],
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_T']], 
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_Td']], 
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_ws']], 
                                 [value if not np.isnan(value) else ma.masked for value in clean_data['mand']['mand_wd']]):
            # define label and plot values
            label = f"{int(p)} hPa | {z} m\n        {T}°C | {mag(rh_from_dp(T, Td))}%        →\n{deg2dir(wd)} at {ws} kts"
            trans, _, _ = skew_ax.get_yaxis_text1_transform(0)
            skew_ax.text(0.24, clean_data['mand']['mand_p'][i], label, 
                     fontsize=9, ha='center', color=gen_txt_clr, transform=trans, 
                     alpha=0.8, weight='bold', clip_on=True)
    skew_ax.text(0.24, clean_data['mand']['mand_p'][-1] - 23, "MANDATORY LEVELS", 
         fontsize=8, ha='center', color=gen_txt_clr, transform=trans, 
         alpha=0.8, weight='bold', clip_on=True)
    #########################################################################
    ...
    proj = ccrs.Mercator()
    map_ax = plt.axes((0.53, 0, 0.3, 0.3), projection=proj, zorder=12)
    map_ax.spines['geo'].set_color(brdr_clr)
    
    # add states, borders, coastlines
    map_ax.add_feature(cfeature.STATES, edgecolor=brdr_clr, alpha=0.7, linestyle='-', linewidth=1, zorder=6)
    map_ax.add_feature(cfeature.BORDERS.with_scale('50m'), edgecolor=brdr_clr, alpha=1, linewidth=1, zorder=12)
    map_ax.add_feature(cfeature.COASTLINE.with_scale('50m'), edgecolor=brdr_clr, alpha=1, linewidth=1, zorder=12)
    
    # define spine color based on darkmode settings
    map_ax.spines["top"].set_color(brdr_clr)
    map_ax.spines["left"].set_color(brdr_clr)
    map_ax.spines["right"].set_color(brdr_clr)
    map_ax.spines["bottom"].set_color(brdr_clr)

    # plot a dropsonde location marker
    map_ax.plot(clean_data['drop-info']['latlon'][1], clean_data['drop-info']['latlon'][0]
                , marker='o', mfc='none', markeredgewidth=1.5, color='black', markersize='10', transform=ccrs.PlateCarree(), zorder=13)
    map_ax.plot(clean_data['drop-info']['latlon'][1], clean_data['drop-info']['latlon'][0],
                marker='+', color='black', markeredgewidth=2, markersize='16', transform=ccrs.PlateCarree(), zorder=12)

    # if add_sat = True, plot satellite data on the "big map" and then add a smaller map in the corner
    # else, just plot the big map and redefine some of the cfeature properties and zoom-level
    if add_sat == True:
        zoom_factor = 4
        map_ax.set_extent([clean_data['drop-info']['latlon'][1] + zoom_factor, 
                       clean_data['drop-info']['latlon'][1] - (zoom_factor + 1), 
                       clean_data['drop-info']['latlon'][0] + (zoom_factor + 1), 
                       clean_data['drop-info']['latlon'][0] - zoom_factor])
        map_ax.set_box_aspect(0.7) 
        
        # create a custom IR colormap
        ir_colors = [
            '#FFFFFF',   # White
            '#000000',  # Black
            '#8B0000',  # Dark Red
        ...
        # build a linear-segmented matplotlib cmap 
        n_colors = 100
        ir_cmap = LinearSegmentedColormap.from_list('ir_colormap', ir_colors, N=n_colors)
        
        # get satellite data and projection from get_sat_img() function defined above 
        sat_data, geos, sat_x, sat_y = get_sat_img(clean_data) 
        
        # plot satellite data on the "big map" using axes.imshow
        im = map_ax.imshow(sat_data, origin='upper',
                  extent=(sat_x.min(), sat_x.max(), sat_y.min(), sat_y.max()),
                  transform=geos,
                  regrid_shape=2000,
                  aspect='auto',
        ...
        # create a special map title that includes a formatted datetime str of valid sat data time
        dt_str = np.datetime_as_string(sat_data.time.values)
        goes_date = dt_str[5:7] + "-" + dt_str[8:10] + "-" + dt_str[2:4] + " " + dt_str[11:16] + "Z"
        map_title = f"DROPSONDE LOCATION | GOES-16 Ch. 7 | {goes_date}  "
        
        # define and build smaller corner map
        zoom_factor = 10
        smmap_ax = plt.axes((0.5375, 0.2, 0.1, 0.1), projection=ccrs.Mercator(), zorder=13)
        smmap_ax.add_feature(cfeature.OCEAN.with_scale('50m'), color='navy', alpha=0.4, zorder=3)
        smmap_ax.add_feature(cfeature.LAND.with_scale('50m'), color='darkgreen', alpha=0.4, zorder=3)
        smmap_ax.add_feature(cfeature.STATES, edgecolor='darkgreen', alpha=0.7, linestyle='-', linewidth=0.25, zorder=6)
        smmap_ax.add_feature(cfeature.BORDERS.with_scale('50m'), edgecolor='darkgreen', alpha=1, linewidth=0.25, zorder=12)
        smmap_ax.add_feature(cfeature.COASTLINE.with_scale('50m'), edgecolor='darkgreen', alpha=1, linewidth=0.25, zorder=12)
        smmap_ax.set_box_aspect(0.7)
        smmap_ax.set_extent([clean_data['drop-info']['latlon'][1] + zoom_factor, 
                             clean_data['drop-info']['latlon'][1] - zoom_factor, 
                             clean_data['drop-info']['latlon'][0] + zoom_factor, 
                             clean_data['drop-info']['latlon'][0] - zoom_factor])
        
        # plot dropsonde location
        smmap_ax.plot(clean_data['drop-info']['latlon'][1], clean_data['drop-info']['latlon'][0],
                      marker='o', mfc='none', markeredgewidth=1.5, color='black', markersize='8', transform=ccrs.PlateCarree(), zorder=10)
        smmap_ax.plot(clean_data['drop-info']['latlon'][1], clean_data['drop-info']['latlon'][0],
                      marker='+', color='black', markeredgewidth=2, markersize='12', transform=ccrs.PlateCarree(), zorder=10)
    else: 
        zoom_factor = 7
        map_ax.set_extent([clean_data['drop-info']['latlon'][1] + zoom_factor, 
                       clean_data['drop-info']['latlon'][1] - zoom_factor, 
                       ...
                       clean_data['drop-info']['latlon'][0] - zoom_factor])
        map_ax.set_box_aspect(0.7) 
        map_ax.add_feature(cfeature.OCEAN.with_scale('50m'), color='navy', alpha=0.4, zorder=3)
        map_ax.add_feature(cfeature.LAND.with_scale('50m'), color='darkgreen', alpha=0.4, zorder=3)
        map_title = f"DROPSONDE LOCATION"
    ########################################################################
    ...
    #################################################################
    ### CREATE PLOT TITLE BASED ON THE DATA ###
    #################################################################
    top_title    = f"{clean_data['drop-info']['source']}"
    credit_title = f"SOUNDERPY | KYLE J GILLETT (@WXKYLEGILLETT)" 
    left_title   = f"MISSION: {clean_data['drop-info']['mission-id']} | SONDE: #{clean_data['drop-info']['obs-number']}"
    middle_title = f"{clean_data['drop-info']['valid-time'][1]}-{clean_data['drop-info']['valid-time'][2]}-{clean_data['drop-info']['valid-time'][0]} {clean_data['drop-info']['valid-time'][3]}Z"
    right_title  = f"LOCATION: {clean_data['drop-info']['drop-lctn']} | [{clean_data['drop-info']['latlon'][0]}°N {clean_data['drop-info']['latlon'][1]*-1}°W]"
    ackn_title   = f"POWERED BY TROPYCAL, METPY & SOUNDERPY    "
    
    plt.figtext(0.823, 1.07, " ")
    plt.figtext(0.823, -0.03, " ")
    
    plt.figtext(0, 1.035, top_title,  color=gen_txt_clr,
                fontsize=20, weight='bold', ha='left')
    plt.figtext(0, 1.009, left_title, color=gen_txt_clr,
                fontsize=14, ha='left')
    plt.figtext(0.5, 1.009, middle_title,  color=gen_txt_clr,
                fontsize=14, ha='right')
    plt.figtext(0.815, 1.009, right_title, color=gen_txt_clr,
                fontsize=14, ha='right')
    plt.figtext(0.828, -0.02, ackn_title, 
                fontsize=10, alpha=0.5, color='cornflowerblue', weight='bold', ha='right')
    plt.figtext(0.815, 1.035, credit_title, 
                fontsize=8, alpha=0.5, color='cornflowerblue', weight='bold', ha='right')
    plt.figtext(0.815, 0.305, map_title, color=gen_txt_clr,
                fontsize=9, alpha=0.4, weight='bold', ha='right')
    #################################################################
    ...
    img = Image.open(urlopen('https://user-images.githubusercontent.com/100786530/251580013-2e9477c9-e36a-4163-accb-fe46780058dd.png'))
    #                  side-side  up-down  size   size
    imgax = fig.add_axes([-0.01, 0.90, 0.08, 0.08], anchor='SE')
    imgax.imshow(img)
    imgax.axis('off')
    ##################################################################
