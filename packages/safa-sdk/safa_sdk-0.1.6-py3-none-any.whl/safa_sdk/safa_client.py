...
from safa_sdk.constants import SAFA_AUTH_TOKEN, STORE_CRED_KEY, STORE_PROJECT_KEY
from safa_sdk.http_client import HttpClient
...
class SafaClient:
    ...
    def __init__(self, store: Optional[SafaStore] = None):
        """
        ...
        if store is None:
            store = SafaStore()
        self.http_client = HttpClient(SafaClient.BASE_URL)
        self.store = store
    ...
    def hgen(self, version_id: str, hgen_data: Dict):
        """
        Runs HGEN.
        :param version_id: ID of version of artifacts to use.
        :param hgen_data: The request data.
        :return: Job response.
        """
        response = self.http_client.post(f"hgen/{version_id}", data=hgen_data)
        return cast(Dict, response)
