...
"""
    Triogonometric base class voor Sym Express 3
...
class SymFuncTrigonoBase( symFuncBase.SymFuncBase ):
  """
  Base class for trigonometische functions
  """

  # def _convertFuncSinCosComplex( self, elem ):
  #  # convert sin(atan(x)/n) = ( (cos(atan(x)) + i sin(atan(x)))^(1/n) - (cos(atan(x)) - i sin(atan(x)))^(1/n) ) / (2 i)
  #  #         cos(atan(x)/n) = ( (cos(atan(x)) + i sin(atan(x)))^(1/n) + (cos(atan(x)) - i sin(atan(x)))^(1/n) ) / 2
  #
  #  # print( "_convertFuncSinCosComplex" )
  #
  ...
  def _optimzeSinCosTan( self, elemFunc ):
  ...
  def _convertFuncSinCosTan( self, elem ):
    ...
    elemnew = None
    # for iCnt2 in range( 0, len( symTrigonometricData.trigonometricdata )):
    for tri in symTrigonometricData.trigonometricdata:
      # tri = symTrigonometricData.trigonometricdata [ iCnt2 ]
      if tri[ 0 ] != funcname:
        continue
      if (  tri[ 1 ] != iSign
         or tri[ 2 ] != iCounter
  ...
  def _convertSinCosAtan( self, elem ):
    ...
    if ( isinstance( elem.elements[ 0 ], symexpress3.SymExpress) and elem.name == 'cos' ):
      ...
      # TODO uitzoeken, andere oplossen = loskoppelen...
      return None
