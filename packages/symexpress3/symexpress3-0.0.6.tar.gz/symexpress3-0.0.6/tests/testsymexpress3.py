...
if len( sys.argv ) > 0 :
  for iCnt in range( 1, len( sys.argv )):
    ...
    if cOpt == '--help' :
      ...
      print( '--showoptimized Show optimzed expressions' )
...
for dData in testData :
  ...
  if cActions[ 0 ] == 'optimze_writeOutSum' :
    oResult.optimize()
  else:
  ...
  for cAction in cActions :
    ...
    if cAction == 'optimzeNormal' :
      oExpress.optimizeNormal()
    elif cAction == 'optimzeSpecial' :
      oExpress.optimizeSpecial()
    else:
