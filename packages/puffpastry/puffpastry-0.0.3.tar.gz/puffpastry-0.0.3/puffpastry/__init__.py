...
def create(identifier: str, force: bool = True):
    ...
    global _APP, _REGISTRY, _CACHE
    if _APP is not None and force is False:
        raise KeyError("Application object already initialized.")
...
def provides(identifier: str):
    ...
    if identifier in _REGISTRY:
        raise KeyError(
            f"identifier {identifier} is already assigned an object in the application."
        )
