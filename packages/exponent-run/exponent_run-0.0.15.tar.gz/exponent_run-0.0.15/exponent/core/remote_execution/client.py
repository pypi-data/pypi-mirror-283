...
class RemoteExecutionClient:
    ...
    async def get_execution_requests(
        ...
        response = await self.api_client.get(
            f"/api/remote_execution/{chat_uuid}/requests",
        )
        return await deserialize_list_api_response(response)

    async def post_execution_result(
        self, chat_uuid: str, response: RemoteExecutionResponse
    ) -> None:
        await self.api_client.post(
            f"/api/remote_execution/{chat_uuid}/result",
    ...
    async def disconnect_signal_received(self, chat_uuid: str) -> bool:
        response = await self.api_client.get(
            f"/api/remote_execution/{chat_uuid}/signal/disconnect/check",
            timeout=60,
        )
        if response.status_code != http_status.OK:
            raise Exception(
        ...
        signal = await deserialize_api_response(response, SignalResponse)
        return signal.status == SignalStatus.sent

    async def acknowledge_disconnect_signal(self, chat_uuid: str) -> None:
        response = await self.api_client.post(
            f"/api/remote_execution/{chat_uuid}/signal/disconnect/acknowledge",
            timeout=60,
        )
        if response.status_code != http_status.OK:
            raise Exception(
