...
from detail.client.instrumentation.http import HttpInstrumentor
...
def before_request():
	...
	A.set_attribute('http.request.headers',json.dumps(request.headers.to_wsgi_list(),cls=DetailEncoder));HttpInstrumentor.start_capturing_http()
def after_request(response):A=response;B=trace.get_current_span();B.set_attribute('http.status_code',A.status_code);B.set_attribute('http.response.headers',json.dumps(A.headers.to_wsgi_list(),cls=DetailEncoder));B.set_attribute('http.response.body',A.data);gc.collect();HttpInstrumentor.stop_capturing_http();return A
def call_wrapper(wrapped,instance,args,kwargs):
	A='wsgi'
	with get_tracer(A).start_as_current_span(A):return wrapped(*args,**kwargs)
