...
def addPackageAction(pkgpath: str, force: bool, branch: str, **_) -> None:
  from modelbit import add_package
  add_package(pkgpath, force=force, branch=branch)


def deletePackageAction(name: str, version: str, branch: str, **_) -> None:
  from modelbit import delete_package
  delete_package(name=name, version=version, branch=branch)


def listPackageAction(name: Optional[str], branch: str, **_) -> None:
...
def addModelAction(name: str, path: str, branch: str, **_) -> None:
  from modelbit import add_model
  add_model(name=name, file=path, branch=branch)


def getModelAction(name: str, path: str, branch: str, **_) -> None:
  from modelbit import get_model
...
def initializeParser() -> argparse.ArgumentParser:
  ...
  model_parser = subparsers.add_parser('model', help="Manage file-based models in the registry")
  ...
  model_add_parser.set_defaults(func=addModelAction)
  ...
  model_get_parser.set_defaults(func=getModelAction)
