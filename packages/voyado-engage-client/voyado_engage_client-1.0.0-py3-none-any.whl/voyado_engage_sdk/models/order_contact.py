...
class OrderContactMatchKeyType(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, OrderContactMatchKeyType._member_map_.values())
        )


@JsonMap({"match_key": "matchKey", "match_key_type": "matchKeyType"})
class OrderContact(BaseModel):
    ...
    :type match_key_type: OrderContactMatchKeyType
    """

    def __init__(self, match_key: str, match_key_type: OrderContactMatchKeyType):
        self.match_key = match_key
        self.match_key_type = self._enum_matching(
            match_key_type, OrderContactMatchKeyType.list(), "match_key_type"
        )
