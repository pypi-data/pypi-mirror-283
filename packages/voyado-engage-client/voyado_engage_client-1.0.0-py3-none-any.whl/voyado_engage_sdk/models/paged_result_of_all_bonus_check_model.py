...
class PagedResultOfAllBonusCheckModel(BaseModel):
    """PagedResultOfAllBonusCheckModel

    :param items: items, defaults to None
    :type items: List[AllBonusCheckModel], optional
    ...
    def __init__(
        self,
        items: List[AllBonusCheckModel] = None,
        total_count: int = None,
        offset: int = None,
        count: int = None,
    ):
        if items is not None:
            self.items = self._define_list(items, AllBonusCheckModel)
