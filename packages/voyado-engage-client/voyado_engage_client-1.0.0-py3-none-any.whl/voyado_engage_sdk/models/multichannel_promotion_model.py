...
class MultichannelPromotionModelStatus(Enum):
    ...
    def list():
        ...
        return list(
            map(
                lambda x: x.value,
                MultichannelPromotionModelStatus._member_map_.values(),
            )
        )
...
class MultichannelPromotionModel(BaseModel):
    """MultichannelPromotionModel

    :param id_: Internal Id in Voyado
    :type id_: str
    :param status: Current status of the promotion, defaults to None
    :type status: MultichannelPromotionModelStatus, optional
    ...
    def __init__(
        ...
        id_: str,
        name: str,
        status: MultichannelPromotionModelStatus = None,
        ...
        if status is not None:
            self.status = self._enum_matching(
                status, MultichannelPromotionModelStatus.list(), "status"
            )
