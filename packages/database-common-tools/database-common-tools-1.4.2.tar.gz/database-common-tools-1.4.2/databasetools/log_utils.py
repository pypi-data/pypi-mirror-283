...
class MultiprocessingLogHandler(logging.Handler):
    def __init__(self, filename, mode='a', time_unit='D', backup_count=3, encoding='utf-8'):
        ...
        self.queue = Queue(-1)
        self.handler = handlers.TimedRotatingFileHandler(filename, when=time_unit, backupCount=backup_count, encoding=encoding)
...
def setup_multiprocess_logging(log_file, time_unit='D', backupCount=3, encoding='utf-8'):
    """
    ...
    logger.setLevel(logging.INFO)
    handler = MultiprocessingLogHandler(log_file, time_unit, backupCount, encoding)
    handler.setFormatter(log_format)
    logger.addHandler(handler)
    return logger
