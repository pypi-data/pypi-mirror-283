...
class RpcHelper(BaseRpcHelper):
    ...
    def assert_compare_grpc_exceptions(self, exc_hg, exc_git,
                                       same_details=True,
                                       skip_structured_errors_issue=None,
                                       structured_errors_handler=None):
        ...
        if skip_structured_errors_issue is None:
            ...
            if structured_errors_handler is not None:
                ...
                git_cls = handler['git_cls']
                try:
                    git_code, _, git_error = parse_structured_error(exc_git,
                                                                    git_cls)
                    git_has_error = True
                ...
                try:
                    hg_code, _, hg_error = parse_structured_error(
                        exc_hg, handler.get('hg_cls', git_cls))
                    hg_has_error = True
                except LookupError:  # pragma no cover
                    # unlikely case: caller is expecting structured errors
                    hg_has_error = False

                assert hg_has_error is git_has_error
                if hg_has_error:
                    assert hg_code == git_code
                    git_norm = handler.get('git_normalizer')
                    if git_norm is not None:
                        git_norm(git_error)
                    assert handler['to_git'](hg_error) == git_error

                # already done and a priori not directly comparable
                md_hg.pop(STATUS_DETAILS_KEY, None)
                md_git.pop(STATUS_DETAILS_KEY, None)
            assert md_hg == md_git
