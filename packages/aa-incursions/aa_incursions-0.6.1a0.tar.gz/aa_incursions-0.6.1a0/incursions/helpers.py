    ...
    try:
        embed.add_field(name="Max Spawn Stats",
                        value=f'''
                            ...
                            {(incursion.mobilizing_timestamp - incursion.established_timestamp).total_seconds() / timedelta(days=5).total_seconds():.2f}% total possible `established` time used
                        ''')
    except Exception:
        pass
    ...
    try:
        embed.add_field(name="Max Spawn Stats",
                        value=f'''
                            ...
                            {(incursion.ended_timestamp - incursion.withdrawing_timestamp).total_seconds() / timedelta(days=1).total_seconds():.2f}% total possible `withdrawing` time used
                        ''')
    except Exception:
        pass
    return embed
