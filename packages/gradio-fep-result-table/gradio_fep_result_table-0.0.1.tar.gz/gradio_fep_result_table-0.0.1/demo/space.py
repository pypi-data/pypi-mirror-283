...
```bash
pip install gradio_fep_result_table
```
...
""", elem_classes=["md-custom"], header_links=True)
    ...
    gr.ParamViewer(value=_docs["fep_result_table"]["members"]["__init__"], linkify=[])


    gr.Markdown("### Events")
    gr.ParamViewer(value=_docs["fep_result_table"]["events"], linkify=['Event'])
...
def predict(
    ...
    return value
```
""", elem_classes=["md-custom", "fep_result_table-user-fn"], header_links=True)
