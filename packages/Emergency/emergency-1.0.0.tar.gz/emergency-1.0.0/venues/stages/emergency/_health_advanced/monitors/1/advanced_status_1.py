...
def check_1 ():
	import emergency
	import emergency.procedures.data_nodes.tiny as emergency_db
	...
	records_1 = emergency_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)

	scan = emergency.start (
	...
	records_2 = emergency_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)
	assert (len (records_2) == (len (records_1) + 1))
	
	last_record = emergency_db.last_record (
		db_directory = normpath (join (variable, f"status_db"))
	)
