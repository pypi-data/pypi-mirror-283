...
from elsai_agent_hub.tools.read_pdf import ReadPdf
...
class SalesAgent:
    ...
    def __invoke_model(self, pdf_file_path: str, domain: str, 
                       prompt: str, recipient_name:str, 
                       recipient_designation: str, 
                       sender_name: str, sender_designation: str, 
                       ai_service: str) -> bool:
        self.logger.info(fr"Preparing mail content for {recipient_name}")
        try:    
            if ai_service == "azure":
                llm = azure_openai()
            else:
            ...
            llm_response = llm_chain.invoke({"product_details": product_text,
    ...
    def sales_pitch(self,
        ...
        try:
            ...
            if not csv_file_path:
                raise ValueError("Please provide the recipient details in the CSV file, In the column name of Name,Mail,Designation,Domain")
            
            if not Path(pdf_file_path).exists():
                raise ValueError(fr"The file {pdf_file_path} does not exist in the given path.")
            
            if not Path(csv_file_path).exists():
                raise ValueError(fr"The file {csv_file_path} does not exist in the given path.")
            ...
            missing_columns = [field for field in required_columns if field not in df.columns]
            if missing_columns:
            ...
            for index, row in df.iterrows():
                ...
                llm_response = self.__invoke_model(pdf_file_path, domain, prompt, name, designation, sender_name, sender_designation, ai_service)

                if llm_response:
                    self.logger.info(fr"Sending mail content to {name}")
                    EmailSender().send_email(self.login, self.password, self.sender, mail, llm_response['subject'], llm_response['body'], attachment)
                    self.logger.info(fr"Mail sent to {name}")
                    df.loc[index, "Mail Status"] = "Sent"
                else:
                    self.logger.error(fr"Unable to send the mail to {name}")
                    df.loc[index, "Mail Status"] = "Not Sent"
            df.to_csv(csv_file_path, index=False)
            return {"status":"Updated the status in same CSV under the column name of 'Mail Status'"}      
        except Exception as exc:
