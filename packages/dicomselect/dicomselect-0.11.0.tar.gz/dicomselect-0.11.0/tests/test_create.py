...
def test_input_temp():
    ...
    def header_func(reader: DICOMImageReader):
        nest = reader.metadata['radiopharmaceutical_information_sequence']
        with reader as dcms:
            print(dcms)
        print(nest)
        return {'lol': 1}

    db.create('tests/input/temp', max_workers=1, max_rows=1000, additional_dicom_tags=additional_dicom_tags,
              custom_header_func=header_func,
              reader_prefer_mode=dicomselect.ReaderPreferMode.PreferPydicomImage | dicomselect.ReaderPreferMode.PreferPydicomMetadata)
