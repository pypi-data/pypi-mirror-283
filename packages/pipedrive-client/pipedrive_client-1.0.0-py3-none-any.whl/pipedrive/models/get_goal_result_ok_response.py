...
class GoalType3(BaseModel):
    ...
    :type params: TypeParams4, optional
    """

    def __init__(self, name: str = None, params: TypeParams4 = None):
        if name is not None:
            self.name = name
        if params is not None:
            self.params = self._define_object(params, TypeParams4)


@JsonMap({"id_": "id", "type_": "type"})
class GoalAssignee3(BaseModel):
...
@JsonMap({})
class GoalExpectedOutcome3(BaseModel):
...
class DataGoal3(BaseModel):
    ...
    def __init__(
        ...
        owner_id: int = None,
        title: str = None,
        type_: GoalType3 = None,
        assignee: GoalAssignee3 = None,
        interval: str = None,
        duration: GoalDuration3 = None,
        expected_outcome: GoalExpectedOutcome3 = None,
        is_active: bool = None,
        ...
        if title is not None:
            self.title = title
        if type_ is not None:
            self.type_ = self._define_object(type_, GoalType3)
        if assignee is not None:
            self.assignee = self._define_object(assignee, GoalAssignee3)
        if interval is not None:
            self.interval = interval
        if duration is not None:
            self.duration = self._define_object(duration, GoalDuration3)
        if expected_outcome is not None:
            self.expected_outcome = self._define_object(
                expected_outcome, GoalExpectedOutcome3
            )
...
@JsonMap({})
class GetGoalResultOkResponseData(BaseModel):
    """GetGoalResultOkResponseData

    :param progress: The numeric progress of the goal, defaults to None
    :type progress: int, optional
    :param goal: goal, defaults to None
    :type goal: DataGoal3, optional
    """

    def __init__(self, progress: int = None, goal: DataGoal3 = None):
        if progress is not None:
            self.progress = progress
        if goal is not None:
            self.goal = self._define_object(goal, DataGoal3)


@JsonMap({})
class GetGoalResultOkResponse(BaseModel):
    """GetGoalResultOkResponse

    :param success: If the request was successful or not, defaults to None
    :type success: bool, optional
    :param data: data, defaults to None
    :type data: GetGoalResultOkResponseData, optional
    """

    def __init__(self, success: bool = None, data: GetGoalResultOkResponseData = None):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_object(data, GetGoalResultOkResponseData)
