...
class DataFieldType16(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, DataFieldType16._member_map_.values()))


@JsonMap({"id_": "id"})
class AddProductFieldCreatedResponseData(BaseModel):
    ...
    :type field_type: DataFieldType16
    ...
    def __init__(
        ...
        self.field_type = self._enum_matching(
            field_type, DataFieldType16.list(), "field_type"
        )
        if id_ is not None:
            self.id_ = id_
        if key is not None:
...
@JsonMap({})
class AddProductFieldCreatedResponse(BaseModel):
    """AddProductFieldCreatedResponse

    :param success: If the response is successful or not, defaults to None
    :type success: bool, optional
    :param data: data, defaults to None
    :type data: AddProductFieldCreatedResponseData, optional
    """

    def __init__(
        self, success: bool = None, data: AddProductFieldCreatedResponseData = None
    ):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_object(data, AddProductFieldCreatedResponseData)
