...
class DataFieldType15(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, DataFieldType15._member_map_.values()))


@JsonMap({"id_": "id"})
class GetProductFieldsOkResponseData(BaseModel):
    ...
    :type field_type: DataFieldType15
    ...
    def __init__(
        ...
        self.field_type = self._enum_matching(
            field_type, DataFieldType15.list(), "field_type"
        )
        if id_ is not None:
            self.id_ = id_
        if key is not None:
...
@JsonMap({})
class GetProductFieldsOkResponse(BaseModel):
    """GetProductFieldsOkResponse

    :param success: If the response is successful or not, defaults to None
    :type success: bool, optional
    :param data: Array containing data for all product fields, defaults to None
    :type data: List[GetProductFieldsOkResponseData], optional
    :param additional_data: Additional data for the product field, such as pagination, defaults to None
    ...
    def __init__(
        self,
        success: bool = None,
        data: List[GetProductFieldsOkResponseData] = None,
        additional_data: dict = None,
    ):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_list(data, GetProductFieldsOkResponseData)
