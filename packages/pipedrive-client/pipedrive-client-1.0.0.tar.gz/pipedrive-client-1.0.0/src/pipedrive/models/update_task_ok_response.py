...
class DataDone5(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, DataDone5._member_map_.values()))


@JsonMap({"id_": "id"})
class UpdateTaskOkResponseData(BaseModel):
    ...
    def __init__(
        ...
        if done is not None:
            self.done = self._enum_matching(done, DataDone5.list(), "done")
...
@JsonMap({})
class UpdateTaskOkResponse(BaseModel):
    """UpdateTaskOkResponse

    :param success: success, defaults to None
    :type success: bool, optional
    :param data: data, defaults to None
    :type data: UpdateTaskOkResponseData, optional
    ...
    def __init__(
        self,
        success: bool = None,
        data: UpdateTaskOkResponseData = None,
        additional_data: dict = None,
    ):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_object(data, UpdateTaskOkResponseData)
