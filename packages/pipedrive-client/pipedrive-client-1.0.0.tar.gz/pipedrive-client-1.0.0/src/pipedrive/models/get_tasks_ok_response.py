...
class DataDone2(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, DataDone2._member_map_.values()))


@JsonMap({"id_": "id"})
class GetTasksOkResponseData(BaseModel):
    ...
    def __init__(
        ...
        if done is not None:
            self.done = self._enum_matching(done, DataDone2.list(), "done")
...
@JsonMap({})
class GetTasksOkResponseAdditionalData(BaseModel):
...
@JsonMap({})
class GetTasksOkResponse(BaseModel):
    """GetTasksOkResponse

    :param success: success, defaults to None
    :type success: bool, optional
    :param data: data, defaults to None
    :type data: List[GetTasksOkResponseData], optional
    :param additional_data: The additional data of the list, defaults to None
    :type additional_data: GetTasksOkResponseAdditionalData, optional
    """

    def __init__(
        self,
        success: bool = None,
        data: List[GetTasksOkResponseData] = None,
        additional_data: GetTasksOkResponseAdditionalData = None,
    ):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_list(data, GetTasksOkResponseData)
        if additional_data is not None:
            self.additional_data = self._define_object(
                additional_data, GetTasksOkResponseAdditionalData
            )
