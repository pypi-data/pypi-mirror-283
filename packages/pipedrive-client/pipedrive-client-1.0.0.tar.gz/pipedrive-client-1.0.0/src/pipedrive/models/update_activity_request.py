...
class UpdateActivityRequestDone(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, UpdateActivityRequestDone._member_map_.values())
        )
...
class UpdateActivityRequest(BaseModel):
    ...
    :param participants: List of multiple persons (participants) this activity is associated with. It requires a structure as follows: `[{"person_id":1,"primary_flag":true}]`, defaults to None
    ...
    :type done: UpdateActivityRequestDone, optional
    """
    ...
    def __init__(
        ...
        done: UpdateActivityRequestDone = None,
    ):
        ...
        if done is not None:
            self.done = self._enum_matching(
                done, UpdateActivityRequestDone.list(), "done"
            )
