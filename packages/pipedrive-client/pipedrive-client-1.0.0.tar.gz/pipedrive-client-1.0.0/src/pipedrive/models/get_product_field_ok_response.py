...
class DataFieldType17(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, DataFieldType17._member_map_.values()))


@JsonMap({"id_": "id"})
class GetProductFieldOkResponseData(BaseModel):
    ...
    :type field_type: DataFieldType17
    ...
    def __init__(
        ...
        self.field_type = self._enum_matching(
            field_type, DataFieldType17.list(), "field_type"
        )
        if id_ is not None:
            self.id_ = id_
        if key is not None:
...
@JsonMap({})
class GetProductFieldOkResponse(BaseModel):
    """GetProductFieldOkResponse

    :param success: If the response is successful or not, defaults to None
    :type success: bool, optional
    :param data: data, defaults to None
    :type data: GetProductFieldOkResponseData, optional
    """

    def __init__(
        self, success: bool = None, data: GetProductFieldOkResponseData = None
    ):
        if success is not None:
            self.success = success
        if data is not None:
            self.data = self._define_object(data, GetProductFieldOkResponseData)
