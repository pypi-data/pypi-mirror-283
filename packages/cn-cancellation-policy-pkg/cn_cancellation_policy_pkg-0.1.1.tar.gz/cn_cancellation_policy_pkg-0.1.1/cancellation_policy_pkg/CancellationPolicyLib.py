        ...
        if len(pricing) > 0:
            ...
            for k, policy in enumerate(pricing):
                ...
                if policy['ChargeType'] == 'Fixed':
                    ...
                    if can_amount == total_tbo:
                        can_amount = 0
                else:
                    if int(round(policy['CancellationCharge'])) == 0:
                        can_amount = 0
                    else:
                        percentage = int(round(policy['CancellationCharge'] / 100))
                        prcent_amount = int(round(percentage * total_tbo))
                        can_amount = total_tbo - prcent_amount
                        if percentage == 1:
