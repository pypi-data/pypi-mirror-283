...
def clean_key (url):
    """Removes key from URL, for safer error messages."""
    ...
    def __init__(self, data, wait=True, sleep_notification=None, key=None, id=None):
        ...
        if data is not None:
            ...
            try:
                ...
                raise RequestSubmissionError(None, e.reason, "URL %s" % clean_key(self.url))
...
class Probe():
    ...
    def __init__(self, id, fetch=True):
        ...
        try:
            conn = urllib.request.urlopen(JsonRequest(self.url))
        except urllib.error.HTTPError as e:
            if e.code == 404:
                raise ProbeNotFound
            else:
