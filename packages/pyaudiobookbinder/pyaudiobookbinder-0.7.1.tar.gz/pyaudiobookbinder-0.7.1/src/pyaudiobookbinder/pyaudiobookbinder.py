...
class PyAudiobookBinder:
    ...
    def __init__(self, directory="",
        ...
        if title == "" or title is None:
            print(f"Title not provided. Extracting from directory {self.directory}")
        ...
        if author == "":
            print(f"Author not provided. Extracting from directory {self.directory}")
        ...
        if image == "":
            print(f"Cover image not provided. Searching in directory {self.directory} for cover.jpg or cover.png.")
    ...
    def extract_book_info_from_directory(self) -> tuple[str, str]:
        """
        ...
        leaf_directory = os.path.basename(os.path.normpath(self.directory))

        # Separating title and author based on underscore
        parts = leaf_directory.split("_")
    ...
    def extract_title(self) -> str:
        ...
        """
        title, _ = self.extract_book_info_from_directory()        
        return title
    ...
    def extract_author(self) -> str:
        ...
        """
        _, author = self.extract_book_info_from_directory()
...
""")
    ...
    if is_notebook():
        ...
        print(f"Selected first directory in AudioBookSources: {directory}")
    else:
        a = parser.parse_args()
