...
class scu():
    ...
    HTTP rest API.
    """
    def __init__(self, address='', ip='', port='8080', debug=False, lims_az=(-270.0, 270, 3.0), lims_el=(15, 90, 1.35), dish_type='mke', post_put_delay=0.2, use_socket=True, influx_token='', kwargs_requests = None, wait_done_by_uuid=False, **kwargs):
    ...
    def deactivate_lowpowermode(self):
        uuid = self.scu_put("/devices/command", {"path":"acu.dish_management_controller.set_power_mode","params":{"action":"0"}}).text
        if self.wait_done_by_uuid:
            self.wait_uuid_done(uuid)
        return uuid
    
    def activate_lowpowermode(self):
        uuid = self.scu_put("/devices/command", {"path":"acu.dish_management_controller.set_power_mode","params":{"action":"1"}}).text
        if self.wait_done_by_uuid:
            self.wait_uuid_done(uuid)
        return uuid
