...
class UnconSO:
    """
    This class contains uncostrained global test functions for benchmarking DFO algorithms.

    These tests are adopted from the following website (converted from their matlab version): https://www.sfu.ca
    ...
    _name : str
        Test function name
    _dtype : DType
        Numpy precision format

    Methods
    -------
    test_problem()
        Calls the test function's evaluator based on the predefined name
    """
    ...
    def ackley(self) -> List[float]:
        """ The following link has more info about the test:
...
class ConSO:
    ...
    def weight_min_speed_reducer(self):
        """ Weight minimization of a speed reducer. It involves the design of a speed reducer for a small aircraft
        engine.

        ref: [1] Integral Global Optimization: Theory, Implementation and Applications, 2012.
             [2] A test-suite of non-convex constrained optimization problems from the real-world and some baseline
             results, 2020
        """
...
class Run(_UM):
    ...
    """
    uncon_test_names: List[str] = field(init=False)
    con_test_name: List[str] = field(init=False)
    report_path: str = os.path.abspath("..\\tests")
    field_names: List[str] = field(init=False)
    file_writer: csv.DictWriter = field(init=False)
    test_suite: str = "uncon"
    num_tests: int = 15
    tests_path: str = os.path.abspath("..\\suite\\unconstrained")
    ...
    def __post_init__(self):
        ...
        self.con_test_names = ["g1", "g2", "g3", "griewank", "geom_prog", "himmelblau", "pressure_vessel", "tc_spring",
                               "weight_min_speed_reducer", "wbeam"]

        file: str = os.path.join(self.report_path, 'BM_report.csv')
        df = pd.read_csv(file)
        if df.empty:
    ...
    def add_row(self, name: str, run_index: int, nv: int, nc: int, nb_success: int, it: int,
        ...
        ----------
        name : str, test name
        run_index: int, optimization run index
        nv: int, no. of variables
        nc: int, no. of constraints
        nb_success: int, no. of success points found during optimization
        it: int, the no. of iterations
    ...
    def get_list_of_seeds(self, nb_runs: int) -> List[float]:
        """ Get a list of different random generator seeds """
    ...
    def BM_statistics(self):
        """ Visualization of the BM statistics """

        if self.test_suite == "uncon":
            names: List[str] = self.uncon_test_names
        else:
            names: List[str] = self.con_test_names

        # Read and plot error stats
        df = pd.read_csv(os.path.abspath(os.path.join(self.report_path, 'BM_report.csv')), usecols=["NAME", "F_ERR"],
                         skipinitialspace=True)
        DF = pd.DataFrame(df)
        GDF = DF.groupby("NAME")
        ax = GDF.boxplot(subplots=False, rot=45)
        ax.set_ylabel("|f_optimal-f_exact|")
        ax.set_title("Performance improvement")
        plt.xticks(list(range(1, len(names) + 1)), names)
        plt.show()

        # Read and plot no. of blackbox evaluations stats
        df = pd.read_csv(os.path.abspath(os.path.join(self.report_path, 'BM_report.csv')), usecols=["NAME", "#BBEVAL"],
                         skipinitialspace=True)

        DF = pd.DataFrame(df)
        GDF = DF.groupby("NAME")
        ax = GDF.boxplot(subplots=False, rot=45)
        ax.set_ylabel("# BBEVAL")
        ax.set_title("BB evaluations consumed")
        plt.xticks(list(range(1, len(names) + 1)), names)
        plt.show()

        # Read and plot optimization run-time stats
        df = pd.read_csv(os.path.abspath(os.path.join(self.report_path, 'BM_report.csv')),
                         usecols=["NAME", "RUNTIME_SEC"],
                         skipinitialspace=True)
        DF = pd.DataFrame(df)
        GDF = DF.groupby("NAME")
        ax = GDF.boxplot(subplots=False, rot=45)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Please enter the BM suite name and the path of the report file you would like to analyze!")
    R = Run()
    R.test_suite = sys.argv[1]
    R.report_path = sys.argv[2]
    R.BM_statistics()
