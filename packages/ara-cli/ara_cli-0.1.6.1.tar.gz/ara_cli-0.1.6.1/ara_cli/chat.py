...
class Chat:
    ...
    def execute_clear(self, *args, **kwargs):
        ...
        if user_input.lower() != 'y':
            return
        self.overwrite_chat_file(self.chat_name)
        self.chat_history = self.load_chat_history(self.chat_name)
    ...
    def execute_load(self, *args, additional_input="", **kwargs):
        ...
        suffix = "\n```\n"
        if self.load_file(file_name, prefix=prefix, suffix=suffix):
            print(f"Loaded contents of file {file_name}")
    ...
    def execute_load_givens(self, *args, additional_input="", **kwargs):
        file_name = additional_input
        if file_name == "":
            file_name = "prompt.data/config.prompt_givens.md"
        # directory = os.path.dirname(self.chat_name)
        cwd = os.getcwd()
        # directory = os.path.dirname(self.chat_name)
        # givens_path = f"{directory}/{file_name}"
        givens_path = f"{cwd}/{file_name}"
        if not os.path.exists(givens_path):
            print(f"No givens file found at {givens_path}")
            user_input = input("Please specify a givens file: ")
            givens_path = f"{cwd}/{user_input}"
        if not os.path.exists(givens_path):
            print(f"No givens file found at {givens_path}")
            return
