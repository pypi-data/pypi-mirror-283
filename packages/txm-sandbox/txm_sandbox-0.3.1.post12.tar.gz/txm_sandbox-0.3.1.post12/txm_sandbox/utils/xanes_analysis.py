...
class xanes_analysis:
    ...
    def _interp_ref_spec(self):
        self._lcf_ref_spec = []
        for ii in range(self.lcf_ref.index.size):
            c = interp1d(self.lcf_ref.iloc[ii]["eng"], self.lcf_ref.iloc[ii]["mu"])
            self._lcf_ref_spec.append(c)

    def _lcf_coef_bounds(self):
        self._lcf_coef_bounds = []
        for ii in range(self.lcf_ref.index.size):
            c = (0, 1)
            self._lcf_coef_bounds.append(c)

    @msgit(wd=100, fill="+")
    def lcf(self):
        self._interp_ref_spec()
        self._lcf_coef_bounds()

        self.lcf_ref_spec
        if self.lcf_constr_use:
            self.lcf_func_type = "w_bound_w_constr"
        else:
            self.lcf_func_type = "w_bound_wo_constr"
        args = (self.eng, self.norm_spec, self._lcf_ref_spec, self._lcf_coef_bounds)
        self.lcf_rlt = xm.lcf(self.lcf_func_type, args, num_processes=N_CPU)
