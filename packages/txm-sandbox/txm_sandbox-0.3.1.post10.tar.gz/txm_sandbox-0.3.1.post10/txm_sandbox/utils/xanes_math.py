...
def find_50_peak(model_e, x_e, p_e, model_p, x_p, p_p, ftype="both"):
    ...
    if ftype == "both":
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_both, func_e, x_e, func_p, x_p),
                [
                    [p_e[:, bdi[ii] : bdi[ii + 1]], p_p[:, bdi[ii] : bdi[ii + 1]]]
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_wl, model_e, x_e, func_p, x_p),
                [
                    [p_e[:, bdi[ii] : bdi[ii + 1]], p_p[:, bdi[ii] : bdi[ii + 1]]]
                    for ii in range(N_CPU)
                ],
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_edge, func_e, x_e, model_p, x_p),
                [
                    [p_e[:, bdi[ii] : bdi[ii + 1]], p_p[:, bdi[ii] : bdi[ii + 1]]]
                    for ii in range(N_CPU)
                ],
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_none, model_e, x_e, model_p, x_p),
                [
                    [p_e[:, bdi[ii] : bdi[ii + 1]], p_p[:, bdi[ii] : bdi[ii + 1]]]
                    for ii in range(N_CPU)
                ],
...
def _fit_lcf_np_lstsq(args):
    """
    Fit a linear combination of basis functions to data with np.linalg.lstsq.
    args = (X, y, basis_funcs)
    no bound and constraint conditions are allowed
    """
    X, y, basis_funcs, _ = args
    A = np.column_stack([f(X) for f in basis_funcs])
    coefs, _, _, _ = np.linalg.lstsq(A, y, rcond=None)
    return coefs


def _fit_lcf_lsq_linear(args):
    """
    Fit a linear combination of basis functions to data using scipy.optimize.lsq_linear.
    args = (X, y, basis_funcs, bounds)
    bound but constraint condition is allowed
    """
    X, y, basis_funcs, bounds = args

    def lc(coefs, X):
        return np.sum([c * f(X) for c, f in zip(coefs, basis_funcs)], axis=0)

    A = lambda coefs, X: lc(coefs, X)
    coefs, _, _, _ = lsq_linear(A, y, bounds=bounds, lsq_solver="lsq_linear")
    return coefs


def _fit_lcf_sp_minimize(args):
    """
    Fit a linear combination of basis functions to data using scipy.optimize.minimize with constraints.
    args = (X, y, basis_funcs, bounds, constraint)
    both bound and constraint conditions are allowed
    """
    X, y, basis_funcs, bounds = args

    def objective(coefs):
        lc = np.sum([c * f(X) for c, f in zip(coefs, basis_funcs)], axis=0)
        residuals = y - lc
        return np.sum(residuals**2)

    def constraint_func(coefs):
        # Define your constraint function here
        # For example, let's constrain the sum of coefficients to be equal to 1
        return np.sum(coefs) - 1

    initial_guess = np.zeros(len(basis_funcs))
    bounds = tuple(zip(*bounds))  # Convert bounds to the format required by minimize
    constr = {"type": "eq", "fun": constraint_func}  # Define the constraint
    result = minimize(
        objective, initial_guess, bounds=bounds, constraints=constr
    )  # Use SLSQP (Sequential Least Square Programming)
    return result.x


LCF_FUNCS = {
    "wo_bound_wo_constr": _fit_lcf_np_lstsq,
    "w_bound_wo_constr": _fit_lcf_lsq_linear,
    "w_bound_w_constr": _fit_lcf_sp_minimize,
}


def lcf(fit_func_type, *args, num_processes=4):
    """
    Parallelize the linear combination curve fitting using multiprocessing.
    """
    X, y, basis_funcs, bounds = args
    pool = mp.Pool(processes=num_processes)
    chunk_size = len(X) // num_processes
    _args = [
        (X[i : i + chunk_size], y[i : i + chunk_size], basis_funcs, bounds)
        for i in range(0, len(X), chunk_size)
    ]
    results = pool.map(LCF_FUNCS[fit_func_type], _args)
    pool.close()
    pool.join()
