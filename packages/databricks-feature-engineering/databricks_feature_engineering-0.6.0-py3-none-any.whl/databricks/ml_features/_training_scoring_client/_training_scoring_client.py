...
class TrainingScoringClient:
    ...
    def create_training_set(
        ...
        **kwargs,
        ...
        self._warn_if_non_photon_for_native_spark(
        ...
        return TrainingSet(
            ...
            kwargs.get(
                _USE_SPARK_NATIVE_JOIN,
                self._get_default_use_spark_native_join(feature_spec),
            ),
        )
    ...
    def _warn_if_non_photon_for_native_spark(self, use_native_spark):
        if use_native_spark and not self._spark_client.is_photon_cluster():
            _logger.warning(
    ...
    def _get_default_use_spark_native_join(self, feature_spec: FeatureSpec):
        ...
        # Note that this case is technically not needed since this fn by default returns False.
        # However we still add this in case we decide to use native spark by default in the future.
        max_feature_tables_for_native_spark = 10
        if len(feature_spec.table_infos) > max_feature_tables_for_native_spark:
            return False
