...
from studio.app.models.asset_instance import AssetInstance
...
from studio.app.models.project import ProjectTaskTypeLink

from studio.app.utils import fields, events
...
def create_casting_link(entity_in_id, asset_id, nb_occurences=1, label=""):
    ...
    try:
        ...
        current_app.logger.warning(
            "Attempt to create duplicated entity links (concurrent modification) via studio.app.services.breakdown_service.create_casting_link (raised by sqlalchemy.orm.exc.StaleDataError)"
        )
    except DetachedInstanceError:
        current_app.logger.warning(
            "Attempt to create duplicated entity links (attempt to access unloaded attributes on a mapped instance that is detached) via studio.app.services.breakdown_service.create_casting_link (raised by sqlalchemy.orm.exc.DetachedInstanceError)"
        )
    return link
