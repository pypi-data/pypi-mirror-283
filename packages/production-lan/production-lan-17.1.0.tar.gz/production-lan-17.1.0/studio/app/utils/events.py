...
from studio.app.stores import publisher_store
from studio.app.models.event import ApiEvent
from studio.app.utils import fields
...
def emit(event, data={}, persist=True, project_id=None):
    ...
    from studio.app.config import ENABLE_JOB_QUEUE

    for func in event_handlers.values():
        if ENABLE_JOB_QUEUE:
            from studio.app.stores.queue_store import job_queue
...
def save_event(event, data, project_id=None):
    ...
    """
    try:
        from studio.app.services.persons_service import get_current_user_raw
