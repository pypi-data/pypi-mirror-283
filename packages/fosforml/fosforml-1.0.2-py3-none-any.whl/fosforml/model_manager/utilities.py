...
from fosforml.model_manager import snowflakesession as SnowflakeSession
import pandas as pd
...
class DatasetManager:
    ...
    def get_or_create_dataset_object(self,model_name,version_name,session):
        datasets_obj = Dataset(session=session,
                                database=self.session_instance.connection_params[0]["connectionDetails"]['defaultDb'],
                                schema =self.session_instance.connection_params[0]["connectionDetails"]['defaultSchema'],
    ...
    def get_dataset_purpose(self,dataset_name):
