...
class Strategy:
        ...
        for topic in topics:
            ...
            if topic in data:
                for values in data[topic]:
                    ...
                    for key in values.keys():
                        value_deque = self.split_data_map[topic][key]
                        if len(value_deque) == 0:
                            value_deque = collections.deque(maxlen=deque_length)
                        value_deque.append(values[key])
                ...
                if topic in self.split_data_map:
                    if len(data_map) < 1:
                        for [key, value] in kv_data:
                            self.split_data_map[topic][key].append(value) 

                topic_end_time = self.split_data_map[topic]['end_time']
    ...
    async def resync_split_ringbuffer(self, topics: List[str], data: Dict[str, List[Dict[str,str]]]):
        """
        resync
        """
        for topic in topics:
    ...
    async def resync_ringbuffer(self, topics: List[str], data: Dict[str, List[Dict[str, str]]]):
        """
        Do something
        """
        for topic in topics:
