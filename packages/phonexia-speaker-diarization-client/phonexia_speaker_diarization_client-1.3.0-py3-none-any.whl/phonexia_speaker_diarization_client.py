...
import \
    phonexia.grpc.technologies.speaker_diarization.v1.speaker_diarization_pb2 as diarization
import \
    phonexia.grpc.technologies.speaker_diarization.v1.speaker_diarization_pb2_grpc as diarization_grpc
import soundfile
...
class speaker_diarization_client:
    ...
    @staticmethod
    def time_to_duration(time: float) -> Optional[
    ...
    def file_to_request(
            self, file: Path, start: float, end: float, use_raw_audio: bool,
        ...
        config = diarization.DiarizeConfig(max_speakers=max_speakers,
            ...
            with soundfile.SoundFile(file) as r:
                ...
                for data in r.blocks(blocksize=r.samplerate, dtype="int16"):
                    ...
                    yield diarization.DiarizeRequest(
                        audio=phx_common.Audio(content=data.flatten().tobytes(),
    ...
    def process_file(
        ...
        response = self.send_diarize_request(
            self.file_to_request(file=in_file, start=start, end=end,
                                 use_raw_audio=use_raw_audio))

        output = open(output_file, "w",
                      encoding="utf8") if output_file else sys.stdout  # noqa: SIM115
        ...
        if self.format == "lab":
            ...
            output.write(
                MessageToJson(response, always_print_fields_with_no_presence=True))
    ...
    def process_dir(
        ...
        logging.info("Scanning directory %s for *.%s", os.path.abspath(in_dir),
                     input_suffix)
        ...
        for file in filtered_files:
            in_file = in_dir / file
            out_file = (
                None if output is None else (Path(output) / file).with_suffix(
                    "." + self.format)
            )
            self.process_file(in_file=in_file, start=start, end=end,
                              output_file=out_file, use_raw_audio=use_raw_audio)
...
def main():
    ...
    try:
        ...
        if args.in_file:
            client.process_file(in_file=args.in_file, start=args.start, end=args.end,
                                output_file=args.out_file,
                                use_raw_audio=args.use_raw_audio)

        elif args.in_dir:
            client.process_dir(in_dir=args.in_dir, start=args.start, end=args.end,
                               output=args.out_dir,
