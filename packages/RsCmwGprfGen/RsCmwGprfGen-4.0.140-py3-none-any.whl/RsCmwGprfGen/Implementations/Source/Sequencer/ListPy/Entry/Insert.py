...
class InsertCls:
	...
	def set(self, after_index: int = None) -> None:
		...
		Snippet: driver.source.sequencer.listPy.entry.insert.set(after_index = 1) \n
		Inserts a new entry into the sequencer list, after the selected entry. You can specify <AfterIndex> to select that entry.
		Or you can select an entry via method RsCmwGprfGen.Source.Sequencer.ListPy.index. \n
			:param after_index: integer Index of the entry to be selected
		"""
		param = ''
		if after_index:
