...
class AllCls:
	...
	def get(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.TargetSyncState = None) -> List[enums.GeneratorState]:
		...
		Snippet: value: List[enums.GeneratorState] = driver.source.sequencer.state.all.get(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'SOURce:GPRF:GENerator<Instance>:SEQuencer:STATe:ALL? {param}'.rstrip())
