...
class FrequencyCls:
	...
	def set(self, index: int, frequency: float) -> None:
		...
		Snippet: driver.source.sequencer.listPy.frequency.set(index = 1, frequency = 1.0) \n
	...
	def get(self, index: int) -> float:
		"""SCPI: SOURce:GPRF:GENerator<Instance>:SEQuencer:LIST:FREQuency \n
		Snippet: value: float = driver.source.sequencer.listPy.frequency.get(index = 1) \n
		Defines or queries the RF generator frequency for the sequencer list entry with the selected <Index>. For the supported
		frequency range, see 'Frequency ranges'. \n
			:param index: integer
	...
	def get_all(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.source.sequencer.listPy.frequency.get_all() \n
		Defines the RF generator frequencies for all sequencer list entries. For the supported frequency range, see 'Frequency
		ranges'. \n
			:return: frequency: numeric Comma-separated list of values, one value per list entry Unit: Hz
		"""
	...
	def set_all(self, frequency: List[float]) -> None:
		"""SCPI: SOURce:GPRF:GENerator<Instance>:SEQuencer:LIST:FREQuency:ALL \n
		Snippet: driver.source.sequencer.listPy.frequency.set_all(frequency = [1.1, 2.2, 3.3]) \n
		Defines the RF generator frequencies for all sequencer list entries. For the supported frequency range, see 'Frequency
		ranges'. \n
			:param frequency: numeric Comma-separated list of values, one value per list entry Unit: Hz
		"""
