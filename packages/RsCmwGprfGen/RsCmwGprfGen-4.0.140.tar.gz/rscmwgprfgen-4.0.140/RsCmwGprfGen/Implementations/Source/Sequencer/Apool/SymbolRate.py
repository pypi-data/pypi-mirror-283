...
class SymbolRateCls:
	...
	def get_all(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.source.sequencer.apool.symbolRate.get_all() \n
		Queries the sample rates of the ARB files in the file pool. \n
			:return: sample_rate: float Comma-separated list of values, one value per file Unit: Samples per second
		"""
	...
	def get(self, index: int) -> float:
		...
		Snippet: value: float = driver.source.sequencer.apool.symbolRate.get(index = 1) \n
		Queries the sample rate of the ARB file with the specified <Index>. \n
			:param index: integer
			:return: sample_rate: float Unit: Samples per second"""
