...
class TxCls:
	...
	def set(self, index: float, usage: List[bool]) -> None:
		"""SCPI: SOURce:GPRF:GENerator<Instance>:SEQuencer:LIST:CMWS:USAGe:TX \n
		Snippet: driver.source.sequencer.listPy.singleCmw.usage.tx.set(index = 1.0, usage = [True, False, True]) \n
		No command help available \n
			:param index: No help available
			:param usage: No help available
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('index', index, DataType.Float), ArgSingle.as_open_list('usage', usage, DataType.BooleanList, None))
	...
	def get(self, index: float) -> List[bool]:
		"""SCPI: SOURce:GPRF:GENerator<Instance>:SEQuencer:LIST:CMWS:USAGe:TX \n
		Snippet: value: List[bool] = driver.source.sequencer.listPy.singleCmw.usage.tx.get(index = 1.0) \n
		No command help available \n
			:param index: No help available
