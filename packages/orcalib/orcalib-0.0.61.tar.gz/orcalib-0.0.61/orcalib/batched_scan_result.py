...
class BatchedScanResult:
    ...
    def to_records_list(self) -> list[list[dict[str, Any]]]:
        """
        Convert the values of a vector column to a list of lists of dictionaries

        Returns:
            A list of lists of dictionaries. The outer list represents the batches, the inner list
                represents the memories, and the innermost dictionary contains a mapping of column
                names to values.
        """
        return [[{col: val for col, val in zip(self.column_dict.keys(), memory)} for memory in batch] for batch in self]
...
@deprecated("use BatchedScanResult instead")
class BatchedVectorScanResult_V2:
    ...
    def _extract_memory_values(self, memory, column_slice: ColumnSlice | None):
        if column_slice is None:
            return memory[:]
...
class BatchedScanResultBuilder:
    ...
    def add_feature(self, name: str, feature_type: OrcaTypeHandle, values: list[list[Any]] | torch.Tensor):
        ...
        Args:
            ...
            feature_values: The list of values for the feature
        """
