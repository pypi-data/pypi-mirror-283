...
@LastEditTime: 2024-07-09 11:55:16
...
class RedisExHelper:
    ...
    def init(self, config_dict=None, decode_responses=True, go_heavy_connection=False):
        ...
        """
        if share_config.get_value("is_redis_cluster", False) == False:
            if not config_dict:
            ...
            return RedisHelper.redis_cluster_init(config_dict=config_dict, go_heavy_connection=go_heavy_connection)
     
    @classmethod
    def cluster_init(self, config_dict=None, go_heavy_connection=False):
        """
        :description: redis集群初始化
        :param config_dict：连接串配置
        :param go_heavy_connection：是否启用连接池对象去重,共用连接池对象
        :return: redis_client
        :last_editors: HuangJianYi
        """
        config_dict = config.get_value("redis_cluster")
        config_go_heavy_connection = share_config.get_value("go_heavy_connection", None)
        if config_go_heavy_connection != None:
            go_heavy_connection = config_go_heavy_connection
        if go_heavy_connection == False:
            redis_client = RedisHelper.redis_cluster_init(config_dict=config_dict)
            return redis_client
        else:
            key = CryptoHelper.md5_encrypt(str(config_dict))
            if key in self.redis_clients.keys():
