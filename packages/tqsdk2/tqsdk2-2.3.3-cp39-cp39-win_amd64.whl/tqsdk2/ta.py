...
def ATR(df, n):
    ...
    new_df["atr"] = tqsdk2.tafunc.ma(new_df["tr"], n)
    return new_df
...
def BIAS(df, n):
    ...
    """
    ma1 = tqsdk2.tafunc.ma(df["close"], n)
...
def BOLL(df, n, p):
    ...
    """
    new_df = pd.DataFrame()
    mid = tqsdk2.tafunc.ma(df["close"], n)
...
def DMI(df, n, m):
    ...
    hd = df["high"] - pre_high
    ld = pre_low - df["low"]
    admp = tqsdk2.tafunc.ma(pd.Series(np.where((hd > 0) & (hd > ld), hd, 0)), n)
    admm = tqsdk2.tafunc.ma(pd.Series(np.where((ld > 0) & (ld > hd), ld, 0)), n)
    ...
    new_df["adx"] = tqsdk2.tafunc.ma(ad, m)
...
def KDJ(df, n, m1, m2):
    ...
    new_df["k"] = tqsdk2.tafunc.sma(rsv, m1, 1)
    new_df["d"] = tqsdk2.tafunc.sma(new_df["k"], m2, 1)
...
def MACD(df, short, long, m):
    ...
    """
    new_df = pd.DataFrame()
    eshort = tqsdk2.tafunc.ema(df["close"], short)
    elong = tqsdk2.tafunc.ema(df["close"], long)
    new_df["diff"] = eshort - elong
    new_df["dea"] = tqsdk2.tafunc.ema(new_df["diff"], m)
...
def RSI(df, n):
    ...
    """
    lc = df["close"].shift(1)
    rsi = tqsdk2.tafunc.sma(pd.Series(np.where(df["close"] - lc > 0, df["close"] - lc, 0)), n, 1) / \
          tqsdk2.tafunc.sma(np.absolute(df["close"] - lc), n, 1) * 100
...
def DMA(df, short, long, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["ddd"] = tqsdk2.tafunc.ma(df["close"], short) - tqsdk2.tafunc.ma(df["close"], long)
    new_df["ama"] = tqsdk2.tafunc.ma(new_df["ddd"], m)
    return new_df
...
def EXPMA(df, p1, p2):
    ...
    """
    new_df = pd.DataFrame()
    new_df["ma1"] = tqsdk2.tafunc.ema(df["close"], p1)
    new_df["ma2"] = tqsdk2.tafunc.ema(df["close"], p2)
    return new_df
...
def CR(df, n, m):
    ...
    new_df["crma"] = tqsdk2.tafunc.ma(new_df["cr"], m).shift(int(m / 2.5 + 1))
    return new_df
...
def CDP(df, n):
    ...
    new_df["ah"] = tqsdk2.tafunc.ma(cdp + pt, n)
    new_df["al"] = tqsdk2.tafunc.ma(cdp - pt, n)
    new_df["nh"] = tqsdk2.tafunc.ma(2 * cdp - df["low"], n)
    new_df["nl"] = tqsdk2.tafunc.ma(2 * cdp - df["high"], n)
    return new_df
...
def HCL(df, n):
    ...
    """
    new_df = pd.DataFrame()
    new_df["mah"] = tqsdk2.tafunc.ma(df["high"], n)
    new_df["mal"] = tqsdk2.tafunc.ma(df["low"], n)
    new_df["mac"] = tqsdk2.tafunc.ma(df["close"], n)
    return new_df
...
def ENV(df, n, k):
    ...
    """
    new_df = pd.DataFrame()
    new_df["upper"] = tqsdk2.tafunc.ma(df["close"], n) * (1 + k / 100)
    new_df["lower"] = tqsdk2.tafunc.ma(df["close"], n) * (1 - k / 100)
    return new_df
...
def PUBU(df, m):
    ...
    """
    pb = (tqsdk2.tafunc.ema(df["close"], m) + tqsdk2.tafunc.ma(df["close"], m * 2) + tqsdk2.tafunc.ma(df["close"], m * 4)) / 3
...
def BBI(df, n1, n2, n3, n4):
    ...
    """
    bbi = (tqsdk2.tafunc.ma(df["close"], n1) + tqsdk2.tafunc.ma(df["close"], n2) + tqsdk2.tafunc.ma(df["close"], n3) + tqsdk2.tafunc.ma(
        df["close"], n4)) / 4
...
def DKX(df, m):
    ...
    new_df["d"] = tqsdk2.tafunc.ma(new_df["b"], m)
    return new_df
...
def BBIBOLL(df, n, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["bbiboll"] = (tqsdk2.tafunc.ma(df["close"], 3) + tqsdk2.tafunc.ma(df["close"], 6) + tqsdk2.tafunc.ma(df["close"],
                                                                                           12) + tqsdk2.tafunc.ma(
        df["close"], 24)) / 4
...
def ADTM(df, n, m):
    ...
    new_df["adtmma"] = tqsdk2.tafunc.ma(new_df["adtm"], m)
    return new_df
...
def B3612(df):
    ...
    """
    new_df = pd.DataFrame()
    new_df["b36"] = tqsdk2.tafunc.ma(df["close"], 3) - tqsdk2.tafunc.ma(df["close"], 6)
    new_df["b612"] = tqsdk2.tafunc.ma(df["close"], 6) - tqsdk2.tafunc.ma(df["close"], 12)
    return new_df
...
def DBCD(df, n, m, t):
    ...
    """
    new_df = pd.DataFrame()
    bias = (df["close"] - tqsdk2.tafunc.ma(df["close"], n)) / tqsdk2.tafunc.ma(df["close"], n)
    dif = bias - bias.shift(m)
    new_df["dbcd"] = tqsdk2.tafunc.sma(dif, t, 1)
    new_df["mm"] = tqsdk2.tafunc.ma(new_df["dbcd"], 5)
    return new_df
...
def DDI(df, n, n1, m, m1):
    ...
    new_df["ddi"] = diz - dif
    new_df["addi"] = tqsdk2.tafunc.sma(new_df["ddi"], n1, m)
    new_df["ad"] = tqsdk2.tafunc.ma(new_df["addi"], m1)
    return new_df
...
def LWR(df, n, m):
    ...
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.sma(rsv, m, 1)), columns=["lwr"])
    return new_df
...
def MASS(df, n1, n2):
    ...
    """
    ema1 = tqsdk2.tafunc.ema(df["high"] - df["low"], n1)
    ema2 = tqsdk2.tafunc.ema(ema1, n1)
...
def MI(df, n):
    ...
    new_df["mi"] = tqsdk2.tafunc.sma(new_df["a"], n, 1)
    return new_df
...
def MICD(df, n, n1, n2):
    ...
    ami = tqsdk2.tafunc.sma(mi, n, 1)
    new_df["dif"] = tqsdk2.tafunc.ma(ami.shift(1), n1) - tqsdk2.tafunc.ma(ami.shift(1), n2)
    new_df["micd"] = tqsdk2.tafunc.sma(new_df["dif"], 10, 1)
    return new_df
...
def MTM(df, n, n1):
    ...
    new_df["mtmma"] = tqsdk2.tafunc.ma(new_df["mtm"], n1)
    return new_df
...
def PRICEOSC(df, long, short):
    ...
    Example::
        ...
        from tqsdk2 import TqApi, TqAuth, TqSim
        from tqsdk2.ta import PRICEOSC
    ...
    """
    ma_s = tqsdk2.tafunc.ma(df["close"], short)
    ma_l = tqsdk2.tafunc.ma(df["close"], long)
...
def PSY(df, n, m):
    ...
    Example::
        ...
        from tqsdk2 import TqApi, TqSim
        from tqsdk2.ta import PSY
    ...
    """
    new_df = pd.DataFrame()
    new_df["psy"] = tqsdk2.tafunc.count(df["close"] > df["close"].shift(1), n) / n * 100
    new_df["psyma"] = tqsdk2.tafunc.ma(new_df["psy"], m)
    return new_df
...
def RC(df, n):
    ...
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.sma(rc.shift(1), n, 1)), columns=["arc"])
    return new_df
...
def RCCD(df, n, n1, n2):
    ...
    new_df = pd.DataFrame()
    rc = df["close"] / df["close"].shift(n)
    arc = tqsdk2.tafunc.sma(rc.shift(1), n, 1)
    new_df["dif"] = tqsdk2.tafunc.ma(arc.shift(1), n1) - tqsdk2.tafunc.ma(arc.shift(1), n2)
    new_df["rccd"] = tqsdk2.tafunc.sma(new_df["dif"], n, 1)
    return new_df
...
def ROC(df, n, m):
    ...
    new_df["rocma"] = tqsdk2.tafunc.ma(new_df["roc"], m)
    return new_df
...
def SLOWKD(df, n, m1, m2, m3):
    ...
    fastk = tqsdk2.tafunc.sma(rsv, m1, 1)
    new_df["k"] = tqsdk2.tafunc.sma(fastk, m2, 1)
    new_df["d"] = tqsdk2.tafunc.sma(new_df["k"], m3, 1)
    return new_df
...
def SRDM(df, n):
    ...
    dmf = np.where((df["high"] + df["low"]) >= (df["high"].shift(1) + df["low"].shift(1)), 0,
                   np.where(np.absolute(df["high"] - df["high"].shift(1)) > np.absolute(df["low"] - df["low"].shift(1)),
                            np.absolute(df["high"] - df["high"].shift(1)), np.absolute(df["low"] - df["low"].shift(1))))
    admz = tqsdk2.tafunc.ma(pd.Series(dmz), 10)
    admf = tqsdk2.tafunc.ma(pd.Series(dmf), 10)
    new_df["srdm"] = np.where(admz > admf, (admz - admf) / admz, np.where(admz == admf, 0, (admz - admf) / admf))
    new_df["asrdm"] = tqsdk2.tafunc.sma(new_df["srdm"], n, 1)
    return new_df
...
def ZDZB(df, n1, n2, n3):
    ...
    new_df["b"] = tqsdk2.tafunc.ma(a, n2)
    new_df["d"] = tqsdk2.tafunc.ma(a, n3)
    return new_df
...
def DPO(df):
    ...
    """
    dpo = df["close"] - (tqsdk2.tafunc.ma(df["close"], 20)).shift(11)
...
def LON(df):
    ...
    new_df["ma1"] = tqsdk2.tafunc.ma(new_df["lon"], 10)
    return new_df
...
def SHORT(df):
    ...
    new_df["ma1"] = tqsdk2.tafunc.ma(new_df["short"], 10)
    return new_df
...
def MV(df, n, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["mv1"] = tqsdk2.tafunc.sma(df["volume"], n, 1)
    new_df["mv2"] = tqsdk2.tafunc.sma(df["volume"], m, 1)
    return new_df
    ...
    new_df["a"] = np.absolute(np.where(df["close"] > df["close"].shift(1),
                                       ...
                                       np.where(df["close"] < df["close"].shift(1), df["close"] - np.where(
                                           df["close"].shift(1) > df["high"], df["close"].shift(1), df["high"]),
                                                0)).cumsum())
    new_df["b"] = tqsdk2.tafunc.sma(new_df["a"], n, 1)
    new_df["e"] = tqsdk2.tafunc.sma(new_df["a"], m, 1)
    return new_df
...
def VOSC(df, short, long):
    ...
    """
    vosc = (tqsdk2.tafunc.ma(df["volume"], short) - tqsdk2.tafunc.ma(df["volume"], long)) / tqsdk2.tafunc.ma(df["volume"], short) * 100
...
def VRSI(df, n):
    ...
    """
    vrsi = tqsdk2.tafunc.sma(
        pd.Series(np.where(df["volume"] - df["volume"].shift(1) > 0, df["volume"] - df["volume"].shift(1), 0)), n,
        1) / tqsdk2.tafunc.sma(np.absolute(df["volume"] - df["volume"].shift(1)), n, 1) * 100
...
def MA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.ma(df["close"], n)), columns=["ma"])
    return new_df
...
def SMA(df, n, m):
    ...
    """
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.sma(df["close"], n, m)), columns=["sma"])
    return new_df
...
def EMA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.ema(df["close"], n)), columns=["ema"])
    return new_df
...
def EMA2(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.ema2(df["close"], n)), columns=["ema2"])
    return new_df
...
def TRMA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(tqsdk2.tafunc.trma(df["close"], n)), columns=["trma"])
    return new_df
...
def BS_VALUE(df, quote, r=0.025, v=None):
    ...
    Args:
        ...
        quote (tqsdk2.objs.Quote): 需要计算理论价的期权对象，其标的合约应该是 df 序列对应的合约，否则返回序列值全为 nan
    ...
    Example1::
        ...
        from tqsdk2.ta import BS_VALUE
    ...
    Example2::
        ...
        from tqsdk2 import TqApi
        from tqsdk2.ta import BS_VALUE
        from tqsdk2.tafunc import get_his_volatility
    ...
    if v is None:
        v = tqsdk2.tafunc._get_volatility(df["close"], df["duration"], quote.trading_time)
        if math.isnan(v):
            return pd.DataFrame(np.full_like(df["close"], float('nan')), columns=["bs_price"])
    t = tqsdk2.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)
    return pd.DataFrame(data=list(tqsdk2.tafunc.get_bs_price(df["close"], quote.strike_price, r, v, t, quote.option_class)),
                        columns=["bs_price"])
...
def OPTION_GREEKS(df, quote, r=0.025, v=None):
    ...
    Args:
        ...
        quote (tqsdk2.objs.Quote): 期权对象，应该是 df 中多 K 线序列中的期权合约对象，否则返回序列值全为 nan。
    ...
    Example::
        ...
        from tqsdk2 import TqApi
        from tqsdk2.ta import OPTION_GREEKS
    ...
    if not (quote.ins_class.endswith("OPTION") and quote.instrument_id == df["symbol"][0] and quote.underlying_symbol == df["symbol1"][0]):
        ...
        t = tqsdk2.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)  # 到期时间
        if v is None:
            v = tqsdk2.tafunc.get_impv(df["close1"], df["close"], quote.strike_price, r, 0.3, t, quote.option_class)
        d1 = tqsdk2.tafunc._get_d1(df["close1"], quote.strike_price, r, v, t)
        new_df["delta"] = tqsdk2.tafunc.get_delta(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
        new_df["theta"] = tqsdk2.tafunc.get_theta(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
        new_df["gamma"] = tqsdk2.tafunc.get_gamma(df["close1"], quote.strike_price, r, v, t, d1)
        new_df["vega"] = tqsdk2.tafunc.get_vega(df["close1"], quote.strike_price, r, v, t, d1)
        new_df["rho"] = tqsdk2.tafunc.get_rho(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
    return new_df
...
def OPTION_VALUE(df, quote):
    ...
    Example::
        ...
        from tqsdk2 import TqApi
        from tqsdk2.ta import OPTION_VALUE
...
def OPTION_IMPV(df, quote, r=0.025):
    ...
    Example::
        ...
        from tqsdk2 import TqApi
        from tqsdk2.ta import OPTION_IMPV
    ...
    his_v = tqsdk2.tafunc._get_volatility(df["close1"], df["duration"], quote.trading_time)
    his_v = 0.3 if math.isnan(his_v) else his_v
    t = tqsdk2.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)  # 到期时间
    return pd.DataFrame(
        data=list(tqsdk2.tafunc.get_impv(df["close1"], df["close"], quote.strike_price, r, his_v, t, quote.option_class)),
        columns=["impv"])
...
def VOLATILITY_CURVE(df: pd.DataFrame, quotes: dict, underlying: str, r=0.025):
    ...
    Example::
        ...
        from tqsdk2 import TqApi, TqAuth
        from tqsdk2.ta import VOLATILITY_CURVE
    ...
    for symbol_title in symbol_titles:
        ...
        t = tqsdk2.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)
        res_dict[quote.strike_price] = tqsdk2.tafunc.get_impv(df[base_close_title],
