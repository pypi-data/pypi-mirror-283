...
class Serializer:
    ...
    def add_header(
        ...
        self.headers[key] = self._serialize_value(data, explode=explode, quote_str_values=False)
        return self
    ...
    def add_cookie(
        ...
        self.cookies.append(f"{key}={self._serialize_value(data, explode=explode, quote_str_values=False)}")
        return self
    ...
    def add_query(
        ...
        if style == "form":
            separator = f"&{key}=" if explode and isinstance(data, list) else ("&" if explode else ",")
            ...
            query_param = "".join(
                f"{key}[{k}]={quote(self._serialize_value(v))}&" for k, v in data.items()
            ).rstrip("&")
    ...
    def _serialize_value(
        self, data: Any, separator: str = ",", explode: bool = False, quote_str_values: bool = True
    ) -> str:
        """
