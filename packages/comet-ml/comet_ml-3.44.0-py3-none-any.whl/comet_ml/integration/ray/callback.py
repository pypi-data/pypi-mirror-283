...
class CometTrainLoggerCallback(ray.tune.logger.LoggerCallback):
    ...
    Args:
        ...
        experiment_name: string (optional) = Custom name for the Comet experiment. If ``None``, a name
            is generated automatically. Defaults to ``None``
        api_key: string (optional), Comet API key. Defaults to ``None``.
        workspace: string (optional), Comet workspace name. Defaults to ``None``.
        project_name: string (optional), Comet project name. Defaults to ``None``.
        experiment_key: string (optional), experiment key to be used for logging. Defaults to ``None``.
        mode: string (optional), controls how the Comet experiment is started, 3 options are possible:
        ...
        online: boolean (optional), if True, the data will be logged to Comet server, otherwise it will be stored locally in offline experiment
            Defaults to ``True``.
