...
class IuKey:
    countNonce = 0
    @staticmethod
    def generateId(input_string:str = None, size: int = 32, isHash:bool = False):
        IuKey.countNonce +=1 
        if  IuText.StringEmpty(input_string):
            #current_time_ns = time.time_ns()
            #input_string = str(current_time_ns)
            iD = IuCryptHash.toSha256Bytes(f"{uuid.uuid1()}{IuKey.countNonce}")[:size]
        else:
            if isHash:
                iD = IuCryptHash.toSha256Bytes(f"{input_string}")[:size]
            else:
