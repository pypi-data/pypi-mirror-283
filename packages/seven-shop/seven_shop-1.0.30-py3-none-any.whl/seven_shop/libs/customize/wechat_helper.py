...
:LastEditTime: 2024-07-09 11:27:24
...
class WechatPayHelper(object):
    ...
    def wechat_jsapi_pay(self,order_id, title, price, open_id,attach):
        """
        ...
        headers = {
            "Authorization": self._create_authorization(json.dumps(body),"POST","/v3/pay/transactions/jsapi"),
            "Accept": "application/json",
            "Content-Type": "application/json"
        }

        result = HTTPHelper.post("https://api.mch.weixin.qq.com/v3/pay/transactions/jsapi",json = body, headers = headers)
        if result.status_code == 200:
            # 构建唤起支付信息
            prepay_id = f"prepay_id={json.loads(result.text)['prepay_id']}"
            ...
            sign = base64.b64encode(self.signer.sign(digest)).decode('utf-8')
