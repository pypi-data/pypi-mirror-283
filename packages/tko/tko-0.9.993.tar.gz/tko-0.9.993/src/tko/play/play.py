...
class Play:
    ...
    def str_quest(self, in_focus: bool, q: Quest, lig: str) -> Sentence:
        ...
        if in_focus:
            ...
            if self.flags_on(Const.quest_bar):
    ...
    def show_flags_bar(self, frame: Frame):
        ...
        add_flag(Const.count,   "Count",   f"[{Const.key_show_count}]")
        add_flag(Const.minimum,   "Minimum",   f"[{Const.key_show_minimum}]")
        add_flag(Const.down,    "Down",    f"[{Const.key_show_down}]")
        add_flag(Const.xp,    "XP",    f"[{Const.key_show_xp}]")
        add_flag(Const.percent, "Percent", f"[{Const.key_show_percent}]")
        add_flag(Const.admin,   "Admin",   f"[{Const.key_admin}]")
        add_flag(Const.group_bar, "GroupBar", f"[{Const.key_group_bar}]")
        add_flag(Const.quest_bar, "QuestBar", f"[{Const.key_quest_bar}]")
    ...
    def show_help(self):
        help: Input = Input().add_extra_exit("h").warning().ljust()
        self.add_input(help)
        help.set_header(Sentence().addf("/", " Help "))
        help.addt("Barras alternáveis")
        help.adds(Sentence().addt("  ").addf("", f"{Const.key_flags_bar}").addt(" - Mostrar ou esconder a barra de flags"))
        help.adds(Sentence().addt("  ").addf("", f"{Const.key_help_bar}").addt(" - Mostrar ou esconder a barra de ajuda"))
        help.adds(Sentence().addt("  ").addf("", f"{Const.key_skills_bar}").addt(" - Mostrar ou esconder a barra de skills"))
        help.addt("")
        help.addt("Controles")
        ...
        help.addt(f"      {Const.key_open_link} - Abrir tarefa em uma aba do browser")
        help.addt(f"      {Const.key_down_task} - Baixar um tarefa de código para seu dispositivo")
        help.addt("")
        help.addt("Flags")
        help.addt("  Muda a forma de exibição dos elementos")
        help.addt("")
        help.addt("Extra")
        help.addt(f"  {Const.key_set_lang} - Mudar a linguagem de download dos rascunhos")
    ...
    def show_top_bar(self, frame: Frame) -> None:
        ...
        add_flag(Const.flags_bar, "FlagsBar", f"[{Const.key_flags_bar}]")
        add_flag(Const.help_bar, "HelpBar", f"[{Const.key_help_bar}]")
        add_flag(Const.skills_bar, "SkillsBar", f"[{Const.key_skills_bar}]")
    ...
    def show_items(self):
        ...
        bottom_sy = 0
        if self.flags_on(Const.help_bar):
            bottom_sy = 1
    ...
    def check_disable_skills(self):
        lines, cols = Fmt.get_size()
        if cols < 50:
            if self.flags_on(Const.skills_bar):
                self.flags_toggle("skills_bar")
    ...
    def main(self, scr):
        ...
        while not self.exit:
            ...
            if value == ord(Const.key_quit) or value == 27:
                self.add_input(Input().addt("Saindo do aplicativo").set_exit_fn(self.set_exit).add_extra_exit("q").warning())
                ...
                self.flags_toggle(Const.help_bar)
            elif value == ord(Const.key_skills_bar):
                self.flags_toggle(Const.skills_bar)
                self.check_disable_flags()
            elif value == ord(Const.key_flags_bar):
                self.flags_toggle(Const.flags_bar)
                self.check_disable_skills()
            elif value == ord(Const.key_help):
                self.show_help()
            elif value == ord(Const.key_show_count):
                self.flags_toggle(Const.count)
                self.send_ligado("Mostrando contagem de tarefas feitas ", Const.count, Const.key_show_count)
            elif value == ord(Const.key_show_xp):
                self.flags_toggle(Const.xp)
                self.send_ligado("Mostrando xp em missões e tarefas ", Const.xp, Const.key_show_xp)
            elif value == ord(Const.key_show_percent):
                self.flags_toggle(Const.percent)
                self.send_ligado("Mostrando percentuais ", Const.percent, Const.key_show_percent)
            elif value == ord(Const.key_show_minimum):
                self.flags_toggle(Const.minimum)
                self.send_ligado("Mostrando mínimo valor para conclusão de Missão ", Const.minimum, Const.key_show_minimum)
            elif value == ord(Const.key_show_down):
                self.flags_toggle(Const.down)
                self.send_ligado("Mostrando caminho das tarefas baixadas ", Const.down, Const.key_show_down)
            elif value == ord(Const.key_group_bar):
                self.flags_toggle(Const.group_bar)
                self.send_ligado("Mostrando barra de porcentagem nos Grupos ", Const.group_bar, Const.key_group_bar)
            elif value == ord(Const.key_quest_bar):
                self.flags_toggle(Const.quest_bar)
                self.send_ligado("Mostrando barra de porcentagem nas Missões ", Const.quest_bar, Const.key_quest_bar)
            elif value == ord(Const.key_admin):
                self.flags_toggle(Const.admin)
                self.send_ligado("Desbloqueando acesso a todas as missões ", Const.admin, Const.key_admin)
                self.reload_options()
            elif value == ord(">"):
                self.process_expand()
            elif value == ord("<"):
