...
def raise_incomplete_config(config: AuthConfig):
    if config.login_handler is None:
        click.echo("Your Login Details have not been Set. Please reset your client id")
        raise click.Abort


pass_auth = click.make_pass_decorator(AuthConfig, ensure=True, )
...
def cli(config: WsConfig, context: click.Context):
    ...
    try:
        context.obj = adapter.validate_python(config.storer.get_key(AuthConfig.Key_))
    except TypeError as err:
        click.echo(f"error loading the store key '{AuthConfig.Key_}'\n"
                   f"Error:\n    {err}\n"
...
@pass_config
@pass_auth
def login(login_config: AuthConfig, config: WsConfig,user_name: str, password: str):
    ...
    config.login = login_config.login_handler.login(SigninDetails(username=user_name, password=password))
    config.store()
