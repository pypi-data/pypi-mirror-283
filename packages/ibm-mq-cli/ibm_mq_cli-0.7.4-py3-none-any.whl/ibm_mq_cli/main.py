...
def put_message(args):
    """Put a message on a queue in the specified queue manager."""
    with tempfile.NamedTemporaryFile(delete=False, mode='w') as msg_file:
        msg_file.write(args.message)
        msg_file_path = msg_file.name
    command = f'cat {msg_file_path} | /opt/mqm/samp/bin/amqsputc {args.queue_name} {args.qm_name}'
    output = run_mq_command(command)
    print(f"Message put on queue {args.queue_name} in queue manager {args.qm_name}.\n{output}")
    os.remove(msg_file_path)

def get_message(args):
    """Get a message from a queue in the specified queue manager."""
