...
def put_message(args):
    ...
    env = os.environ.copy()
    env['MQSERVER'] = 'DEV.APP.SVRCONN/TCP/localhost(1414)'
    env['MQSAMP_USER_ID'] = 'app'

    mq_command = f"/opt/mqm/samp/bin/amqsputc {args.queue_name} {args.qm_name}"
    process = subprocess.Popen(mq_command, shell=True, env=env, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def main():
    ...
    parser_put_message.add.argument("queue_name", help="Name of
