...
def put_message_on_queue(args):
    """Put a message on the specified queue."""
    command = f'echo "{args.message}" | /opt/mqm/samp/bin/amqsputc {args.queue_name} {args.qm_name}'
    output = run_command(command)
    print(f"Message put on queue {args.queue_name} in queue manager {args.qm_name}.\n{output}")

def get_message_from_queue(args):
    """Get a message from the specified queue."""
    command = f'/opt/mqm/samp/bin/amqsgetc {args.queue_name} {args.qm_name}'
    output = run_command(command)
    print(f"Message retrieved from queue {args.queue_name} in queue manager {args.qm_name}.\n{output}")
...
def main():
    ...
    parser_configure_queue.add.argument("queue_name", help="Name of the queue to configure")
    parser_configure_queue.add.argument("configuration", help="Configuration string for the queue")
    ...
    parser_start_qm.add.argument("name", help="Name of the queue manager to start")
    ...
    parser_stop_qm.add.argument("name", help="Name of the queue manager to stop")
    ...
    parser_display_queues.add.argument("qm_name", help="Name of the queue manager")
    ...
    parser_get_permissions.add.argument("qm_name", help="Name of the queue manager")
    parser_get_permissions.add.argument("user", help="Name of the user")
    ...
    parser_put_message.add.argument("qm_name", help="Name of the queue manager")
    parser_put_message.add.argument("queue_name", help="Name of the queue")
    parser_put_message.add.argument("message", help="Message to put on the queue")
    parser_put_message.set_defaults(func=put_message_on_queue)
    parser_put_message.epilog = """
    Example usage:
    ibm-mq-cli put_message QM1 DEV.QUEUE.1 "Hello, World!"
    ibm-mq-cli put_message MyQueueManager MyQueue "Test Message"
    """
    ...
    parser_get_message.add.argument("qm_name", help="Name of the queue manager")
    parser_get_message.add.argument("queue_name", help="Name of the queue")
    parser_get_message.set_defaults(func=get_message_from_queue)
    parser_get_message.epilog = """
    Example usage:
    ibm-mq-cli get_message QM1 DEV.QUEUE.1
    ibm-mq-cli get_message MyQueueManager MyQueue
    """
