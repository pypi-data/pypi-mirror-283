...
class widget:
    ...
    """
    def __init__(self,connector=1, typ = "signal", pos = (580,60), timeout=1 ):
    ...
    def refresh(self):
        ...
        if input1 != "xxxxxx":
            ...
            if input1 == "signal": self.typ = "signal"
            if input1 == "dial": self.typ = "dial"
            if input1 == "box": self.typ = "box"

            res = input1ext.split(";")
            self.raw = res[0] # table text box split ,comma,
            ...
            self.value0100 = 50
            if len(res)>2 and self.value is not None:
                mini,maxi=0,100
    ...
    def display(self, frame ):
        #if self.is_active():
        if self.typ == "signal":
            frame = signal_strength( frame, position=self.position , percentage= self.value0100 , value=self.value )
        if self.typ == "dial":
            frame = dial( frame, position=self.position , percentage= self.value0100 , value=self.value )
        if self.typ == "box":
            frame = text_box( frame, position=self.position , title =  "" , values = self.raw.replace(",","\n") )
        return frame
...
class Camera(BaseCamera):
        ...
        if not (vidnum is None) and (vidnum==-1):
            ...
            if (cap.find("screenshot.jpg")==0) and ('pyautogui' in globals()):
                ...
                frame = iprint( blank_image, drtext,
                                "di99" , position,  color_rgb=(0,255,0)  ) # green LED
            ...
            if cap is None and vidnum is None:
                ...
                frame = force_center_text(frame, overtext , posy = 35, fg_bgra = (0,0,0) ) # blue
                txt_img = "xxxxxxxxxxx"
    ...
    def frames( ):
        ...
        mywidgets.append( widget(2,pos=(585,120), typ = "dial") )
        mywidgets.append( widget(3,pos=(585,260), typ= "box" ) )
        mywidgets.append( widget(4,pos=(585,370), typ = "box" ) )
            ...
            if ret: #********************************************************* operations
                ...
                if senh.add_frame(frame):  # it is a proper image....
                    ...
                    if overtext is not None:
                        frame = force_center_text( frame, overtext, posy = 400, fg_bgra= (0,255, 0) )
                        # DIAL --------
                        post = (590,430)
                        frame = dial(frame,post , color=(0,255,0) ,
                                     radius = 30,
                                     thickness = 15,
                    ...
                    if baduser is not None:
                        frame = force_center_text( frame, f"error! => {baduser}", posy = 440, fg_bgra= (0,0,255 ), fosize = 32 )
                    ...
                    for i in mywidgets:
                        i.refresh()
                        if i.is_active(): frame = i.display(frame)
