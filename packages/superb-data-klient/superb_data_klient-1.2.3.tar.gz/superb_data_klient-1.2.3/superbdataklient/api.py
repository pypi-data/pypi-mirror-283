...
from typing import List, Generator, Dict, Any, Optional, Tuple, Union
...
from ._internal_utils import TokenHolder, _get_azure_storage_url, _sanitize_url, _insert_proxy_auth, _parse_azp_from_token
...
class SDKClient:
    ...
    def __init__(self, **kwargs: Union[str, int]) -> None:
        ...
        if 'username' in kwargs and 'password' in kwargs:
            self._token_holder = TokenHolder(domain=self._env.domain, realm=self._env.realm, client_id=self._env.client_id, session=self.session)
            ...
            while True:
                ...
                try:
                    ...
                    if e.response.status_code == 404:
                        ...
                        print(f"The Opensearch scroll id has expired. Consider setting a larger scroll_duration than '{scroll_duration}'", file=sys.stderr)
                        break
                    else:
                        raise
