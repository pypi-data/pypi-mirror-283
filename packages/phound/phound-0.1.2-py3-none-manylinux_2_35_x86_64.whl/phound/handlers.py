...
from phound.events import AudioChunk, Call, CallAttendee, Event, EventType, Message, PlaybackStatus
...
class Chat:
    ...
    def send_message(
        ...
        self._client.send_message(self.persona_uid,
                                  self._get_chat_id(chat_id, persona_uid),
                                  text,
    ...
    def show_typing(self, timeout: int = 60, chat_id: str = "", persona_uid: str = "") -> None:
        self._client.show_typing(self.persona_uid, self._get_chat_id(chat_id, persona_uid), timeout, self._channel_id)

    def get_history(
        self, depth: int = 10, start_message_id: str = "0", chat_id: str = "", persona_uid: str = ""
    ) -> List[Message]:
        self._client.request_chat_history(
            self.persona_uid, self._get_chat_id(chat_id, persona_uid), start_message_id, self._channel_id, depth)
        return self._event_listener.wait_event(EventType.CHAT_HISTORY).body

    def _get_chat_id(self, chat_id: str, persona_uid: str) -> str:
        if chat_id:
            to_chat_id = chat_id
        elif persona_uid:
        ...
        if not to_chat_id:
            raise ChatError("Either chat_id or persona_uid must be provided")
        return to_chat_id
...
class BaseCallHandler(BaseHandler):
    ...
    def record(self, file_path: str) -> None:
        self._client.record_call(self._call_id, file_path)
    ...
    def _handle_event(self, event: Event) -> None:
        ...
        if event.type == EventType.CALL_INCOMING:
            self.on_incoming_call(event.body)
            ...
            if attendee.persona_uid != self.persona_uid:
                self.on_attendee_join(attendee)
    ...
    def _intentionally_end_call(self) -> None:
        ...
        # avoid calling on_hangup if call ended intentionally
        self._client.request_next_event(self._channel_id)
        self._event_listener.wait_event(EventType.CALL_HANGUP)
        self._is_active = False
