...
def guess_text_format(text: str) -> MessageTextFormat:
    regex = re.compile("|".join([f"<{t}>.*</{t}>" for t in Tag]))
    return MessageTextFormat.HTML if re.search(regex, text) else MessageTextFormat.PLAIN
...
def get_chat_id(from_uid: str, to_uid: str) -> str:
    first, last = sorted([from_uid, to_uid])
    return _uid_to_hex(first) + _uid_to_hex(last)
