...
class BaseDynamodbModel(BaseModel):
    ...
    def validate_dynamodb_item(cls, data_dict: dict, from_raw: bool = False) -> Self:
        ...
        if from_raw:
            ...
            logger.info("Data dict after serialization: {}".format(data_dict))
        data_dict = {item_key: cls.validate_value(item_value) for item_key, item_value in data_dict.items()}
        logger.info("Data dict after validation: {}".format(data_dict))
        data_model =  cls.validate_item(item_dict=data_dict)
        logger.info("Dynamodb item model after validation: {}".format(data_model))
        return data_model
