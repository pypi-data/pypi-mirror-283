...
class ImageCreator:
    ...
    def get_ms_token(self):
        ...
        response = self.requests.post(url, data=payload, headers=headers)import datetime
import json
import requests
...
class ImageCreator:
    ...
    def login(self, code):
        ...
        self.expires_in = datetime.datetime.now() + datetime.timedelta(seconds=r["expires_in"])
    ...
    def get_ms_cookies(self):
        ...
        self.copilot_cookies = formatted_cookies
    ...
    def gen_image(self, query, locale="EN", removeBackground=False) -> list:
        ...
        headers = {
            ...
            "Cookie": self.copilot_cookies,
        }

        response = self.requests.post(f"{url}create", data=payload, headers=headers)
        r = response.json()
        payload = json.dumps({
        ...
        while not images:
            response = self.requests.post(f"{url}retrieve", data=payload, headers=headers)
    ...
    def get_ms_cokies(self):
        ...
        # Obtém os cookies do objeto de resposta e os converte em uma string formatada
        cookies = response.cookies
    ...
    def gen_image(self, query, locale="EN", removeBackground=True) -> list:
        url = "https://www.bing.com/api/swiftkey/v1/image-creator/"
