...
class ImageCreator:
    ...
    async def login(self, code):
        ...
        response = await self.requests.post(url, data=payload, headers=headers)
    ...
    async def refresh_token(self):
        ...
        response = await self.requests.post(url, data=payload, headers=headers, timeout=60)
    ...
    async def tone_rewrite(self, query):
        if not self.copilot_cokies:
            await self.get_ms_cokies()
        await self.check_and_refresh_token()
    ...
    async def text_improve(self, query):
        if not self.copilot_cokies:
            await self.get_ms_cokies()
        await self.check_and_refresh_token()
    ...
    async def gen_image(self, query, locale="EN", removeBackground=False) -> list:
        if not self.copilot_cokies:
            await self.get_ms_cokies()
        await self.check_and_refresh_token()
    ...
    async def gen_sticker(self, query, locale="EN", removeBackground=True) -> list:
        if not self.copilot_cokies:
            await self.get_ms_cokies()
        await self.check_and_refresh_token()
