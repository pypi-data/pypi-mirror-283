...
def make_salt(
    ...
    if envx.read(salt_file_name) and not overwrite:
        ...
        print(envx.read(salt_file_name)['env_path'])
        lazyfile.save(
            file=salt_file_name.split('.')[0],
            content=salt_file_content,
            suffix=salt_file_name.split('.')[1],
            path=envx.read(salt_file_name)['env_path'],
            overwrite=overwrite
        )
        return {
            'code': 0,
            'msg': 'ok',
            'salt_str': random_ak,
            'salt_dir': envx.read(salt_file_name)['file_dir'],
