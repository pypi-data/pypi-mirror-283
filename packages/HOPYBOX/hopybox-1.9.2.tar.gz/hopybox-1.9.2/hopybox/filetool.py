...
class Remove():
  def __init__(self,path):      
    self.set_path = os.path.join(os.path.expanduser('~'),'.hopybox','.File_Recycle')
    self.path = path
    self.abspath = os.path.abspath(path)
  @property
  def bin(self):
    shutil.move(self.path,self.set_path)
    tip_tick('Successfully moved to the file Recycle bin')
  def back(self):
    back_path = input('\033[95mWhich folder do you want to restore the file to ?')
    shutil.move(os.path.join(self.set_path,self.path),back_path)
    tip_tick(f'Successfully restored to {back_path}')
