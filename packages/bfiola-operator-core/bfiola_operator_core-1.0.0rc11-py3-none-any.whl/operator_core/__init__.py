...
Resource: TypeAlias = "NamespacedResource | GlobalResource"
SomeResource = TypeVar("SomeResource", bound=Resource, contravariant=True)


def resource_init_subclass(resource_cls: type[SomeResource]):
    """
    ...
    for attr in ("apiVersion", "kind", "_plural"):
        if getattr(resource_cls, attr) is None:
            raise RuntimeError(f"{attr} undefined: {resource_cls}")

    if resource_cls._immutable_fields is None:
        resource_cls._immutable_fields = set()

    group, version = resource_cls.apiVersion.split("/")
    resource_cls._api_info = lightkube.generic_resource.create_api_info(
        group, version, resource_cls.kind, resource_cls._plural
    )

    lightkube.core.resource_registry.resource_registry.register(resource_cls)


def resource_from_dict(resource_cls: type[SomeResource], v: dict) -> SomeResource:
    return resource_cls(**v)


def resource_to_dict(resource: Resource) -> dict:
    return pydantic.RootModel[resource.__class__](resource).model_dump()


class NamespacedResource(lightkube.core.resource.NamespacedResource, Generic[ResourceSpec]):
    """
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        resource_init_subclass(cls)

    @classmethod
    def from_dict(cls, v, **kwargs):
        """
        Creates a resource from a dict
        NOTE: required for lightkube
        """
        return resource_from_dict(cls, v)

    def to_dict(self, **kwargs):
        """
        Converts the resource to a dict
        NOTE: required for lightkube
        """
        return resource_to_dict(self)


class GlobalResource(lightkube.core.resource.GlobalResource, Generic[ResourceSpec]):
    """
...
class Operator:
    ...
    async def resource_create(
        ...
        *,
        ...
        """
        model = pydantic.RootModel[resource_cls].model_validate(dict(body)).root
    ...
    async def resource_update(
        ...
        *,
        ...
        for diff_item in diff_items:
            if diff_item[1] in resource_cls._immutable_fields:
