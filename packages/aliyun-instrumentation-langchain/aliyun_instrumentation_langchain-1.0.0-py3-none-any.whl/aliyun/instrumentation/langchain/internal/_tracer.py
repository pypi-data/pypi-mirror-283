...
def audit_timing(wrapped: Any, _: Any, args: Any, kwargs: Any) -> Any:
    ...
    try:
        ...
        logger.debug(f"{wrapped.__name__}: {latency_ms:.2f}ms")
...
class OpenInferenceTracer(BaseTracer):
    ...
    def _start_trace(self, run: Run) -> None:
        ...
        logger.debug(f"_start_trace: {run}")
...
def _langchain_run_type_to_span_kind(run_type: str) -> AliyunSpanKindValues:
    try:
        return AliyunSpanKindValues(run_type.upper())
    except ValueError:
        return AliyunSpanKindValues.UNKNOWN
...
def _convert_io(obj: Optional[Mapping[str, Any]]) -> Iterator[str]:
    ...
    if len(obj) == 1 and isinstance(value := next(iter(obj.values())), str):
        ...
        yield AliyunMimeTypeValues.JSON.value
...
SERVICE_USER_ID = SpanAttributes.SERVICE_USER_ID
SERVICE_USER_NAME = SpanAttributes.SERVICE_USER_NAME
LLM_SPAN_KIND = SpanAttributes.GEN_AI_SPAN_KIND
LLM_MODEL_NAME = SpanAttributes.GEN_AI_MODEL_NAME
...
TOOL_CALL_FUNCTION_DESCRIPTION = ToolCallAttributes.TOOL_CALL_FUNCTION_DESCRIPTION
TOOL_CALL_FUNCTION_THOUGHTS = ToolCallAttributes.TOOL_CALL_FUNCTION_THOUGHTS
...
LLM_PROMPT_TEMPLATE = SpanAttributes.GEN_AI_PROMPT_TEMPLATE
LLM_PROMPT_TEMPLATE_VARIABLES = SpanAttributes.GEN_AI_PROMPT_VARIABLES
LLM_PROMPT_TEMPLATE_VERSION = SpanAttributes.GEN_AI_PROMPT_VERSION

LLM_REQUEST_PARAMETERS = SpanAttributes.GEN_AI_REQUEST_PARAMETERS
LLM_REQUEST_MODEL_NAME = SpanAttributes.GEN_AI_REQUEST_MODEL_NAME
LLM_REQUEST_MAX_TOKENS = SpanAttributes.GEN_AI_REQUEST_MAX_TOKENS
LLM_REQUEST_TEMPERATURE = SpanAttributes.GEN_AI_REQUEST_TEMPERATURE
LLM_REQUEST_TOP_P = SpanAttributes.GEN_AI_REQUEST_TOP_P
LLM_REQUEST_STREAM = SpanAttributes.GEN_AI_REQUEST_IS_STREAM
LLM_REQUEST_STOP_SEQUENCES = SpanAttributes.GEN_AI_REQUEST_STOP_SEQUENCES
LLM_REQUEST_TOOL_CALLS = SpanAttributes.GEN_AI_REQUEST_TOOL_CALLS

LLM_RESPONSE_MODEL_NAME = SpanAttributes.GEN_AI_RESPONSE_MODEL_NAME
LLM_RESPONSE_FINISH_REASON = SpanAttributes.GEN_AI_RESPONSE_FINISH_REASON

LLM_PROMPTS = SpanAttributes.GEN_AI_PROMPT
CONTENT = SpanAttributes.CONTENT

LLM_OUTPUT_MESSAGES = SpanAttributes.GEN_AI_COMPLETION

LLM_USAGE_PROMPT_TOKENS = SpanAttributes.GEN_AI_USAGE_PROMPT_TOKENS
LLM_USAGE_COMPLETION_TOKENS = SpanAttributes.GEN_AI_USAGE_COMPLETION_TOKENS
LLM_USAGE_TOTAL_TOKENS = SpanAttributes.GEN_AI_USAGE_TOTAL_TOKENS
...
SERVICE_NAME = SpanAttributes.SERVICE_NAME
SERVICE_VERSION = SpanAttributes.SERVICE_VERSION
SERVICE_OWNER_ID = SpanAttributes.SERVICE_OWNER_ID
SERVICE_OWNER_SUB_ID = SpanAttributes.SERVICE_OWNER_SUB_ID

EMBEDDING_TEXT = EmbeddingAttributes.EMBEDDING_TEXT
EMBEDDING_VECTOR = EmbeddingAttributes.EMBEDDING_VECTOR
LLM_INPUT_MESSAGES = SpanAttributes.GEN_AI_PROMPT
