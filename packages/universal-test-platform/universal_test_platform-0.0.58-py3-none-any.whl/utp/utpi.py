...
def appRcc():
    subprocess.Popen('python -m robot --report NONE --outputdir output --logtitle "Task log" src/App/',shell=True,env=os.environ).wait()
    

@click.command("web-rcc",help="run robot test for app by rcc, this shouldn't call directly")
def webRcc():
    subprocess.Popen('python -m robot --report NONE --outputdir output --logtitle "Task log" src/Web/',shell=True,env=os.environ).wait()
