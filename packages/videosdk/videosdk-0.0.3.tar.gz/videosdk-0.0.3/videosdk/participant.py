...
class VideoStreamTrack(CustomVideoTrack):
    kind = "video"
...
class Participant:
    ...
    def remove_all_listeners(self):
        ...
        """
        logging.debug("Removing all event listeners for %s", self.__id)
    ...
    def __listen(self):
        ...
        def on_stream_disabled(consumer: Consumer):
            ...
            if consumer.appData["peerId"] == self.__id:
                if consumer.id in self.__streams:
                    ...
                    self.__emit(
                        VideoSDKEvents.EV_STREAM_DISABLED.value, stream)
        ...
        def on_media_status_changed(data):
            ...
            if data["peerId"] == self.__id:
                ...
                logging.debug(
                    f"Participant :: {self.id} :: media-status-changed")
                self.__emit(
                    VideoSDKEvents.EV_MEDIA_STATUS_CHANGED.value,
                    {"peerId": data["peerId"], "kind": data["kind"],
                        "newState": data["newState"]},
                )

        self.__internal_event_listeners[Events.PARTICIPANT_MEDIA_STATE_CHANGED] = on_media_status_changed
        ...
        def on_video_quality_changed(data):
            ...
            if data["peerId"] == self.__id:
                logging.debug(
                    f"Participant :: {self.id} :: video-quality-changed")
                self.__emit(
                    VideoSDKEvents.EV_VIDEO_QUALITY_CHANGED.value,
                    {"peerId": data["peerId"], "prevQuality": data["prevQuality"],
                        "currentQuality": data["currentQuality"]},
                )

        self.__internal_event_listeners[Events.VIDEO_QUALITY_CHANGED] = on_video_quality_changed
    ...
    def capture_image(self, filepath="captured_image.jpg", desr_height: int = None, desr_width: int = None):
        """
    ...
    async def async_capture_image(self, filepath="captured_image.jpg", desr_height=None, desr_width=None) -> Image:
        """
        ...
        """
        try:
            streams = [stream for stream in self.__streams.values()
                       if stream.kind == "video"]

            if len(streams) == 0:
                raise Exception("Video Track Not Found")

            video_track = VideoStreamTrack(streams[0].track)
            ...
            logging.debug(f"Image captured and saved as {filepath}")
            ...
            logging.error(f"Error in capture_image_from_stream: {e}")
            self.__room_client.__emit_error(f"Not Able to capture image {e}")
