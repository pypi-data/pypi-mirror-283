...
from .utils import PubSubPublishConfig, PubSubSubscribeConfig, RecordingConfig, ServerConfig, TranscriptionConfig, get_server_config
...
class RoomClient(AsyncIOEventEmitter):
    def __init__(self, **kwargs: Union[RoomClientConfig, Any]):
        ...
        self.__loop: AbstractEventLoop = kwargs.get(
        ...
        self.__use_sharing_simulcast: bool = kwargs.get(
            "use_sharing_simulcast")
        ...
        logging.debug("consume: %s", self.__consume)
        logging.debug("produce: %s", self.__produce)
        logging.debug(
            f"Meeting Room Client initialized with room_id: {self.__room_id}")
    ...
    def join(self):
        ...
        try:
            ...
            logging.error(f"Failed to Join :: {e}")
    ...
    async def async_join(self):
        ...
        try:
            if not self._joined:
                ...
                self.emit(Events.MEETING_STATE_CHANGED,
                          {"state": "CONNECTING"})
    ...
    async def __wait_for(
        ...
        try:
            ...
            logging.error(f"signal :: Timeout")
    ...
    async def __handle_incoming_messages(self):
        try:
            ...
            logging.error(
                f"signal :: error while handling signal messages :: {e}"
            )
        ...
        try:
            ...
            if message.get("method") == "newDataConsumer":
                if self.__consume:
                    ...
                    logging.debug(
                        "I don't want to consume newDataConsumer event")
                    await self.__reject(
                        message=message["data"],
                        code=403,
                        reason="I don't want to consume",
            ...
            logging.error(f"signal :: Error while websocket request: {e}")
            self.__emit_error("Internal socket Error")
    ...
    async def __handle_notifications(self, message: Dict[str, Any]):
        try:
            ...
            if message["method"] == "producerScore":
                ...
                logging.debug(
                    f"signal :: producer score {producerId} :: {score}")
            ...
            if message["method"] == "consumerLayersChanged":
                consumerId = message["data"]["consumerId"]
                logging.debug(
                    f"signal :: consumer layers changed {consumerId}")
                if consumerId in self.__consumers:
            ...
            if message["method"] == "pubsubMessage":
                logging.debug(f"signal :: pubsub message {message['data']}")
                self.emit(Events.PUBSUB_MESSAGE, message["data"])

        except Exception as e:
            logging.error(
                f"signal :: Error processing websocket notification :: {e}", exc_info=True
            )
            self.__emit_error("Internal socket Error")
        ...
        try:
            ...
            self.__device = Device(
                handlerFactory=AiortcHandler.createFactory(
                    tracks=[self.__audio_stream_track,
                            self.__video_stream_track]
                )
            )
            ...
            logging.error(f"RTC :: error while device loading :: {e}")
    ...
    async def handle_send_transport(self):
        ...
        try:
            ...
            logging.error(
                f"RTC :: error while send createWebRtcTransport() :: {e}"
            )
        ...
        async def on_connect(dtlsParameters: DtlsParameters):
            try:
                logging.debug(
                    f"RTC :: send transport connecting at ::  {time.time()}")
                reqId = self.__generate_random_number()
                req = {
                    "request": True,
                    "id": reqId,
                ...
                ans = await self.__wait_for(self.__answers[reqId], timeout=15)
                logging.debug(
                    f"RTC :: send transport connected at ::  {time.time()}")
            except Exception as e:
                logging.error(
                    f"RTC :: error while send connectWebRtcTransport() :: {e}"
                )
        ...
        async def on_produce(kind: str, rtpParameters: RtpParameters, appData: dict):
            try:
                ...
                logging.error(f"RTC :: error while produce() :: {e}")
    ...
    async def handle_join(self):
        try:
            ...
            logging.error(f"RTC :: error while join() :: {e}")
    ...
    async def handle_produce(self):
        ...
        try:
            ...
            logging.error(f"RTC :: error while create video producer :: {e}")
        ...
        try:
            ...
            logging.error(f"RTC :: error while creating audio producer :: {e}")
    ...
    async def handle_recv_transport(self):
        ...
        try:
            ...
            logging.error(
                f"RTC :: error while recv createWebRtcTransport() :: {e}"
            )
        ...
        async def on_connect(dtlsParameters: DtlsParameters):
            try:
                ...
                logging.error(
                    f"RTC :: error while recv connectWebRtcTransport() :: {e}"
                )
    ...
    async def handleConsume(
        ...
        try:
            ...
            logging.debug(
                f"creating {kind} consumer for participant :: {peerId}")
            ...
            logging.debug(
                f"created {kind} consumer for participant :: {peerId}")
            ...
            logging.error(f"RTC :: error while Creating Consumer :: {e}")
    ...
    async def async_enable_webcam(
        ...
        try:
            logging.debug(
                f"RTC :: webcam_enabled {self.webcam_enabled}")
            if not self.webcam_enabled:
                self.webcam_enabled = True
            ...
            if custom_track is not None:
                ...
                if self.__video_stream_track.readyState == "ended" and self.__custom_camera_video_track is None:
            ...
            def on_producer_close():
                logging.debug(
                    f"video producer close :: {self.__video_producer.id}")

            @self.__video_producer.observer.on("pause")
            def on_producer_close():
                logging.debug(
                    f"video producer pause :: {self.__video_producer.id}")
            ...
            logging.error(f"MEDIA :: error while Enable Webcam :: {e}")
    ...
    async def async_disable_webcam(self):
        try:
            ...
            logging.error(f"MEDIA :: error while Disable Webcam :: {e}")
    ...
    async def async_enable_mic(self, custom_track: Optional[MediaStreamTrack] = None):
        try:
            ...
            if custom_track is not None:
                self.__audio_producer: Producer = (
                    await self.__send_transport.produce(
                ...
                if self.__audio_stream_track.readyState == "ended" and self.__custom_microphone_audio_track is None:
            ...
            def on_producer_close():
                logging.debug(
                    f"audio producer close :: {self.__audio_producer.id}")

            @self.__audio_producer.observer.on("pause")
            def on_producer_close():
                logging.debug(
                    f"audio producer pause :: {self.__audio_producer.id}")
            ...
            logging.error(f"MEDIA :: error while Enable Mic {e}")
    ...
    async def async_start_recording(self, recording_config: Optional[RecordingConfig] = None):
        try:
            ...
            logging.error(f"MEDIA :: error while start recording {e}")
            self.__emit_error("Not Able to Start Recording")

    def stop_recording(self):
        logging.debug(f"RTC :: recording stopping")
    ...
    async def async_stop_recording(self):
        try:
            ...
            logging.error(f"MEDIA :: error while stop recording {e}")
            self.__emit_error("Not Able to stop Recording")

    def start_transcription(self, transcription_config: Optional[RecordingConfig] = None):
        logging.debug(f"RTC :: transcription starting")
        self.__tasks[f"{TASK_ANY}-transcription-start"] = asyncio.ensure_future(
            self.async_start_transcription(transcription_config)
        )

    async def async_start_transcription(self, transcription_config: Optional[TranscriptionConfig] = None):
        try:
            reqId = self.__generate_random_number()
            req = {
                "request": True,
            ...
            logging.error(f"MEDIA :: error while start transcription {e}")
    ...
    def leave(self):
        ...
        try:
            ...
            logging.error(f"Failed to Leave :: {e}")
    ...
    async def async_enable_peer_mic(self, peerId):
        try:
            ...
            logging.error(f"Signal :: Error while Enable Peer Mic :: {e}")
    ...
    async def async_disable_peer_mic(self, peerId):
        try:
            logging.debug(f"disable peer mic called {peerId}")
            reqId = self.__generate_random_number()
            req = {
                "request": True,
                "id": reqId,
            ...
            logging.error(f"Signal :: Error while Disable Peer Mic :: {e}")
    ...
    async def async_enable_peer_webcam(self, peerId):
        try:
            ...
            logging.error(f"Signal :: Error while Enable Peer Webcam :: {e}")
            self.__emit_error("Not Able to Enable Peer Webcam")

    def disable_peer_webcam(self, peerId):
        self.__tasks[f"{TASK_ANY}-disable-peer-webcam-{peerId}"] = asyncio.ensure_future(
    ...
    async def async_disable_peer_webcam(self, peerId):
        try:
            ...
            logging.error(f"Signal :: Error while Disable Peer Webcam :: {e}")
    ...
    async def async_remove_peer(self, peerId):
        try:
            ...
            logging.error(f"Signal :: Error while Remove Peer :: {e}")
    ...
    async def async_add_custom_video_track(self, track: MediaStreamTrack):
        try:
            ...
            logging.error(f"RTC :: Error while Adding Custom Track :: {e}")
    ...
    async def async_handle_pubsub_publish(self, pubsub_config: PubSubPublishConfig):
        try:
            ...
            logging.error(f"RTC :: Error while Publishing {e}", exc_info=True)
    ...
    async def async_handle_pubsub_subscribe(self, pubsub_config: PubSubSubscribeConfig):
        try:
            ...
            logging.error(f"RTC :: Error while Subscribe {e}", exc_info=True)
