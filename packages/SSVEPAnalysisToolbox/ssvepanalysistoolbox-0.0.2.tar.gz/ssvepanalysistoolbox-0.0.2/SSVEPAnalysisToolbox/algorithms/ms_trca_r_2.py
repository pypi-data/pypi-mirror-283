    ...
    def fit(self,
        ...
        for filterbank_idx in range(filterbank_num):
            for class_idx in range(1,stimulus_num+1):
                ...
                separated_trainSig_1_tmp = [separated_trainSig_1[i] for i in range(start_idx, end_idx)]
                # Calulcate spatial filter
                W = _TDCA_ls_calW([np.swapaxes(trainSig[:,filterbank_idx,:,:], 1,2) for trainSig in separated_trainSig_tmp],
                                [np.swapaxes(trainSig[:,filterbank_idx,:,:], 1,2) for trainSig in separated_trainSig_1_tmp],
                                self.n_jobs,
...
def _TDCA_ls_calW(trainSig, trainSig_1, n_jobs = None, LSconfig = None):
    ...
    if n_jobs is None:
        Y = []
        for trainSig_single_freq in trainSig_1:
            Y.append(
