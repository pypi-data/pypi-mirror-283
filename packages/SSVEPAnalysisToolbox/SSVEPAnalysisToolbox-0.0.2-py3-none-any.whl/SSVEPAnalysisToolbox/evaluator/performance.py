...
def cal_performance_onedataset_crossSubj_diffsiglen(evaluator: object,
                                                     dataset_idx: int,
                                                     tw_seq: List[float],
                                                     train_or_test: str) -> Tuple[ndarray, ndarray]:
    """
    Calculate acc and ITR for one dataset
    Evaluations will be carried out on each signal length
    Under each signal length, there may be several trials. Acc and itr values of all these trials will be averaged.
    ...
    acc_store = np.zeros((len(model_container),1, len(tw_seq)))
    itr_store = np.zeros((len(model_container),1, len(tw_seq)))
    for tw_idx, tw in enumerate(tw_seq):
        trial_info = {'dataset_idx':[dataset_idx],
                        'tw':tw}
        trial_idx = evaluator.search_trial_idx(train_or_test, trial_info)
        ...
        for method_idx in range(len(model_container)):
            ...
            acc_store[method_idx,0,tw_idx] = mean(tmp_acc)
            itr_store[method_idx,0,tw_idx] = mean(tmp_itr)
    return acc_store, itr_store
