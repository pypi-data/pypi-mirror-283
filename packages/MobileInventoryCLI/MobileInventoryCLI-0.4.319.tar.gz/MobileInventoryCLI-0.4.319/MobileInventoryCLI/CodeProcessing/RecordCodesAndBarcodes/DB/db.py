...
class ClipBoordEditor:
    def reset_table(self):
        ...
        ClipBoord.metadata.create_all(ENGINE)
        print(f"{Fore.orange_red_1}A restart is required!")
    ...
    def pastable(self):
        with Session(ENGINE) as session:
            paste=session.query(ClipBoord).filter(ClipBoord.defaultPaste==True).first()
            if paste:
    ...
    def list_(self,short=False,searchable=None,delete=False,update=False,set_defaultPaste=False):
        try:
            search=None
            with Session(ENGINE) as session:
                if searchable:
                ...
                if ct == 0:
                    ...
                    return
                if update or delete or set_defaultPaste:
                    fieldname='CB'
                    mode='Selection'
                    h=f'{Prompt.header.format(Fore=Fore,mode=mode,fieldname=fieldname,Style=Style)}'
                    index=None
                    ...
                    if set_defaultPaste:
                        if index not in [None,]:
                            def mkBoolean(text,data):
                                try:
                                    if text in "y,yes,true,True,1".split(","):
                                        return True
                                    else:
                                        r=False
                                        try:
                            ...
                            results[index].defaultPaste=Prompt.__init2__(self,func=mkBoolean,ptext="Set Default?",helpText="y,yes,true,True,1",data={'default':results[index].defaultPaste})
...
new|n - new text to save
edit|upd - edit a ClipBoord Value by its cbid or selected text
rm|del - delete a text by its cbid or by its selected text
search|s - search for a specific note by string
list|l - list all notes, long version
lists|ls - list all notes, short version
clear_all|ca - clear all notes
reset_table - drop table and recreate table, for when a column is added/modified
dflt|default - set which entry is to be the default pastable, where if multiples are set default, only the latest is used
reset_default|rst_dflt - set all defaultPaste=False
pastable|paste|pst - show the pastable that will be used by the reset of the system
'''
        self.autoClean()
        while True:
            try:
                ...
                if doWhat in [None,]:
                    return
                elif doWhat.lower() in 'new|n'.split('|'):
                    self.new_cb()
