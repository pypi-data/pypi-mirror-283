...
class ScriptControl(gui.FormLayout):
    ...
    def build_ui(self):
        ...
        self.name_layout = self.build_script_name_layout()
        pm.setParent(self)
    ...
    def build_script_name_layout(self):

        form = pm.formLayout(nd=100)
        pm.setParent(form)

        label = pm.text(width=k.LABEL_WIDTH, align="right", label=" Script name")
        self.script_name_tf = pm.textField(width=200)
        self.wrap_batch_cb = pm.checkBox(label="As mayabatch")
        ...
        form.attachForm(label, "left", k.FORM_SPACING_X)
        form.attachNone(label, "right")
        form.attachForm(label, "top", k.FORM_SPACING_Y)
        form.attachForm(label, "bottom", k.FORM_SPACING_Y)

        form.attachControl(self.script_name_tf, "left", k.FORM_SPACING_X, label)
        form.attachControl(
            self.script_name_tf, "right", k.FORM_SPACING_X, self.wrap_batch_cb
        )
        form.attachForm(self.script_name_tf, "top", k.FORM_SPACING_Y)
        form.attachForm(self.script_name_tf, "bottom", k.FORM_SPACING_Y)

        form.attachNone(self.wrap_batch_cb, "left")
        form.attachControl(
            self.wrap_batch_cb, "right", k.FORM_SPACING_X, self.clipboard_btn
        )
        form.attachForm(self.wrap_batch_cb, "top", k.FORM_SPACING_Y)
        form.attachForm(self.wrap_batch_cb, "bottom", k.FORM_SPACING_Y)

        form.attachNone(self.clipboard_btn, "left")
        form.attachForm(self.clipboard_btn, "right", k.FORM_SPACING_X)
        form.attachForm(self.clipboard_btn, "top", k.FORM_SPACING_Y)
        form.attachForm(self.clipboard_btn, "bottom", k.FORM_SPACING_Y)
    ...
    def bind_script(self, node, prefix):
        ...
        wrap_batch_attribute = node.attr(prefix + "WrapBatch")
        wrap_batch_value = wrap_batch_attribute.get() or False
        pm.checkBox(
            self.wrap_batch_cb,
            edit=True,
            value=wrap_batch_value,
            changeCommand=pm.Callback(self.on_wrap_batch_change, wrap_batch_attribute),
        )

    def on_script_change(self, attribute):
        attribute.set(self.script_name_tf.getText().strip())

    def on_wrap_batch_change(self, attribute):
        attribute.set(self.wrap_batch_cb.getValue())
    ...
    def copy_script_to_clipboard(self, node, prefix):
        ...
        script = f'MAYA_MODULE_PATH="{stormmodpath}" PATH="$PATH:{mayabinpath}" {script}'
