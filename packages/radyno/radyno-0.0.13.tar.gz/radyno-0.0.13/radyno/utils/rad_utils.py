...
class H5_rad():
    ...
    def extract_files(self):    
        ...
        self.nmacro     = nmacro
        if nmacro != 1:    
            IDrange     = np.sort(np.random.choice(IDrange, size=maxpart,replace=False)).astype(int)
            self.IDrange    = IDrange
        IDsize          = len(IDrange)      
    ...
    def div_extimate(self):
        ...
        print()
        print("x rad angle =",self.thxlim)
        print("y rad angle =",self.thylim)
        print()
