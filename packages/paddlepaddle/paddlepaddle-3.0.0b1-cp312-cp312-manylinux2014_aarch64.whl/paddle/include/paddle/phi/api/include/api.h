...
PADDLE_API Tensor accuracy_check(const Tensor& x, const Tensor& y, const std::string& fn_name, int64_t res_idx);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> detection_map(const Tensor& detect_res, const Tensor& label, const paddle::optional<Tensor>& has_state, const paddle::optional<Tensor>& pos_count, const paddle::optional<Tensor>& true_pos, const paddle::optional<Tensor>& false_pos, int class_num, int background_label = 0, float overlap_threshold = .5f, bool evaluate_difficult = true, const std::string& ap_type = "integral");
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> tdm_sampler(const Tensor& x, const Tensor& travel, const Tensor& layer, bool output_positive = true, const std::vector<int>& neg_samples_num_list = {}, const std::vector<int>& layer_offset_lod = {}, int seed = 0, int dtype = 2);
...
PADDLE_API Tensor moe(const Tensor& x, const Tensor& gate, const Tensor& bmm0, const Tensor& bias0, const Tensor& bmm1, const Tensor& bias1, const std::string& act_type = "gelu");
