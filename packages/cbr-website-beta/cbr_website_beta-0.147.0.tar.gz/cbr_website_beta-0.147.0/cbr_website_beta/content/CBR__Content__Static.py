import cbr_static
from cbr_website_beta.content.markdown.Markdown_Parser import markdown_parser
from cbr_website_beta.utils._for_osbot.for_OSBot_Utils  import files_names_in_folder
from osbot_utils.base_classes.Type_Safe                 import Type_Safe
from osbot_utils.utils.Files                            import files_list, file_exists, file_contents, path_combine_safe, parent_folder, folders_in_folder, folders_names_in_folder, files_names
...
class CBR__Content__Static(Type_Safe):
    ...
    def base_folder(self, target_file):
        file_path   = self.path_to_file(target_file)
        folder_path = parent_folder(file_path)
        root_folder = self.path_static_content()
        base_folder = folder_path.replace(root_folder, '')
        return base_folder
