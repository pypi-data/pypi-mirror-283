...
def test_bkreport_prod_mcsimulation_success(bkreport):
    ...
    assert simulation_condition is not None, "SimulationCondition element should be present."
    sim_description_param = next(
        (
            param
            for param in simulation_condition
            if param.tag == "Parameter" and param.attrib["Name"] == "SimDescription"
        ),
        None,
    )
    assert sim_description_param is not None, "SimDescription parameter should be present."
    assert sim_description_param.attrib["Value"] == bkreport.simDescription
...
def test_bkreport_prod_mcsimulation_noinputoutput_success(bkreport):
    ...
    * Simulation conditions because the application used is Gauss
    ...
    # Because we are using Gauss, sim conditions should be present
    simulation_condition = root.find("SimulationCondition")
    assert simulation_condition is not None, "SimulationCondition element should be present."
    sim_description_param = next(
        (
