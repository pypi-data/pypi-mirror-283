...
def test_form(setup_athena):
    athena_dir = setup_athena

    filename = 'test_form.py'
    code = f'''def run(athena):
    athena.client().post('http://{API_HOST}/api/echo', lambda r: r
        .body.form({{'foo': 'bar'}}))'''
    ...
    assert response_body['headers']['Content-Type'] == 'application/x-www-form-urlencoded'
    assert response_body['form']['foo'] == ['bar']

def test_form_append(setup_athena):
    athena_dir = setup_athena

    filename = 'test_form_append.py'
    code = f'''def run(athena):
    athena.client().post('http://{API_HOST}/api/echo', lambda r: r
        .body.form({{'foo': 'bar'}})
        .body.form_append('foo', ['baz', 'qux']))'''
    with open(os.path.join(athena_dir, filename), 'w') as f:
        f.write(code)
    ...
    assert response_body['headers']['Content-Type'] == 'application/x-www-form-urlencoded'
    assert response_body['form']['foo'] == ['bar', 'baz', 'qux']
