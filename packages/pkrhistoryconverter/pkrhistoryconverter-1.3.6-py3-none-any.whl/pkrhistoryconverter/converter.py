...
from pkrcomponents.actions.action import CallAction, BetAction, RaiseAction, FoldAction, CheckAction
...
from pkrcomponents.tournaments.tournament import Tournament
from pkrcomponents.utils.exceptions import (ShowdownNotReachedError, CannotParseWinnersError, NotSufficientBetError,
                                            NotSufficientRaiseError)

from pkrhistoryconverter.utils.exceptions import HandConversionError
...
class HandHistoryConverter:
    ...
    def get_winners(self):
        ...
        except CannotParseWinnersError:
            raise HandConversionError
        except ValueError:
            winners = self.data.get("winners")
            for name, reward in winners.items():
                player = self.table.players[name]
                player.win(reward.get("amount"))
