...
from lmnr_cli.parser.parser import runnable_graph_to_template_vars
...
def pull(pipeline_name, pipeline_version_name, project_api_key, loglevel):
    ...
    res = requests.get(
        "http://localhost:8000/v1/pipeline-version-by-name",
    ...
    class_name = pipeline_name.replace("_", "").replace("-", "")
    # TODO: Properly convert to camel case
    ...
    cookiecutter(
        ...
        default_config={"cookiecutters_dir": os.path.abspath("."), "replay_dir": os.path.abspath(".")},  # type: ignore
        no_input=True,
