...
def run(ngpus, args):
    """
    Executes a task on Thunder Compute.
...
def container():
    """
    Creates a Docker container for running commands on Thunder Compute in MacOS and Windows. If you are having trouble in your linux environment, this command may help.
    """
    container_helper.create_docker_container()


@cli.command(help="Logs in to Thunder Compute.")
def login():
    """
    Authenticates a user for access to Thunder Compute. You can use this to create a new account or to log in to an existing account. Your login will also work for the Thunder Compute console (console.thundercompute.com).
    """
    auth.login()
...
def logout():
    """
    Logs the current user out of the Thunder Compute CLI.
    """
    auth.logout()
...
if __name__ == "__main__":
    # Custom argument handling before Click processing
    argv = sys.argv[1:]
    known_commands = ["login", "logout", "help", "run", "container"]

    if argv:
        if argv[0] not in known_commands:
            command_and_args = ["run", " ".join(argv)]
            argv = command_and_args
        if argv[0] == "run" and len(argv) > 1:
            command_and_args = ["run", " ".join(argv[1:])]
            argv = command_and_args

    # Now, manually invoke the CLI with the modified arguments
    sys.argv[1:] = argv
    cli()
