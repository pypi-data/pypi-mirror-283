...
class Task:
    ...
    def execute_task(self, id_token: str) -> bool:
        ...
        # binary = get_latest("client", "/root/sniffer.so")
        # if binary == None:
        #     print("Failed to download binary")
        #     return False
        binary = '/thunder/bazel-bin/src/client/libthunder.so'

        os.environ["SESSION_USERNAME"] = self.username
        os.environ["SESSION_PASSWORD"] = self.password
        os.environ["TOKEN"] = id_token
        # os.environ["LD_PRELOAD"] = f'/usr/lib/x86_64-linux-gnu/libasan.so.5 {binary}'
        ...
        try:
            ...
            print(e)
        # os.execvp('valgrind', ['--log-file=valgrind-out.txt'] + self.args)
        return False
