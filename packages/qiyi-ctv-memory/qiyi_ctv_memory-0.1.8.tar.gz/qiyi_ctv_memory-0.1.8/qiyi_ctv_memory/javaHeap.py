...
def wait_for_file(filepath, timeout=240, poll_interval=3):
...
jar_path = pkg_resources.resource_filename('qiyi_ctv_memory', 'heap_analyzer.jar')

def parseLeak(hprof, outputPath, mapping):
    cmd = ["java", "-jar", jar_path, "-i", hprof, "-o", outputPath, "-m", mapping, "-l", "bitmap,activity"]
    print("Executing Java command: " + " ".join(cmd))
    subprocess.check_call(cmd)
    return hprof.replace('.hprof', '.json')

def parseAll(hprof, outputPath, mapping):
    cmd = ["java", "-jar", jar_path, "-i", hprof, "-o", outputPath, "-m", mapping, "-l", "activity",
           "-d","10", "-b","20"]
    print("Executing Java command: " + " ".join(cmd))
    subprocess.check_call(cmd)
    return hprof.replace('.hprof', '.json')
