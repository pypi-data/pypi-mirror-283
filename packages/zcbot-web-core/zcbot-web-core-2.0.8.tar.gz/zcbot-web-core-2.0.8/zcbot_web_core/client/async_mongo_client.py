...
class AsyncMongo(object):
    ...
    # 插入或更新
    async def insert_or_update(self, collection, data, id_key='_id', upsert=True, multi=False):
        if not multi:
    ...
    # 以主键更新
    async def update_by_pk(self, collection, pk_val, data, upsert=False):
        return await self.db[collection].update_one({'_id': pk_val}, {'$set': data}, upsert=upsert)

    # 批量更新
    async def batch_update(self, collection, filter, datas, *args, **kwargs):
        return await self.db[collection].update_many(filter, {'$set': datas})
...
async def test_mongo_cursor():
    mongo_uri = "mongodb://root:Dangerous!@192.168.10.99:27017"
    mongo_db = "zcbot_pool"
    aio_mongo = AsyncMongo(mongo_uri, mongo_db)
    print(aio_mongo)

    collection = "fastsku_meta_config"
    cursor = await aio_mongo.list_with_cursor(collection)
    async for document in cursor:
        print("document =", document)
    await cursor.close()
...
async def test_mongo():
    ...
    collection = "fastsku_meta_config"
    # rows = await aio_mongo.distinct(collection, dist_key="tenantCode")
    rows = await aio_mongo.list_with_page(collection)
...
async def test_insert_or_update():
    mongo_uri = "mongodb://root:Dangerous!@192.168.10.99:27017"
    mongo_db = "zcbot_pool"
    aio_mongo = AsyncMongo(mongo_uri, mongo_db)
    print(aio_mongo)
    ...
    collection = "demo0101"
    # rows = await aio_mongo.distinct(collection, dist_key="tenantCode")
    data = {"_id": 3, "name": "zhangsan", "friend": "zhaosi88455"}
    result = await aio_mongo.insert_or_update(collection, data)
    # result = await aio_mongo.insert(collection, data)
...
async def test_update():
    ...
    collection = "demo0101"
    # rows = await aio_mongo.distinct(collection, dist_key="tenantCode")
    data = {"_id": 2, "name": "zhangsan", "friend": "zhaosi88455"}
    _filter = {"_id": 2}
    data = {"name": "99ss9ujh"}
    # result = await aio_mongo.insert_or_update(collection, data)
    result = await aio_mongo.update(collection=collection, filter=_filter, data=data)
...
async def test_save():
    ...
    _filter = {"_id": 4}
    data = {"name": "ojgztysbnc", "friend": "sabbzx"}
    result = await aio_mongo.save(collection=collection, filter=_filter, save_data=data, upsert=True)
...
async def test_update_by_pk():
    ...
    data = {"name": "azzzzz", "friend": "ffffff"}
    result = await aio_mongo.update_by_pk(collection=collection, pk_val=4, data=data)
...
async def test_batch_update():
    ...
    collection = "demo0101"
    _filter = {"_id": {"$in": [1, 2, 3]}}
    data = {"friend": "zzzz"}
    result = await aio_mongo.batch_update(collection=collection, filter=_filter, datas=data)
...
async def test_delete():
    ...
    result = await aio_mongo.delete(collection=collection, filter={"_id": 2})
...
async def test_bulk_write():
    ...
    mongo_uri = "mongodb://root:Dangerous!@192.168.10.99:27017"
    mongo_db = "partner_staples"
    aio_mongo = AsyncMongo(mongo_uri, mongo_db)
    print(aio_mongo)

    cursor = await aio_mongo.list_with_cursor("pt_sku_work")
    print("start load")
    bulk_list = []
    async for document in cursor:
        bulk_list.append(InsertOne(document))
    print("start write")
    result = await aio_mongo.bulk_write(collection="pt_sku_work2", bulk_list=bulk_list)
    print("result =", result)
    print("all done")
...
async def test_create_index():
    ...
    result = await aio_mongo.create_index(collection=collection, fields=_fields)
...
async def test_aggregate():
    mongo_uri = "mongodb://root:Dangerous!@192.168.10.199:27017"
    mongo_db = "ecprice_jslink"
    aio_mongo = AsyncMongo(mongo_uri, mongo_db)
    print(aio_mongo)
    _fields = {}
    _query = {}
    _sort = {"genTime": -1}
    pipelines = [
        {"$lookup": {"from": "bs_link_pool", "localField": "linkId", "foreignField": "_id", "as": "sku"}},
        {"$replaceRoot": {"newRoot": {"$mergeObjects": [{"$arrayElemAt": ["$sku", 0]}, "$$ROOT"]}}},
        {"$project": {"sku": 0, **_fields}},
        {"$match": _query},
        {"$sort": _sort}
    ]
    cursor = await aio_mongo.aggregate(collection=f"ecp_pingan_high_material", pipelines=pipelines)
    async for row in cursor:
        print(row)
...
async def test_aggregate_page():
    ...
    rows, count = await aio_mongo.aggregate_page(collection=f"ecp_pingan_high_material", pipelines=pipelines)
...
if __name__ == '__main__':
    ...
    # loop.run_until_complete(test_mongo_cursor())
    # loop.run_until_complete(test_insert_or_update())
    # loop.run_until_complete(test_update())
    # loop.run_until_complete(test_delete())
    # loop.run_until_complete(test_aggregate())
    # loop.run_until_complete(test_aggregate_page())
