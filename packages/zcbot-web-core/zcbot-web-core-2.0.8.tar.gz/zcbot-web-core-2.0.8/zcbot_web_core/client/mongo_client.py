...
class Mongo(object):
    ...
    # 插入或更新
    def insert_or_update(self, collection, data, id_key='_id', upsert=True, multi=False):  # changed
        if not multi:
    ...
    def update_by_pk(self, collection, pk_val, data, upsert=False):
        return self.db[collection].update_one({'_id': pk_val}, {'$set': data}, upsert=upsert)

    # 批量更新
    def batch_update(self, collection, filter, datas, *args, **kwargs):
        return self.db[collection].update_many(filter, {'$set': datas})
