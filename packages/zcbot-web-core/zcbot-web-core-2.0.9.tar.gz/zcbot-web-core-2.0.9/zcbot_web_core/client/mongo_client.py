    ...
    def insert_or_update(self, collection, data=None, update=None, id_key='_id', upsert=True, multi=False):  # changed
        if not multi:
            if data and not update:
                ...
                result = self.db[collection].update_one({id_key: data[id_key]}, update, upsert=upsert)
            else:
            ...
            if data and not update:
                ...
                result = self.db[collection].update_many({id_key: data[id_key]}, update, upsert=upsert)
            else:
                raise BizException("data和update不能同时存在或同时为空")
        return result
