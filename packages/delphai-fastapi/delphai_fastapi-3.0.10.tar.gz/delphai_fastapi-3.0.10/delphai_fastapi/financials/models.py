...
class AddRevenueEntry(CamelModel):
    ...
    revenue: Union[float, str, None] = Field(
        description="Revenue amount datapoint", examples=["22m"]
    )
...
class RemovedResponse(CamelModel):
    removed: int
...
class FundingEvent(CamelModel):
    ...
    amount: Optional[float] = Field(None, description="Amount of investment")
    date: Optional[datetime.date] = Field(
        None, description="Date of the investment", examples=["2024-01-01"]
    )
    ...
    investor_name: Optional[str] = Field(None, description="Name of the investor")
    investor_url: Optional[str] = Field(None, description="Website of the investor")
