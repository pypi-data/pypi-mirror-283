...
# logger = BotrunAskFolderLogger().get_logger()

def botrun_ask_folder(google_drive_folder_id: str, force=False) -> None:
    """
    ...
    # logger.info(f"Downloading files from Google Drive folder ID: {google_drive_folder_id}")
    drive_download(
    ...
    # logger.info(f"Splitting text files in directory: {google_drive_folder_id}")
    run_split_txts(
    ...
    # logger.info(f"Starting embeddings to Qdrant for folder ID: {google_drive_folder_id}")
