...
####################################################################
def updateOpenPathID(account: dict, credentials: NeonCredentials):
    ...
    response = requests.patch(url, json=data, headers=credentials.headers)
...
####################################################################
def updateDID(account: dict, credentials: NeonCredentials):
    assert int(account.get("Account ID")) > 0
...
####################################################################
def appendMemberships(account: dict, credentials: NeonCredentials, detailed=False):
...
####################################################################
def getMemberById(id: int, credentials: NeonCredentials, detailed=False):
    ...
    account = appendMemberships(account, credentials=credentials, detailed=detailed)
    return account
...
####################################################################
def getNeonAccounts(searchFields, credentials: NeonCredentials, neonAccountDict={}):
    # Output Fields
    # 85 is DiscourseId
    ...
    while True:
        ...
        response = requests.post(url, json=data, headers=credentials.headers)
...
####################################################################
def getOrphanOpAccounts(credentials: NeonCredentials, neonAccountDict={}):
    searchFields = [
        {"field": "Membership Expiration Date", "operator": "BLANK"},
        {"field": "OpenPathID", "operator": "NOT_BLANK"},
    ]

    return getNeonAccounts(
        searchFields, credentials=credentials, neonAccountDict=neonAccountDict
    )
...
####################################################################
def getOrphanDiscourseAccounts(credentials: NeonCredentials, neonAccountDict={}):
    searchFields = [
        {"field": "Membership Expiration Date", "operator": "BLANK"},
...
####################################################################
def getMembersFast(credentials: NeonCredentials, neonAccountDict={}):
...
####################################################################
def getAccountsByType(type: str, credentials: NeonCredentials, neonAccountDict={}):
...
####################################################################
def getRealAccounts(credentials: NeonCredentials):
    accountCount = 0
    activeSubscriptions = 0

    neonAccountDict = getMembersFast(credentials=credentials)
    # Staff accounts might not have any membership records
    neonAccountDict = getAccountsByType(
        STAFF_TYPE, credentials=credentials, neonAccountDict=neonAccountDict
    )
    # former Staff accounts might not have any membership records
    neonAccountDict = getOrphanDiscourseAccounts(
        credentials=credentials, neonAccountDict=neonAccountDict
    )
    neonAccountDict = getOrphanOpAccounts(
        credentials=credentials, neonAccountDict=neonAccountDict
    )
    ...
    for account in neonAccountDict:
        ...
        neonAccountDict[account] = appendMemberships(
            neonAccountDict[account], credentials=credentials
        )
