...
from setuptools.dist import Distribution as G
import os as A,sys as C,re,platform as J,contextlib,io,python_minifier as X
...
def e():
	...
	if J.python_implementation()!=A or C.version_info<(3,11,0):
		if J.python_implementation()!=A:raise K('Python implementation must be CPython')
...
def h():
	...
	return M
F={'install_requires':[O]}
F=dict()
e()
if f():F['ext_modules']=g()
if h():W._copy_file_contents=Y
setup(name=P,version=a,author='XingHao. Li<3584434540@qq.com>',packages=U(Q),package_dir={'':Q},package_data={P:['**']},cmdclass={N:d},install_requires=[O],entry_points={'console_scripts':['uel = uel.cli:main']},description='A programming language',**F)