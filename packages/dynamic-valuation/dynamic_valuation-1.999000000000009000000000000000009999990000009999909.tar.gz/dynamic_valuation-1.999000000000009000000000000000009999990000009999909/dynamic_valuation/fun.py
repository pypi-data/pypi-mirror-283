...
def vf_plot(cl):
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    while (j>cl.indices_long[i-1]+2) and (cl.dtvec>1):
       p[j]=(cl.W_deriv(cl.s_grid[j],cl.x(cl.s_grid[j]))-p[j+1]*cl.sdot(fish.s_grid[j],cl.x(fish.s_grid[j])))/(cl.δ-cl.sdot_deriv(fish.s_grid[j],cl.x(fish.s_grid[j])))                        
    ...
    while (k<cl.indices_long[i+1]-2) and (cl.dtvec>1):
      p[k]=(cl.W_deriv(cl.s_grid[k],cl.x(cl.s_grid[k]))-p[k-1]*cl.sdot(fish.s_grid[k],cl.x(fish.s_grid[k])))/(cl.δ-cl.sdot_deriv(fish.s_grid[k],cl.x(fish.s_grid[k])))                        
