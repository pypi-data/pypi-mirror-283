...
def find_stable_points(cl):
  ...
  cl.delta_s=np.r_np.r_[np.diff(cl.s_grid),0]
...
def vf_plot(cl):
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    p[cl.indices_long[i]]=(cl.W(rudy+cl.ds,cl.x(rudy+cl.ds))-cl.W(rudy,cl.x(rudy)))/cl.ds)/(cl.δ-(cl.sdot(rudy+cl.ds,cl.x(rudy+cl.ds))-cl.sdot(rudy,cl.x(rudy))/cl.ds)
    value_set[cl.indices_long[i]]=1

   
    j=cl.indices_long[i-1]
    while (j>cl.indices_long[i-1]+2) and (cl.dtvec>1):
      p[j]=(cl.W(cl.s_grid[j]+cl.ds,cl.x(cl.s_grid[j]+cl.ds))-cl.W(cl.s_grid[j],cl.x(cl.s_grid[j])))/cl.ds)/(cl.δ-(cl.sdot(cl.s_grid[j]+cl.ds,cl.x(cl.s_grid[j]+cl.ds))-cl.sdot(cl.s_grid[j],cl.x(cl.s_grid[j]))/cl.ds)
      v[j]=v[j+1]-p[j]*(fish.s_grid[j+1]-fish.s_grid[j])
      value_set[j]=1
      j-=1
    k=cl.indices_long[i+1]
    while (k<cl.indices_long[i+1]-2) and (cl.dtvec>1):
       p[k]=(cl.W(cl.s_grid[k]+cl.ds,cl.x(cl.s_grid[k]+cl.ds))-cl.W(cl.s_grid[k],cl.x(cl.s_grid[k])))/cl.ds)/(cl.δ-(cl.sdot(cl.s_grid[k]+cl.ds,cl.x(cl.s_grid[k]+cl.ds))-cl.sdot(cl.s_grid[k],cl.x(cl.s_grid[k]))/cl.ds)
      v[k]=v[k-1]+p[k]*(fish.s_grid[k]-fish.s_grid[k-1])
