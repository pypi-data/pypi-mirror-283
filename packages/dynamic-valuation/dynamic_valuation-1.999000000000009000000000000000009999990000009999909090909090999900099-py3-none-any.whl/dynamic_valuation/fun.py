...
def find_stable_points(cl):
  ...
  cl.delta_s=np.r_[np.diff(cl.s_grid),0]
  ...
  cl.sax.plot(cl.s_grid,cl.ds+0*cl.s_grid)
  cl.sax.plot(cl.s_grid,0*cl.s_grid-cl.ds)
...
def vf_plot(cl):
  ...
  value_set=0*v
  p=value_set.copy()
  cl.roots_long=np.unique(np.r_[cl.s_min,cl.roots,cl.s_max])
  cl.indices_long=np.unique(np.r_[0,cl.indices,len(cl.s_grid)-1])
  ...
  for i in range(1,len(cl.roots_long)-1):
    rudy=cl.roots_long[i]
    v[cl.indices_long[i]]=cl.W(rudy,cl.x(rudy))/cl.δ
    v[cl.indices_long[i]-1]=cl.W(rudy,cl.x(rudy))/cl.δ
    v[cl.indices_long[i]+1]=cl.W(rudy,cl.x(rudy))/cl.δ

    p[cl.indices_long[i]]=cl.W_deriv(rudy,cl.x(rudy))/(cl.δ-cl.sdot_deriv(rudy,cl.x(rudy)))
    p[cl.indices_long[i]-1]=cl.W_deriv(rudy,cl.x(rudy))/(cl.δ-cl.sdot_deriv(rudy,cl.x(rudy)))
    p[cl.indices_long[i]+1]=cl.W_deriv(rudy,cl.x(rudy))/(cl.δ-cl.sdot_deriv(rudy,cl.x(rudy)))


    value_set[cl.indices_long[i]]=1
    value_set[cl.indices_long[i]+2]=1

    value_set[cl.indices_long[i]-1]=1

    

   
    j=cl.indices_long[i]-2
    while (j>cl.indices_long[i-1]) and (cl.dtvec[j]>.1):
       p[j]=(cl.W_deriv(cl.s_grid[j],cl.x(cl.s_grid[j]))+((p[j+2]-p[j+1])/cl.delta_s[j+1])*cl.sdot(cl.s_grid[j],cl.x(cl.s_grid[j])))/(cl.δ-cl.sdot_deriv(cl.s_grid[j],cl.x(cl.s_grid[j])))                        
       v[j]=v[j+1]-p[j]*(cl.s_grid[j+1]-cl.s_grid[j])
       value_set[j]=1
       j-=1
    k=cl.indices_long[i]+2
    while (k<cl.indices_long[i+1]) and (cl.dtvec[j]>.1):
      p[k]=(cl.W_deriv(cl.s_grid[k],cl.x(cl.s_grid[k]))+((p[k-1]-p[k-2])/cl.delta_s[k-1])*cl.sdot(cl.s_grid[k],cl.x(cl.s_grid[k])))/(cl.δ-cl.sdot_deriv(cl.s_grid[k],cl.x(cl.s_grid[k])))                        
      v[k]=v[k-1]+p[k]*(cl.s_grid[k]-cl.s_grid[k-1])
      value_set[k]=1
      k+=1
  ...
  rhs=(np.gradient(v)/np.gradient(cl.s_grid)*cl.sdotvec+cl.wvec)/cl.δ
  cl.v=v
  cl.p=p
  ...
  cl.vax.scatter(cl.s_grid[value_set>0], v[value_set>0],color='black',s=9,zorder=3,label='Linear Approximation near Stable Fixed Point')

  cl.vax.plot(cl.s_grid, rhs,color='blue',label=r"$\frac{W(s)+V'(s)\dot{s}(s)}{\delta}$",zorder=1,lw=6)
  ...
  cl.vax.set_title('Diagnostic/Fit for the Value Function Differential Equation');
  cl.vfig2,cl.vax2=plt.subplots()

  cl.vax2.plot(cl.s_grid, cl.v,color='red',lw=4,label='V(s)')
  cl.vax2.set_title('Value Function');
  cl.vax2.set_xlabel('Capital stock: $s$', fontsize=12)


  cl.vax2.legend()
  cl.pfig,cl.pax=plt.subplots()

  cl.pax.plot(cl.s_grid, np.gradient(v)/np.gradient(cl.s_grid),color='red',lw=4,label=r"$p(s)=V'(s)$")
  cl.pax.set_title('Shadow Price');
  cl.pax.set_xlabel('Capital stock: $s$', fontsize=12)
