...
def vf_plot(cl):
  ...
  for i in range(len(cl.roots)):
    rudy=cl.roots[i]
    v[cl.indices[i]]=cl.W(rudy,cl.x(rudy))/(1-cl.β)
    value_set[cl.indices[i]]=1
  ...
  cl.wvec=(cl.wvec0+cl.wvec1)/2
  cl.sdotvec=(cl.sdotvec0+cl.sdotvec1)/2
  #absdot=[np.abs(i),0) for i in cl.sdotvec]
  cl.dtvec=np.maximum(.01,abs(cl.delta_s/cl.sdotvec))
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    v[cl.indices_long[i]]=cl.W(rudy,cl.x(rudy))/(1-cl.β)
    value_set[cl.indices_long[i]]=1
    j=cl.indices_long[i]-1
    while (j>cl.indices_long[i-1]):# and (cl.dtvec[j]>.01):
      v[j]=cl.β**(cl.dtvec[j])*v[j+1]+cl.wvec[j]*cl.dtvec[j]*cl.β**(cl.dtvec[j]/2) 
      value_set[j]=1
      j-=1
    k=cl.indices_long[i]+1
    while (k<cl.indices_long[i+1]):# and (cl.dtvec[j]>.01):
      v[k]=cl.β**(cl.dtvec[k])*v[k-1]+cl.wvec[k]*cl.dtvec[k]*cl.β**(cl.dtvec[k]/2) 
      value_set[k]=1
      k+=1
