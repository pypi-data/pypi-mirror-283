...
def vf_plot(cl):
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    while (j>cl.indices_long[i-1]+2):
       p[j]=(cl.W_deriv(cl.s_grid[j],cl.x(cl.s_grid[j]))+p[j+1]*cl.sdot(cl.s_grid[j],cl.x(cl.s_grid[j])))/(cl.δ-cl.sdot_deriv(cl.s_grid[j],cl.x(cl.s_grid[j])))                        
       v[j]=v[j+1]-p[j]*(cl.s_grid[j+1]-cl.s_grid[j])
       value_set[j]=1
       j-=1
    k=cl.indices_long[i]+1
    while (k<cl.indices_long[i+1]-2):
      p[k]=(cl.W_deriv(cl.s_grid[k],cl.x(cl.s_grid[k]))+p[k-1]*cl.sdot(cl.s_grid[k],cl.x(cl.s_grid[k])))/(cl.δ-cl.sdot_deriv(cl.s_grid[k],cl.x(cl.s_grid[k])))                        
