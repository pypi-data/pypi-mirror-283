...
def vf_plot(cl):
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    j=cl.indices_long[i]
    while (j>cl.indices_long[i-1]+2) and (cl.sdotvec0[j]<9*cl.ds):
      v[j]=val+w_s/(cl.δ-sdot_s)*(cl.s_grid[j]-rudy)
      value_set[j]=1
      j-=1
    k=cl.indices_long[i+1]
    while (k<cl.indices_long[i+1]-2) and (cl.sdotvec0[k]>-9*cl.ds):
      v[k]=val+w_s/(cl.δ-sdot_s)*(cl.s_grid[k]-rudy)
      value_set[k]=1
      k+=1
