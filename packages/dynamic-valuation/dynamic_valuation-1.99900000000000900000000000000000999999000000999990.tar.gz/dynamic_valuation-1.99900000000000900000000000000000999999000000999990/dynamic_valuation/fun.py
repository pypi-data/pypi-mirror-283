...
def vf_plot(cl):
  ...
  for i in range(1,len(cl.roots_long)-1):
    ...
    while (j>cl.indices_long[i-1]+2) and (cl.dtvec>1):
      p[j]=(cl.W_deriv(fish.s_grid[j],cl.x(fish.s_grid[j]))/(cl.δ-(cl.sdot_deriv(fish.s_grid[j],cl.x(fish.s_grid[j]))
      v[j]=v[j+1]-p[j]*(fish.s_grid[j+1]-fish.s_grid[j])
      value_set[j]=1
      j-=1
    k=cl.indices_long[i+1]
    while (k<cl.indices_long[i+1]-2) and (cl.dtvec>1):
      p[k]=(cl.W_deriv(fish.s_grid[k],cl.x(fish.s_grid[k]))/(cl.δ-(cl.sdot_deriv(fish.s_grid[k],cl.x(fish.s_grid[k]))
