...
def get_indices_around_roots(root_indices, vector, threshold):
  """
  Finds indices around roots where magnitude is less than threshold,
  returning separate lists for each root.

  Args:
      root_indices: A list containing indices of roots in the vector.
      vector: The vector to search for elements within the buffer.
      threshold: The maximum magnitude for an element to be considered within the buffer.

  Returns:
      A list of lists, where each inner list contains indices around a root that satisfy the magnitude criteria.
  """
  all_indices = []
  for root_index in root_indices:
    indices = []
    # Search left until threshold is crossed or vector beginning is reached
    i = root_index - 1
    while i >= 0 and abs(vector[i]) < threshold:
      indices.append(i)
      i -= 1

    # Search right until threshold is crossed or vector end is reached
    i = root_index + 1
    while i < len(vector) and abs(vector[i]) < threshold:
      indices.append(i)
...
def vf_plot(cl):
  ...
  cl.buffered_indices = get_indices_around_roots(cl.indices,cl.sdotvec0,cl.ds)
