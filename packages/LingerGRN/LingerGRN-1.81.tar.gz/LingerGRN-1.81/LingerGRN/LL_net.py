...
def TF_RE_binding(GRNdir,adata_RNA,adata_ATAC,genome,method,outdir,species):
    from tqdm import tqdm
    import numpy as np
    import pandas as pd
...
def load_TFbinding_scNN(GRNdir,outdir,genome):
    import pandas as pd
    import numpy as np
    genome_map=pd.read_csv('genome_map.txt',sep='\t',header=None)
    genome_map.index=genome_map[0].values 
    ...
    Match2=pd.read_csv('Match_TF_motif_'+genome_map.loc[genome][1]+'.txt',sep='\t',header=0)
...
def load_RE_TG_scNN(outdir):   
    ...
    distance,REs,TGs=list2mat(dis,'RE','gene','distance')
    distance=pd.DataFrame(distance,index=REs,columns=TGs)
    cis=pd.read_csv(outdir+'cell_population_cis_regulatory.txt',sep='\t',header=None)
    cis.columns=['RE','TG','score']
    cisGRN,REs,TGs=list2mat(cis,'RE','TG','score')
    cisGRN=pd.DataFrame(cisGRN,index=REs,columns=TGs)
    REoverlap=list(set(cisGRN.index)&set(distance.index))
    cisGRN=cisGRN.loc[REoverlap]
    distance=distance.loc[REoverlap]
    geneoverlap=list(set(cisGRN.columns)&set(distance.columns))
    cisGRN=cisGRN[geneoverlap]
    distance=distance[geneoverlap]
    return distance,cisGRN
...
def cis_reg(GRNdir,adata_RNA,adata_ATAC,genome,method,outdir,species): 
    from tqdm import tqdm
    chrom=['chr'+str(i+1) for i in range(22)]
    chrom.append('chrX')
    if method=='baseline':
...
## cell annotation
    ...
    return resultall  
def cell_type_specific_cis_reg_scNN(distance,cisGRN,RE,TG):
    import numpy as np
    import pandas as pd
    from scipy.sparse import csc_matrix
    from scipy.sparse import coo_matrix
    RE=RE.loc[distance.index]
    ## select the genes
    TGoverlap=list(set(distance.columns)&set(TG.index))
    #target_col_indices = [col_dict[col] for col in TGoverlap]
    TG=TG.loc[TGoverlap]
    ## cell annotation
    TG_temp=TG.values#[:,np.array(label)==celltype].mean(axis=1)
    ...
    RE_temp=RE_temp/RE_temp.mean()+0.1
    cisGRN=cisGRN+0.1
    Score=(RE_temp *cisGRN.values)*(distance.values)*(TG_temp.T)
    Score=pd.DataFrame(Score,index=distance.index,columns=distance.columns)
    data = Score.values[Score.values!=0] 
    rows, cols = np.nonzero(Score.values) 
    coo = coo_matrix((data,(rows,cols)),shape=Score.shape)
    combined = np.zeros([len(data),3], dtype=object) 
...
def cell_type_specific_cis_reg(GRNdir,adata_RNA,adata_ATAC,genome,celltype,outdir,method): 
    ...
    if (celltype=='all')&(method!='scNN'):
        ...
        distance,cisGRN=load_RE_TG_scNN(outdir)
        for label0 in labelset:
            label0=str(label0)
            temp=adata_ATAC.X[np.array(label)==label0,:].mean(axis=0).T
            RE=pd.DataFrame(temp,index=adata_ATAC.var['gene_ids'].values,columns=['values'])
            ...
            TG=pd.DataFrame(temp,index=adata_RNA.var['gene_ids'].values,columns=['values'])
            del temp
            result=cell_type_specific_cis_reg_scNN(distance,cisGRN,RE,TG)
...
def trans_reg(GRNdir,method,outdir,species):
    import ast
    import pandas as pd
    from scipy.sparse import coo_matrix
    import numpy as np
