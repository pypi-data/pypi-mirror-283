...
class ProjectConfig(ProjectConfigBase, ProjectConfigLintMixin, ProjectConfigDirsMixin):
    ...
    def save(self, path=None):
        ...
        with open(path or self.path, mode="w+", encoding="utf8") as fp:
            ...
            with open(path.replace('.ini', '.json') or self.path.replace('.ini', '.json'), mode="w+", encoding="utf8") as nfp:
                tuples = [{s: self.items(s, None, True)} for s in self.sections()]
                contents_json = json.dumps(tuples[0], ensure_ascii=False, indent=2)
                # click.echo("contents json: %s" % contents_json)
                nfp.seek(0)
                nfp.truncate()
                nfp.write(contents_json.strip() + "\n")
