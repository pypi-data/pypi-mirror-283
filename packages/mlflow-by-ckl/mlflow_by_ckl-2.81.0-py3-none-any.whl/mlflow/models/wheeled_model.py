...
class WheeledModel:
    ...
    def save_model(self, path, mlflow_model=None):
        ...
        local_model_path = _download_artifact_from_uri(
            self._model_uri, output_path=path
        )
        ...
        conda_env = _extract_conda_env(
            model_metadata.flavors.get(FLAVOR_NAME, {}).get(ENV, None)
        )
        ...
        if conda_env is None and not os.path.isfile(pip_requirements_path):
            raise MlflowException(
                "Cannot add libraries for model with no logged dependencies.",
                BAD_REQUEST,
            )
        ...
        print("Downloading wheels for the model, REQUIRMENTS:L")
        ocr_reqs = [
            line for line in open(pip_requirements_path, "r") if "spark-ocr" in line
        ]
        ocr_req_path = pip_requirements_path + ".ocr"
        open(ocr_req_path, "w").write("\n".join(ocr_reqs))

        no_ocr_reqs = [
            line for line in open(pip_requirements_path, "r") if "spark-ocr" not in line
        ]
        open(pip_requirements_path, "w").write("\n".join(no_ocr_reqs))

        read_and_print_file(pip_requirements_path)
        print("V2")
        read_and_print_file(ocr_req_path)
        WheeledModel._download_wheels(
            pip_requirements_path=ocr_req_path, dst_path=wheels_dir, no_deps=True
        )
        # RM !! urllib3-2.2.1-py3-none-any.whl
        os.system(f"rm {wheels_dir}/urllib3-2.2.1-py3-none-any.whl")
        ...
        os.system(f"rm {wheels_dir}/pillow-10.4.0-cp39-cp39-manylinux_2_28_x86_64.whl")
        os.system(f"rm {wheels_dir}/Pillow-10.4.0-cp39-cp39-manylinux_2_28_x86_64.whl")

        os.system(f"rm {wheels_dir}/pillow-10.3.0-cp39-cp39-manylinux_2_28_x86_64.whl")
        os.system(f"rm {wheels_dir}/Pillow-10.3.0-cp39-cp39-manylinux_2_28_x86_64.whl")
        os.system(f"rm {wheels_dir}/pillow-10.2.0-cp39-cp39-manylinux_2_28_x86_64.whl")
        os.system(f"rm {wheels_dir}/Pillow-10.2.0-cp39-cp39-manylinux_2_28_x86_64.whl")
        os.system(f"rm {wheels_dir}/numpy-1.24.3-cp39-cp39-manylinux_2_17_x86_64.manylinux2014_x86_64.whl")
        os.system(f"rm {wheels_dir}/numpy-1.26.4-cp39-cp39-manylinux_2_17_x86_64.manylinux2014_x86_64.whl")


        print("CONTENTS OF WHEEL DIR:")
        r = capture_cmd_output(f'ls {wheels_dir}')
        print(r)
        ...
        shutil.copy2(
            ...
            os.path.join(local_model_path, _ORIGINAL_REQ_FILE_NAME),
        )
    ...
    @classmethod
    def _download_wheels(cls, pip_requirements_path, dst_path, no_deps=False):
        """
        ...
        if no_deps:
            download_command = (
                ...
                f"{pip_requirements_path} --no-cache-dir --no-deps"
            )
        else:
            download_command = (
                f"{sys.executable} -m pip wheel {pip_wheel_options} --wheel-dir={dst_path} -r"
        ...
        try:
            subprocess.run(
                [download_command], check=True, shell=True, capture_output=True
            )
        except subprocess.CalledProcessError as e:
            raise MlflowException(
                f"An error occurred while downloading the dependency wheels: {e.stderr}, {e.stdout}"
            )

    def _overwrite_pip_requirements_with_wheels(
        self, pip_requirements_path, wheels_dir
    ):
        """
