...
def _is_mlflow_requirement(requirement_string):
    """
    Returns True if `requirement_string` represents a requirement for mlflow (e.g. 'mlflow_by_ckl==1.2.3').
    """
...
def _generate_mlflow_version_pinning() -> str:
    """
    Returns a pinned requirement for the current MLflow version (e.g., "mlflow_by_ckl==3.2.1").
    ...
    if _MLFLOW_TESTING.get():
        ...
        return f"mlflow_by_ckl=={VERSION}"
    ...
    # tests.
    return f"mlflow_by_ckl=={version.major}.{version.minor}.{version.micro - 1}"


def _contains_mlflow_requirement(requirements):
    """
    Returns True if `requirements` contains a requirement for mlflow (e.g. 'mlflow_by_ckl==1.2.3').
    """
