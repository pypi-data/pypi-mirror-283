...
from python_sage_imap.helpers.email import EmailMessage, EmailIterator
...
def test_email_message_initialization():
    ...
    expected_date = convert_to_local_time(parsed_date).replace(microsecond=0).isoformat()
...
def test_decode_payload_latin1_error():
    ...
    part.set_payload(b'\x80abc')
    assert email_message.decode_payload(part) == '\x80abc'  # "\x80" is valid in latin-1 but may not display correctly
...
def create_test_message(subject="Test", from_address="test@example.com", to_address="to@example.com", body="This is a test email", is_multipart=False, with_attachments=False):
    if is_multipart:
        message = MIMEMultipart()
