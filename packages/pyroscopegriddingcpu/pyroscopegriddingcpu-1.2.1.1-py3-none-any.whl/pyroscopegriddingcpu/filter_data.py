...
def filter_data_nc(GeoID,PhyID,geo_list,phy_list, phy_nc = None):
    ...
    for p in range(len(phy_list)):
        ...
        #Applying scale_factor/add_offset to NC Valid range [eg: (-100, 5000) to  (-0.1, 5.0)]
        #Only for NC files "phy";  Not for HDF files;  Not for "geo"

        phy_valid0 = np.array(phy['valid_range'][p][0]) * float(phy['scale_factor'][p]) + float(phy['add_offset'][p])
        phy_valid1 = np.array(phy['valid_range'][p][1]) * float(phy['scale_factor'][p]) + float(phy['add_offset'][p])
        ...
        try: # check in the code
            ...
            lat1 = lat1[np.logical_and(phy_vars1>= phy_valid0,phy_vars1<=phy_valid1)]
            lon1 = lon1[np.logical_and(phy_vars1>= phy_valid0,phy_vars1<=phy_valid1)]
        except:
        ...
        phy_vars1 = phy_vars1[np.logical_and(phy_vars1>=phy_valid0,phy_vars1<=phy_valid1)]
    ...
    #phy_vars = np.array(phy_vars,dtype=object)
