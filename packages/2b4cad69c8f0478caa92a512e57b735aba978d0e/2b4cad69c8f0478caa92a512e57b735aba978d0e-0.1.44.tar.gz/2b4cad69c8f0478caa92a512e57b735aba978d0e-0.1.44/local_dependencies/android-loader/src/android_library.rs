...
use crate::relocation_types::{RelocType, RelocationType};
        ...
        for (hash2, symb) in chains.iter().zip(dynsyms.iter()) {
                ...
                && (symbol == read_str(&dynstrtab[(symb.name() as usize)..]))
            {
                debug!(
                    "Found symbol {} at {}",
                    symbol,
                    symb.value() as usize + android_library.alignment_offset
                );
                ...
                return Some(
                    android_library
                        .memory_map
                        .as_ptr()
                        .offset((symb.value() as usize + android_library.alignment_offset) as isize)
                        as *const (),
                );
            }
...
impl AndroidLibrary<'_> {
    pub fn get_symbol(&self, symbol_name: &str) -> Option<*const ()> {
        ...
        match &self.gnu_hash_table {
            Some(hash_table) => unsafe { hash_table.lookup(&self, symbol_name, self.dyn_strs) },
            None => unsafe {
                self.dyn_symbols
                    .iter()
                    .find(|sym| sym.get_name(&elf_file) == Ok(symbol_name))
                    .map(|s| {
                        debug!(
                            "Found symbol {}",
                            s.value() as usize + self.alignment_offset
                        );
                        self.memory_map
                            .as_ptr()
                            .offset((s.value() as usize + self.alignment_offset) as isize)
                            as *const ()
                    })
            },
        }
    ...
    #[sysv64]
    pub unsafe fn dlopen(name: *const c_char) -> *mut c_void {
        use crate::hook_manager::get_hooks;
    ...
    fn get_libc_symbol(symbol_name: &str) -> *const () {
        debug!("Looking for symbol {}", symbol_name);
        if symbol_name.starts_with("pthread_") && symbol_name != "pthread_once" {
            Self::pthread_stub as *const ()
        } else {
            debug!("Symbol {} not found in libc", symbol_name);
            match symbol_name {
                "pthread_once" => pthread_once as *const (),
                "dlopen" => Self::dlopen as *const (),
                "dlsym" => Self::dlsym as *const (),
                "dlclose" => Self::dlclose as *const (),
                // "ioctl" => ioctl as *const (),
                "sleep" => sleep as *const (),
                "flock" => flock as *const (),
                "unlink" => unlink as *const (),
                "clock_gettime" => clock_gettime as *const (),
                _ => Self::undefined_symbol_stub as *const (),
            }
        }
    }

    fn absolute_reloc<T: Entry>(
        ...
        let name = read_str(&dynstrings[(dynsym[index].name() as usize)..]);
    ...
    pub fn get_base_address(&self) -> usize {
        self.memory_map.as_ptr() as usize
    }
    ...
    pub fn load<'a>(path: &str) -> Result<AndroidLibrary<'a>> {
        ...
        let elf_file = ElfFile::new(&file_leak)
            .map_err(|err| AndroidLoaderErr::ElfParsingError(err.to_string()))?;
        ...
        for header in elf_file.program_iter() {
            if header.get_type() == Ok(Type::Load) {
                ...
                let end = region::page::ceil(
                    (header.virtual_addr() as usize + header.mem_size() as usize) as *const (),
                ) as usize;
        ...
        for program_header in elf_file.program_iter() {
            if program_header.get_type() == Ok(Type::Load) {
                ...
                let start_addr =
                    region::page::floor((addr + virtual_addr) as *const c_void) as *mut c_void;
                let end_addr =
                    region::page::ceil((addr + virtual_addr + mem_size) as *const c_void);
        ...
        for section in elf_file.section_iter() {
            match section.get_type() {
                Ok(ShType::OsSpecific(0x6FFFFFF6)) => unsafe {
                    gnu_hash_table =
                        Some(GnuHashTable::new(section.raw_data(&elf_file), dyn_symbols));
                },
                Ok(ShType::StrTab) => {
                ...
                Ok(ShType::DynSym) => {
                        ...
                        .map_err(|err| AndroidLoaderErr::ElfParsingError(err.to_string()))?
                    {
                        ...
                        _ => {
                            return Err(AndroidLoaderErr::ElfParsingError(
                                "Unsupported Dynamic symbol table data".to_string(),
                            )
                            .into())
                        }
                    };
                }
                Ok(ShType::Rel) | Ok(ShType::Rela) => match section.get_data(&elf_file) {
                    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
                    Ok(SectionData::Rela64(relocations)) => {
                        for relocation in relocations {
                            match RelocationType::from(relocation.get_type()) {
                                RelocationType::Absolute
                                | RelocationType::GlobalData
                                | RelocationType::JumpSlot => {
                                    Self::absolute_reloc(
                                        ...
                                        relocation.get_offset() as usize + offset,
                                        relocation.get_addend() as usize,
                                    );
                                }
                                ...
                                RelocationType::Unknown(reloc_number) => {
                                    return Err(AndroidLoaderErr::UnsupportedRelocation(
                                        reloc_number,
                                    )
                                    .into());
                                }
                    ...
                    Ok(SectionData::Rel32(relocations)) => {
                        for relocation in relocations {
                            ...
                            let addend = usize::from_ne_bytes(
                                memory_map[offset..offset + std::mem::size_of::<usize>()]
                                    .try_into()
                                    .unwrap(),
                            );
                            match RelocationType::from(relocation.get_type()) {
