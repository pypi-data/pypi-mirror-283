...
def bootstrap(array,stat,weights=None,nreal=100):
    real = np.zeros(nreal)                       # declare an empty list to store the bootstrap realizations
    if weights is None:
        weights = np.ones((len(array))) 
    for l in tqdm(range(0,nreal)):               # loop over the L bootstrap realizations       
        samples = random.choices(array, k=len(array),weights=weights) # n Monte Carlo simulations, sample with replacement
        real[l] = stat(samples)                  # calculate the realization of the statistic and append to list
    return real                                  # return the list of realizations of the statistic
