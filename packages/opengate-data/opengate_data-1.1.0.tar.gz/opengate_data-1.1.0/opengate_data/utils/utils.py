...
def send_request(method: str, headers: dict | None = None, url: str = '', data: dict[str, Any] | str = None,
                 files: [str, [str, bytes, str]] = None, json_payload: dict[str, Any] = None, params: Any = None) -> Response | dict[
    ...
    try:
        if method == 'post':
            response = requests.post(url, headers=headers, data=data, params=params, files=files, json=json_payload, verify=False,
                                     timeout=3000)
        elif method == 'get':
            response = requests.get(url, headers=headers, verify=False, timeout=3000)
        elif method == 'put':
            response = requests.put(url, headers=headers, data=data, files=files, json=json_payload, verify=False,
                                    timeout=3000)
        elif method == 'delete':
