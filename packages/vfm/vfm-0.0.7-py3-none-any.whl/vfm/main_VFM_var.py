...
class VFMProcessor:
    ...
    def load_data(self):
        ...
        # for well in self.wells:
            # print(f"PI data Time for well {well}:", self.pi_data[well]['Time'].head())
            # print(f"BTP data Time for well {well}:", self.btp_data[well]['Time'].head())
        # print("BDO data Time:", self.bdo_data['Time'].head())
    ...
    def process_data(self):
        ...
        for well in self.wells:
            ...
            T_us_PI = self.pi_data[well]['T_us_PI'].values
            P_us_PI = self.pi_data[well]['P_us_PI'].values
            P_ds_SDV_PI = self.pi_data[well]['P_ds_SDV_PI'].values
            DeltaPManifold_PI = self.pi_data[well]['DeltaPManifold_PI'].values
            Qgi_PI = self.pi_data[well]['Qgi_PI'].values
            u_PI = self.pi_data[well]['u_PI'].values
            u_RWT = self.pi_data[well]['u_RWT'].values
            Fc = self.pi_data[well]['P_us_RWT'].values/self.pi_data[well]['P_us_PI'].values
            T_us_RWT = self.pi_data[well]['T_us_RWT'].values
            P_us_RWT = self.pi_data[well]['P_us_RWT'].values
            P_ds_SDV_RWT = self.pi_data[well]['P_ds_SDV_RWT'].values
            DeltaPManifold_RWT = self.pi_data[well]['DeltaPManifold_RWT'].values
            Qgi_RWT = self.pi_data[well]['Qgi_RWT'].values


            P_us_SDV = self.pi_data[well]['P_ds_SDV_PI'].values
            P_ds_PI = P_us_SDV + self.pi_data[well]['DeltaPManifold_PI'].values
            P_us_SDV = self.pi_data[well]['P_ds_SDV_RWT'].values
            P_ds_RWT = P_us_SDV + self.pi_data[well]['DeltaPManifold_RWT'].values
            ...
            pi_time_series = self.pi_data[well]['Time'].values.astype('datetime64[s]')
            btp_indices = []
            ...
            for i, pi_time in enumerate(pi_time_series):
                ...
                # print(f"Processing i = {i}, k = {k}, TimeBTP = {self.pi_data[well]['TimeBTP'].iloc[i]}, PI Time = {pi_time}")
            ...
            ts = (pi_time_series[1] - pi_time_series[0]).astype('float') / (24 * 3600)  # in days
            # print(f"Debug: Time step (ts) calculated: {ts}")
            inv_ts = int(1 / ts)
            inv_day = int(1 / (24 * ts))
            ...
            if self.flag_init == 1:
                ...
                for i in range(n_BDO):
                    while k < len(pi_time_series) and pi_time_series[k] <= Time_BDO[i]:
                        if k == n_pi - 1:
                            break
                        k += 1
                ...
                for i in pos_sim[::inv_ts]:
                    for j in range(1):
                        ...
                        if idx < n_pi and idx + inv_ts < n_pi:
                            T_us_est[idx] = np.mean(T_us_PI[idx:idx + inv_ts])
                            P_us_est[idx] = np.mean(P_us_PI[idx:idx + inv_ts])
                            P_ds_est[idx] = np.mean(P_ds_SDV_PI[idx:idx + inv_ts])
                            Qgi_est[idx] = np.mean(Qgi_RWT[idx:idx + inv_ts])
                            opening_est[idx] = np.mean(u_PI[idx:idx + inv_ts])
                            if P_us_est[idx] != 0:
                            ...
                            results = VFM(
                                ...
                                P_ds_est[idx], P_ds_SDV_RWT[idx], Qgi_est[idx], Qgi_RWT[idx],
                ...
                for i in pos_sim:
                    if self.flag_init == 3:
                        ...
                        k = 0
                        while np.isnan(opening_PI[i - k]) or opening_PI[i - k] == 0:
                            if i - k == 0 or i > n_pi - 3:
                                break
                            if opening_PI[i - k] == 0 and (P_us_PI[i + 2 * inv_day] < P_us_PI[i] or P_us_PI[i + 3 * inv_day] < P_us_PI[i]):
                                break
                            k += 1
                        opening_est[i] = opening_PI[i - k]
                        pos_min = min(pos_min, i - k)
                    ...
                    try:
                        results = VFM(
                            T_us_est[i], T_us_RWT[i], P_us_est[i], P_us_RWT[i],
                            P_ds_est[i], P_ds_RWT[i], Qgi_est[i], Qgi_RWT[i],
                            opening_est[i], opening_RWT[i], Fc_est[i], self.pi_data[well]['rhoo_SC'].iloc[i],
            ...
            # Ensure pos_cons is valid before calling plotting_delumping
            if pos_cons.size > 0:
                # print(f"pos_cons: {pos_cons}")
                # print(f"pos_cont: {pos_cont}")
