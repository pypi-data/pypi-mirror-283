...
class Parquets:
    ...
    def save_symbol_parquet(self, sd: SymbolDetail, df: pl.DataFrame, period: Period):
        match period:
    ...
    def merge_parquets(self):
        ...
        for root, dirs, files in self.fs.walk(search='depth'):
            ds: List[str] = root.strip('/').split('/')
            if len(ds) == 3 and ds[-1].lower() == Period.Tick.name.lower():
                ...
                for year, combines in yearly.items():
                    ...
                    if len(combines) > 1:
                        ...
                        df: pl.DataFrame = df.sort(by=self.pks[1])
                        path_tmp: str = f'{root}/{year}.parquet.tmp'
                        with self.fs.open(path=path_tmp, mode='wb') as f:
                            f: io.BytesIO
                            df.write_parquet(file=f, compression='zstd')
                        ...
                        for file in combines:
                            if os.path.basename(file) != os.path.basename(dst_path):
...
if __name__ == '__main__':
    p = Parquets(fs_url='smb://writer:440229fa@172.16.7.81:445/quote_jq?direct-tcp=true')
    p.merge_parquets()
