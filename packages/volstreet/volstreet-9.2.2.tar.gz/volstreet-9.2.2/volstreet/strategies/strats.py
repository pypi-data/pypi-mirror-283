...
def delta_hedged_strangle(
    ...
    file_name = (
        ...
        f"{underlying.name.lower()}_{strategy_tag.lower().strip().replace(' ', '_')}\\"
        f"{date}.json"
    )
    ...
    recording_task = timed_executor(60)(recording_task)
...
def trend_v2(
    ...
    at_market: bool = False,
    notification_url: Optional[str] = None,
    strategy_tag: Optional[str] = "Trend V2",
) -> list[dict]:
    # Entering the main function
    if time(*exit_time) < current_time().time():
        notifier(
...
def reentry_straddle(
    ...
    at_market: bool = False,
    notification_url: Optional[str] = None,
    strategy_tag: Optional[str] = "Reentry straddle",
) -> list[dict]:
    ...
    def get_state() -> str:
        return "neutral" if len(reentry_position.active_options) == 2 else "directional"

    def new_append_position_status(self, **additional_info) -> None:
        self._append_position_status(state=get_state(), **additional_info)

    reentry_position._append_position_status = reentry_position.append_position_status
