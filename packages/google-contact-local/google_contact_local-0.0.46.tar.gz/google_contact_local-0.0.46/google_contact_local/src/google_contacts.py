...
from importer_local.importer_sync_conflict_resolution import (
    ImporterSyncConflictResolution, UpdateStatus as ImporterUpdateStatus)
...
class GoogleContacts(GoogleAccountLocal, metaclass=MetaLogger,
        ...
        while next_google_contact_index <= self.google_contact_seq_end:
            try:
                ...
                for index, google_contact_connection in enumerate(google_connections):
                    ...
                    contact_row_dict = self.contacts_local.upsert_contact_dict_with_return_dict(contact_dict)
                    self._update_contact_dict_with_upsert_result(contact_dict=contact_dict, upsert_result_dict=contact_row_dict) or {}
    ...
    def _insert_contact_details_to_db(self, contact_dict: dict, user_external_id: int,
        ...
        try:
            ...
            # insert organization
            self.__sync_organization(contact_dict=contact_dict)
    ...
    def pull_contacts_with_stored_token(self, email_address: str, groups_dicts: list[dict] = None,
                                        start_index: int = 0, end_index: int = 1) -> None:
        user_external_username = email_address  # Because it is only google, username is the email
        ...
        token_data = self.user_externals_local.get_auth_details(  # TODO: fix typing
            username=user_external_username, system_id=GoogleContactConstants.GOOGLE_SYSTEM_ID, profile_id=self.profile_id)
        if not token_data:
        ...
        self.pull_people_api(email_address=email_address, username=user_external_username, groups_dicts=groups_dicts)
    ...
    def __sync_organization(self, contact_dict: dict):
        update_status = self.organizations_local.get_update_status(
            last_modified_timestamp=contact_dict.get("last_modified_timestamp"),
            main_profile_id=contact_dict.get("main_profile_id"))
        if update_status == UpdateStatus.UPDATE_CIRCLEZ:
            organizations_job_titles_dicts_list = contact_dict.get("organizations_job_titles_dicts_list")
            ...
            for organization_job_title_dict in organizations_job_titles_dicts_list:
                ...
                organzation_ml_ids_list = upsert_organization_result.get("organization_ml_ids_list")
                organization_job_title_dict["organization_id"] = organization_id
                organization_job_title_dict["organization_ml_ids_list"] = organzation_ml_ids_list
    ...
    def __insert_organization_profile(self, contact_dict: dict, ) -> list:
        if contact_dict.get("created_for_contact_profile_id"):
            profiles_ids_list = [contact_dict.get("created_for_contact_profile_id")]
        else:
            profiles_ids_list = contact_dict.get("profiles_ids_list") if contact_dict.get("profiles_ids_list") else []
            ...
            if len(profiles_ids_list) != 1:
                profiles_ids_list = []
        linked_group_results_list = contact_dict["linked_group_results_list"]
        if not linked_group_results_list or not profiles_ids_list:
            return []
        profile_id = profiles_ids_list[0]
        organization_profiles_ids_list = []
        for linked_group_result in linked_group_results_list:
            organization_id = linked_group_result.get("organization_id")
            if organization_id is None:
                continue
            job_title_id = linked_group_result.get("job_title_id")
            job_title_ml_id = linked_group_result.get("job_title_ml_id")
            organization_ml_id = linked_group_result.get("organization_ml_ids_list")[0] if linked_group_result.get(
                "organization_ml_ids_list") else None
            data_dict = {
        ...
        return organization_profiles_ids_list
    ...
    def __insert_link_contact_groups(self, contact_dict: dict) -> list:
        ...
        organizations_job_titles_dicts_list = contact_dict.get("organizations_job_titles_dicts_list") or []
        for organization_ids_info_dict in organizations_job_titles_dicts_list:
            ...
            organization_ml_ids_list = organization_ids_info_dict.get("organization_ml_ids_list")
    ...
    def __conflict_resolution(self, last_modified_timestamp: str, contact_id: int) -> ImporterUpdateStatus:
        """
        ...
        update_status: ImporterUpdateStatus = sync_conflict_resolution.get_update_status(
            last_modified_timestamp=last_modified_timestamp,
    ...
    def _update_contact_dict_with_upsert_result(self, *, contact_dict: dict, upsert_result_dict: dict) -> dict:
