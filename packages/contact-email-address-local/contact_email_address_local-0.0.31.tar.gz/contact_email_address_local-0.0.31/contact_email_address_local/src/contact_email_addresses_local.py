...
class ContactEmailAdressesLocal(GenericMapping, metaclass=MetaLogger,
    ...
    def insert_contact_and_link_to_email_address(
        ...
        if not contact_email_address_str:
            contact_phone_number = self.select_one_value_by_column_and_value(schema_name="contact",
                                                                             select_clause_value='phone1',
                                                                             view_table_name='contact_view',
                                                                             column_name='contact_id',
                                                                             column_value=contact_dict['contact_id'])
            email_address_from_phone_number = self.select_one_value_by_column_and_value(schema_name="contact",
                                                                                        select_clause_value='email1',
                                                                                        view_table_name='contact_view',
                                                                                        column_name='phone1',
                                                                                        column_value=contact_phone_number)
