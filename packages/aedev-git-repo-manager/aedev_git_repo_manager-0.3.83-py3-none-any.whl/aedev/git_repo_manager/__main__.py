...
def pypi_versions(pip_name: str) -> List[str]:
    ...
    try:
        ...
        versions = sorted(list(response.json()['releases'].keys()), key=Version)
        return versions
    except (KeyError, ValueError, Exception):  # requests.exceptions.JSONDecodeError
        return [""]         # invalid pip_name/page-not-found/never released to PyPi
...
def show_actions(ini_pdv: PdvType):
    ...
    if compact:
        cae.po(f"  --- registered and available actions (locally and on {host_domain}):")
        for act in actions:
