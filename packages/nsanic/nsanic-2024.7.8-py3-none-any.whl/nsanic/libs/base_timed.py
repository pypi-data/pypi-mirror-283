...
class BaseTimed:
    ...
    __month_map = {1: 31, 2: 28, 3: 31, 4: 30, 5: 31, 6: 30, 7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31}
    __units_map = {'second': 1, 'minute': 60, 'hour': 3600, 'day': 86400}

    def __init__(self, looping=None, run_tz='UTC'):
        self.run_tz = run_tz
        if not looping:
            looping = asyncio.get_event_loop()
        self.__run_loop = looping
        self.__logs = MultLogger(folder=self.__class__.__name__.lower())
    ...
    def log_err(self, *err):
        if not err:
            return
        self.__logs.info(*err) if self.__logs else print(*err)
        ...
        async def loop_fun():
            ...
            while 1:
                ...
                try:
                    ...
                    self.log_info('循环任务运行出错', err)

        if interval and interval <= 2:
            self.log_info('存在间隔小于2秒的任务, 将不加入循环任务执行')
        if not fun_param:
            fun_param = []
    ...
    def add_timed_task(self, name: str, timed: Union[list, tuple], fun, fun_param: Union[list, tuple] = None):
        async def timed_fun():
            cur_dt = tool_dt.cur_dt(tz=self.run_tz)
            _, nt, over_sta = self.__get_next_datetime(timed, cur_dt)
            ((await fun(*fun_param)) if asyncio.iscoroutinefunction(fun) else fun(*fun_param))
            while 1:
                try:
                    (await fun(*fun_param)) if asyncio.iscoroutinefunction(fun) else fun(*fun_param)
        ...
        if not fun_param:
            fun_param = []
        self.remove_task(name)
        tk = self.__run_loop.create_task(timed_fun(), name=name)
        self.TASK_MAP.update({name: tk})
    ...
    def cult_second_dt(self, dt, add_str: str, add_count: int = 1):
        ...
        if add_str == 'month':
            ...
            for i in range(add_count):
                month += 1
                if month <= 12:
                    days_arr.append(self.get_month_days(year, month))
                else:
                    year += 1
                    month = month - 12
                    days_arr.append(self.get_month_days(year, month))
        ...
        total_sec = sum([(365 if not self.check_leap(dt.year + i) else 366) for i in range(add_count)]) * 86400
        return tool_dt.to_datetime(int(dt.timestamp()) + total_sec, tz=self.run_tz)

    def get_month_days(self, year: int, month: int):
        is_leap = self.check_leap(year)
        is_leap and self.__month_map.update({2: 29})
        return self.__month_map.get(month)
