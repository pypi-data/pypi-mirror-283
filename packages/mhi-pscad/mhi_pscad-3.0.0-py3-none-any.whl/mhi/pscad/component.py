...
import logging
import csv
from typing import Any, Dict, List, Optional, Tuple, TYPE_CHECKING, NewType
...
class ParameterMixin(CommandMixin):            # pylint: disable=abstract-method
    """
    ...
    class DummyCodec:
        ...
        def _encode(self, key, val):           # pylint: disable=unused-argument
            if isinstance(val, bool):
                return "true" if val else "false"
            return str(val)

        def _decode(self, key, val):           # pylint: disable=unused-argument
            return val
    ...
    def _param_codec(self):
        if '_form_codec' not in self.__class__.__dict__:
            ...
            self.__class__._form_codec = codec # pylint: disable=protected-access
    ...
    @property
    def iid(self) -> int:
        """See ZComponent.iid - stub for export_parameters"""
        return 0
...
class MovableMixin(CommandMixin):              # pylint: disable=abstract-method
    """
    ...
    def bounds(self):
        ...
        return Rect(*self._context.get_prop(self, 'bounds'))
...
class OrientableMixin(MovableMixin):           # pylint: disable=abstract-method
    """
...
class ZComponent(Remotable, ParameterMixin):
    ...
    @rmi
    def _event(self,                        # pylint: disable=too-many-arguments
...
class Component(ZComponent, OrientableMixin):  # pylint: disable=abstract-method
    """
...
class Wire(Component):     # pylint: disable=abstract-method, too-many-ancestors
...
class StickyWire(Wire):    # pylint: disable=abstract-method, too-many-ancestors
    """
...
class Bus(Wire):           # pylint: disable=abstract-method, too-many-ancestors
...
class ACLine(Wire,         # pylint: disable=abstract-method, too-many-ancestors
             ModuleMixin):
    """
    ...
    def __repr__(self):
        scope, name = self._defn_name
        return f'{self.__class__.__name__}("{scope}:{name}", {self.iid})'
...
class UserCmp(Component,   # pylint: disable=abstract-method, too-many-ancestors
              ModuleMixin):
    """
    ...
    def parameters(self,                      # pylint: disable=arguments-differ
    ...
    @deprecated("Use component.parameters(...)")
    def set_parameters(self,                  # pylint: disable=arguments-differ
                       scenario: Optional[str] = None,
                       **kwargs) -> None:
    ...
    @deprecated("Use component.parameters()")
    def get_parameters(self,                  # pylint: disable=arguments-differ
                       scenario: Optional[str] = None
                       ) -> Optional[Parameters]:
        """
