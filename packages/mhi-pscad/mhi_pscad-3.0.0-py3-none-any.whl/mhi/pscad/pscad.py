...
class PSCAD(Application, Remotable):   # pylint: disable=too-many-public-methods
    """
    ...
    * :func:`mhi.pscad.application()`
    ...
    def _settings_codec(self):
        ...
        codec = PSCAD._setting_codec

        names = self.project_names()
        names[0] = '(Current)'
        bb_tgt_ns_codec = MapCodec({name: str(idx)
                                    for idx, name in enumerate(names)})
        codec._coding['bb_target_namespace'] = bb_tgt_ns_codec  # pylint: disable=protected-access
    ...
    def navigate_to(self,                   # pylint: disable=too-many-arguments
