"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
from clerk_dev.types import BaseModel, Nullable
from pydantic import model_serializer
from typing import Optional, TypedDict
from typing_extensions import NotRequired


class UpdateInstanceRestrictionsRequestBodyTypedDict(TypedDict):
    allowlist: NotRequired[Nullable[bool]]
    block_disposable_email_domains: NotRequired[Nullable[bool]]
    block_email_subaddresses: NotRequired[Nullable[bool]]
    blocklist: NotRequired[Nullable[bool]]
    ignore_dots_for_gmail_addresses: NotRequired[Nullable[bool]]
    

class UpdateInstanceRestrictionsRequestBody(BaseModel):
    allowlist: Optional[Nullable[bool]] = None
    block_disposable_email_domains: Optional[Nullable[bool]] = None
    block_email_subaddresses: Optional[Nullable[bool]] = None
    blocklist: Optional[Nullable[bool]] = None
    ignore_dots_for_gmail_addresses: Optional[Nullable[bool]] = None
    
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["allowlist", "block_disposable_email_domains", "block_email_subaddresses", "blocklist", "ignore_dots_for_gmail_addresses"]
        nullable_fields = ["allowlist", "block_disposable_email_domains", "block_email_subaddresses", "blocklist", "ignore_dots_for_gmail_addresses"]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in self.model_fields.items():
            k = f.alias or n
            val = serialized.get(k)

            if val is not None:
                m[k] = val
            elif not k in optional_fields or (
                    k in optional_fields
                    and k in nullable_fields
                    and (self.__pydantic_fields_set__.intersection({n}) or k in null_default_fields) # pylint: disable=no-member
                ):
                m[k] = val

        return m
        
