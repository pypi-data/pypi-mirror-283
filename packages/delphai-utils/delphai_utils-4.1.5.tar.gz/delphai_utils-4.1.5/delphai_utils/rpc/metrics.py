...
def set_application_info(**kwargs):
...
def message_published(*, exchange, routing_key, type, priority, payload_size):
    labels = dict(
        exchange=exchange,
...
def server_request_processed(priority, method, error, queued_for, elapsed):
...
def client_request_processed(priority, service, method, elapsed):
    labels = dict(
