...
class MecordTaskExecutorThread(Thread):
    ...
    def appendTask(self, data, timeout, country, callback, gettask, ext=""):
        ...
        try:
            ...
            taskUtils.taskPrint(taskUUID, f"{name} === addQueue {country} task : {taskUUID}")
        except:
            pass
...
class MecordLongConnectThread(Thread):
    ...
    def receive_GetTask(self, body):
        ...
        if len(rsp.list) > 0:
            taskUtils.taskPrint(None, f"{current_thread().name}=== get data packet, has {len(rsp.list)} task")
        for it in rsp.list:
    ...
    def GetTask(self):
        if not os.path.exists(stop_thread_file):
            ...
            if self.mtype:
                print(f"{current_thread().name} waiting next {req.limit} task, widgets: {req.widgets}")
            else:
