...
def device_status(stdscr, idx, flag):
    ...
    if flag:
        ...
        try:
            real_stdsrc(idx, 0, scr_str(f"Device Usage                  CPU : {mdict['main']['cpu']}%                  Memory : {mdict['main']['mem']}%                  GPU : {mdict['main']['gpu']}%".ljust(ll*3-2)))
        except:
            ...
            async def get_usage(pid):
                try:
                    if pid == 'main':
                        ...
                        io_counters = process.io_counters()
                        disk_read = f"{(io_counters.read_bytes - read) / (1024 ** 2):.2f}MB/s"
                        disk_write = f"{(io_counters.write_bytes - write) / (1024 ** 2):.2f}MB/s"
                        mdict[pid].update({
                            "cpu": cpu_usage,
                            "read": disk_read,
                            "write": disk_write,
            ...
            while idx < len(sys.argv):
                if sys.argv[idx] == "-thread":
                    ...
                    if threadNum < 1 or threadNum > 500:
                        ...
                        return
                if sys.argv[idx] in ["product", "us", "sg", "test", "plus"]:
                    env += sys.argv[idx]
...
def main():
    ...
    cmd = 'service start plus -h 3 -l 10'
    # cmd = 'status'
    # cmd = 'widget enable all'
    # cmd = 'widget mark 224706f5-436c-5602-8257-85855c7127d5'
    cmd = 'service start'
    # cmd = r'widget add C:\Users\amiao\Desktop\mtest\mtest04'
    for _ in cmd.split(' '):
        sys.argv.append(_)
    if len(sys.argv) >= 2:
        module = sys.argv[1]
