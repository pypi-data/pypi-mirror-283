...
class AIFunctionHelper:
    ...
    async def _async_wrapper(self, func, decorator_kwargs, *args, **func_kwargs):
        ...
        if return_hint is None or return_hint == str:
            wrapper_model = create_model(
                f"{func.__name__.capitalize()}Result",
                result=(str, ...)
            )
        elif not (isinstance(return_hint, type) and issubclass(return_hint, BaseModel)):
            wrapper_model = create_model(
                f"{func.__name__.capitalize()}Result",
                result=(return_hint, ...)
            )
        ...
        # Convert the result to the appropriate Pydantic model if necessary
        if return_hint and issubclass(return_hint, BaseModel):
            if isinstance(actual_result, dict):
                actual_result = return_hint(**actual_result)
            elif isinstance(actual_result, list) and issubclass(get_origin(return_hint), list):
                item_type = get_args(return_hint)[0]
                actual_result = return_hint([item_type(**item) if isinstance(item, dict) else item for item in actual_result])

        # Call the original function with the extracted result
        if has_ai_result_param:
            if asyncio.iscoroutinefunction(func):
                result = await func(ai_result=actual_result, **arg_values)
            else:
                result = func(ai_result=actual_result, **arg_values)
        else:
            if asyncio.iscoroutinefunction(func):
        ...
        # If the return hint is None or str, ensure we return a string
        if return_hint is None or return_hint == str:
