def test_binning():
    ...
    import numpy as np
    input_flim_data = np.arange(100).reshape(2, 1, 2, 5, 5)  # (utime, time, z, y, x)
    ...
    image_binned_2D = apply_binning(input_flim_data, bin_size=2, binning_3D=False)
    image_binned_3D = apply_binning(input_flim_data, bin_size=2, binning_3D=True)
