...
class PhasorPlotterWidget(PlotterWidget):
    def __init__(self, napari_viewer):
        ...
        self.tau_lines_line_edit_widget.setPlaceholderText('Comma-separated list of tau values in ns')
        self.tau_lines_container.layout().addWidget(self.tau_lines_line_edit_widget)
    ...
    def add_tau_lines(self, ax, tau_list, frequency, harmonic=1):
        ...
        for tau in tau_list:
            ...
            ax.annotate(text=f'{tau * 1E9:.2f} ns', xy=(g, s), fontsize=8, color=dot.get_color(), annotation_clip=True)

    def add_tau_lines_from_widget(self):
        tau_lines_text = self.tau_lines_line_edit_widget.text()
        tau_list = [float(tau) for tau in tau_lines_text.split(',') if tau]# and tau != 'Comma-separated list of tau values in ns']
        if tau_list:
            if self.frequency:
                self.add_tau_lines(self.graphics_widget.axes, tau_list, frequency=self.frequency, harmonic=self.harmonic)
                self.graphics_widget.draw_idle()
            else:
                warnings.warn('Frequency is not set. Please calculate phasor plot first.')
    ...
    def on_show_hide_tau_lines(self):
        if self.tau_lines_button.isChecked():
            ...
            # redraw phasor plot
            self.run(features=self.layer_select.value.features,
