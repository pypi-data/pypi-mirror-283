...
def get_n_largest_cluster_labels(features_table: 'pandas.DataFrame', n: int=1, clustering_id: str='MANUAL_CLUSTER_ID') -> List[int]:
    ...
    sorted_cluster_ids = unique_cluster_ids[np.argsort(counts)[::-1]] + 1 # noise clusters become 0 and unselected become 1 (matching labels layer)
    sorted_cluster_ids = sorted_cluster_ids[sorted_cluster_ids > 0] # remove noise cluster (0)
...
def split_n_largest_cluster_labels(labels_layer: "napari.layers.Labels", clusters_labels_layer: "napari.layers.Labels", clustering_id: str, n: int=1) -> List["napari.layers.Labels"]:
    ...
    cluster_labels = get_n_largest_cluster_labels(features_table, n, clustering_id)
    cluster_individual_labels_layer_list = []
    for label in cluster_labels:
        cluster_individual_labels_layer_list.append(manual_label_extract(clusters_labels_layer, label))
...
class Split_N_Largest_Cluster_Labels(Container):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self._viewer.layers.events.inserted.connect(self._labels_layer_combobox.reset_choices)
        self._viewer.layers.events.removed.connect(self._labels_layer_combobox.reset_choices)
        ...
        self._viewer.layers.events.inserted.connect(self._clusters_labels_layer_combobox.reset_choices)
        self._viewer.layers.events.removed.connect(self._clusters_labels_layer_combobox.reset_choices)
        ...
        self._labels_layer_combobox.changed.connect(self._clustering_id_combobox.reset_choices)
    ...
    def _get_layers(self, widget):
        ...
        List[napari.layers.Layer]
            list of layers
        """        
        return [layer for layer in self._viewer.layers if isinstance(layer, self.input_layer_types)]
    ...
    def _get_valid_clusters_column_names(self, widget):
        ...
        return [column_name for column_name in features_table.columns if '_CLUSTER_ID' in column_name]
