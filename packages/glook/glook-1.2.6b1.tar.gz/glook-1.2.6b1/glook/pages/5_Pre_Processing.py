...
if sup_unsup == 'Unsupervised Learning':
	...
	preprocessing_action = st.sidebar.radio('Select preprocessing action', [
		'Select Pre-Processing Step⬇️', 'Drop Column (For Col)', 'Treat Missing (For Col)',
		'Treat Missing (Full DF)', 'Change Data Type (For Col)', 'Drop Duplicates (Full DF)',
		'Treat Outliers (For Col)', 'Treat Outliers (Full DF)', 'Apply Transformation (For Col)',
		'Column Unique Value Replacement (For Col)', 'Discretize Variable (For Col)',
		'Dummy Variables (Full DF)', 'Dummy Variable (For Col)', 'Apply Scaling (Full DF)'], help="(For Col) means -> operation on particular Column, (Full DF) means -> operation on full DataFrame")
	...
	try:
		if preprocessing_action == 'Select Pre-Processing Step⬇️':
			...
			st.dataframe(df)
			print(155)
		elif preprocessing_action == 'Change Data Type (For Col)':
		...
		elif preprocessing_action == 'Drop Column (For Col)':
			if st.button('Apply', on_click=callback
				) or st.session_state.button_clicked:
				modified_df = df.copy()
				...
				if confirm_change:
					...
					st.rerun()
					print(277)
		elif preprocessing_action == 'Drop Duplicates (Full DF)':
			if st.button('Apply', on_click=callback
				) or st.session_state.button_clicked:
				modified_df = df.copy()
				...
				if confirm_change:
					...
					st.rerun()
					print(282)
		elif preprocessing_action == 'Treat Outliers (For Col)':
		...
		elif preprocessing_action == 'Treat Outliers (Full DF)':
			...
			if st.button('Apply', on_click=callback
				...
				if confirm_change:
					...
					st.rerun()
					print(347)
		elif preprocessing_action == 'Treat Missing (For Col)':
		...
		elif preprocessing_action == 'Treat Missing (Full DF)':
			print(409)
		...
		elif preprocessing_action == 'Apply Transformation (For Col)':
		...
		elif preprocessing_action == 'Dummy Variables (Full DF)':
		...
		elif preprocessing_action == 'Dummy Variable (For Col)':
			encoding_method = st.selectbox('Select encoding method', [
				'One-Hot Encoding', 'Label Encoding'])
			opt = st.toggle('drop_first')
			if st.button('Apply', on_click=callback
		...
		elif preprocessing_action == 'Apply Scaling (Full DF)':
			...
			if st.button('Apply', on_click=callback
				...
				if confirm_change:
					...
					st.rerun()
					print(598)
		elif preprocessing_action == 'Discretize Variable (For Col)':
		...
		elif preprocessing_action == 'Column Unique Value Replacement (For Col)':
			print(43)
			print(659)
	...
	try:
		# st.write('Session State:->', st.session_state['shared'])
		preprocessing_action = st.sidebar.radio('Select preprocessing action', [
		'Select Output Variable ↗️ & Select Pre-Processing Step⬇️', 'Drop Column (For Col)', 'Treat Missing (For Col)',
		'Treat Missing (Full DF)', 'Change Data Type (For Col)',
		'Treat Outliers (For Col)', 'Treat Outliers (Full DF)', 'Drop Duplicates (Full DF)', 'Apply Transformation (For Col)',
		'Column Unique Value Replacement (For Col)', 'Discretize Variable (For Col)',
		'Dummy Variables (Full DF)', 'Dummy Variable (For Col)', 'Apply Scaling (Full DF)'], help="(For Col) means -> operation on particular Column, (Full DF) means -> operation on full DataFrame")
		st.sidebar.write(":orange[Confirmed Changes] ⤵️")
		if len(st.session_state.full_flow) == 0:
		...
		if 'df_pre' in st.session_state:
			...
			if preprocessing_action == 'Select Output Variable ↗️ & Select Pre-Processing Step⬇️':
				...
				if y_var is not None:
					...
					if y_nul >= 1:
						st.sidebar.warning("The Y variable contains missing values. Make sure to treat it with the `Treat Missing (For Col)`", icon="🚨")
			
			
			if preprocessing_action == 'Drop Column (For Col)':
