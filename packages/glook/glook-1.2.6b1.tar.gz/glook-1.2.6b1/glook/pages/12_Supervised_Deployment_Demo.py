	...
	if type_ == 'Upload File':
		...
		if choice == "Upload '.csv' or '.xlsx'":
			for act in Flow:
				if act == 'Drop Column (For Col)':
				...
				elif act == 'Change Data Type (For Col)':
					...
					try:
						...
						print("e15", e)
				elif act == 'Treat Missing (For Col)':
				...
				elif act == 'Treat Missing (Full DF)':
					try:
				...
				elif act == 'Drop Duplicates (Full DF)':
				...
				elif act == 'Treat Outliers (For Col)':
					try:
				...
				elif act == 'Treat Outliers (Full DF)':
				...
				elif act == 'Apply Transformation (For Col)':
				...
				# else:
				#     st.write("No file named 'apply_transformation_on_col.pkl' found in the current directory.")
				elif act == 'Column Unique Value Replacement (For Col)':
					try:
				...
				elif act == 'Discretize Variable (For Col)':
				...
				elif act == 'Dummy Variable (For Col)':
				...
				elif act == 'Dummy Variables (Full DF)':
				...
				elif act == 'Apply Scaling (Full DF)':
		...
		for column, details in column_details.items():
			...
			if details["type"] == "numeric":
				user_inputs[column] = st.number_input(
					...
					min_value=details["min"],
					max_value=details["max"],
					value=details["default"], key=unique_key  # Use the random row's value as the default
				)
				...
				user_inputs[column] = st.selectbox(
					f"Select a value for {column}",
					options=details["options"],
					index=list(details["options"]).index(details["default"]), key=unique_key  # Set the default selection
				)
			
			elif details["type"] == "boolean":
				user_inputs[column] = st.checkbox(
					f"Toggle {column}",
					value=details["default"], key=unique_key  # Use the random row's boolean value
				)
	...
	if st.button("Predict", type="primary", use_container_width=True):
		...
		try:
			if type_ == 'Upload File':
				...
				st.success("Success ✅")
				st.warning("This feature is Not Yet Released")
