...
def load_configuration(
        ...
        command_line_config: Optional[SleepGuardConfiguration]
...
def install_default_configuration() -> pathlib.Path:
    ...
    filename = "default_configuration.yaml"
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        default_config_file = pathlib.Path(sys._MEIPASS)
        default_config_file = default_config_file / "sleepguard" / "resources" / filename

        shutil.copyfile(default_config_file, user_config_file)
    else:
        with as_file(files(resources).joinpath(filename)) as default_config_file:
            shutil.copyfile(default_config_file, user_config_file)

    return user_config_file


def main(config: SleepGuardConfiguration):
    """
    ...
    log.debug(f"Using configuration: {config}")
...
def signal_handler(sig, frame):
    ...
    """
    log = logging.getLogger(APP_NAME)
    log.info(f"Terminating Sleep Guard")
...
if __name__ == "__main__":
    ...
    configuration = load_configuration(program_options.config_path, program_options.configuration)
