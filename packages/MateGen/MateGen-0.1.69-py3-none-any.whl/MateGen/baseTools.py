...
def write_google_env(google_search_key, cse_id):
    ...
    if load_dotenv(dotenv_path):
        ...
        set_key(env_file, 'GOOGLE_SEARCH_KEY', google_search_key)
        set_key(env_file, 'CSE_ID', cse_id)
...
def write_zhihu_env(cookie, user_agent):
    ...
    if load_dotenv(dotenv_path):
        set_key(env_file, 'ZHIHU_SEARCH_COOKIE', cookie)
        set_key(env_file, 'ZHIHU_SEARCH_USER_AGENT', user_agent)      
...
def write_github_token_to_env(github_token):
    ...
    # 如果 .env 文件不存在，创建一个
    if load_dotenv(dotenv_path):
        set_key(env_file, 'GITHUB_TOKEN', github_token)
...
def write_oss_env(access_key_id, access_key_secret, endpoint, bucket_name):
    ...
    # 如果.env文件不存在，创建一个
    if load_dotenv(dotenv_path):
        set_key(env_file, 'OSS_ACCESS_KEY_ID', access_key_id)
        set_key(env_file, 'OSS_ACCESS_KEY_SECRET', access_key_secret)
        set_key(env_file, 'OSS_ENDPOINT', endpoint)
        set_key(env_file, 'OSS_BUCKET_NAME', bucket_name)
...
def write_kaggle_env(headers, cookies):
    ...
    # 如果.env文件不存在，创建一个
    if load_dotenv(dotenv_path):
        set_key(env_file, 'HEADERS', headers)
        set_key(env_file, 'COOKIES', cookies)
