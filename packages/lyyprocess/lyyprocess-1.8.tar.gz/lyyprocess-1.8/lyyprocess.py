...
def is_self_already_running(if_exit=True, debug=False, kill_existing=False):
    """
    判断程序自身是否已经运行。
    如果参数if_exit为真，则直接退出。
    ...
    if debug:
        print(f"当前进程名称: {current_process.name()}, PID: {current_process.pid}")

    for proc in psutil.process_iter(["pid", "name"]):
        if debug:
            print(f"进程名称: {proc.info['name']}, PID: {proc.info['pid']}")

        # 检查进程名称是否与当前进程相同，但排除python解释器自身
        if proc.info["name"] != "python.exe" and proc.info["name"] == current_process.name() and proc.pid != current_process.pid:
            if kill_existing:
                try:
                    ...
                    proc.wait()       # 等待进程终止
                    if debug:
                        print(f"已结束同名进程: {proc.info['name']}, PID: {proc.pid}")
                except psutil.NoSuchProcess:
                    if debug:
                        print(f"进程 {proc.pid} 已经被终止或不存在。")
                except psutil.AccessDenied:
                    if debug:
                        print(f"没有权限结束进程: {proc.info['name']}, PID: {proc.pid}")
                except Exception as e:
                    if debug:
                        print(f"结束进程时发生错误: {e}")
            else:
                if if_exit:
                    sys.exit("检测到同名进程正在运行。")
                return True
    return False
