...
class Hierarchy(models.Model):
    ...
    def get_hierarchy(name):
        return Hierarchy.objects.get_or_create(
            name=name, defaults=dict(base_url="/"))[0]
    ...
    def get_root(self):
        ...
        try:
            ...
            return Section.objects.create(
                label="Root", slug="", hierarchy=self, is_root=True)
...
class Section(models.Model):
    ...
    def is_first_child(self):
        return SectionChildren.objects.filter(
            parent=self.get_parent(),
            ordinality__lt=self.get_ordinality()
        ).count() == 0

    def is_last_child(self):
        return SectionChildren.objects.filter(
            parent=self.get_parent(),
            ordinality__gt=self.get_ordinality()
        ).count() == 0
    ...
    def get_ordinality(self):
        if self.is_root:
            return -1
        return SectionChildren.objects.get(
            parent=self.get_parent(), child=self).ordinality

    def get_children(self):
        return [
            sc.child for sc in SectionChildren.objects.filter(
                parent=self).order_by("ordinality")
        ]
    ...
    def get_descendents(self):
        """ returns flattened depth-first traversal of this section
        and its children """
...
class PageBlock(models.Model):
    ...
    def __str__(self):
        return "%s [%d]: %s" % (
            self.section.label, self.ordinality, self.label)
    ...
    def render(self, **kwargs):
        if hasattr(self.content_object, "template_file"):
            t = get_template(
                getattr(self.content_object, "template_file"))
            d = kwargs
