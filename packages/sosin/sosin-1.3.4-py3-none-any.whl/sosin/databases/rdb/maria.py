...
class MariaDB(Database):
    ...
    def __init__(self, db_config:dict, cursor_type:str = None) -> None:
    ...
    def insert(self, table:str, columns: str, value: tuple) -> Union[int, bool]:
        ...
        result = super().insert(sql_qr, value)
        if result:
    ...
    def insert_many(self, table:str, columns: str, values: list, ignore:bool=False) -> bool:
        ...
        return super().insert_many(sql, values)
    ...
    def truncate(self, table:str, forcing=True) -> bool:
        ...
        """
        try:
            self.execute(f'SET FOREIGN_KEY_CHECKS = {int(forcing)}; TRUNCATE TABLE {table};')
            self.commit()
            return True
        except:
            return False
