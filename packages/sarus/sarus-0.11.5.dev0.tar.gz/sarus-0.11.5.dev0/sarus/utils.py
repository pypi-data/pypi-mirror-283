...
from sarus_data_spec.transform import custom_code as transform_custom_code
from sarus_data_spec.dataspec_validator.typing import DataspecPrivacyPolicy
from sarus_data_spec.manager.ops.processor.standard.custom_code import (
    execute_custom_code,
)
...
def custom_code(
    image_name: str, wrapper: srt.DataSpecWrapper
) -> srt.DataSpecWrapper:
    ...
    try:
        manifest = client.images.get_registry_data(image_name)
        digest = manifest.id
    except:  # noqa: E722
        digest = "unknown"
    print(f"Digest:{digest}")
    result_datasepc = transform_custom_code(image_name, digest)(dataspec)
    ds_wrapper = DataFrame.from_dataspec(result_datasepc)
    return ds_wrapper


def run_local_custom_code(
    path: str,
    wrapper: srt.DataSpecWrapper,
    image_name="local_custom_code",
) -> pd.DataFrame:
    docker_client = docker.from_env()
    print("The image is being built...")
    _ = docker_client.images.build(path=path, tag=image_name, rm=True)
    synth_dataspec, _ = wrapper.alternative_dataspec(target_epsilon=0)
    print(DataspecPrivacyPolicy.SYNTHETIC.value)

    if synth_dataspec.prototype() == sp.Dataset:
        iterator = synth_dataspec.to(t.Iterator[pa.RecordBatch])
    else:
        raise ValueError("Can't run local code on scalar value.")
    input_table = pa.Table.from_batches([batch for batch in iterator])
    output_table = execute_custom_code(input_table, image_name)
