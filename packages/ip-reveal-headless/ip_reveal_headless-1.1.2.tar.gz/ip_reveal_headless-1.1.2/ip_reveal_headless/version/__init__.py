...
PACKAGE_URL = 'https://pypi.org/pypi/ip-reveal-headless'
...
class PyPiVersionInfo:
    ...
    Since:
        v3.0
    """
    __url = 'https://pypi.org/pypi/ip-reveal-headless/json'
    ...
    def check_for_update(self, include_pre_releases=False):
        ...
        # Handle instances where pre-releases are included but there are no pre-releases available.
        if (include_pre_releases or self.include_pre_release_for_update_check) and self.latest_pre_release is not None:
    ...
    def update(self):
        """
        Checks for updates to ip-reveal-headless.
        ...
        try:
            if self.update_available:
                ...
                print(f'\n\n[bold green]No update available.[/bold green]'
                      f'\nCurrent version: [bold cyan]{parse_version()}[/bold cyan]'
                      f'\nLatest PyPi Version: [bold cyan]{self.latest}[/bold cyan]'
                      f'\n{local_newer_statement}'
                      )
            ...
            print(f'An error occurred during the update check: {str(e)}: {e.with_traceback()}')
    ...
    def print_version_info(self, check_for_update=False):
        """
        ...
        if check_for_update:
            ...
            if self.update_available:
                ...
                table.add_row('Update Available', '[bold red]No[/bold red]')
                table.add_row('Latest Version', f'{self.latest_stable}')

                if self.installed_newer_than_latest:
                    table.add_row('Local Version Newer', '[bold green]Yes[/bold green] (Development Build)')
