...
def critical_pairs_PI_QI(A: IntegralAlgebra, 
    ...
    assert LC_Q == 1, "LC(Q) should be normalized"
    P_I = P.get_P_I()
    Q_I = Q.get_P_I()
    ...
    B_Q = A.LM_LC(A.polynomials_product(LM_Q_I_i2plus, LM_Q_N_i1plus_pow))[0]
    #then C_Q
    if B_Q == IM(1):
        C_Q = 0
    else:
        C_Q = B_Q.cut("1+") # B_Q]1+]
    # convert B_Q and C_Q to IntegralPolynomial to do the products below
    B_Q = IntegralPolynomial(B_Q)
    C_Q = IntegralPolynomial(C_Q) 
    U_left_term = A.polynomials_product(P_circled_alpha, B_Q)
    U_right_term = A.product_P_Coeff(
                        A.integrate_polynomial(
                            A.polynomials_product(C_Q, P_circled_alpha)
                        ), 
                    -1)
    U = A.polynomials_add(U_left_term, U_right_term)
    # we add the coeff in front of U : 
    l_P = LC_P_N
    U = A.product_P_Coeff(U, 1/(alpha_value* l_P**(alpha_value -1)))
    ...
    B_P = A.LM_LC(A.polynomials_product(LM_P_I_i2plus, LM_P_N_i1plus_pow))[0]
    #then C_Q
    if B_P == IM(1):
        C_P = 0
    else:
        C_P = B_P.cut("1+") # B_P]1+]
    # convert B_P and C_P to IntegralPolynomial to do the products below
    B_P = IntegralPolynomial(B_P)
    C_P = IntegralPolynomial(C_P)
    

    V_left_term = A.polynomials_product(Q_circled_beta, B_P)
    V_right_term = A.product_P_Coeff(
                        A.integrate_polynomial(
                            A.polynomials_product(C_P, Q_circled_beta)
                        ), 
                    -1)
    V = A.polynomials_add(V_left_term, V_right_term)
    # we add the coeff in front of V : 
    l_Q = LC_Q_N
    V = A.product_P_Coeff(V, -1/(beta_value* l_Q**(beta_value -1))) 
    ...
    if not S.is_zero():
        assert A.IMO_le(A.LM_LC(S)[0], A.LM_LC(U)[0])
    return S
...
def critical_pairs_PI_QN(A: IntegralAlgebra, 
    ...
    P_I = P.get_P_I()
    Q_N = Q.get_P_N()
    if P_I.is_zero() or Q_N.is_zero():
        return None
    LM_P_I,LC_P_I = A.LM_LC(P_I)
    LM_Q_N,LC_Q_N = A.LM_LC(Q_N)
    if (LM_P_I != LM_P) or (LM_Q_N != LM_Q):
        return None
    ...
    m_B_P = A.monomials_product(m,B_P)
    V = A.integrate_polynomial(A.polynomials_product(Q,m_B_P))
    # we add the coeff in front of V
    l_Q = LC_Q_N
    V = A.product_P_Coeff(V, -1/(l_Q)) 
...
def critical_pairs(A: IntegralAlgebra, 
    ...
    for P,Q in all_distinct_pairs_in_R:
        P_norm = A.normalize_LC_of_P(P)[0]
        Q_norm = A.normalize_LC_of_P(Q)[0]
