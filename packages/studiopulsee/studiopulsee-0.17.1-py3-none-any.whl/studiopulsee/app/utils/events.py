...
from studiopulsee.app.stores import publisher_store
from studiopulsee.app.models.event import ApiEvent
from studiopulsee.app.utils import fields
...
def emit(event, data={}, persist=True, project_id=None):
    ...
    from studiopulsee.app.config import ENABLE_JOB_QUEUE

    for func in event_handlers.values():
        if ENABLE_JOB_QUEUE:
            from studiopulsee.app.stores.queue_store import job_queue
...
def save_event(event, data, project_id=None):
    ...
    """
    try:
        from studiopulsee.app.services.persons_service import get_current_user_raw
