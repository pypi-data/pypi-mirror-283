...
COPY /build2docker-entrypoint /usr/local/bin/build2docker-entrypoint
ENTRYPOINT ["/usr/local/bin/build2docker-entrypoint"]
...
class BuildPack:
    ...
    Specifically used for creating Dockerfiles for use with build2docker only.
    ...
    def __init__(self, base_image):
        ...
        if sys.platform.startswith("win"):
            self.log.warning(
                ...
                "support is experimental in build2docker."
            )
        self.platform = ""
    ...
    def build(
        ...
        for fname in ("build2docker-entrypoint", "python3-login"):
