...
# Flask endpoint to handle document queries
...
def query_document():
    ...
    if not query_text or not api_key or not folder_path:
        return jsonify({"error": "Query text, API key, and folder path are required"}), 400
    ...
    if not os.path.exists(folder_path):
        ...
        return jsonify({"error": "The folder path does not exist"}), 400
