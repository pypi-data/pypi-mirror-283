...
def compute_greek_coefficients(p11: np.array, p12: np.array, p22: np.array, p33: np.array, p34: np.array, p44: np.array, angle_grid: np.array, num_coeff: int):
    ...
    for i in range(0, num_coeff):
        ...
        lm_a1[i] = simpson(p11 * lpoly_00, cos_theta) / simpson(lpoly_00 * lpoly_00, x=cos_theta)
        lm_a4[i] = simpson(p44 * lpoly_00, cos_theta) / simpson(lpoly_00 * lpoly_00, x=cos_theta)

        if i >= 2:
            lm_b1[i] = simpson(p12 * lpoly_02, cos_theta) / simpson(lpoly_02 * lpoly_02, x=cos_theta)
            lm_b2[i] = -simpson(p34 * lpoly_02, cos_theta) / simpson(lpoly_02 * lpoly_02, x=cos_theta)

            temp1 = simpson((p22 + p33) * lpoly_22, cos_theta) / simpson(lpoly_22 * lpoly_22, x=cos_theta)
            temp2 = simpson((p22 - p33) * lpoly_2m2, cos_theta) / simpson(lpoly_2m2 * lpoly_2m2, x=cos_theta)
