...
def _get_changed(folder: str, changed_files: List[Path]) -> set:
    return {_parse_folder_parts(folder, f) for f in changed_files if str.startswith(f.as_posix(), folder)}

def _get_changed_bricks(top_dir: str, changed_files: List[Path], namespace: str) -> list:
