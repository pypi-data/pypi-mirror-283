...
# %% Funktion Solver
    ...
    def results_to_excel(self,path):
        ...
        self.optimize_excel_col_width(path)
        
    def optimize_excel_col_width(self,path):
        '''
    ...
    def show_lp_system(self,window_size="1225x850",width=500):
        ...
        for obj in self.obj_lst:
            ...
            for eqs in eq_lst:
                ...
                timesteps_str = f'für t = [{self.summarize_intervals(timesteps_lst)}]'
                label = ttk.Label(frame, text=timesteps_str, anchor="center")
                label.pack(pady=10, fill=tk.BOTH)
                fig = self.create_figure_for_display(eq)
    ...
    def create_figure_for_display(self,eq):
        fig, ax = plt.subplots(figsize=(1.5,  0.5))  # Anpassen der Höhe an die Anzahl der Gleichungen #!anpassen
        ax.axis('off')

        y_position = 0.4
        fig.text(0.5, y_position, f"${sp.latex(self.convert_to_sympy_equation(eq))}$", fontsize=12, ha='center', va='center')    

        return fig

    def convert_to_sympy_equation(self,equation):
