...
class GenericQueryManager(QueryManager):
    ...
    async def delete(self, item: GenericModel):
        await self.stmt.delete(item)
    ...
    async def count(
        ...
        try:
            ...
            count, _ = self.stmt.all()
            return count
        finally:
            self._init_query()

    async def execute(self, many=True) -> GenericModel | list[GenericModel] | None:
        try:
            _, items = self.stmt.all()
            if many:
                return items
