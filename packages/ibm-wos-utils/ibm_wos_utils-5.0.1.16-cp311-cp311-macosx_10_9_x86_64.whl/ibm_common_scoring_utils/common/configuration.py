...
class Configuration():
    ...
    def __validate_configuration_for_fm(self,configuration:dict,missing_values:list):
        ...
        #TODO Following section is needed for FMaaS. Validations will be removed once deployment based scoring is in place for FM'
        if configuration.get("prompt_template_asset_details") is None:
                missing_values.append("prompt_template_asset_details")
                
        if configuration.get("platform_url") is None:
...
class LLMConfiguration():
    """
        Class to set the configuration of LLM models for scoring needs 
    """
    def __init__(self,configuration) -> None:
        self.validate_cofiguration(configuration)
        
        #Set the properties as needed 
        self.model_type = configuration.get("task_type")
        self.credentials = configuration.get("credentials")
        self.service_type = configuration.get("service_type")
        self.scoring_url = configuration.get("scoring_url")
    
    @staticmethod
    def validate_cofiguration(configuration):
        pass
