...
class WMLProvider(ServiceProvider):
    ...
    def get_headers(self,token=None):
        ...
        if token is None:
            if self.credentials.wml_location in [AuthProvider.CLOUD.value, AuthProvider.CLOUD_REMOTE.value]:
                #Cloud
                token = get_iam_token(self.credentials.apikey,auth_url=self.credentials.auth_url)
            else:
                #CPD case
    ...
    def score(self,df,**kwargs)-> pd.DataFrame:
        ...
        if self.config.is_foundation_model:
            execution_type = kwargs.get("execution_type") or ExecutionType.ASYNC.value
    ...
    def score_fm_with_pta(self,config:Configuration,headers, df:pd.DataFrame,execution_type):
        ...
        model_parameters = get(prompt_template_asset, "prompt.model_parameters")
        ...
        #Set scoring url to FMaaS url
        self.config.scoring_url = f"{self.credentials.url}/ml/v1-beta/generation/text?version=2023-05-29"
                
        #Get prompt string with parameters:
        try:
            input_api_response = get_prompt_string(self.config.platform_url,asset_id,self.get_headers(token=self.credentials.wml_token),project_id=self.config.project_id,space_id=self.config.space_id)
            input_string = input_api_response.get("input")
        except Exception as ex:
            self.logger.log_error(f"Error getting prompt string from the API . Reason:{str(ex)}.Trying inbuilt utility function ...")
        ...
        for row in rows :
            ...
            if execution_type == ExecutionType.SYNC.value: #Sync execution is kept as of now for testing purpose , Can you be removed later.
                score_response, response_time = score_fmaas(self.config.scoring_url,payload,headers)
                ...
                scoring_tasks.append(ScoringTask(headers,payload,self.config.scoring_url, config))
                
        #Execute in parallel for async mode
        if execution_type == ExecutionType.ASYNC.value:
            try:
                response_rows = AsyncIOExecutor.execute_http_tasks(self.config.batch_size, *scoring_tasks)
            except asyncio.TimeoutError:
                raise Exception("Timeout during scoring")
            
            
        #Construct response
        response_df = pd.DataFrame(response_rows,columns=["prompt_response",config.prediction,"response_time"])
        return response_df
