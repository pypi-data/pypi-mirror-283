...
class sm_contribution(models.Model):
    ...
    current_remuneration_mode = fields.Selection(
        [     # models.env['sm_contributions.sm_contribution_interest']._fields['remuneration_mode'].selection
            ('euros', _('Euros')),
            ('moneder', _('Moneder de Mobilitat')),
        ],
        string="Modalitat remuneració actual",
        compute="compute_current_remuneration_mode",
        store=True,
    )
    ...
    @api.depends('contributions_line.interest_model', 'contributions_line.interest_model.remuneration_mode')
    def compute_current_remuneration_mode(self):
        for record in self:
            if not record.contributions_line:
                record.current_remuneration_mode = ''
            else:
                current_contribution_line = record.contributions_line.filtered(lambda x: int(x.year) <= datetime.now().year).sorted(key='year')
                current_contribution_line = current_contribution_line[-1] if current_contribution_line else record.contributions_line.sorted(key='year')[-1]
                
                record.current_remuneration_mode = current_contribution_line.interest_model.remuneration_mode
