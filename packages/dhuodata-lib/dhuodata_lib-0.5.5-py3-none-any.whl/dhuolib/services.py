...
class ServiceML(ABC):
    ...
    def __init__(self, service_uri: str = 'http://localhost:8000'):
...
class ExperimentMLAPI(ServiceML):
    ...
    def download_pickle(
        ...
        url = f"{self.service_uri}/dowload/batch/{experiment_name}/{model_name}?model_stage={model_stage}&run_id={run_id}&type_model={type_model}"
...
class RunsMLAPI(ServiceML):
    ...
    def create(self, run_params):
        response = requests.post(
            f"{self.service_uri}", json=run_params, headers={"Content-Type": "application/json"}
        )
        return self._handle_response(response)

    def upload_chunk(self, experiment_name: str, file_name: str, file_path: str, chunk: bytes, content_type: str):
        response = requests.post(
...
class ProjectMLAPI:
    ...
    def create_project(self, project_name):
        ...
        return requests.post(f"{self.service_uri}/project", json=body)
    ...
    def deploy_script(
        ...
        response = requests.post(f"{self.service_uri}/deploy",
    ...
    def run_pipeline(self, project_name: str):
        body = {"project_name": project_name}
        response = requests.post(
            f"{self.service_uri}/cluster/run", json=body, headers=self.headers)
...
class ServiceAPIMLFacade:
    def __init__(self, service_uri):
        self.experiment_api = ExperimentMLAPI(service_uri)
        self.model_api = ModelMLAPI(service_uri)
        self.runs_api = RunsMLAPI(service_uri)
        self.project_api = ProjectMLAPI(service_uri)
