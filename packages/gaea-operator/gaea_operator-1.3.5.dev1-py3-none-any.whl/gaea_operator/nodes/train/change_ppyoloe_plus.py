...
from gaea_operator.config import PPYOLOEPLUSMConfig, KEY_NETWORK_ARCHITECTURE
...
from gaea_operator.utils import write_file, is_base64, ModelTemplate, NETWORK_ARCHITECTURE
...
def change_ppyoloe_plus_train(args):
    """
    Train component for change_ppyoloe_plus model.
    """
    ...
    train_advanced_parameters[KEY_NETWORK_ARCHITECTURE] = \
        NETWORK_ARCHITECTURE[ModelTemplate.CHANGE_PPYOLOE_PLUS_NAME][
            train_advanced_parameters[KEY_NETWORK_ARCHITECTURE]]
    bcelogger.info(f"Advanced parameters: {train_advanced_parameters}")
    # 3. 下载预训练模型
    pretrain = {"change-ppyoloe_s": "ppyoloe_crn_s_obj365_pretrained",
                "change-ppyoloe_m": "ppyoloe_crn_m_obj365_pretrained",
                "change-ppyoloe_l": "ppyoloe_crn_l_obj365_pretrained",
                "change-ppyoloe_x": "ppyoloe_crn_x_obj365_pretrained"}
    ...
    trainer.launch()
    trainer.paddledet_export(model_dir=args.output_model_uri, is_change_model=True)
...
if __name__ == "__main__":
    args = parse_args()
    change_ppyoloe_plus_train(args=args)
