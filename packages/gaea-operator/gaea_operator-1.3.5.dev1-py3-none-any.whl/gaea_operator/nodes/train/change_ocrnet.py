...
from gaea_operator.config import OCRNetConfig, KEY_NETWORK_ARCHITECTURE
...
def change_ocrnet_train(args):
    ...
    dataset = CityscapesDataset(windmill_client=windmill_client,
    ...
    train_advanced_parameters[KEY_NETWORK_ARCHITECTURE] = \
        NETWORK_ARCHITECTURE[ModelTemplate.CHANGE_OCRNET_NAME][train_advanced_parameters[KEY_NETWORK_ARCHITECTURE]]
    ...
    bcelogger.info(f"Start export model from {args.output_model_uri}")
    input_shape = [-1, 6, int(train_advanced_parameters['eval_height']), int(train_advanced_parameters['eval_width'])]
