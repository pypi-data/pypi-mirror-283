...
class Executor:
    ...
    def __init__(self):
        ...
        if FLAGS.embedding_mode:
            model_root = '/modelset/model/base/'
            model_dir = os.path.join(model_root, FLAGS.model_name)
        else:
            model_root = '/modelset/model/'
            model_dir = os.path.join(model_root, FLAGS.model_name.split('/')[-1].replace('.', '___'))
