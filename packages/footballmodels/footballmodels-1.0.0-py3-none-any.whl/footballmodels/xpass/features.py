...
def x_pass_features(data: pd.DataFrame) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    data_n["pass_angle"] = F.col_get_qualifier_value(data_n, display_name="Angle").astype(float)
...
def x_pass_features_v2(data, use_positions_as_features=False):
    ...
    data_n["is_losing_big"] = data_n["game_state"].apply(lambda x: x == "LosingBig").astype(int)
    data_n["is_winning_big"] = data_n["game_state"].apply(lambda x: x == "WinningBig").astype(int)
    data_n["is_losing_small"] = data_n["game_state"].apply(lambda x: x == "LosingSmall").astype(int)
    data_n["is_winning_small"] = data_n["game_state"].apply(lambda x: x == "WinningSmall").astype(int)
    data_n["is_drawing"] = data_n["game_state"].apply(lambda x: x == "Drawing").astype(int)
    data_n["is_fwd"] = data_n["position"].apply(lambda x: x in ["RF", "LF", "CF"]).astype(int)
    data_n["is_att_mid"] = (
        data_n["position"].apply(lambda x: x in ["RCAM", "RM", "LM", "LCAM", "CAM", "LWF", "RWF", "SS"]).astype(int)
    )
    data_n["is_mid"] = data_n["position"].apply(lambda x: x in ["RCM", "LCM", "RCDM", "LCDM", "CDM", "CM"]).astype(int)
    data_n["is_cb"] = data_n["position"].apply(lambda x: x in ["RCB", "LCB", "CB"]).astype(int)
    data_n["is_fb"] = data_n["position"].apply(lambda x: x in ["RB", "LB", "RWB", "LWB"]).astype(int)
