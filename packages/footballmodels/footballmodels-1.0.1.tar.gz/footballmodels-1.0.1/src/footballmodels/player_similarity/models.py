...
class SimilarityAlgorith(ABC):
    ...
    def _get_searched_player(self, data: pd.DataFrame, player: str, squad: str, season: int, comp: str):
        return data[
            (data["player"] == player) & (data["squad"] == squad) & (data["season"] == season) & (data["comp"] == comp)
        ]
    ...
    def find(
        ...
        if filter_kwargs:
            ...
            if self._get_searched_player(filtered_data, player, squad, season, comp).empty:
                player_data = self._get_searched_player(search_data, player, squad, season, comp)
                filtered_data = pd.concat([filtered_data, player_data])
        return filtered_data.sort_values("distance").head(n)

    @abstractmethod
    def _find(self, data: pd.DataFrame, player: str, squad: str, season: int, comp: str):
        pass
