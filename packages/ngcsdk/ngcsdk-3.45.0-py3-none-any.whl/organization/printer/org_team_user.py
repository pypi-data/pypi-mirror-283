...
class OrgTeamUserPrinter(NVPrettyPrint):
    ...
    def print_user_details(  # noqa: D102
        ...
        user_roles = user_details.user.roles or []
        ...
        org_roles = {}
        team_roles = {}
        if (include_org or include_teams) and user_roles is not None:
            for role in user_roles or []:
                if include_org and role.org and not role.team:
                    org_name = role.org.name
                    org_display_name = role.org.displayName
                    full_org_name = org_name
                    if org_display_name:
                        full_org_name = org_display_name + " (" + org_name + ")"
                    org_roles[full_org_name] = ",".join(role.orgRoles)
                if include_teams and role.team:
                    team_roles[role.team.name] = ",".join(role.teamRoles)
                    # handle case when org role is not returned.
                    if include_org and not org_roles:
                        org_name = role.org.name
                        org_display_name = role.org.displayName
