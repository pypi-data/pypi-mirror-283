...
def clone_repo():
    ...
    stages = ["Installing Django Default structure...", "Building Django Default structure...", "Finishing progress..."]
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            # Display progress bar for cloning with colored stage names
            print(Fore.LIGHTCYAN_EX + "\nOperation started:\n" + Style.RESET_ALL)
            alive_progress_bar(5, [stages[0]])
            subprocess.run(["git", "clone", repo_url, tmp_dir], check=True, stdout=subprocess.DEVNULL,
            ...
            for item in os.listdir(tmp_dir):
                ...
                d_path = os.path.join(os.path.dirname(os.getcwd()), item)
            ...
            print(Fore.LIGHTGREEN_EX + f"Django Structure successfully built !!!" + Style.RESET_ALL)
    except subprocess.CalledProcessError as e:
        print(Fore.LIGHTRED_EX + f"An error occurred while cloning the repository: {e}" + Style.RESET_ALL)
