...
class SpecialsFixturesResponse(BaseModel):
...
:type leagues: List[SpecialsFixturesLeague], optional
"""
    def __init__(self, sport_id: int = None, last: int = None, leagues: List[SpecialsFixturesLeague] = None):
        if sport_id is not None:
            self.sport_id = sport_id
        if last is not None:
            self.last = last
        if leagues is not None:
            self.leagues = self._define_list(leagues, SpecialsFixturesLeague)
