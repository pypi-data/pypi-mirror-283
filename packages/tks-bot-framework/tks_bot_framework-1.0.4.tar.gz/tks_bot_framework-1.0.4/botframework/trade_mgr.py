...
class TradeMgr:
    ...
    async def create_first_entry_for_pos_idx(self, topic_name_trade, pos_idx, provider_trade_id, provider_signal_id, status_of_position:StatusOfTrade, price, is_hot_signal:bool, market, data_source, direction:Direction, tp, sl, position_size_in_percentage, percentage_of_position, timestamp=None):
        ...
        await database.produce_message(topic_name=topic_name_trade, key=str(pos_idx), value=trade_data)
    ...
    async def create_additional_entry_for_pos_idx(self, ksqldb_query_url, topic_name_trade, view_name_trade, pos_idx, new_status_of_position: StatusOfTrade = None, price = None, provider_signal_id = None, percentage_of_position = None, timestamp = None, buy_order_id = None, sell_order_id = None, new_buy_order = False, new_sell_order = False, execution_confirmed = False, tp_sl_reached=False):
        if timestamp is None:
            timestamp = time.time() * 1000
        trade_data = await self.get_latest_trade_data_by_pos_idx(ksqldb_query_url, view_name_trade, pos_idx)
        # Unpack the list into variables
        (data_time_of_data_entry, data_pos_idx, data_provider_trade_id, data_status_of_position, data_is_hot_signal,
        ...
        if new_status_of_position == StatusOfTrade.SELLING:
            new_time_of_position_closing = timestamp
        else:
            new_time_of_position_closing = data_time_of_position_closing
        # Update buy order if buy_order_id is provided
        if buy_order_id and buy_order_id in buy_orders:
            buy_orders[buy_order_id]["execution_confirmed"] = execution_confirmed
        # Update sell order if sell_order_id is provided
        if sell_order_id and sell_order_id in sell_orders:
            sell_orders[sell_order_id]["execution_confirmed"] = execution_confirmed
        ...
        await database.produce_message(topic_name=topic_name_trade, key=str(pos_idx), value=new_trade_data)
    ...
    async def get_active_positions(self, ksqldb_query_url, view_name, direction: Direction = None):
        algo_config = utils.get_app_config()
        position_capacities = algo_config.get('position_capacities', [])
        # Filter position capacities by direction if specified.
        if direction is not None:
            filtered_positions = [pc['position_capacity']['id'] for pc in position_capacities if pc['position_capacity']['direction'] == direction]
        else:
            filtered_positions = [pc['position_capacity']['id'] for pc in position_capacities]
        occupied_positions = []
        if len(filtered_positions) == 0:
            return occupied_positions  # Return early if there are no positions to check
        
        logger.info(f"Filtered positions: {filtered_positions}")
        
        for pos_idx in filtered_positions:
            ...
            logger.info(f"Results for POS_IDX {pos_idx}: {results}")
            
            if results:
                latest_message = results[0]  # The latest message for the given pos_idx
                
                logger.info(f"Latest message for POS_IDX {pos_idx}: {latest_message}")
                
                # Adjust the indices based on your data structure
                status_of_position = latest_message[3]  # Assuming 'status_of_position' is at index 3
                
                logger.info(f"Status of position for POS_IDX {pos_idx}: {status_of_position}")
    ...
    async def get_realized_and_unrealized_profit_and_loss_of_position(self, ksqldb_query_url, view_name, pos_idx: int, market, exchange=None) -> tuple:
        ...
        current_price = 3000 #await MarketMgr().get_last_price(market, exchange)
