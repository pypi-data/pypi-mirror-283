...
class ModernSwitch(QWidget):
    ...
    def __init__(self, parent=None, name='', par:TREpar=None):
        ...
        FlagWindowRun=self.window().FlagRun if hasattr(self.window(),'FlagRun') else False
    ...
    def startTimer(self):
        if not self.FlagRunning and hasattr(self.window(),'gearMovie'):
            self.FlagRunning=True
            self.gearMovie=self.window().gearMovie
                    ...
                    if len(data)>2:
                        if data[2]:
                            ...
                            for data2 in data2list:
                                stringa=QKeySequence(data2).toString(QKeySequence.NativeText) 
                                tip+=' ('+stringa+')'
    ...
    def treeContextMenuEvent(self, event):
        ...
        if item is None: 
            ...
            if 'items' in self.additionalButtonBars:
                buttonBars+=[bar for bar in self.additionalButtonBars['items']]
        ...
        for bar in buttonBars:
            ...
            for type, b in bar.buttons.items():
                ...
                if b.isVisible() and b.isEnabled():
                    name.append(bar.buttonData[type]['name'])
...
class ProjectTree(PaIRSTree):
    ...
    def titleFont(self,label:QLabel,fPixelSize=None):
        if fPixelSize is None:
            if hasattr(self.window(),'GPApar'): fPixelSize=self.window().GPApar.fontPixelSize
...
class StepButtonBar(QWidget):   
    def __init__(self, buttonData=stepData, buttonSize=processButtonSize, FlagInvisible=True):
        ...
        for type in buttonData:
            ...
            if all([f in list(data) for f in ('icon', 'name', 'type')]):
                ...
                def buttonIcon(b,d):
                    iconOn = TreeIcons.icons[d['icon']]
                    iconOff = TreeIcons.icons[d['icon'].replace('.png','_off.png')]
                    b.clicked.connect(lambda:self.setButtonIcon(b,iconOn,iconOff))
                    setattr(b,'setButtonIcon',lambda:self.setButtonIcon(b,iconOn,iconOff))
    ...
    def setButtonIcon(self,button:QToolButton,iconOn,iconOff):
        if button.isEnabled():
            if button.isChecked():
                button.setIcon(iconOn)
            else:
...
class ProcessTree(PaIRSTree):
        ...
        for i in range(self.topLevelItemCount()):
            ...
            for processPrev in self.itemList[1:]:
                try:
                    ...
                    pri.Error.red(inst)
                    pass
        if hasattr(self.window(),'setLinks'):
                ...
                for ITE,ind_old,dep_old,link_old in zip(ITEs,inds_old_i,deps_old_i,links_old_i):
                    for k,ind_slave_old in enumerate(dep_old):
                        if ind_slave_old[2] not in inds_new_list: #stiamo rigenerando l'albero
                            ...
                            self.window().setLinks(ind_slave_old,ind_old,FlagUnlink,ind_slave_new,ITE.ind)     
                            if ind_slave_new[1]==0 and ITE.ind[1]==0:     
                                self.window().setLinks(ind_slave_new,ITE.ind)
                        pass
                    if link_old in inds_new_list: 
                        link_new=copy.deepcopy(link_old)
                        link_new[2]=inds_new[link_new[2]]
                        self.window().setLinks(ind_old,link_old,FlagUnlink,ITE.ind,link_new)    
                        if link_new[1]==0 and ITE.ind[1]==0:     
                            self.window().setLinks(ITE.ind,link_new)   
                        pass 
    ...
    def createProcessItem(self,type,ind=None,FlagNewItem=True):
        self.blockSignals(True)
        if ind==None: ind=self.topLevelItemCount()
        item = QTreeWidgetItem()
    ...
    def setProcessItemWidget(self,item:QTreeWidgetItem,ITE:ITEpar):
        ...
        switch.setVisible(ITE.procdata.flagRun<=0 and ITE.Process!=ProcessTypes.cal and not self.FlagBin)
            ...
            for i in indexes:
                for ITE in self.itemList[0][i]:
                    ...
                    for dep in ITE.dependencies:
                        self.window().setLinks(dep,ITE.ind,FlagUnlink=True)
                    if ITE.link: self.window().setLinks(ITE.ind,ITE.link,FlagUnlink=True)
    ...
    def restore_action(self):
        ...
        self.restoreTree.paste_above_below_action(FlagAbove=not self.FlagBin,row=0 if not self.FlagBin else self.restoreTree.topLevelItemCount()-1,FlagPasteDeleted=True)

    def paste_above_below_action(self,FlagAbove=True,row=None,FlagPasteDeleted=False): 
...
class StepItemWidget(QWidget):
    ...
    def __init__(self, ITE:ITEpar, parent=None):
        ...
        self.setMinimumWidth(stepNameWidth+self.label_size+self.bar_width+secondLevelSpacing*3)
        self.setFromITE(ITE)
    ...
    def setFromITE(self,ITE:ITEpar):
        ...
        FlagGif=False
        if ITE.flagRun==-2 and ITE.label=='running' and hasattr(self.window(),'runningMovie'):
            if hasattr(self.window(),'procdata') and self.window().procdata is ITE.procdata:
                FlagGif=True
        if FlagGif:
            self.label.setPixmap(QPixmap())
            self.label.setMovie(self.window().runningMovie)
        else:
        ...
        if ITE.Step==StepTypes.cal:
            ...
            self.progressBar.setValue(ITE.progress)  # Set the initial value of the progress bar
...
class PaIRS_Explorer(gPaIRS_Tab):         
    ...
    def __init__(self,parent=None,widgets=[]):
        ...
        itemWidths=[
            titleNameWidth+ModernSwitch.switchWidth+3, #first level
        ...
        self.processTree.setMinimumWidth(max(itemWidths)+self.processTree.columnWidth(0)+40)
        ...
        def stepPageAction(type):
            ...
            c=list(ITE.children).index(type)
            child=item.child(c)
        ...
        if item:  
            ...
            for c in list(stepData):    
                ...
                if c in ITEs[0].children:
                    ...
                    flagRunnable=flagRunnable and ITEs[ind+1].flagRun==0 and ITEs[0].flagRun!=-2
                    b.setEnabled(b.isVisible() and flagRunnable)
    ...
    def selectStep(self):
        ...
        for k,w in enumerate(self.widgets):
            ...
            if w.TABname=='Calibration':
                ...
                FlagVisible.append(par is not None)
        ...
        FlagBridge=True
        if i is not None: 
            #self.window().setTABpars_at(self.ITEpar.ind,FlagAdjustPar,FlagBridge,widget=self.widgets[i])
    ...
    def setITElayout(self,ITE:ITEpar=None):
        ...
        if ITE.flagRun==0:
            ...
            while ITEs[j].Step!=ITE.Step and j<len(ITEs)-1:
                if not ITEs[j].OptionDone==1 and ITEs[j].Step in mandatory:
                    FlagDone=False
            ...
            #if hasattr(self.window(),'reset_step') and not os.path.exists(procdata.procOutName()) and not ITE.FlagInit:
            #    self.window().reset_step(procdata.ind)
            #    return
