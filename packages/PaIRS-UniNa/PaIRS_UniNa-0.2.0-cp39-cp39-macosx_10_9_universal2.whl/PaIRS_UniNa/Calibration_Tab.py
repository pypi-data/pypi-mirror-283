...
#*************************************************** Adjusting parameters
    def adjustCALpar(self):
        ...
        self.CALpar.OptionDone = 1 if all(self.CALpar.calEx) and len(self.CALpar.calList)==self.CALpar.ncam else 0
        return
              
#*************************************************** Layout
    def setCALlayout(self):
        self.ui.label_ncam.adjustSize()
        self.ui.spin_ncam.setEnabled(self.CALpar.Process!=ProcessTypes.spiv)
        ...
        if self.CALpar.OptionDone==1:
            self.CALpar.warningMessage='Calibration files correctly identified!'
        else:
            warningMessage=''
            nFiles=len(self.CALpar.calList)
            if nFiles<self.CALpar.ncam:
                warningMessage+=f'Number of calibration files ({nFiles}) lower than number of cameras specified ({self.CALpar.ncam})!'
            if not all(self.CALpar.calEx):
                n=sum([1 if not ex else 0 for ex in self.CALpar.calEx])
                if warningMessage: warningMessage+='\n'
                warningMessage+=f'{n} out of {self.CALpar.ncam} calibration files missing!'
            self.CALpar.warningMessage=warningMessage
...
#******************** Actions
    ...
    def button_import_action(self):
        filenames, _ = QFileDialog.getOpenFileNames(self,\
            "Select image files from the current directory", filter='*.cal',\
