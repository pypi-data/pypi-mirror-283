...
class Input_Tab(gPaIRS_Tab):
    ...
    def __init__(self,parent: QWidget =None, flagInit= __name__ == "__main__"):
        ...
        self.TABname='Input'
        self.ui: Ui_InputTab
        self.exImTree=self.ui.exImTree=GlobalImageTree(self)
...
#*************************************************** Adjusting parameters
    def adjustINPpar(self):
        ...
        if not self.INPpar.FlagCam:
            self.INPpar.inp_ncam=self.ncamMinimum()
        else: 
        ...
        FlagWarn=self.INPpar.OptionValidPath==0
        if not self.INPpar.FlagInit: 
            self.ui.imTreeWidget.button_scan_list_action()
        if not FlagWarn:
            for imExc in self.INPpar.imEx:
                for imExf in imExc:
                    for ex in imExf:
                        if not ex:
                            FlagWarn=True
        ...
        self.INPpar.OptionDone=1 if (len(self.INPpar.imList[0][0])>0 and not FlagWarn) else -1 if len(self.INPpar.imList[0][0])>0 else 0
...
#*************************************************** Layout
    def setINPlayout(self):
        ...
        if self.INPpar.OptionDone==1:
            self.INPpar.warningMessage='Input image files correctly identified!'
        else:
            FlagWarn=False
            for imExc in self.INPpar.imEx:
                for imExf in imExc:
                    for ex in imExf:
                        if not ex:
                            FlagWarn=True
                            break
            if self.INPpar.OptionValidPath==0:
                self.INPpar.warningMessage='Invalid input path!'
            elif len(self.INPpar.imList[0][0])==0:
                self.INPpar.warningMessage='Image set empty!'
            elif FlagWarn:
                self.INPpar.warningMessage='Some image files missing!'
            else:
                self.INPpar.warningMessage='Issues with identifying input image files!'
        self.ui.name_tab.setFixedWidth(self.ui.name_tab.sizeHint().width())
        self.ui.label_done.setPixmap(self.pixmap_done if self.INPpar.OptionDone==1 else self.pixmap_warnc)
        self.ui.label_done.setToolTip(self.INPpar.warningMessage)
        return
...
#******************** Layout & Adjustments
    def setImageNumberSpinLimits(self):
        if self.INPpar.isDifferentFrom(self.INPpar_old,fields=['imSet','frame_1','frame_2','inp_ncam','FlagTR']):
            ind_in, ind_fin, npairs, npairs_max= self.getIndNpairs()
            self.INPpar.ind_in=min([max([ind_in,self.INPpar.ind_in]),ind_fin])
            self.INPpar.npairs=min([self.INPpar.npairs,npairs])
    ...
    def getIndNpairs(self):
        if self.INPpar.imSet.count:
            ...
            ind_in=ind_in_2=0
            ind_fin=-1
        npairs_max=npairs=ind_fin-ind_in+1 
        if not all(self.INPpar.frame_2): 
            npairs_max=int(npairs/2)+npairs%2
            npairs=int(npairs/2)
        if self.INPpar.FlagTR_Import: npairs=(1+int(any(self.INPpar.frame_2)))*npairs-1
        if npairs==0: ind_in=ind_fin=0
        return ind_in, ind_fin, npairs, npairs_max
        
    def adjustExampleImageList(self):
        exImListPar=['path','frame_1','frame_2','inp_ncam','ind_in','npairs','step','FlagTR']
...
#******************** Settings
    def image_list_set(self):
        if self.INPpar.isDifferentFrom(self.ui.imTreeWidget.imTree,fields=['imList','imEx']):
            ...
            self.ui.imTreeWidget.setLists(self.INPpar.path,self.INPpar.imList,self.INPpar.imEx,selection=self.INPpar.selection,FlagOnlyPrepare=False)
