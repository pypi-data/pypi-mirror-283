...
def schedule_job(job_config: dict, time_unit: str, tempo: int) -> None:
    logger.info(f'Adicionando job {job_config.get("job_name")} ao schedule de {tempo} em {tempo} {time_unit}')
    ...
    if func is not None:
        if time_unit == 'M':  # Minutos
            # Agendamento dos jobs com o wrapper para monitorar tempo de execução
            schedule.every(tempo).minutes.do(job_wrapper(func), job_config)
            #schedule.every(tempo).minutes.do(func, job_config)
        elif time_unit == 'H':  # Horas
            schedule.every(tempo).hours.do(func, job_config)
        elif time_unit == 'D':  # Dias
            schedule.every(tempo).days.do(func, job_config)
...
# Ajuste feito por Ernane Crato - para tentar resolver o problema de JOB parado
# Função para rodar os jobs pendentes
def run_and_monitor_jobs():
    while True:
        # Verifica e executa jobs pendentes
        schedule.run_pending()
        reexecute_failed_jobs()  # Verifica e reexecuta jobs falhados
        time.sleep(1)  # Intervalo para evitar uso excessivo de CPU

# Ajuste feito por Ernane Crato - para tentar resolver o problema de JOB parado
# Monitoramento e reexecução de jobs que falharam
def monitor_jobs():
    while True:
        # Log dos jobs agendados
        for job in schedule.jobs:
            logging.info(f"Job {job.job_func.__name__} agendado para executar em {job.next_run}")
        reexecute_failed_jobs()
        sleep(600)  # Intervalo de monitoramento


# Estrutura para armazenar o status dos jobs
job_status = {}

# Atualizando o wrapper para armazenar o status de execução
def job_wrapper(job_func):
    def wrapped():
        job_name = job_func.__name__
        start_time = time.time()
        try:
            logging.info(f"Iniciando {job_name}")
            job_func()
            job_status[job_name] = 'success'
            logging.info(f"Finalizado {job_name} com sucesso")
        except Exception as e:
            logging.error(f"Erro ao executar {job_name}: {e}")
            job_status[job_name] = 'failed'
        end_time = time.time()
        duration = end_time - start_time
        logging.info(f"{job_name} levou {duration:.2f} segundos para ser concluído")
    return wrapped

# Função para reexecutar jobs falhados
def reexecute_failed_jobs():
    for job_name, status in job_status.items():
        if status == 'failed':
            logging.info(f"Reexecutando {job_name}")
            job = next(job for job in schedule.jobs if job.job_func.__name__ == job_name)
            job.job_func()  # Executar o job novamente
            job_status[job_name] = 'success'  # Atualizar o status
...
def main():
    ...
    if src.exibir_interface_grafica:
        ...
        while True:
            try:
                # Codigo Original - Antes da Alteração do Ernane
                #schedule.run_pending()
                #sleep(5)

                #Novo Código do Ernane Crato, para tentar resolver o problema do Job que não RODA
                scheduler_thread = threading.Thread(target=run_and_monitor_jobs)
