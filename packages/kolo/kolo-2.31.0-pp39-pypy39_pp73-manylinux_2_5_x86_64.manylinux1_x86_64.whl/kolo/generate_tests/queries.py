...
class DjangoUpdate:
    ...
    def template_parts(self):
        for field in self.fields:
            yield f"{self.variable_name}.{field.name} = {field.value_repr}"
        yield f"{self.variable_name}.save()"
...
class SQLParser:
    ...
    def build_creates(self, query_dict, columns, row, seen_mutations, schema_data):
        ...
        return inserts, imports, skipped_inserts, updates
    ...
    def parse_sql_queries(self, sql_queries, schema_data):
                ...
                if not any(
                    ...
                    if (
                        ...
                        if isinstance(where.this, sqlglot.expressions.EQ):
                            ...
                            new_inserts, new_imports, new_skips, new_updates = (
                                self.build_creates(
                                    query_dict,
                                    [column],
                                    [value],
        ...
        value_imports = build_value_imports(updated_creates, asserts)
        return updated_creates, imports, asserts, value_imports, skipped_inserts
