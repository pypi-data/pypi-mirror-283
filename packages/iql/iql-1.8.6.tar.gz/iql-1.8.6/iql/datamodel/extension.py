...
class IqlExtension:
    ...
    def pivot(self, working_df: DataFrame, pivot_option: Iterable[str]) -> DataFrame:  # noqa: C901
        ...
        if isinstance(column, str):
            column_set = set(working_df[column].unique())
        else:
            column_set = set()
        ...
        column_set.update(working_df.columns)  # make sure all columns in column_set
        working_df = working_df.reindex(columns=column_set, fill_value=None)
