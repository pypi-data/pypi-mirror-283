...
import inspect
from typing import AsyncGenerator, Generator, Callable, Literal, Union, cast, Protocol
...
from .utils import MessageDict, Message, ApiReturnObj, FileDataDict
...
class ChatInterface(Blocks):
    ...
    def _append_multimodal_history(
        ...
        for x in message.files:
            history.append(
                {"role": "user", "content": cast(FileDataDict, x.model_dump())}
            )
    ...
    async def _submit_fn(
        ...
        *args,
        ...
        if self.is_async:
            ...
            response: (
                ApiReturnObj | Message | MessageDict
        ...
        if self.multimodal and isinstance(message, MultimodalData):
            self._append_multimodal_history(
                message, cast(MessageDict, new_response), history
            )
        else:
