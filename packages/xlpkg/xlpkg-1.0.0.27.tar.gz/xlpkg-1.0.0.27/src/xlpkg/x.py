...
def f_app(filename=__file__,host='127.0.0.1',port=86,reload=True):
    import uvicorn
    fname,fext=os.path.splitext(os.path.basename(filename))
    app=fname+":app"
    uvicorn.run(app, host=host, port=port, reload=reload)

def f_uvirun(f=__file__,host='127.0.0.1',port=86):
    import uvicorn
    a=os.path.basename(f)
    print(a)
    fname,fext=os.path.splitext(a)
    print(a,fname)
    app=fname+':app'
    ...
    try:
        with conn.cursor(cursor=pymysql.cursors.DictCursor) as cur:
            ...
            if row>0:
                ...
                print(f_c(31), str(i).zfill(2)+' 无数据!',f_c(0))
                pass
    except Exception as e:
        print(e)
    pass
...
def f_dir07(path):
    for item in os.scandir(path):
        if item.is_file():
            ...
            print(f_c(31),x.f_isdir(item),f_c(),item.path)
            f_dir07(item)
    return(files)
    pass
