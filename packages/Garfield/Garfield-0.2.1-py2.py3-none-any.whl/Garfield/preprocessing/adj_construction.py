...
def graph_computing(adj_coo, cell_num, k):
    edgeList = []
    distMat = distance.cdist(adj_coo, adj_coo, 'euclidean')  # compute all distances at once
    for node_idx in range(cell_num):
        nearest_indices = np.argsort(distMat[node_idx])[:k+1]
        nearest_dists = distMat[node_idx, nearest_indices[1:k+1]]
...
def graph_construction(adj_coo, cell_N, k):
    edge_list = graph_computing(adj_coo, cell_N, k)
    graph_dict = edgeList2edgeDict(edge_list, cell_N)
...
# Now, you would call the function with the actual 'rna' and 'atac' data:
# result_matrix = process_data(rna_data, atac_data)
