...
def preprocessing_adt(
    ...
    if used_pca_graph:
        ...
        sc.pp.neighbors(adata_hvg, n_neighbors=n, metric=metric, use_rep='X')
...
def preprocessing(
    ...
    if profile == 'RNA':
        ...
        assert sub_data_type in ['rna', 'atac', 'adt'], 'Sub_data_type must be "rna", "atac", or "adt".'
        ...
        if data_type == 'single-modal':
            ...
            if graph_const_method is None:
                graph_dict = graph_construction(adata.obsm['spatial'], adata.shape[0], k=n)
