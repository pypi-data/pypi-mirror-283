...
class Pipeline:
    ...
    def update_qa_queries(self, queries):
        """Update the QA queries."""
        self.qa_queries = queries
    ...
    def send_email(self, subject, body, recipients=None):
        ...
        if not recipients:
            if hasattr(self, 'recipients'):
                ...
                raise errors.SMTPConfigurationError('No recipient passed or defined within pipeline attributes.')
    ...
    def execute_query(self, query, temp_table_name=None):
        ...
        try:
            ...
            raise errors.TaskError(f"Failed to execute query: {error_message}")
    ...
    def create_temp_table(self, query_job, temp_table_name):
        ...
        try:
            ...
            raise errors.TaskError(f"Failed to create temporary table {temp_table_name}: {error_message}")
    ...
    def execute_task(self, task):
        ...
        try:
            ...
            try:
                ...
                raise errors.TaskError(f"Failed to translate query for task '{task.name}': Missing key {str(e)} in temp tables")
