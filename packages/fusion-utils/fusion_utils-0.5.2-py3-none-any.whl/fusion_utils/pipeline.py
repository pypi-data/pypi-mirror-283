...
def send_completion_email(self):
    ...
    for task_name, updates in self.status.items():
        ...
        status_summary.append(f"<span class='{ 'pass' if latest_update.get('success', False) else 'fail' }'>Task '{task_name}': {latest_update['status']} at {latest_update['timestamp']} (Execution Time: {execution_time_str})</span>")
        ...
        body = f"Pipeline {self.name} has completed with the following status:<br><br>{summary}<br><br>Total Execution Time: {total_execution_time_str}"
