...
class Pipeline:
    ...
    def send_completion_email(self):
        ...
        for task_name, updates in self.status.items():
            ...
            status_summary.append(f"<span class='{ 'pass' if latest_update.get('success', False) else 'fail' }'>Task '{task_name}': {latest_update['status']} at {latest_update['timestamp']} (Execution Time: {self.task_execution_times.get(task_name, 'N/A'):.2f} seconds)</span>")
        ...
        body = f"Pipeline {self.name} has completed with the following status:<br><br>{summary}<br><br>Total Execution Time: {self.total_execution_time:.2f} seconds"
    ...
    def add_task(self, task):
        ...
        if task.is_qa:
            temp_table = None  # QA tasks don't need temp tables
        else:
    ...
    def execute_qa_task(self, task):
        print(f"Executing QA task '{task.name}'")
        try:
            ...
            if condition and not condition(result):
                raise TaskError(f"QA check failed for task '{task.name}'")
            else:
                print(f"QA check passed for task '{task.name}'")
