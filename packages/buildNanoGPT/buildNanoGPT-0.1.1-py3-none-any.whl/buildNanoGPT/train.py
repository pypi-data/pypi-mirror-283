...
def train_GPT(raw_model, train_loader, val_loader, train_cf, ddp_cf, use_compile=False, log_dir="log", log_file="log.txt"):
    """"Training GPT"""
    if ddp_cf.master_process:
    ...
    optimizer = raw_model.configure_optimizers(weight_decay=0.1, learning_rate=6e-4, device_type=ddp_cf.device_type)
    for step in range(train_cf.max_steps):
        ...
        if (step % train_cf.val_check_steps == 0 or last_step) and (not use_compile):
            hellaswag_eval(raw_model, step, log_file, ddp_cf)
        ...
        if ((step > 0 and step % train_cf.val_check_steps == 0) or last_step) and (not use_compile):
            generate_text(raw_model, enc, ddp_cf)
        # determine and set the learning rate for this iteration
        lr = get_lr(step, train_cf)
        # training loop
        train_step(raw_model, optimizer, train_loader, lr, step, t_start, log_file, train_cf, ddp_cf)
