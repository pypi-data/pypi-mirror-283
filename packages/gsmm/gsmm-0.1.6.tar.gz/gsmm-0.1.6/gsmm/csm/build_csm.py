...
def assign_reaction_confidences(#model: cobra.Model, expression_data: pd.DataFrame, gene_id_column: str, 
                                scores_column: str) -> pd.DataFrame:
    """
    Assigns confidence levels to reactions based on associated genes.
    ...
    Args:
        ...
        scores_column (str): Name of the expression values' column in `expression_data` used for gene confidence levels.

    Returns:
        pd.DataFrame: DataFrame with assigned confidence levels for reactions.

    Notes:
        This function assigns confidence levels to reactions based on the associated genes'
        confidence levels provided in `expression_data`. Specific biomass reactions are assigned
...
def run_model_reconstruction(data_path: str,
                             scores_column: str,
                             model_path: Optional[str] = model_paths,
                             base_model_path: Optional[str] = base_model_path, 
                             gene_id_column: Optional[str] = gene_id_column,
    ...
    Args:
        ...
        model_path (Optional[str], optional): Path to the SBML model file (default: model_paths).
        base_model_path (Optional[str], optional): Path to save the filtered SBML model (default: base_model_path).
