...
class FineTune_Uunsloth_Mistral_7b:
    ...
    def selective_merge(self, base_model_path, fine_tuned_model_path, output_dir):
        # Load models
        base_model = AutoModelForCausalLM.from_pretrained(base_model_path, cache_dir=self.cache_dir).to('cuda')
        fine_tuned_model = AutoModelForCausalLM.from_pretrained(fine_tuned_model_path, cache_dir=self.cache_dir).to('cuda')
