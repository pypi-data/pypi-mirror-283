...
class Sylphe:
    ...
    def __init__(self, get_place=False, places='location.p', reverse_places='reverse_location.p'):
        
        self.sequences_db = None
        self.indices_db = None
        self.components_db = None
    ...
    def read_sequences(self) :
        ...
        sequences = self.sequences_db[sylphe_col_sequences.keys()]
    ...
    def read_indices(self):
        ...
        def _check_dates(length):
            ...
            c = row['count']
            s = f'end-begin dates != values length for: {idx} {keycode} : len != (DateEnd - DateBegin + 1)  {length} {c} != ( {end} - {begin} + 1 = {end - begin + 1})',  
        ...
        indices = self.indices_db[sylphe_col_indices.keys()]
    ...
    def read_components(self):
        ...
        components = self.components_db[sylphe_col_components.keys()]
    ...
    def db_access(self, dbaccess):
        try:
            import pyodbc
            #print('import_access')
            logger.info('import_access')
            connection_string = (
                r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};'
                rf'DBQ={dbaccess};'
            )
            conn = pyodbc.connect(connection_string)
            logger.info('import_access connection ok')

            lst = []
            for table, df in zip(['Sequence', 'Component', 'Index'], [self.sequences_access, self.components_access, self.indices_access]):
                query = f"SELECT * FROM {table}"
                lst.append(pd.read_sql(query, conn))
            return lst
    ...
    def read_file(self, filename):
        self.sequences_db, self.indices_db, self.components_db = self.db_access(filename)
        keycode_parent = Path(filename).stem

        return self.to_dataset(root_keycode=keycode_parent)
