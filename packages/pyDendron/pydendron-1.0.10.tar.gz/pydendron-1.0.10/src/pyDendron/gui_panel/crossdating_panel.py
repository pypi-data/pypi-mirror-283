...
class CrossDatingPanel(Viewer):
    ...
    def __init__(self, dataset, parameters, cfg_path, **params):
        ...
        #self.bt_export = pn.widgets.Button(name='Excel Export', icon='swicth', button_type='primary', align=('start', 'center'), width=2*bt_size)
        #self.bt_export.on_click(self.on_export)
...
#        self.warray = self.wtabulator
        ...
        self.tabs = pn.Tabs(('Array', self.warray),
                           ...
                           ('Density', pn.Column('Computed on all *scores*. Threshold and rank are disallowed.', self.bt_svg, self.whist)),
    ...
    #     #logger.info(f'set_tabulator  {len(df)}, {time.time()}')
        ...
        try:
            if Path(self.cfg_file).is_file():
                with open(self.cfg_file, 'r') as fd:
                    data = json.load(fd)
                    self.param = self.Param(**self.Param.param.deserialize_parameters(data['param']))
...
#                self.dataset_package.dt_data.at[idx, DATE_BEGIN] = date_begin
    ...
    def on_stop(self, event):
        #self.cross_dating.is_stop = True
        logger.info('on_stop')
        #self.cross_dating.stop()
        #self.bt_compute.disabled = False
        #self.bt_stop.disabled = True
        #self.clean()
        #self.progress_info.object = f'<span>stop process</span>'
