...
class Dataset(param.Parameterized):
    ...
    def notify_changes(self, message):
        ...
        logger.info(f'--> dataset notify_changes: {message}')
    ...
    def update(self, update_tree=False):
        ...
        #print(self.sequences.columns)
        #print(sequences_dtype_dict)
        #print(self.sequences[SITE_LATITUDE])
    ...
    def cut(self, triplets: List[Tuple[int, int, int]], dest_path:  List[int], notify=True) -> str:
        """
    ...
    def append(self, dataset, verify_integrity=True, reindex=True, notify=True):
        """
    ...
    def edit_component(self, idx_parent, idx_child, value, notify=True):
    ...
    def edit_sequence(self, idxs, column, value, notify=True):
    ...
    def shift_offsets(self, parent_idx, notify=True):
        """
    ...
    def copy_dates_to_offsets(self, parent_idx, notify=True):
        """
    ...
    def set_offsets_to_dates(self, parent_idx, notify=True):
        """
        Get children of `parent_idx`, copy dates to offsets and shift to 0 (if `shift` is True).
        """
        data = self.get_data(parent_idx, max_depth=1, include_parent=False)
    ...
    def set_chononology_info(self, idx, means, weights, offsets, data_type, data_samples, sequences=None):
        ...
        self.log_sequences(idx, f'Chronology update')
