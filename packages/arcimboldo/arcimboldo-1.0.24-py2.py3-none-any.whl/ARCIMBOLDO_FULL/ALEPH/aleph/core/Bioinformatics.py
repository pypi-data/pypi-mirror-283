...
def get_rmsd_and_RT(ca_list1,ca_list2,full_ca, transform=True, n_iter=5):
    ...
    transf, rmsd_list = fit_wellordered(atom_list_a, atom_list_b, n_iter=n_iter, full_output=True,
...
def getRMSDfromCommonCore(dicc_1, dicc_2):
    ...
    if len(core.keys()) > 0:
        ...
        transf, rmsd_list = fit_wellordered(ref1, comp, n_iter=None, full_output=True)
        if len(rmsd_list) > 0:
            rmsd_1 = rmsd_list[-1][1]
        else:
            rmsd_1 = -1
        transf, rmsd_list = fit_wellordered(ref2, comp, n_iter=None, full_output=True)
        if len(rmsd_list) > 0:
        ...
        if algorithm == "nigels-core2":
            ...
            for cren in liran:
                ...
                for tren in range(len(listValiCmp)):
                    ...
                    transf, rmsd_list = fit_wellordered(refi, compi, n_iter=n_iter, full_output=True, n_stdv=2,
            ...
            for tren in range(len(listValiCmp)):
                ...
                transf, rmsd_list = fit_wellordered(refi, compi, n_iter=n_iter, full_output=True)
...
# Functions from https://github.com/csb-toolbox/CSB
# Stopped working in pypi
