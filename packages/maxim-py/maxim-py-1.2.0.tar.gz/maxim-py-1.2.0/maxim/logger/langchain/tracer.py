...
class MaximLangchainTracer(BaseCallbackHandler):
    ...
    def _validate_maxim_metadata(self, metadata: Dict[str, Any]):
        ...
        valid_keys = ['session_id', 'trace_id', 'span_id', 'generation_tags',
                      'retrieval_tags', 'trace_tags', 'retrieval_name', 'trace_name', 'llm_call_name']
        ...
        **kwargs: Any
        ...
        if metadata is not None:
            ...
            self.metadata_store.set(
                str(run_id), metadata.get("maxim", None), 60*10)
            generation_name = metadata.get(
                "maxim", {}).get("llm_call_name", None)
        ...
        self.generations.set(str(run_id), generation_id, 60*10)
    ...
    def on_llm_error(
        ...
        if run_id is not None:
            ...
            if generation_id is not None:
                container = self._get_container(run_id)
                self.logger.generation_error(generation_id, generation_error)
                if container == "local_trace":
                    self.logger.trace_end(str(run_id))

    def on_chain_start(
        self, serialized: Dict[str, Any], inputs: Dict[str, Any], **kwargs: Any
    ) -> Any:
        print("[MaximSDK] we don't support chain flows yet")

    def on_chain_end(self, outputs: Dict[str, Any], **kwargs: Any) -> Any:
        print("[MaximSDK] Chains are not supported")
    ...
    def on_tool_start(
        ...
        print("[MaximSDK] Tools are not yet supported")
    ...
    def on_text(self, text: str, **kwargs: Any) -> Any:
        print("[MaximSDK] Text are not supported")
